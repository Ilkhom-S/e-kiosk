    ...
    namespace Driver {
        ...
        class IDevice {
          public:
            ...
            static const char
                *StatusSignal; // = SIGNAL(status(SDK::Driver::EWarningLevel::Enum, const QString &, int));
          ...
          public:
            /// Возвращает название подключенного устройства. Обязан выдать корректное значение после вызова
            /// initialize().
            virtual QString getName() const = 0;

            /// Переформировывает список параметров для авто-поиска и устанавливает 1-й набор параметров из этого
            /// списка.
            virtual IDetectingIterator *getDetectingIterator() = 0;

            /// Подключает и инициализирует устройство. Возвращает истину, если устройство готово к работе (установлен
            /// порт и т.д.).
