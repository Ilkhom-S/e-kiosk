...
const char FiscalsForSessionTotal[] =
...
const char TransferOperatorAddress[] =
    "transfer_operator_address";                            // 1005 (Адрес оператора перевода).
const char TransferOperatorINN[] = "transfer_operator_inn"; // 1016 (ИНН оператора перевода).
const char TransferOperatorName[] =
    "transfer_operator_name"; // 1026 (Наименование оператора перевода).
const char TransferOperatorPhone[] =
    "transfer_operator_phone"; // 1075 (Телефон оператора перевода).
...
const char AgentFlagsReg[] =
    "agent_flags_reg"; // 1057 (Признак(и) платежного агента из итогов регистрации и на платеже).
...
const char PayOffSubjectQuantity[] =
    "payoff_subject_quantity";                              // 1023 (Количество предмета расчета).
...
const char PayOffSubjectUnitPrice[] =
    "payoff_subject_unit_price"; // 1079 (Цена за единицу предмета расчета с учетом скидок и
                                 // наценок).
const char PayOffSubjectTaxAmount[] =
    "payoff_subject_tax_amount";                        // 1200 (Сумма НДС за предмет расчета).
const char PayOffSubjectType[] = "payoff_subject_type"; // 1212 (Признак предмета расчета).
const char PayOffSubjectMethodType[] =
    "payoff_subject_method_type"; // 1214 (Признак способа расчета).
...
const char PrePaymentFiscalTotal[] =
    "prepayment_fiscal_total"; // 1215 (Сумма по чеку (БСО) предоплатой (зачетом аванса)).
const char PostPaymentFiscalTotal[] =
    "postpayment_fiscal_total"; // 1216 (Сумма по чеку (БСО) постоплатой (в кредит)).
const char CounterOfferFiscalTotal[] =
    "counter_offer_fiscal_total"; // 1217 (Сумма по чеку (БСО) встречным предоставлением).
...
const char TaxAmount06[] =
    "tax_amount_06"; // 1106 (Сумма НДС чека по расчетной ставке 18/118 (20/120)).
