...
enum Enum {
    ...
    RIARightsProvision, /// Предоставление прав на использование результатов интеллектуальной
                        /// деятельности (РИД, RIA)
    Payment,               /// Платеж
...
struct SUnitData {
    ...
    SUnitData(
        ...
        : sum(aSum), VAT(aVAT), name(aName), providerINN(aProviderINN),
          payOffSubjectType(aPayOffSubjectType), payOffSubjectMethodType(aPayOffSubjectMethodType),
          section(aSection) {}
};

typedef QList<SUnitData> TUnitDataList;

/// Фискальные данные платежа  !!!!!!!!!!!!!!  НЕ забываем обновить метод
/// FiscalProtocol::createRequest и parseRequestJsonDoc
struct SPaymentData {
    ...
    SPaymentData(const TUnitDataList &aUnitDataList,
                 EPayOffTypes::Enum aPayOffType,
                 EPayTypes::Enum aPayType = EPayTypes::None,
                 ETaxSystems::Enum aTaxSystem = ETaxSystems::None,
                 EAgentFlags::Enum aAgentFlag = EAgentFlags::None)
        : payOffType(aPayOffType), unitDataList(aUnitDataList), taxSystem(aTaxSystem),
          payType(aPayType), agentFlag(aAgentFlag) {}
