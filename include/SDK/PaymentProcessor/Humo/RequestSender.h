...
public:
    ...
    typedef std::function<bool(
        const QByteArray &, QByteArray &, ESignatureType, const QByteArray &)>
        TResponseVerifier;
...
protected:
    ...
    Response *request(NetworkTask::Type aType,
                      const QUrl &aUrl,
                      Request &aRequest,
                      ESignatureType aSignatureType,
                      ESendError &aError,
    ...
    bool defaultRequestSigner(const QByteArray &aRequest,
                              QByteArray &aSignedRequest,
                              ESignatureType aSignatureType,
                              QByteArray &aSignature);

    /// Метод, производящий последние преобразования над заголовками и телом HTTP запроса перед
    /// отправкой.
    bool defaultRequestModifier(Request &aRequest,
                                NetworkTask::TByteMap &aHeaders,
                                QByteArray &aEncodedAndSignedData);

    /// Метод, проверяющий запросы по умолчанию.
    bool defaultResponseVerifier(const QByteArray &aSignedResponse,
