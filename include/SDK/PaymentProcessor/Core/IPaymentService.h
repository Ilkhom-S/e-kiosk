...
public:
    ...
    virtual QList<IPayment::SParameter>
    calculateCommission(const QList<IPayment::SParameter> &aParameters) = 0;
    ...
    virtual QMap<qint64, QList<SDK::PaymentProcessor::IPayment::SParameter>>
    getPaymentsFields(const QList<qint64> &aIds) = 0;

    /// Обновление поля платежа.
    virtual bool updatePaymentField(qint64 aPayment,
                                    const IPayment::SParameter &aField,
                                    bool aForceUpdate = false) = 0;

    /// Обновление полей платежа.
    virtual bool updatePaymentFields(qint64 aPayment,
                                     const QList<IPayment::SParameter> &aFields,
                                     bool aForceUpdate = false) = 0;

    /// Выполнение шага платежа в онлайне.
    virtual void
    processPaymentStep(qint64 aPayment, EPaymentStep::Enum aStep, bool aBlocking = false) = 0;

    /// Конвертация переданного платежа к типу aTargetType, поддерживаема этой фабрикой. В случае
