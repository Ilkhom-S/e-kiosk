...
public:
    ...
    virtual QVariantMap createFiscalTicket(qint64 aPaymentId,
                                           const QVariantMap &aPaymentParameters,
    ...
    virtual QStringList getReceipt(qint64 paymentId,
                                   const QVariantMap &aPaymentParameters,
                                   const SDK::Driver::SPaymentData &aPaymentData) = 0;

    /// Выполнить сервисную операцию (инкассация, Z-отчет, X-отчет).
    virtual bool serviceRequest(ERequestType::Enum aType,
                                QStringList &aReceipt,
