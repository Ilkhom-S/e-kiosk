...
public:
    ...
    struct SParameter {
        ...
        SParameter(QString aName,
                   QVariant aValue,
                   bool aUpdated = false,
                   bool aCrypted = false,
                   bool aExternal = false)
            : name(aName), value(aValue), crypted(aCrypted), updated(aUpdated),
              external(aExternal) {}

        /// Проверяет, является ли параметр пустым.
        inline bool isNull() const { return name.isEmpty(); }
