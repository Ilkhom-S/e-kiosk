    ...
    namespace PaymentProcessor {
        ...
        class IPaymentFactory {
          public:
            ...
            /// Конвертация переданного платежа к типу aTargetType, поддерживаемо этой фабрикой. В случае ошибки
            /// возвращает false.
            virtual bool convertPayment(const QString &aTargetType, IPayment *aPayment) = 0;

            /// Возвращает уточнённое описание для провайдера, который должен проводиться с помощью этой фабрики
            /// платежей. Если фабрика не может уточнить описание (или оператор сейчас использовать нельзя), то она
            /// возвращает невалидную структуру. Пример: пиновая фабрика постепенно загружает списки номиналов карт и
            /// обновляет их в настройках провайдера при вызове этого метода.
