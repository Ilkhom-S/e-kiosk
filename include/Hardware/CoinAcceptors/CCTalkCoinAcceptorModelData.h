...
public:
    CModelData() {
        ...
        data()["NRI"]["G13"].unsupported =
            CCCTalk::TUnsupported() << CCCTalk::Command::BaseYear << CCCTalk::Command::CreationDate
                                    << CCCTalk::Command::SoftLastDate;

        add("NRI", "BV", "Pelicano");
        data()["NRI"]["BV"].unsupported = CCCTalk::TUnsupported() << CCCTalk::Command::BaseYear
