...
#define STAR_VALUE(aParameters, aValue, aKey) aParameters.insert(aKey, QStringList() << aValue);
        ...
        add(ESTARMemorySwitchTypes::LoopHoldEjector,
            6,
            0x0,
            LPEType,
            LPEData,
        ...
        // TODO: отключено - нет у некоторых принтеров. Если команда установки ASB где-то не
        // работает - кастомизировать ASB-мем-свич и включить
        //  ASB - автовыдача статуса при его изменении
        // add(ESTARMemorySwitchTypes::ASB, 7, 0xC, false, CHardware::AutomaticStatus, "automatic
        // status emitting");

        // Presenter paper position status (ASB) - статус презентера включен в ответ на запрос
        // статуса
        add(ESTARMemorySwitchTypes::ASBPresenter,
            6,
            0x4,
            false,
            CHardware::Printer::PresenterStatusEnable,
            "presenter status enable",
            TModels() << TUP592 << TUP992 << UnknownEjector);

        // 180 degree rotation print - печатаем задом наперед и с задержками на каждой строе.
        add(ESTARMemorySwitchTypes::VerticalMountMode,
            2,
            0xC,
            false,
            CHardware::Printer::VerticalMountMode,
        ...
        add(ESTARMemorySwitchTypes::AutoRetractionTimeout,
            5,
            0x0,
            CHardware::Printer::AutoRetractionTimeout,
            PCTData,
            "presenter capture timer");

        // BM function.
        add(ESTARMemorySwitchTypes::BlackMark,
            1,
            0x8,
            false,
            CHardware::Printer::BlackMark,
            "black mark");
        ...
        add(ESTARMemorySwitchTypes::PowerOnReaction,
            6,
            0x8,
            CHardware::Printer::PowerOnReaction,
            POnRData592,
            "paper in presenter at power on",
            TModels() << TUP592);
        ...
        add(ESTARMemorySwitchTypes::Codepage,
            3,
            0x8,
            CHardware::Codepage,
            codepageData,
