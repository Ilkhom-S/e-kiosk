...
#include <Hardware/Printers/StarMemorySwitchesBase.h>
...
#define STAR_VALUE(aParameters, aValue, aKey) aParameters.insert(aKey, QStringList() << aValue);
                ...
                add(ESTARMemorySwitchTypes::LoopHoldEjector, 6, 0x0, LPEType, LPEData, "presenter mode",
                    TModels() << TUP592 << TUP992 << UnknownEjector);

                // TODO: отключено - нет у некоторых принтеров. Если команда установки ASB где-то не работает -
                // кастомизировать ASB-мем-свич и включить
                //  ASB - автовыдача статуса при его изменении
                // add(ESTARMemorySwitchTypes::ASB, 7, 0xC, false, CHardware::AutomaticStatus, "automatic status
                // emitting");

                // Presenter paper position status (ASB) - статус презентера включен в ответ на запрос статуса
                add(ESTARMemorySwitchTypes::ASBPresenter, 6, 0x4, false, CHardware::Printer::PresenterStatusEnable,
                    "presenter status enable", TModels() << TUP592 << TUP992 << UnknownEjector);

                // 180 degree rotation print - печатаем задом наперед и с задержками на каждой строе.
                add(ESTARMemorySwitchTypes::VerticalMountMode, 2, 0xC, false, CHardware::Printer::VerticalMountMode,
                    "180 degree rotation print");

                // Таймаут авторетракта
                TMSWParameters PCTData;
                fillExcept(PCTData, "0", "00000000");

                add(ESTARMemorySwitchTypes::AutoRetractionTimeout, 5, 0x0, CHardware::Printer::AutoRetractionTimeout,
                    PCTData, "presenter capture timer");
                ...
                add(ESTARMemorySwitchTypes::PowerOnReaction, 6, 0x8, CHardware::Printer::PowerOnReaction, POnRData592,
                    "paper in presenter at power on", TModels() << TUP592);

                TMSWParameters POnRData992;
                fillExcept(POnRData992, Cut, "1");

                add(ESTARMemorySwitchTypes::PowerOnReaction, 6, 0x8, CHardware::Printer::PowerOnReaction, POnRData992,
                    "paper in presenter at power on", TModels() << TUP992);
