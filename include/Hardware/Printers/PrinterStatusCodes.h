...
const int PaperEndVirtual =
    213; /// Бумага закончилась по показания датчика скорого окончания бумаги.
const int WrongFWConfiguration = 214; /// Неверная конфигурация прошивки.
const int OFDNotSent =
    215; /// Нет связи с ОФД сервером (виртуальный статус фискального сервера).
} // namespace Warning
...
const int PrinterFRCollapse =
    230; /// Глобальная ошибка принтера в составе ФР, печать невозможна.
...
const int OFDNotSent =
    237; /// Нет связи с ОФД сервером (виртуальный статус фискального сервера).
} // namespace Error
} // namespace PrinterStatusCode
