...
struct SMSWParameter {
    ...
    SMSWParameter(int aNumber,
                  int aIndex,
                  int aBitAmount,
                  const QStringList &aDataTypes,
                  const TMSWParameters &aParameters,
                  const QString &aDescription,
                  const TModels &aModels)
        : number(aNumber), index(aIndex), bitAmount(aBitAmount), dataTypes(aDataTypes),
          parameters(aParameters), description(aDescription), models(aModels) {}
};
...
public:
    ...
    bool update(const TMemorySwitchTypes &aSTARMemorySwitchTypes,
    ...
    bool getConfigParameter(ESTARMemorySwitchTypes::Enum aType,
                            const TMemorySwitches &aMemorySwitches,
                            QVariant &aValue);

protected:
    /// Добавить данные составного параметра мем-свича.
    void add(ESTARMemorySwitchTypes::Enum aParameterType,
             int aNumber,
             int aIndex,
             const QStringList &aDataTypes,
             const TMSWParameters &aParameters,
    ...
    bool getParameterValue(ESTARMemorySwitchTypes::Enum aParameterType,
                           const QVariantMap &aConfiguration,
                           QStringList &aValue);

    /// Проверить достаточность ключей конфига для работы с параметром мем-свича.
    bool hasConfigKeys(ESTARMemorySwitchTypes::Enum aParameterType,
                       const QVariantMap &aConfiguration);
    ...
    bool hasValidData(ESTARMemorySwitchTypes::Enum aParameterType,
                      const TMemorySwitches &aMemorySwitches);
...
protected:
    void add(ESTARMemorySwitchTypes::Enum aType,
             const QString &aName,
