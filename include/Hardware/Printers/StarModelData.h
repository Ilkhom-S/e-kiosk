...
struct SModelData {
    ...
    SModelData(bool aVerified,
               const QString &aDeviceId,
               double aMinFirmware,
               bool aCutter,
               bool aEjector,
               bool aHeadThermistor,
               bool aVoltageSensor,
               bool aInnerPaperEndSensor,
               int aFeeding3,
               int aFeeding4)
        : verified(aVerified), deviceId(aDeviceId), minFirmware(aMinFirmware), cutter(aCutter),
          ejector(aEjector), headThermistor(aHeadThermistor), voltageSensor(aVoltageSensor),
          innerPaperEndSensor(aInnerPaperEndSensor), feeding3(aFeeding3), feeding4(aFeeding4) {}
};
...
private:
    void add(const QString &aModel,
             const QString &aDeviceId,
             double aMinFirmware,
             bool aVerified = false,
             bool aCutter = true,
             bool aEjector = false,
             bool aHeadThermistor = false,
             bool aVoltageSensor = false,
             bool aInnerPaperEndSensor = true,
             int aFeeding3 = 5,
             int aFeeding4 = 4) {
        append(aModel,
               SModelData(aVerified,
                          aDeviceId,
                          aMinFirmware,
                          aCutter,
                          aEjector,
