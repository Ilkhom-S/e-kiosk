...
const int Natural2[] = {
    3, 7, 2, 4, 10, 3, 5, 9, 4, 6, 8}; /// Коэффициенты 2-го КЧ для ИНН физ. лица.
} // namespace Factors
...
const TStatusCodes XReportFiscalErrors = getFiscalStatusCodes(SDK::Driver::EWarningLevel::Error)
                                         ...
                                         << FRStatusCode::Error::EKLZ << FRStatusCode::Error::FM;
...
inline char joinData(const QList<char> &aData) {
    return std::accumulate(
        aData.begin(), aData.end(), ASCII::NUL, [](char aResult, char aLocalData) -> char {
            return aResult | aLocalData;
        });
}
...
public:
    void add(SDK::Driver::TVAT aVAT, int aGroup) { append(aVAT, SData(aGroup, aVAT)); }
};
...
struct SPayTypeData {
    ...
    SPayTypeData(int aValue, const QString &aDescription)
        : value(char(aValue)), description(aDescription) {}
};
...
#define ADD_OPERATION_MODE(aName)                                                                  \
    append(SDK::Driver::EOperationModes::aName, CFR::FiscalFields::aName##Mode)
    ...
    /// Константа для очистки режимов работы от мусора из ФН (ранее это был призак банковского
    /// агента).
