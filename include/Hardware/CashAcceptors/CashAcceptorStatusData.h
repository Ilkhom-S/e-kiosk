...
const TStatusSet SpecialStatuses = TStatusSet()
                                   << Rejected << Cheated << StackerFull << StackerOpen;
const TStatusSet BadSpecialStatuses = TStatusSet() << Cheated << StackerFull << StackerOpen;
const TStatusSet ErrorStatuses = TStatusSet()
                                 << Error << StackerFull << StackerOpen << MechanicFailure;
const TStatusSet MainStatuses = GeneralStatuses + SpecialStatuses;
const TStatusSet LongStatuses = NormalStatuses + BadStatuses + ErrorStatuses +
                                (TStatusSet() << Busy << BillOperation << Escrow);
} // namespace Set
