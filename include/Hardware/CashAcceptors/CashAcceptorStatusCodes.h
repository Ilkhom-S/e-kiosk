...
const int Inhibit =
    601; /// Выключен на прием денег, не может бы включен до отработки определенных действий.
const int Enabled = 602; /// Включен на прием денег.
} // namespace Normal

/// Работа с купюрой/монетой. OK.
namespace BillOperation {
const int Accepting = 620; /// Принимает купюру. Необходимо дать валидатору команду на укладывание в
                           /// стекер или выброс.
const int Escrow = 621; /// Принял купюру, и ждет дальнейшего решения: stack или return. Дается
                        /// информация о купюре.
...
const int Accept =
    666; /// Ошибка принятия купюры вследствие удержания извне //TODO: проверить, что это так.
const int EmptyInlet = 667;  /// Нет купюр на входе купюроприемника.
const int HostCommand = 668; /// Неправильный формат команды хоста.
const int Communication = 669; /// Ошибка связи с устройством (CRC, таймаут ожидания пакета либо
                               /// тех. посылки, внутренние ошибки).
const int Separating = 671; /// Ошибка разделения купюр, может приводить к потере денег.
const int Initialize =
    672; /// Warning. Ошибка стороннего ПО и/или прошивки устройства (например, работа с памятью).
...
const int NoteHighLocation =
    677; /// Неправильное расположение купюры в начале конвейера или около входа в конвейер.
} // namespace OperationError
...
const int SeparatingMotor =
    691; /// Неисправен мотор сепаратора (для купюроприемником с несколькими стекерами).
...
const int String =
    724;             /// Неисправен датчик "ленточного" мошенничества (выдергивание монеты лентой).
