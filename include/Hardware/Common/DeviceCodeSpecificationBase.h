...
struct SDeviceCodeSpecification {
  ...
  bool
  operator==(const SDeviceCodeSpecification &aDeviceCodeSpecification) const {
...
template <class T>
class DeviceCodeSpecificationBase
    : public CSpecification<T, SDeviceCodeSpecification> {
public:
  DeviceCodeSpecificationBase() {
    setDefault(SDeviceCodeSpecification(0, UnknownDeviceCodeDescription));
  }

  void appendStatus(T aCode, int aStatusCode,
