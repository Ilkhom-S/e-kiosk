...
class CommonDeviceCodeSpecification : public IDeviceCodeSpecification, protected DeviceCodeSpecificationBase<char> {
  ...
  protected:
    /// Начать ветку спецификации дополнительных девайс-кодов и установить для нее значение по умолчанию. Вызывать,
    /// когда все спецификации обычных девайс-кодов определены.
...
class BitmapDeviceCodeSpecification : public IDeviceCodeSpecification, protected DeviceCodeSpecificationBase<int> {
        ...
        foreach (int shift, mBuffer.keys()) {
            ...
            if (byteNumber < aBuffer.size()) {
                ...
                aSpecifications.insert(
                    "", SDeviceCodeSpecification(DeviceStatusCode::Error::Unknown, "answer size is too small"));
            }
        }
    }
  ...
  protected:
    ...
    virtual void addStatus(int aByte, int aBit, int aStatusCode, const QString &aDescription = "",
                           bool aInvert = false) {
