    ...
    namespace Plugin {
        ...
        inline TParameterList createSimpleNamedList(const QStringList &aModels, const QString &aDefault) {
            ...
            return TParameterList() << SPluginParameter(CHardwareSDK::ModelName, false, CPPT::ModelName, QString(),
                                                        aDefault, aModels, true)
                                    << SPluginParameter(CHardwareSDK::InteractionType, true, CPPT::InteractionType,
                                                        QString(), interactionType, QStringList() << interactionType)
            ...
            TParameterList create(const QStringList &aModels, const QString &aDefault) {
                ...
                return createSimpleNamedList<T1>(aModels, aDefault)
                       << SPluginParameter(CHardwareSDK::RequiredResource, SPluginParameter::Text, false,
                                           CPPT::RequiredResource, QString(), "Common.Driver.IOPort.System.COM",
                                           QVariantMap(), true)
                       << SPluginParameter(CHardwareSDK::OptionalPortSettings, false, QString(), QString(),
                                           optionalPortSettings[0], optionalPortSettings, true);
            }
        };
        ...
        template <class T1> struct SNamedList<T1, DSDKIT::ItOPOS> {
            TParameterList create(const QStringList &aModels, const QString &aDefault) {
                ...
                return createSimpleNamedList<T1>(aModels, aDefault)
                       << setMultipleExistence()
                       << SPluginParameter(CHardware::OPOSName, true, CPPT::OPOSName, QString(), possibleNames[0],
                                           possibleNames);
            }
        };
