/* @file Интерфейс клиента сторожевого сервиса. */
...
class IWatchServiceClient {
  ...
  public:
    ...
    /// слот с сигнатурой onCommandReceived(const QString& aSender, const
    /// QString& aTarget, const QString& aType, const QStringList & aTail).
    virtual bool subscribeOnCommandReceived(QObject *aObject) = 0;

    /// Подписывает aObject на получение оповещения от сервиса о требованием
    /// завершить работу. Объект должен иметь слот с сигнатурой
    /// onCloseCommandReceived().
    virtual bool subscribeOnCloseCommandReceived(QObject *aObject) = 0;

    /// Подписывает aObject на получение оповещения о разрыве связи со
    /// сторожевым сервисом. Объект должен иметь слот с сигнатурой
    /// onDisconnected().
    virtual bool subscribeOnDisconnected(QObject *aObject) = 0;

    /// Подписывает aObject на получение оповещения от сервиса о закрытии любого
    /// модуля. Объект должен иметь слот с сигнатурой onModuleClosed(const
    /// QString
    /// &).
