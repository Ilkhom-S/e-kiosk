...
class ICryptEngine
{
  public:
    ...
    /// Все нижеследующие public-функции thread-safe.
    ...
    virtual bool createKeyPair(int aKeyPair, CCrypt::ETypeEngine aEngine, const QByteArray &aKeyCard, int aKeySize,
                               QString &aErrorDescription) = 0;

    /// Загружает пару ключей в крипто-движок.
    virtual bool loadKeyPair(int aKeyPair, CCrypt::ETypeEngine aEngine, const QString &aSecretKeyPath,
                             const QString &aPassword, const QString &aPublicKeyPath, const ulong aSerialNumber,
                             const ulong aBankSerialNumber, QString &aErrorDescription) = 0;
    ...
    virtual bool signDetach(int aKeyPair, const QByteArray &aRequest, QByteArray &aSignature,
                            QString &aErrorDescription) = 0;

    /// Функции проверки ЭЦП

    /// Проверяет подпись в строке aResponseString парой ключей aKeyPair, оригинальное сообщение складывает в
    /// aOriginalResponse.
    virtual bool verify(int aKeyPair, const QByteArray &aResponseString, QByteArray &aOriginalResponse,
                        QString &aErrorDescription) = 0;

    /// Проверяет подпись aSignature для сроки aResponseString с помощью пары ключей aKeyPair.
    virtual bool verifyDetach(int aKeyPair, const QByteArray &aResponseString, const QByteArray &aSignature,
                              QString &aErrorDescription) = 0;

    /// Функции шифрования данных

    /// Шифрует aData и складывает результат в aResult. Используется пара ключей aKeyPair, тип ключа - aKey.
    virtual bool encrypt(int aKeyPair, const QByteArray &aData, QByteArray &aResult, CCrypt::ETypeKey aKey,
                         QString &aErrorDescription) = 0;
    ...
    /// Шифрует длинные данные aData и складывает результат в aResult. Используется пара ключей aKeyPair, тип ключа -
    /// aKey.
    virtual bool encryptLong(int aKeyPair, const QByteArray &aData, QByteArray &aResult, CCrypt::ETypeKey aKey,
                             QString &aErrorDescription) = 0;

    /// Шифрует длинные данные aData и складывает результат в aResult. Используется пара ключей aKeyPair, выбирается
    /// открытый ключ.
    virtual bool encryptLong(int aKeyPair, const QByteArray &aData, QByteArray &aResult,
                             QString &aErrorDescription) = 0;
    ...
    virtual bool decrypt(int aKeyPair, const QByteArray &aData, QByteArray &aResult, CCrypt::ETypeKey aKey,
                         QString &aErrorDescription) = 0;

    /// Расшифровывает aData и складывает результат в aResult. Используется пара ключей aKeyPair, выбирается закрытый
    /// ключ.
    virtual bool decrypt(int aKeyPair, const QByteArray &aData, QByteArray &aResult, QString &aErrorDescription) = 0;

    /// Расшифровывает длинные данные aData и складывает результат в aResult. Используется пара ключей aKeyPair, тип
    /// ключа - aKey.
    virtual bool decryptLong(int aKeyPair, const QByteArray &aData, QByteArray &aResult, CCrypt::ETypeKey aKey,
                             QString &aErrorDescription) = 0;

    /// Расшифровывает длинные данные aData и складывает результат в aResult. Используется пара ключей aKeyPair,
    /// выбирается закрытый ключ.
    virtual bool decryptLong(int aKeyPair, const QByteArray &aData, QByteArray &aResult,
                             QString &aErrorDescription) = 0;

    /// Функции хранения защищенного файла на токене

    /// Сохранить данные в токен
