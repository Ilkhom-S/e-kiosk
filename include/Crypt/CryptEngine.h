...
public:
    ...
    virtual bool createKeyPair(const QString &aPath,
    ...
    virtual bool createKeyPair(int aKeyPair,
    ...
    virtual bool loadKeyPair(int aKeyPair,
                             CCrypt::ETypeEngine aEngine,
    ...
    virtual bool
    exportSecretKeyToFile(int aKeyPair, const QString &aFilePath, const QByteArray &aPassword);
    ...
    virtual bool
    exportPublicKeyToFile(int aKeyPair, const QString &aFilePath, ulong &aSerialNumber);

    /// Функции подписывания данных ЭЦП

    /// Подписывает переданный запрос парой ключей aKeyPair.
    virtual bool sign(int aKeyPair,
                      const QByteArray &aRequest,
                      QByteArray &aSignature,
                      QString &aErrorDescription);

    /// Подписывает данные из запроса aRequest парой ключей aKeyPair и возвращает подпись в
    /// aSignature.
    virtual bool signDetach(int aKeyPair,
                            const QByteArray &aRequest,
                            QByteArray &aSignature,
                            QString &aErrorDescription);
    ...
    /// Проверяет подпись в строке aResponseString парой ключей aKeyPair, оригинальное сообщение
    /// складывает в aOriginalResponse.
    virtual bool verify(int aKeyPair,
    ...
    virtual bool verifyDetach(int aKeyPair,
                              const QByteArray &aResponseString,
    ...
    /// Шифрует aData и складывает результат в aResult. Используется пара ключей aKeyPair, тип ключа
    /// - aKey.
    virtual bool encrypt(int aKeyPair,
                         const QByteArray &aData,
                         QByteArray &aResult,
                         CCrypt::ETypeKey aKey,
                         QString &aErrorDescription);

    /// Шифрует aData и складывает результат в aResult. Используется пара ключей aKeyPair,
    /// выбирается открытый ключ.
    virtual bool
    encrypt(int aKeyPair, const QByteArray &aData, QByteArray &aResult, QString &aErrorDescription);

    /// Шифрует длинные данные aData и складывает результат в aResult. Используется пара ключей
    /// aKeyPair, тип ключа - aKey.
    virtual bool encryptLong(int aKeyPair,
                             const QByteArray &aData,
                             QByteArray &aResult,
                             CCrypt::ETypeKey aKey,
                             QString &aErrorDescription);
    ...
    /// Шифрует длинные данные aData и складывает результат в aResult. Используется пара ключей
    /// aKeyPair, выбирается открытый ключ.
    virtual bool encryptLong(int aKeyPair,
                             const QByteArray &aData,
                             QByteArray &aResult,
                             QString &aErrorDescription);
    ...
    /// Расшифровывает aData и складывает результат в aResult. Используется пара ключей aKeyPair,
    /// тип ключа - aKey.
    virtual bool decrypt(int aKeyPair,
                         const QByteArray &aData,
                         QByteArray &aResult,
                         CCrypt::ETypeKey aKey,
                         QString &aErrorDescription);
    ...
    virtual bool
    decrypt(int aKeyPair, const QByteArray &aData, QByteArray &aResult, QString &aErrorDescription);

    /// Расшифровывает длинные данные aData и складывает результат в aResult. Используется пара
    /// ключей aKeyPair, тип ключа - aKey.
    virtual bool decryptLong(int aKeyPair,
                             const QByteArray &aData,
                             QByteArray &aResult,
                             CCrypt::ETypeKey aKey,
                             QString &aErrorDescription);
    ...
    /// Расшифровывает длинные данные aData и складывает результат в aResult. Используется пара
    /// ключей aKeyPair, выбирается закрытый ключ.
    virtual bool decryptLong(int aKeyPair,
                             const QByteArray &aData,
                             QByteArray &aResult,
                             QString &aErrorDescription);
