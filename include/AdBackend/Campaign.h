...
struct Campaign {
    ...
    bool isValid() const { return !type.isEmpty() && id > 0; }

    /// Возвращает true, если кампания баннерная
    bool isBanner() const { return (type.contains("banner", Qt::CaseInsensitive) == true); }

    /// Проверка, это кампания по умолчанию?
    bool isDefault() const { return type.endsWith(DefaultChannelPostfix, Qt::CaseInsensitive); }

    /// Сравнение двух кампаний
    bool isEqual(const Campaign &aCampaign) const {
        return id == aCampaign.id && type == aCampaign.type &&
               url.toString() == aCampaign.url.toString() && md5 == aCampaign.md5 &&
               text == aCampaign.text && expired == aCampaign.expired;
    }

    /// Принятие решения о необходимости скачать
    bool isDownloaded() const { return !url.isEmpty() && !md5.isEmpty(); }

    bool isExpired() const { return QDateTime::currentDateTime() > expired || expired.isNull(); }
