...
public:
    ...
    struct SExternalStateHook {
        ...
        SExternalStateHook() {}
        SExternalStateHook(const QString &aTargetScenario,
                           const QString &aTargetState,
                           THookFunction aHook)
    ...
    virtual void signalTriggered(const QString &aSignal,
                                 const QVariantMap &aArguments = QVariantMap()) = 0;
...
signals:
    /// Вошли в состояние
    void
    enteredState(const QString &aScenario, const QString &aState, const QVariantMap &aArguments);
