...
public:
  ...
  static IDatabaseProxy *getInstance(IDatabaseQueryChecker *aErrorChecker,
                                     Type aType = SQLite);
  ...
  virtual bool
  open(const QString &aDatabaseName = CIDatabaseProxy::DefaultDatabase,
  ...
  /// Выполнение DML запроса. Помещает в rowsAffected количество затронутых
  /// строк.
  virtual bool execDML(const QString &aQuery, long &aRowsAffected) = 0;

  /// Выполнение запроса, содержащего, к примеру, COUNT(*). В result записывает
  /// значение ячейки (1,1).
  virtual bool execScalar(const QString &aQuery, long &aResult) = 0;

  /// Выполнение произвольного запроса. Если запрос успешно выполнен, то
  /// результат функции будет не нулевым.
