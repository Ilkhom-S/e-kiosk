...
void testEnumPhonebookEntries() {
    ...
    if (e.isValid()) {
        ...
        std::wcout << "Failed to initialize PhonebookEntryEnumerator: " << e.getLastError()
                   << std::endl;
    }
}
...
void testEnumConnections() {
    ...
    if (c.isValid()) {
        ...
        std::wcout << "Failed to initialize ConnectionEnumerator: " << c.getLastError()
                   << std::endl;
    }
}
...
void testConnectionStatus() {
    ...
    for (std::list<std::wstring>::iterator i = gConnections.begin(); i != gConnections.end(); ++i) {
        ...
        std::wcout << *i
                   << (status == RasApi::EConnectionStatus::Connected ? ": connected "
                                                                      : ": disconnected ")
                   << std::endl;
    }
}
