...
private slots:
    ...
    void testStackTraceConsistencyAcrossPlatforms() {
        ...
        if (!hasAddress) {
            qDebug()
                << "No memory addresses found in stack trace (may be normal for some platforms)";
        }
    }
    ...
    void cleanupTestCase() { qDebug() << "DebugUtils cross-platform tests completed successfully"; }
};
