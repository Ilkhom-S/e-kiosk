...
void BaseValidatorDevices::printDataToHex(const QByteArray &data)
{
    ...
    for (int i=0;i<data.size();i++) {
        ...
        baTmp.append(QByteArray::number(n, 16).toUpper());z
    }
#endif
...
bool BaseValidatorDevices::sendCommand(QByteArray dataRequest, bool getResponse, int timeResponse,  QByteArray& dataResponse, int timeSleep, bool readAll)
{
    bool respOk = false;
    ...
    if(this->isOpened()){
        ...
        if(timeSleep > 0) this->msleep(timeSleep);
