...
//                if (!serialPort->setCharIntervalTimeout(CMDCitizenCBM1000::charTimeOut)) return false;
                if (!serialPort->setBaudRate(QSerialPort::Baud19200)) return false;

                //if(Debuger) qDebug() << "\nPrinter " << CMDCitizenCBM1000::DeviceName << " to Port " << devicePort->deviceName() << " open in " << devicePort->openMode();
