...
private:
	...
	void getSystemInfo()
	{
		...
		QVariantMap ramInfo = getWmicInfo(
			QString("path win32_physicalmemory get devicelocator, manufacturer, capacity, speed, memorytype"), true);
		...
		diskInfo["serialnumber"] =
			getWmicInfo("path win32_physicalmedia where tag='\\\\\\\\.\\\\PHYSICALDRIVE0' get serialnumber")
				.value("serialnumber")
				.toString()
				.trimmed();
