...
private:
  ...
  void getSystemInfo() {
    ...
    // "----------------- OS ----------------- ";
    QVariantMap osInfo =
        getWmicInfo(QString("os get caption, version, csname, csdversion%1")
    ...
    // "----------------- CPU ----------------- ";
    QVariantMap cpuInfo =
        getWmicInfo("cpu get name, numberofcores, numberoflogicalprocessors");
    sysInfo["cpu"] = cpuInfo;

    // "----------------- Motherboard ----------------- ";
    QVariantMap mboardInfo = getWmicInfo(
    ...
    QVariantMap ramInfo =
        getWmicInfo(QString("path win32_physicalmemory get devicelocator, "
    ...
    for (auto &r : ramInfo.value("data").toList()) {
      ...
      QString divider =
          ramInfo.value("manufacturer").toString() != "" ? "|" : "";
    ...
    diskInfo["serialnumber"] =
        getWmicInfo("path win32_physicalmedia where "
    ...
    if (isWinXP) {
      QString s = QByteArray::fromHex(
          diskInfo.value("serialnumber").toString().toLatin1());
      QString sn = "";
    ...
    QVariantMap lDiskInfo =
        getWmicInfo("logicaldisk get caption, size, freespace", true);
    ...
    for (auto &ld : lDiskInfo.value("data").toList()) {
      ...
      auto lDiskFreeSpace =
          lDisk.value("freespace").toDouble() / (1024 * 1024 * 1024);

      QString divider =
          diskInfo.value("diskCaptions").toString() != "" ? ", " : "";
    ...
    diskInfo["size"] =
        qRound(diskInfo.value("size").toDouble() / (1024 * 1024 * 1024));
