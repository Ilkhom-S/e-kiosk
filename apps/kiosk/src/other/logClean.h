...
private:
  void removeTmpDir() {
    ...
    QDirIterator directories(parent_folder, QDir::Dirs | QDir::NoSymLinks |
                                                QDir::NoDotAndDotDot);
  ...
  bool removeFile(const QString &dirName) {
    ...
    if (dir.exists(dirName)) {
      Q_FOREACH (QFileInfo info, dir.entryInfoList(QDir::Files)) {
        QDate _logDate =
            QDate::fromString(info.fileName().left(10), "dd.MM.yyyy");
    ...
    if (dir.exists(dirPath)) {
      Q_FOREACH (QFileInfo info,
                 dir.entryInfoList(QDir::NoDotAndDotDot | QDir::System |
                                   ...
                                   QDir::DirsFirst)) {
        QDate _logDate =
            QDate::fromString(info.fileName().left(10), "yyyy-MM-dd");
