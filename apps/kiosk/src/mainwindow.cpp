...
MainWindow::MainWindow(QWidget *parent)
  ...
  createConnection(db, QString("%1%2").arg(ConstData::Path::Config,
                                           ConstData::FileName::DBName));

  createConnectionFile(dbUpdater,
                       QString("%1%2").arg(ConstData::Path::Config,
                                           ConstData::FileName::DBFile));

  auto logData = QString("EKiosk VERSION - %1\n").arg(ConstData::version);

  toLog(LogLevel::Info, senderName, logData);
...
void MainWindow::init() {
  ...
  connect(adminDialog, SIGNAL(emit_unlockOpenAdminSts()),
          SLOT(unlockAdminOpenSts()));
  ...
  connect(connObject, SIGNAL(emit_ConnectionError()), this,
          SLOT(connectionError()));
  connect(connObject, SIGNAL(emit_Ping(bool)), this,
          SLOT(connectionResult(bool)));
  ...
  connect(connObject, SIGNAL(emit_ConnectionUp()), this,
          SLOT(connectionUpState()));
  connect(connObject, SIGNAL(emit_checkConState()), this,
          SLOT(connectionCheck()));
  ...
  connect(searchDevices, SIGNAL(emitDeviceSearchFinished()),
          SLOT(deviceSearchFinished()));
  ...
  if (checkUserInBase()) {
    ...
    connect(registrationForm, SIGNAL(emitTpl(QString, bool)),
            SLOT(tplSelected(QString, bool)));
    connect(registrationForm,
    ...
    if (!dialDev) {
      toLog(LogLevel::Error, "CONNECTION",
            "На терминале нет установленных модемов...");
    }
...
void MainWindow::checkConfigData(bool skipSearchDevice) {
  ...
  connect(clsValidator, SIGNAL(eNominalDuplicate(int)),
          SLOT(nominalDuplicateGet(int)));
  connect(clsValidator, SIGNAL(showHideDialogAnimate(bool)),
  ...
  connect(clsCoinAcceptor, SIGNAL(eNominalDuplicate(int)),
          SLOT(coinDuplicateGet(int)));
  ...
  connect(watchDogs, SIGNAL(commandDone(bool, int)),
          SLOT(cmdWatchdogDone(bool, int)));
  ...
  loadingGprs->setCopirightText(QString("%1").arg(info),
  ...
  sDevicesForm->setCopirightText(QString("%1").arg(info),
                                 "№ " + config.terminalData.login);
  ...
  connect(getServices, SIGNAL(emit_getServices(bool)), this,
          SLOT(getServicesReturn(bool)));
  connect(getServices, SIGNAL(emit_responseBalance(double, double, double)),
          this, SLOT(getBalanceUser(double, double, double)));
  connect(getServices, SIGNAL(emit_responseIsActive(bool)), this,
          SLOT(isActiveLock(bool)));
  connect(getServices, SIGNAL(lockUnlockAvtorization(bool, int)), this,
          SLOT(avtorizationLockUnlock(bool, int)));
  connect(getServices, SIGNAL(emit_ErrResponse()), this,
          SLOT(getServicesError()));
  connect(getServices, SIGNAL(emit_timeServer(QString)), connObject,
          SLOT(setDateTimeIn(QString)));
  connect(getServices, SIGNAL(emit_banners(QVariantList)), this,
          SLOT(getBanners(QVariantList)));
  ...
  connect(mainPage, SIGNAL(emit_sendStatusValidator()), this,
          SLOT(validatorStatusGet()));
  connect(mainPage, SIGNAL(validator_activate(bool)), this,
          SLOT(validatorInit(bool)));
  connect(mainPage, SIGNAL(validator_activate(bool)), this,
          SLOT(coinAcceptorInit(bool)));
  connect(mainPage, SIGNAL(emit_toLoging(int, QString, QString)),
          SLOT(toLog(int, QString, QString)));
  connect(mainPage, SIGNAL(emit_openAvtorizationDialog()),
          SLOT(openAdminAuthDialog()));
  ...
  connect(payDaemons, SIGNAL(emit_Loging(int, QString, QString)), this,
          SLOT(toLog(int, QString, QString)));
  connect(payDaemons, SIGNAL(emit_to_print(QString)),
          SLOT(toPrintText(QString)));
  connect(payDaemons, SIGNAL(emit_responseBalance(double, double, double)),
          this, SLOT(getBalanceUser(double, double, double)));
  connect(payDaemons, SIGNAL(lockUnlockNonSend(bool)), this,
          SLOT(nonSendPaymentLock(bool)));
  connect(payDaemons, SIGNAL(emit_RestartNet()), SLOT(startToConnection()));
  connect(payDaemons, SIGNAL(emit_RestartTerminal()),
          SLOT(restartTerminalInit()));
  ...
  connect(mainPage, SIGNAL(emit_print_pay(QString)), payDaemons,
          SLOT(get_print_id(QString)));
  connect(mainPage, SIGNAL(emit_checkStatus54()), payDaemons,
          SLOT(checkPayStatus54()));
  ...
  connect(statusDaemons, SIGNAL(getRequestParam()), this,
          SLOT(getDataToSendStatus()));
  connect(statusDaemons, SIGNAL(emit_Loging(int, QString, QString)), this,
          SLOT(toLog(int, QString, QString)));
  connect(statusDaemons, SIGNAL(emit_responseBalance(double, double, double)),
          this, SLOT(getBalanceUser(double, double, double)));
  connect(statusDaemons, SIGNAL(emit_responseIsActive(bool)), this,
          SLOT(isActiveLock(bool)));
  ...
  connect(statusDaemons, SIGNAL(emit_hashToCheck(QString)), this,
          SLOT(checkHash(QString)));
  ...
  connect(userDaemons, SIGNAL(emit_UserData(QString)), this,
          SLOT(setTerminalInfo(QString)));
  ...
  connect(
      ...
      checkOnline,
      SLOT(sendCheckOnlineRequest(QString, QString, QString, double,
                                  QVariantMap)));

  // Демон для отправки эл.чека
  sendReceipt = new SendReceipt(this);
  sendReceipt->setUrl(config.serverAddress);
  connect(sendReceipt, SIGNAL(emitSendReceiptResult(QString, QString, QString)),
          mainPage, SLOT(sendReceiptResult(QString, QString, QString)));
  ...
  connect(mainPage, SIGNAL(emit_otpSend(QString)), sendOtp,
          SLOT(sendOtpRequest(QString)));
  ...
  connect(
      mainPage,
      SIGNAL(emit_sendJsonRequest(QJsonObject, QString, QString, int, int,
                                  QVariantMap)),
      jsonRequest,
  ...
  connect(statusValidatorTimer, SIGNAL(timeout()), this,
          SLOT(validatorStatusGetOnMain()));

  statusCoinAcceptorTimer = new QTimer(this);
  connect(statusCoinAcceptorTimer, SIGNAL(timeout()), this,
          SLOT(coinAcceptorStatusGetOnMain()));

  incasaciyaForm = new IncasaciyaForm(this);
  connect(incasaciyaForm, SIGNAL(execCommand(int)),
          SLOT(getCommandFromIncash(int)));
  connect(incasaciyaForm, SIGNAL(openDialog()),
          SLOT(openAdminAuthEditDialog()));
  ...
  rebootCountSet();
  QTimer::singleShot(config.timerRasReb * 60000, this,
                     SLOT(rebootCountClear()));
  ...
  toLog(LogLevel::Info, senderName, "Поиск устройств...");
  ...
  connect(adminAuthDialog, SIGNAL(emit_openAdminDialog()),
          SLOT(openAdminDialog()));
...
void MainWindow::getServicesRequest() {
  getServices->setAuthData(config.terminalData.token, systemHashGet(),
                           ConstData::version);
...
void MainWindow::getServicesReturn(bool result) {
  if (result) {
    ...
    if (adminDialog->isVisible()) {
      auto data =
          QVariantMap({{"message", "Конфигурация получена успешно..."}});
...
void MainWindow::getServicesError() {
  ...
  if (adminDialog->isVisible()) {
    auto data = QVariantMap(
        {{"message",
          "Истёк таймаут ожидания запроса\nна получение конфигурации..."}});
    adminDialog->setDataToAdmin(AdminCommand::aCmdInfoGetServices, data);
    adminDialog->authButtonSet(true);
  }
...
void MainWindow::connectionError() {
  toLog(LogLevel::Error, "CONNECTION",
        "Ошибка поднятия соединения с " + config.vpnName);
}
...
void MainWindow::connectionError(QString errNum, QString errComment) {
  ...
  case 756: {
    ...
    toLog(LogLevel::Info, "MAIN",
          QString("Количество перезагрузок по ошибке 756 - %1")
              .arg(count_reboot));
...
void MainWindow::rebootModemEntries() {
  ...
  if (config.WDData.port.contains("COM")) {
    ...
    toLog(LogLevel::Info, "WatchDog",
          QString("Даем каманду на перезагрузку модема..."));
...
void MainWindow::getSystemInfo(QVariantMap data) {
  ...
  toLog(LogLevel::Info, senderName, "GET SYSINFO FINISHED");
  QJsonDocument json = QJsonDocument::fromVariant(config.systemInfo);
  toLog(LogLevel::Info, senderName,
        QString(json.toJson(QJsonDocument::Compact)));
}
...
void MainWindow::connectionUpState() {
  ...
  toLog(LogLevel::Info, "CONNECTION",
        "Соединение с " + config.vpnName + " установлено.");
...
void MainWindow::connectionCheck() {
  ...
  // В начале надо проверить соединение с сервером
  toLog(LogLevel::Info, "CONNECTION",
        "Начинаем проверять соединение с сервером");
...
bool MainWindow::checkUserInBase() {
  ...
  config.terminalData.secretLogin =
      decodeStr(vrmSecretLogin, config.coddingKey);
...
bool MainWindow::saveTerminalAuthData(bool update, QString login,
                                      QString token) {
  ...
  if (update) {
    userQuery = QString(
        "UPDATE terminal_data SET login = '%1', token = '%2' WHERE id = 1;");
  } else {
    userQuery = QString(
        "INSERT INTO terminal_data(id, login, token) VALUES(1, '%1', '%2');");
  }
...
bool MainWindow::saveAdminAuthData(bool update, QString secretLogin,
...
void MainWindow::openAdminAuthDialog() {
  ...
  adminAuthDialog->setAuthParam(config.terminalData.secretLogin,
                                config.terminalData.secretPass);
...
void MainWindow::createDialUpConnection(QVariantMap data) {
  ...
  // Создаем соединение
  int status = connObject->createNewDialupConnection(conName, devName, phone,
                                                     login, pass);
...
void MainWindow::startToConnect(QString connection) {
  ...
  if (connection.toUpper() == "LOCAL CONNECTION") {
    ...
    if (isModemConnectionUp(connectionName)) {
      ...
      registrationForm->setStatusText(
          1, tr("Соединение с %1 поднято").arg(connectionName));
...
void MainWindow::sendAuthRequest(QString login, QString otp) {
  ...
  auto cid =
      collectDaemons->firstCollection ? collectDaemons->getCollectionId() : "";

  authRequest->sendAuthRequest(login, otp, hash, cid);
}

void MainWindow::authResponse(QString resultCode, QString login, QString token,
                              QString message) {
  if (registrationForm) {
  ...
  if (resultCode != "" && resultCode.toInt() == 0 && token != "") {
    // Сохраняем данные
    saveTerminalAuthData(false, login, token);
    saveAdminAuthData(true, config.terminalData.secretLogin,
                      config.terminalData.secretPass);
  ...
  if (resultCode == "") {
    ...
    toLog(LogLevel::Warning, "AUTH", "Сервер недоступен или нет связи");
  } else {
    ...
    toLog(LogLevel::Error, "AUTH", message);
...
QString MainWindow::systemHashGet() {
  ...
  QString uuid = diskModel + diskSerial + mboardProduct + mboardSerialnumber +
                 QDir::currentPath();
  QString hash =
      QCryptographicHash::hash(uuid.toUtf8(), QCryptographicHash::Sha256)
          .toHex();
...
void MainWindow::openAdminDialog() {
  ...
  adminDialog->setDataToAdmin(AdminCommand::aCmdInfoGetServices,
                              QVariantMap({{"message", ""}}));
  ...
  QString vrmValidatorInfo = QString("firmware: %1\n"
                                     "serial: %2")
                                 .arg(config.validatorData.partNumber,
                                      config.validatorData.serialNumber);
  ...
  QString vrmCoinAcceptorInfo = QString("model:  %1\n"
                                        "serial:   %2")
                                    .arg(config.coinAcceptorData.partNumber,
                                         config.coinAcceptorData.serialNumber);
  ...
  data["chek_width"] = config.printerData.chekWidth;
  ...
  data["status_validator_jam_in_box_value_counter"] =
      config.statusValidatorJamInBoxValueCounter;
  data["status_validator_jam_in_box_lockers"] =
...
void MainWindow::getCommandFromAdmin(AdminCommand::AdminCmd cmd) {
  ...
  case AdminCommand::aCmdHtmlIncash: {
    ...
    QString trnTo = "";
    QString htmlCenter = collectDaemons->getHtmlInfoBox(
        nonCollectPay, moneyOutCount, moneyOutSum, dateCollect, cId, cTrn,
        trnFrom, trnTo);
  ...
  case AdminCommand::aCmdHideExplorer: {
    proc.startDetached("taskkill", QStringList()
                                       << "/f" << "/IM" << "explorer.exe");
  } break;
  case AdminCommand::aCmdPrintTestCheck: {
    ...
    if (prtStatus) {
      ...
      toLog(LogLevel::Error, "PRINTER",
            QString("Статус (%1)").arg(vrmLst.at(0)));
      return;
    }
  ...
  case AdminCommand::aCmdSaveDeviceParamR: {
    ...
    saveDevice(1, validatorName, validatorPort, config.validatorData.comment,
               1);
    saveDevice(2, printerName, printerPort, printerComment, 1);
    saveDevice(3, config.modemData.name, modemPort, config.modemData.comment,
               1);
    saveDevice(4, config.WDData.name, watchdogPort, config.WDData.comment, 1);
    saveDevice(5, coinAcceptorName, coinAcceptorPort,
               config.coinAcceptorData.comment, 1);

    toLog(LogLevel::Info, "MAIN", "Устройства успешно сохранены.");
  ...
  case AdminCommand::aCmdGetSimInfo: {
    if (connObject->conState != Connection::GetSimData) {
      ...
      if (isModemConnectionUp(connectionName)) {
        data["message"] = QString("Подождите идет разрыв соединения %1...")
                              .arg(connectionName);
        ...
        data["message"] =
            QString(
                "Соединение %1 опущено, начинаем проверять данные SIM карты...")
                .arg(connectionName);
      } else {
      ...
      if (config.modemData.port != "") {
        ...
        if (clsModem->isItYou(modemComment)) {
          ...
          config.modemData.comment =
              modemComment.replace("'", "").replace(".", "");
  ...
  case AdminCommand::aCmdSaveConnParam: {
    ...
    toLog(LogLevel::Info, "MAIN", "Параметры соединения успешно сохранены.");
  ...
  case AdminCommand::aCmdSaveTrmNumSett: {
    ...
    if (deleteTerminalData()) {
      ...
      toLog(LogLevel::Error, "AUTH",
            "Ошибка базы данных при удалении данных терминала");
...
void MainWindow::validatorStatusGetOnMain() {
  ...
  if (page != PageIn::InputNumber && page != PageIn::InputSum &&
      page != PageIn::PrintDialog) {
...
void MainWindow::coinGet(int coin) {
  ...
  if (coin < 100) {
    toLog(LogLevel::Info, "COIN_ACCEPTOR",
          QString("Вставлена монета - %1 дирам").arg(coin));
  } else {
    toLog(LogLevel::Info, "COIN_ACCEPTOR",
...
void MainWindow::incameStatusFromValidator(int sts, QString comment) {
  ...
  if (sts >= 1 && sts < 30) {
    ...
    if (sts == VStatus::Errors::BadStackerPosition) {
      ...
      if (saveBillValidatorEvent("CASHBOX-OPENED",
                                 dt.toString(Qt::ISODateWithMs))) {
        bValidatorEventCheck();
      }
    }

    // Если есть замятие то надо оповестить
    if (sts == VStatus::Errors::ValidatorJammed ||
        sts == VStatus::Errors::StackerJammed) {
...
void MainWindow::deviceSearchResult(int device, int result, QString dev_name,
                                    QString dev_comment, QString dev_port) {
  if (registrationForm) {
    registrationForm->deviceSearchResult(device, result, dev_name, dev_comment,
                                         dev_port);
    return;
  }
  ...
  case SearchDev::search_validator: {
    ...
    case SearchDev::start_search: {
      sDevicesForm->setValidatorSearchText(
          SearchDev::start_search, interfaceText("validator_info_searching"));
      toLog(LogLevel::Info, senderName,
            interfaceText("validator_info_searching"));
    } break;
    case SearchDev::device_found: {
      auto inf =
          interfaceText("validator_info_found")
    ...
    case SearchDev::device_notfound: {
      sDevicesForm->setValidatorSearchText(
          SearchDev::device_notfound, interfaceText("validator_info_notfound"));
  ...
  case SearchDev::search_coin_acceptor: {
    ...
    case SearchDev::device_found: {
      auto inf =
          interfaceText("coin_acceptor_info_found")
              .replace("[v1]", QString("%1 %2").arg(dev_name, dev_comment))
              .replace("[v2]", dev_port);
  ...
  case SearchDev::search_printer: {
    switch (result) {
    case SearchDev::start_search: {
      sDevicesForm->setPrinterSearchText(
          SearchDev::start_search, interfaceText("printer_info_searching"));
      toLog(LogLevel::Info, senderName,
            interfaceText("printer_info_searching"));
    } break;
    case SearchDev::device_found: {
      auto inf =
          interfaceText("printer_info_found")
              .replace("[v1]", QString("%1 %2").arg(dev_name, dev_comment))
              .replace("[v2]", dev_port);
    ...
    case SearchDev::device_notfound: {
      sDevicesForm->setPrinterSearchText(
          SearchDev::device_notfound, interfaceText("printer_info_notfound"));
      toLog(LogLevel::Error, senderName,
  ...
  case SearchDev::search_modem: {
    ...
    case SearchDev::device_found: {
      auto inf =
          interfaceText("modem_info_found")
              .replace("[v1]", QString("%1 %2").arg(dev_name, dev_comment))
              .replace("[v2]", dev_port);
...
void MainWindow::deviceSearchFinished() {
  ...
  toLogingData +=
      "- Уровень сигнала модема   = " + searchDevices->signalQuality + "%" +
      "\n";
  toLogingData +=
      "- Коментарий к модему      = " + searchDevices->modemComment + "\n";
  toLogingData += "- Сим карта присутствует   = " +
                  QString("%1").arg(searchDevices->nowSimPresent) + "\n";
  toLogingData +=
      "- Оператор сим карты       = " + searchDevices->operatorName + "\n";
  toLogingData +=
      "- Баланс сим карты         = " + searchDevices->simBalance + "\n";
  toLogingData +=
      "- Номер сим каты           = " + searchDevices->simNumber + "\n";
  ...
  config.validatorData.partNumber =
      searchDevices->validatorPartNum.replace("'", "");
  config.validatorData.serialNumber = searchDevices->validatorSerialNum;

  config.coinAcceptorData.partNumber =
      searchDevices->coinAcceptorPartNum.replace("'", "");
...
void MainWindow::devicesInitialization() {
  ...
  clsPrinter->setChekWidth(config.printerData.chekWidth);
...
void MainWindow::deviceTest(int device, QString name, QString port,
                            QString comment) {
  QString msgOk = "";
  QString msgError = "";
  ...
  auto receipt = receiptGet(config.tpl)
                     .arg("201801011111110000", "000000", "2018-01-01 11:11:11",
                          "test test", "", "Beeline", "999999999", "00", "00",
                          "0", "44-640-5544");
...
bool MainWindow::getDeviceFromDB() {
  ...
  while (select.next()) {
    ...
    case 1: {
      ...
      if (state == "0") {
        incameStatusFromValidator(VStatus::Errors::NotAvailable,
                                  "Валидатор не найден");
      }
    } break;
    case 2: {
...
void MainWindow::statusPrinter(int status) {
  ...
  if (status == PrinterState::PrinterOK ||
      status & PrinterState::PaperNearEnd) {
...
void MainWindow::toSendMonitoringStatus() {
  ...
  if (oneSendStatus) {
    ...
    config.coinAcceptorData.partNumber =
        config.coinAcceptorData.partNumber.replace("'", "");
...
void MainWindow::openValidatorBox() {
  ...
  QString trnTo = "";
  QString htmlCenter =
      collectDaemons->getHtmlInfoBox(nonCollectPay, moneyOutCount, moneyOutSum,
...
void MainWindow::cmdWatchdogDone(bool state, int aCommand) {
  ...
  if (aCommand == WDProtocolCommands::ResetModem) {
    ...
    toLog(LogLevel::Info, "WatchDogs", stateNote);
...
void MainWindow::setLockList() {
  ...
  lockList[Lock::ErrorValidator].comment =
      "Заблокирован из за ошибки валидатора";

  lockList[Lock::ErrorInterface].lock = false;
  lockList[Lock::ErrorInterface].comment =
      "Заблокирован из за проблемы с интерфейсом";
  ...
  lockList[Lock::ErrorDevice].comment =
      "Заблокирован по ошибке определения устройств";
  ...
  lockList[Lock::Status_11].comment =
      "Заблокирован по статусу терминал не активен";
  ...
  lockList[Lock::ErrorAvtorizat].comment =
      "Заблокирован из за ошибки авторизации";

  lockList[Lock::ErrorTypeTrm].lock = false;
  lockList[Lock::ErrorTypeTrm].comment =
      "Заблокирован по статусу неверный тип терминала";

  lockList[Lock::ErrorNoRoulPay].lock = false;
  lockList[Lock::ErrorNoRoulPay].comment =
      "Заблокирован по статусу нет прав на прием платежей";

  lockList[Lock::MorePayIn].lock = false;
  lockList[Lock::MorePayIn].comment =
      "Заблокирован по причине в системе 1 и более платежей";

  lockList[Lock::ErrorOpenAdminP].lock = false;
  lockList[Lock::ErrorOpenAdminP].comment =
      "Заблокирован по причине открыта админка";

  lockList[Lock::ErrorDublicateNominal].lock = false;
  lockList[Lock::ErrorDublicateNominal].comment =
      "Заблокирован из за дубликата купюры";

  lockList[Lock::ErrorDatabase].lock = false;
  lockList[Lock::ErrorDatabase].comment =
      "Заблокирован из за ошибки базы данных";
}
...
void MainWindow::getCommandFromServer(QVariantList cmdList) {
  ...
  log = QString("Количество поступивших команд с сервера = %1").arg(cmdCount) +
        log;
...
bool MainWindow::getActiveCommand(QString &trn, int &cmd, QString &account,
...
bool MainWindow::commandStatusUpdate(QString trn, QString status) {
  ...
  strUpdateCollect =
      QString("UPDATE terminal_commands SET status='%2' WHERE trn = '%1'")
          .arg(trn, status);
...
void MainWindow::commandExecution(CommandInit::Cmd cmd, QVariantMap meta) {
  ...
  case CommandInit::cRebootTerminal: {
    ...
    if (whenRasReboot) {
      ...
      toLog(
          LogLevel::Info, "MAIN",
          QString(
              "Ставим параметр счетчика перезагрузки по ошибке 756 равным - %1")
              .arg(rCount + 1));
      rebootCountSet(rCount + 1);
      whenRasReboot = false;
    }

    toLog(LogLevel::Info, "MAIN", "Начинаем перезагружать ASO");
  ...
  case CommandInit::cGetIncashment: {
    QString log =
        comment.trimmed().isEmpty()
            ...
            : QString(
                  "Выполняем команду отправки отложенной инкасации с cid: %1")
                  .arg(comment);
  ...
  case CommandInit::cBVFirmwareUpdate: {
    toLog(
        LogLevel::Info, "MAIN",
        QString(
            "Принята команда обновление прошивки купюроприемника на версию %1")
            .arg(comment));
...
void MainWindow::getBalanceUser(double balance, double overdraft,
                                double threshold) {
...
void MainWindow::getTerminalInfo(QVariantMap map) {
  ...
  QString xmlName =
      QString("aso_%1.xml").arg(terminalInfo["path_name"].toString());
...
void MainWindow::jsonResponseSuccess(QVariantMap response,
                                     QString requestName) {
...
void MainWindow::checkLockTerminal() {
  ...
  if (lock == Lock::Ok) {
    ...
    if (pageIn == PageIn::LockTerminal) {
      toLog(LogLevel::Info, senderName,
            QString("Разблокируем терминал, нет ошибок."));
      mainPage->loadHtmlPage(PageIn::Main);
    }
  } else {
    ...
    if (pageIn != PageIn::LockTerminal && pageIn != PageIn::PrintDialog &&
      ...
      toLog(LogLevel::Error, senderName,
            QString("%1").arg(lockList[lock].comment));
  ...
  if (lock) {
    ...
    case Lock::NonMoney: {
      if (config.smsErrValidator && !lockList[state].lock) {
        ...
        toLog(LogLevel::Info, "SMS_CENTER",
              QString(
                  "Необходимо отправить СМС по недостатки средств у агента."));
      }
    } break;
    case Lock::MorePayIn: {
      if (config.smsErrValidator && !lockList[state].lock) {
    ...
    case Lock::Status_11: {
      if (config.smsErrValidator && !lockList[state].lock) {
        ...
        toLog(
            LogLevel::Info, "SMS_CENTER",
            QString(
                "Необходимо отправить СМС по статусу Терминал не активен(11)"));
      }
    } break;
...
QVariantList MainWindow::getServicesInputsFromDB() {
  ...
  if (querySql.exec(sqlQuery)) {
    ...
    while (querySql.next()) {
      ...
      input["help_secondary"] =
          querySql.value(record.indexOf("help_secondary"));
      input["help_ru"] = querySql.value(record.indexOf("help_ru"));
      input["help_en"] = querySql.value(record.indexOf("help_en"));
      input["placeholder_local"] =
          querySql.value(record.indexOf("placeholder_local"));
      input["placeholder_secondary"] =
          querySql.value(record.indexOf("placeholder_secondary"));
      input["placeholder_ru"] =
          querySql.value(record.indexOf("placeholder_ru"));
      input["placeholder_en"] =
          querySql.value(record.indexOf("placeholder_en"));
...
QVariantList MainWindow::getServicesFromDB() {
  ...
  if (querySql.exec(sqlQuery)) {
    ...
    while (querySql.next()) {
      ...
      service["name_secondary"] =
          querySql.value(record.indexOf("name_secondary"));
  ...
  if (querySql.exec(sqlQuery)) {
    ...
    while (querySql.next()) {
      ...
      category["description_local"] =
          querySql.value(record.indexOf("description_local"));
      category["description_ru"] =
          querySql.value(record.indexOf("description_ru"));
      category["description_en"] =
          querySql.value(record.indexOf("description_en"));
...
bool MainWindow::updateHashConfig(QString hash) {
  QSqlQuery sqlQuery(db);
  QString strUpdate =
      QString("UPDATE terminal_extra SET hash = \"%1\" WHERE id = 1").arg(hash);
...
void MainWindow::createSmsSendTable() {
  ...
  if (!createTable.exec(strCreate)) {
    toLog(LogLevel::Info, "DB_CONNECT",
          QString("Не удалось создать таблицу terminal_sms..."));
  }
}
...
void MainWindow::insertSmsContentInf() {
  ...
  userQuery =
      QString("INSERT INTO terminal_sms(sms_id, sms_state, sms_lock_status)"
              " VALUES(%1, " +
              QString::number(SmsSend::OK) + ", " +
              QString::number(SmsSend::Unlock) + ");");

  for (int i = 1; i <= 6; i++) {
    QString vrmQuery = userQuery.arg(i);
    if (!userSql.exec(vrmQuery)) {
      toLog(LogLevel::Error, "DB_CONNECT",
            "Ошибка при вставке смс контента...");
      return;
    }
  }
}
...
void MainWindow::rebootCountClear() {
  ...
  if (afterRestartRas) {
    ...
    connectionError(
        "756",
        "Предыдущие попытки перезагрузки не помогли... повторим ещё раз.");
  }
}
...
void MainWindow::saveDevice(int deviceId, QString deviceName, QString port,
                            QString comment, int state) {
  QSqlQuery updateDevices(db);
  QString strUpdate;

  strUpdate = QString("UPDATE terminal_devices SET name = \"%1\", port = "
                      "\"%2\", comment = \"%3\", state = %4 WHERE id = %5")
                  .arg(deviceName, port, comment, QString::number(state),
...
bool MainWindow::commandExist(QString trn) {
  ...
  QString strQuery =
      QString(
          "SELECT count(*) AS count FROM terminal_commands WHERE trn = '%1';")
          .arg(trn);
...
bool MainWindow::saveCommand(const QString trn, const int cmd,
                             const QString account, const QString comment) {
  QSqlQuery sqlQuery(db);
...
bool MainWindow::saveBillValidatorEvent(const QString event,
                                        const QString dateTime) {
  QSqlQuery sqlQuery(db);

  QString strQuery =
      QString("INSERT INTO terminal_bvalidator(event, date_time, status)"
              " VALUES('%1', '%2', 'new');")
          .arg(event, dateTime);
...
bool MainWindow::getBillValidatorEvent(QString &dateTime, QString event,
                                       QString status) {
  QSqlQuery sqlQuery(db);
...
bool MainWindow::clearDataBase() {
  ...
  QString vrmDateFrom =
      QString("%1-01-01").arg(QDate::currentDate().year() - 1);
  ...
  lstSql2 =
      QString("DELETE FROM terminal_collect WHERE status = 'confirmed'"
              " AND date_create BETWEEN \"%1 00:00:00\" AND \"%2 23:59:59\";")
          .arg(vrmDateFrom, vrmDateTo);

  lstSql3 =
      QString("DELETE FROM terminal_commands WHERE status = 'confirmed';");

  lstSql4 =
      QString("DELETE FROM terminal_bvalidator WHERE status = 'confirmed';");
...
void MainWindow::settingsSave() {
  ...
  settings.setValue("chek_width", config.printerData.chekWidth);
  ...
  settings.setValue("chek_small_beetwen_string",
                    config.printerData.smallBeetwenString);
  ...
  settings.setValue("exist_counter_printer_chek",
                    config.existCounterPrinterChek);
  ...
  settings.setValue("status_validator_jam_in_box",
                    config.statusValidatorJamInBox);
...
void MainWindow::settingsGet() {
  ...
  config.printerData.chekWidth = settings.value("chek_width").toInt();
  ...
  config.printerData.smallBeetwenString =
      settings.value("chek_small_beetwen_string").toBool();
  ...
  config.searchCoinAcceptor =
      settings.value("search_coin_acceptor", true).toBool();
  ...
  config.winPrtChekBottomMargin =
      settings.value("prt_win_bottom_margin").toInt();

  config.existCounterPrinterChek =
      settings.value("exist_counter_printer_chek").toBool();
  ...
  config.smsValueBalanceAgent =
      settings.value("sms_value_balance_agent").toDouble();
  ...
  config.statusValidatorJamInBox =
      settings.value("status_validator_jam_in_box").toBool();
  config.statusValidatorJamInBoxValueCounter =
      settings.value("status_validator_jam_in_box_value_counter").toInt();
  config.statusValidatorJamInBoxLockers =
      settings.value("status_validator_jam_in_box_lockers").toBool();
  config.lockDuplicateNominal =
      settings.value("lock_duplicate_nominal", false).toBool();
...
QVariantMap MainWindow::nominalData() {
  ...
  QFile file(QString("%1/%2/nominals.json")
                 .arg(ConstData::Path::Nominals, config.tpl));
...
void MainWindow::loadStyleSheet() {
  ...
  // Загружаем интерфейс
  QFile file(QString("%1%2").arg(ConstData::Path::Styles,
                                 ConstData::FileName::StyleQSS));
...
QString MainWindow::versionFull() {
  return QString("%1 EPay v %2")
      .arg(ConstData::companyName, ConstData::version);
}
...
QStringList MainWindow::getWinPrinterNames() {
  ...
  if (EnumPrintersW(PRINTER_ENUM_LOCAL | PRINTER_ENUM_CONNECTIONS, NULL, 2,
                    (LPBYTE)printerInfos, size, &size, &numPrinters)) {
    for (uint i = 0; i < numPrinters; i++) {
      printerName =
          QString::fromUtf16((const ushort *)printerInfos[i].pPrinterName);
...
void MainWindow::loadWebSettings() {
  ...
  QFont standardFont = qvariant_cast<QFont>(
      settings.value(QLatin1String("standardFont"), QFont()));
  QFont fixedFont =
      qvariant_cast<QFont>(settings.value(QLatin1String("fixedFont"), QFont()));
