...
CheckConnection::CheckConnection(QObject *parent) : QThread(parent)
{
    ...
    connect(m_pTcpSocket, SIGNAL(error(QAbstractSocket::SocketError)), this, SLOT(slotErrorConnected(QAbstractSocket::SocketError)));

    manager = new QNetworkAccessManager(this);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this, SLOT(replyFinished(QNetworkReply*)));
    connect(manager, SIGNAL(sslErrors(QNetworkReply*,QList<QSslError>)), this, SLOT(handleSslErrors(QNetworkReply*,QList<QSslError>)));
...
void CheckConnection::replyFinished(QNetworkReply *reply)
{
    ...
    if (error_id == QNetworkReply::NoError) {
        ...
        emit emit_toLoging(0, "PING", QString("SupportsSSL: %1 | SSLLibraryBuildVersion: %2 | SSLLibraryVersion: %3").arg(QSslSocket::supportsSsl()).arg(QSslSocket::sslLibraryBuildVersionString(), QSslSocket::sslLibraryVersionString()));
