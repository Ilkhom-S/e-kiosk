...
RasConnection::RasConnection(QObject *parent) : QThread(parent) {
  ...
  connect(this, SIGNAL(emit_TimerStateDialStart()), stateDialTimer,
          SLOT(start()));
...
void RasConnection::setConnectionName(QString name) { conName = name; }
...
void WINAPI RasConnection::RasCallback(HRASCONN hrasconn, UINT unMsg,
  ...
  case RASCS_AuthRetry:
    S = "Auth Retry...";
    D = "The client has requested another validation attempt with a new user "
        "name/password/domain.";
    D += " (Клиент попросил попытку для проверки другого именем пользователя / "
         "пароль / домен.)";
    break;
  ...
  case RASCS_AuthChangePassword:
    ...
    D += " (Клиент обратился с просьбой изменить пароль для данного "
         "пользователя.)";
    break;
  ...
  case RASCS_PrepareForCallback:
    ...
    D += " (Линии составляет около отключить в рамках подготовки к обратного "
         "вызова.)";
    break;
  case RASCS_WaitForModemReset:
    S = "Wait For Modem Reset...";
    D = "The client is delaying in order to give the modem time to reset "
        "itself in preparation for callback.";
    D += " (Клиент задерживает, чтобы дать время модему для сброса себя в "
         "рамках подготовки к обратного вызова.)";
    break;
  case RASCS_WaitForCallback:
    S = "Wait For Callback...";
    D = "The client is waiting for an incoming call from the remote access "
        "server.";
    D += " (Клиент ждет входящего звонка от удаленного доступа к серверу.)";
    break;
  case RASCS_Projected:
    S = "Projected...";
    D = "This state occurs after the RASCS_AuthProject state. It indicates "
        "that projection result information is "
        "available. You can access the projection result information by "
        "calling RasGetProjectionInfo.";
    D += " (Фаза проектирования закончена.)";
    break;
  case RASCS_SubEntryConnected:
    S = "SubEntry Connected...";
    D = "When dialing a multilink phone-book entry, this state indicates that "
        "a subentry has been connected during "
        "the dialing process. The dwSubEntry parameter of a RasDialFunc2 "
        "callback function indicates the index of "
        "the subentry.";
    // When the final state of all subentries in the phone-book entry has been
    // determined, the connection state is RASCS_Connected if one or more
    // subentries have been connected successfully.";
    D += " (При наборе многоканальной телефонной книге запись, это состояние "
         "указывает, что подменю был связан во "
         "время набора номера процесса.)";
    break;
  case RASCS_SubEntryDisconnected:
    S = "SubEntry Disconnected...";
    D = "When dialing a multilink phone-book entry, this state indicates that "
        "a subentry has been disconnected "
        "during the dialing process. The dwSubEntry parameter of a "
        "RasDialFunc2 callback function indicates the "
        "index of the subentry.";
    D += " (При наборе многоканальной телефонной книге запись, это состояние "
         "указывает, что подменю не был связан "
         "во время набора номера процесса.)";
    break;
...
void RasConnection::HangUp() {
  ...
  if (hangUpThis(hRasConn)) {
    emit this->emit_toLoging(
        0, "CONNECTION",
        QString("Соединение %1 успешно опущено").arg(this->conName));
...
bool RasConnection::Dial() {
  ...
  emit this->emit_toLoging(0, "CONNECTION",
                           "Начинаем подымать соединение с " + this->conName);
  ...
  // copy entry name as wide string
  memset(rasDialParams.szEntryName, 0, sizeof(rasDialParams.szEntryName));
  {
    std::wstring wname = conName.toStdWString();
    wcscpy_s(rasDialParams.szEntryName,
             sizeof(rasDialParams.szEntryName) / sizeof(wchar_t),
             wname.c_str());
  }
  ...
  if (RasGetEntryDialParamsW(0, &rasDialParams, &hasSavedPassword) == 0) {
    ...
    dialError = RasDialW(0, 0, &rasDialParams, 1,
                         (void *)RasConnection::RasCallback, &hrc);

    if (dialError != 0) {
      if (dialError == 756) {
        emit this->emit_toLoging(2, "CONNECTION",
                                 "Возникла ошибка 756 при поднятии соединения");
      }
      ...
      RasGetErrorStringW(dialError, buff, sizeof(buff) / sizeof(wchar_t));
...
int RasConnection::createNewDialupConnection(QString conName, QString devName,
  ...
  wcscpy_s(tRasEntry.szLocalPhoneNumber,
           sizeof(tRasEntry.szLocalPhoneNumber) / sizeof(wchar_t),
           (const wchar_t *)phone.utf16());
  wcscpy_s(tRasEntry.szDeviceName,
           sizeof(tRasEntry.szDeviceName) / sizeof(wchar_t),
           (const wchar_t *)devName.utf16());
  wcscpy_s(tRasEntry.szDeviceType,
           sizeof(tRasEntry.szDeviceType) / sizeof(wchar_t), L"modem");
  tRasEntry.dwfOptions = RASEO_ModemLights | RASEO_SecureLocalFiles |
  ...
  dwError = RasSetEntryPropertiesW(0, rasDialParams.szEntryName, &tRasEntry,
                                   sizeof(tRasEntry), NULL, 0);
  ...
  rasDialParams.dwSize = sizeof(RASDIALPARAMSW);
  LPRASDIALPARAMSW lpRasDialParams = &rasDialParams;

  wcscpy_s(rasDialParams.szEntryName,
           sizeof(rasDialParams.szEntryName) / sizeof(wchar_t),
           (const wchar_t *)conName.utf16());
  wcscpy_s(rasDialParams.szPhoneNumber,
           sizeof(rasDialParams.szPhoneNumber) / sizeof(wchar_t),
           (const wchar_t *)phone.utf16());
  wcscpy_s(rasDialParams.szUserName,
           sizeof(rasDialParams.szUserName) / sizeof(wchar_t),
           (const wchar_t *)login.utf16());
  wcscpy_s(rasDialParams.szPassword,
           sizeof(rasDialParams.szPassword) / sizeof(wchar_t),
           (const wchar_t *)pass.utf16());

  dwError = RasSetEntryDialParamsW(0, lpRasDialParams, FALSE);

  if (dwError) {
    emit this->emit_toLoging(2, "CONNECTION",
                             "Ошибка при присвоении параметров соединению");
...
bool RasConnection::HasInstalledModems(QStringList &lstModemList) {
  ...
  if (ERROR_SUCCESS == dwErr) {
    for (DWORD i = 0; i < dwDevices; i++) {
      if (QString::fromUtf16((ushort *)(&(lpRasDevInfo[i].szDeviceType))) ==
          "modem") {
        lstModemList << QString::fromUtf16(
            (ushort *)(&(lpRasDevInfo[i].szDeviceName)));
        bResult = true;
      }
    }
  }
...
bool RasConnection::getConName(QStringList &lstCon) {
  ...
  if (RasEnumConnections(ras, &dSize, &dNumber) == 0) {
    for (DWORD x = 0; x < dNumber; x++) {
      ...
      emit this->emit_toLoging(
          0, "CONNECTION",
          QString("В данный момент соединение %1 уже поднято")
              .arg(lstCon.at(0)));
    }
  }
...
HRASCONN RasConnection::getConnection() {
  ...
  DWORD dwError = RasEnumConnections(rasConnections, &dwBufferSize,
                                     &dwActualConnectionsCount);
  if (dwError == 0) {
    for (DWORD i = 0; i < dwActualConnectionsCount; i++) {
      if (conName ==
          QString::fromUtf16((ushort *)(&(rasConnections[i].szEntryName)))) {
  ...
  if (RasEnumEntries(NULL, NULL, rasentry, &dSize, &dNumber) == 0) {
    for (DWORD i = 0; i < dNumber; i++) {
      connectionEntries.append(
          QString::fromUtf16((ushort *)(&(rasentry[i].szEntryName))));
    }
  }
}

void RasConnection::stopReconnect() { reCallTimer->stop(); }
