									   ...
									   DWORD dwExtendedError)
{
	...
	case RASCS_Projected:
		S = "Projected...";
		D = "This state occurs after the RASCS_AuthProject state. It indicates that projection result information is "
			"available. You can access the projection result information by calling RasGetProjectionInfo.";
		D += " (Фаза проектирования закончена.)";
		break;
	case RASCS_SubEntryConnected:
		S = "SubEntry Connected...";
		D = "When dialing a multilink phone-book entry, this state indicates that a subentry has been connected during "
			"the dialing process. The dwSubEntry parameter of a RasDialFunc2 callback function indicates the index of "
			"the subentry.";
		// When the final state of all subentries in the phone-book entry has been determined, the connection state is
		// RASCS_Connected if one or more subentries have been connected successfully.";
		D += " (При наборе многоканальной телефонной книге запись, это состояние указывает, что подменю был связан во "
			 "время набора номера процесса.)";
		break;
	case RASCS_SubEntryDisconnected:
		S = "SubEntry Disconnected...";
		D = "When dialing a multilink phone-book entry, this state indicates that a subentry has been disconnected "
			"during the dialing process. The dwSubEntry parameter of a RasDialFunc2 callback function indicates the "
			"index of the subentry.";
		D += " (При наборе многоканальной телефонной книге запись, это состояние указывает, что подменю не был связан "
											 ...
											 QString pass)
{
	...
	wcscpy_s(tRasEntry.szDeviceName, sizeof(tRasEntry.szDeviceName) / sizeof(wchar_t), (const wchar_t*)devName.utf16());
