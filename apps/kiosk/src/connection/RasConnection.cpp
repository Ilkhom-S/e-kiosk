...
void WINAPI RasConnection::RasCallback(HRASCONN hrasconn, UINT unMsg, RASCONNSTATE rascs,
    ...
    switch (rascs) {
        ...
        case RASCS_AuthRetry:
            ...
            D += " (Клиент попросил попытку для проверки другого именем пользователя / пароль / "
                 "домен.)";
            break;
        ...
        case RASCS_WaitForModemReset:
            ...
            D = "The client is delaying in order to give the modem time to reset itself in "
                "preparation for callback.";
            D += " (Клиент задерживает, чтобы дать время модему для сброса себя в рамках "
                 "подготовки к обратного вызова.)";
            break;
        ...
        case RASCS_Projected:
            S = "Projected...";
            D = "This state occurs after the RASCS_AuthProject state. It indicates that projection "
                "result information is available. You can access the projection result information "
                "by calling RasGetProjectionInfo.";
            D += " (Фаза проектирования закончена.)";
            break;
        case RASCS_SubEntryConnected:
            S = "SubEntry Connected...";
            D = "When dialing a multilink phone-book entry, this state indicates that a subentry "
                "has been connected during the dialing process. The dwSubEntry parameter of a "
                "RasDialFunc2 callback function indicates the index of the subentry.";
            // When the final state of all subentries in the phone-book entry has been determined,
            // the connection state is RASCS_Connected if one or more subentries have been connected
            // successfully.";
            D += " (При наборе многоканальной телефонной книге запись, это состояние указывает, "
                 "что подменю был связан во время набора номера процесса.)";
            break;
        case RASCS_SubEntryDisconnected:
            S = "SubEntry Disconnected...";
            D = "When dialing a multilink phone-book entry, this state indicates that a subentry "
                "has been disconnected during the dialing process. The dwSubEntry parameter of a "
                "RasDialFunc2 callback function indicates the index of the subentry.";
            D += " (При наборе многоканальной телефонной книге запись, это состояние указывает, "
                 "что подменю не был связан во время набора номера процесса.)";
            break;
...
int RasConnection::createNewDialupConnection(QString conName, QString devName, QString phone,
    ...
    MultiByteToWideChar(CP_UTF8, 0, phone.toUtf8().constData(), -1,
                        (LPWSTR)tRasEntry.szLocalPhoneNumber,
                        sizeof(tRasEntry.szLocalPhoneNumber) / sizeof(WCHAR));
    MultiByteToWideChar(CP_UTF8, 0, devName.toUtf8().constData(), -1,
                        (LPWSTR)tRasEntry.szDeviceName,
                        sizeof(tRasEntry.szDeviceName) / sizeof(WCHAR));
    ...
    dwError =
        RasSetEntryProperties(0, rasDialParams.szEntryName, &tRasEntry, sizeof(tRasEntry), NULL, 0);
