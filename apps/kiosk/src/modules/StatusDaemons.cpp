...
void StatusDaemons::r_RequestRepeet() {
  emit emit_Loging(
      0, senderName,
      QString("Попытка N - %1 отправки статуса терминала на сервер.")
          .arg(count_resend));
...
void StatusDaemons::sendStatusToServer(Sender::Data &a_Data) {
  ...
  if (a_Data.firstSend) {
    ...
    auto osInfo =
        QString("<os version=\"%1\" csname=\"%2\" csdversion=\"%3\" "
            ...
            .arg(os.value("version").toString(), os.value("csname").toString(),
    ...
    auto ramInfo =
        QString("<ram size=\"%1 MB\" speed=\"%2\" memorytype=\"%3\">%4</ram>\n")
    ...
    auto diskInfo =
        QString("<disk serialnumber=\"%1\" size=\"%2 GB\" freespace=\"%3 GB\" "
    ...
    techParames += QString(
        ...
        "</modem>\n"
        "<sim signal=\"" +
        a_Data.modem.signal + "\" bsim=\"" + a_Data.modem.balance +
        "\" number=\"" + a_Data.modem.number + "\">" + a_Data.modem.provider +
        "</sim>\n"
        "<connection>" +
    ...
    forLogData +=
        QString("Версия ПО                 - %1 \n"
                ...
                "Тип соединения            - %13 \n")
            .arg(a_Data.fullVersion, a_Data.validator.port,
                 a_Data.validator.serial, a_Data.validator.name,
                 a_Data.printer.port, a_Data.printer.name, a_Data.modem.port,
                 a_Data.modem.name, a_Data.modem.signal, a_Data.modem.balance,
                 a_Data.modem.number, a_Data.modem.provider, a_Data.connection);
  }
  ...
  QString status =
      QString("<status>\n"
              ...
              bill_str + out_str + "<lock_status>" +
              QString("%1").arg(a_Data.lockStatus) + "</lock_status>\n" +
              action + "</status>\n")
  ...
  emit emit_Loging(0, senderName,
                   QString("Получены данные о состоянии терминала.\n\n%1\n")
                       .arg(forLogData));
  emit emit_Loging(0, senderName,
                   "Начинаем отправлять статус терминала на сервер.");
