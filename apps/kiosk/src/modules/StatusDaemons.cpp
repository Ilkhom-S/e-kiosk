...
void StatusDaemons::sendStatusToServer(Sender::Data &a_Data)
{
    ...
    if (a_Data.firstSend) {
        ...
        auto osInfo = QString("<os version=\"%1\" csname=\"%2\" csdversion=\"%3\" osarchitecture=\"%4\">%5</os>\n")
        ...
        auto mboardInfo = QString("<mboard product=\"%1\" version=\"%2\" serialnumber=\"%3\">%4</mboard>\n")
        ...
        auto diskInfo = QString("<disk serialnumber=\"%1\" size=\"%2 GB\" freespace=\"%3 GB\" local_disks=\"%4\" model=\"%5\">\n"
                                    "<system_disk caption=\"%6\" size=\"%7 GB\" freespace=\"%8 GB\"></system_disk>\n"
        ...
        techParames += QString("<tech_parametrs>\n"
                                    "<validator port=\""+ a_Data.validator.port +"\" serial=\""+ a_Data.validator.serial +"\">"+ a_Data.validator.name +"</validator>\n"
                                    "<coin_acceptor port=\""+ a_Data.coinAcceptor.port +"\" serial=\""+ a_Data.coinAcceptor.serial +"\">"+ a_Data.coinAcceptor.name +"</coin_acceptor>\n"
                                    "<printer port=\""+ a_Data.printer.port +"\" >"+ a_Data.printer.name +"</printer>\n"
                                    "<modem port=\""+ a_Data.modem.port +"\">"+ a_Data.modem.name +"</modem>\n"
                                    "<sim signal=\""+ a_Data.modem.signal +"\" bsim=\""+ a_Data.modem.balance +"\" number=\""+ a_Data.modem.number +"\">"+ a_Data.modem.provider +"</sim>\n"
                                    "<connection>"+ a_Data.connection +"</connection>\n"
                                    + osInfo
                                    + cpuInfo
                                    + mboardInfo
                                    + ramInfo
    ...
    QString status = QString("<status>\n"
                                ...
                                + bill_str + out_str +
                                "<lock_status>"+ QString("%1").arg(a_Data.lockStatus) +"</lock_status>\n"
                                + action +
