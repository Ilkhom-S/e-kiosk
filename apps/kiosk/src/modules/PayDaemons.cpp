...
void PayDaemons::get_new_pay(QVariantMap payment) {
    ...
    QString strCreateNewOperation =
        QString(
            ...
            " (operation_ratio_number, operation_ratio_persent, operation_account_design, "
            "operation_account"
            ", operation_collect_id, operation_date_create, operation_date_confirm, operation_id"
            ", operation_is_send, operation_money_amount, operation_money_cash, "
            "operation_money_denom"
            ", operation_services_id, operation_status, operation_print_status, "
            "operation_services_name, notify_route, extra_info)"
...
void PayDaemons::get_update_pay(QVariantMap payment) {
    ...
    QString strUpdateOperation = QString(
                                     "UPDATE terminal_operation SET"
    ...
    if (!updateOperation.exec(strUpdateOperation)) {
        // if(Debuger) qDebug() << "Error Update Operation for status LOCK in function
        // get_update_pay";
        emit emit_Loging(
            2, senderName,
...
bool PayDaemons::GetOperationCount(const QString &id_trn) {
    ...
    QString operationQuery =
        QString("SELECT count(*) AS count FROM terminal_operation WHERE operation_id = %1 LIMIT 1;")
            .arg(id_trn);
...
QString PayDaemons::getReceiptInfo(QString tranz_id) {
    ...
    if (!getPayData(tranz_id, date_create, prv_name, account, sum_from, sum_to, ratio_sum,
                    ratio_persent)) {
...
bool PayDaemons::getPayData(QString id_trn, QString &date_create, QString &prv_name,
    ...
    QString strOperation =
        QString("SELECT * FROM terminal_operation WHERE operation_id = '%1' LIMIT 1;").arg(id_trn);
    ...
    if (selectOperation.next()) {
        ...
        sum_from =
            selectOperation.value(recordOperation.indexOf("operation_money_cash")).toString();
        sum_to =
            selectOperation.value(recordOperation.indexOf("operation_money_amount")).toString();
...
bool PayDaemons::getPaymentMap(QString &payment, int &count_pay, double &all_sum) {
    ...
    QString strQueryOperation = QString(
        "SELECT * FROM terminal_operation"
    ...
    while (selectOperationForSend.next()) {
        ...
        payment += QString(
                       "<payment trn=\"%1\" fsum=\"%2\" tsum=\"%3\" prv_id=\"%4\" account=\"%5\" "
                       "receipt=\"%6\" time=\"%7\" receipt_sts=\"%8\" bill_info=\"%9\" "
                       "collection_id=\"%10\" notify_route=\"%11\" %12/>\n")
                       .arg(vrmIdTrn, vrmFSum)
                       .arg(vrmTSum)
                       .arg(vrmPrvID, vrmAccount, vrmIdTrn, vrmDateCreate, stsPrint, billInfo,
                            collectionId, vrmNotifyRoute, extra);

        count_pay++;
        all_sum += vrmTSum;

        emit emit_Loging(0, senderName,
                         QString("Платеж с номером- %1 суммой на счет- %2 и транзакцией- %3 в "
                                 "PAY_REQUEST_XML сформирован.")
                             .arg(vrmAccount)
                             .arg(vrmTSum)
                             .arg(vrmIdTrn));
    }
...
bool PayDaemons::updateOperationStatus(const QString &id_trm, const QString &status,
                                       const QString &dateConfirm) {
    QSqlQuery updateOperation(db);
    QString strUpdateOperation;
    QString strSend = "";
    if (dateConfirm != "0") strSend = ", operation_is_send = 1";
    strUpdateOperation = QString(
                             "UPDATE terminal_operation SET operation_status = %2, "
                             "operation_date_confirm = '%3'%4 WHERE operation_id = %1")
...
bool PayDaemons::getCountPayment(int &count) {
    ...
    QString operationQuery = QString(
        "SELECT count(*) AS count FROM terminal_operation WHERE operation_status = 55 LIMIT 1;");
...
void PayDaemons::checkPayStatus54() {
    ...
    QString operationQuery = QString(
        "SELECT count(*) AS count FROM terminal_operation WHERE operation_status = 54 OR "
        "operation_status = 3;");
    // if(Debuger) qDebug() << operationQuery;
...
void PayDaemons::confirmPayments() {
    ...
    // TODO remove after update operation_status = 3
    strUpdateOperation = QString(
        "UPDATE terminal_operation SET operation_status = 55  WHERE operation_status = 54 OR "
        "operation_status = 3;");
