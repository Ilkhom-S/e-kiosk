			...
			if (strTag == "payment")
			{
				...
				else
				{
					emit emit_Loging(0, senderName,
									 QString("Платеж с транзакцией - %1: обновлен на статус - %2 в %3 часов.")
...
void PayDaemons::get_new_pay(QVariantMap payment)
{
	...
	QString strCreateNewOperation =
		QString("INSERT INTO terminal_operation"
				...
				", operation_services_id, operation_status, operation_print_status, operation_services_name, "
	...
	if (!createNewOperation.exec(strCreateNewOperation))
	{
		...
		emit emit_Loging(
			2, senderName,
			QString("Ошибка Базы Данных... Невозможно создать платёж (номер - %1, сумма - %2). Error: %3. Query: %4")
				.arg(account)
				.arg(sumFrom)
...
void PayDaemons::get_update_pay(QVariantMap payment)
{
	...
	if (!updateOperation.exec(strUpdateOperation))
	{
		...
		emit emit_Loging(2, senderName,
						 QString("Ошибка Базы Данных... Невозможно обновить платёж. Error: %1. Query: %2")
...
bool PayDaemons::GetOperationCount(const QString &id_trn)
{
	...
	QString operationQuery =
		QString("SELECT count(*) AS count FROM terminal_operation WHERE operation_id = %1 LIMIT 1;").arg(id_trn);
...
void PayDaemons::get_confirm_pay(QString tranz_id, bool print)
{
	...
	if (print)
	{
		emit emit_Loging(
			0, senderName,
			QString("К платежу с транзакцией- %1 чек был распечатан %2 числа.").arg(tranz_id, vrmDateCreate));
	}
	else
	{
		emit emit_Loging(0, senderName,
						 QString("Платеж с транзакцией- %1 полностью оплачен %2 числа.").arg(tranz_id, vrmDateCreate));
...
bool PayDaemons::getPayData(QString id_trn, QString &date_create, QString &prv_name, QString &account,
...
bool PayDaemons::getPaymentMap(QString &payment, int &count_pay, double &all_sum)
{
	...
	if (!selectOperationForSend.exec(strQueryOperation))
	{
		...
		// if(Debuger) qDebug() << "Error Select Operation Data for Send in function bool
		// PayDaemons::SendNewOperation()";
		return false;
	}
	...
	while (selectOperationForSend.next())
	{
		...
		QString vrmFSum =
			selectOperationForSend.value(recordOperationForSend.indexOf("operation_money_cash")).toString();
		double vrmTSum =
			selectOperationForSend.value(recordOperationForSend.indexOf("operation_money_amount")).toDouble();
		QString billInfo =
			selectOperationForSend.value(recordOperationForSend.indexOf("operation_money_denom")).toString();
		QString vrmPrvID =
			selectOperationForSend.value(recordOperationForSend.indexOf("operation_services_id")).toString();
		QString vrmAccount =
			selectOperationForSend.value(recordOperationForSend.indexOf("operation_account")).toString();
		QString vrmDateCreate =
			selectOperationForSend.value(recordOperationForSend.indexOf("operation_date_create")).toString();
		QString stsPrint =
			selectOperationForSend.value(recordOperationForSend.indexOf("operation_print_status")).toString();
		QString collectionId =
			selectOperationForSend.value(recordOperationForSend.indexOf("operation_collect_id")).toString();
		...
		payment +=
			QString(
				"<payment trn=\"%1\" fsum=\"%2\" tsum=\"%3\" prv_id=\"%4\" account=\"%5\" receipt=\"%6\" time=\"%7\" "
				"receipt_sts=\"%8\" bill_info=\"%9\" collection_id=\"%10\" notify_route=\"%11\" %12/>\n")
				.arg(vrmIdTrn, vrmFSum)
				.arg(vrmTSum)
				.arg(vrmPrvID, vrmAccount, vrmIdTrn, vrmDateCreate, stsPrint, billInfo, collectionId, vrmNotifyRoute,
					 extra);
		...
		emit emit_Loging(
			0, senderName,
			QString("Платеж с номером- %1 суммой на счет- %2 и транзакцией- %3 в PAY_REQUEST_XML сформирован.")
				.arg(vrmAccount)
				.arg(vrmTSum)
				.arg(vrmIdTrn));
	}
...
bool PayDaemons::updateOperationStatus(const QString &id_trm, const QString &status, const QString &dateConfirm)
{
	...
	strUpdateOperation = QString("UPDATE terminal_operation SET operation_status = %2, operation_date_confirm = '%3'%4 "
								 "WHERE operation_id = %1")
	...
	if (!updateOperation.exec(strUpdateOperation))
	{
		...
		// if(Debuger) qDebug() << "Error Update Operation for status UNKNOWN in function bool
		// PayDaemons::UpdateOperationForUnknown(const QString &id_trm)";
		return false;
	}
	return true;
}
...
bool PayDaemons::getCountPayment(int &count)
{
	...
	QString operationQuery =
		QString("SELECT count(*) AS count FROM terminal_operation WHERE operation_status = 55 LIMIT 1;");
...
void PayDaemons::checkPayStatus54()
{
	...
	QString operationQuery = QString(
		"SELECT count(*) AS count FROM terminal_operation WHERE operation_status = 54 OR operation_status = 3;");
	// if(Debuger) qDebug() << operationQuery;
...
void PayDaemons::confirmPayments()
{
	...
	// TODO remove after update operation_status = 3
	strUpdateOperation = QString(
		"UPDATE terminal_operation SET operation_status = 55  WHERE operation_status = 54 OR operation_status = 3;");
