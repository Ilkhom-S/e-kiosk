...
void JsonRequest::setBaseUrl(QString url) {
  baseUrl = url.replace("aso/api/", "")
                .replace("aso/v3/", "") /*.replace("https", "http")*/;
}

void JsonRequest::sendRequest(QJsonObject json, QString url_,
                              QString requestName, int method_,
                              const int timeout, QVariantMap header) {
