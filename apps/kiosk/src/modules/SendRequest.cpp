...
void SendRequest::slotReadyRead()
{
	...
	{
		statusError = 6;
		statusErrorComment = "the SSL/TLS handshake failed and the encrypted channel could not be established. The "
	...
	{
		statusError = 105;
		statusErrorComment = "the proxy requires authentication in order to honour the request but did not accept any "
	...
	{
		statusError = 204;
		statusErrorComment = "the remote server requires authentication to serve the content but the credentials "
	...
	{
		statusError = 205;
		statusErrorComment = "the request needed to be sent again, but this failed for example because the upload data "
