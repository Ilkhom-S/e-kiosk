...
void CollectDaemons::setDataNote(const QDomNode &domElement) {
  emit emit_Loging(
      0, senderName,
      QString("Пришел ответ инкассации от сервера. Парсим данные"));
  ...
  if (cIdUpdate != "") {
    ...
    if (confirmCollection(cIdUpdate)) {
      ...
      emit emit_Loging(0, senderName,
                       QString("Инкассация с ID - %1 на сервере подтвердилась.")
                           .arg(cIdUpdate));
  ...
  while (!domNode.isNull()) {
    if (domNode.isElement()) {
      ...
      if (strTag == "resultCode") {
        ...
        if (sts == "150" || sts == "245" || sts == "11" || sts == "12" ||
            sts == "133") {
...
QString CollectDaemons::getHtmlInfoBox(QString &nonCollectPayment,
                                       int &moneyOutNum, double &moneyOutSum,
                                       QString data, QString &collectionId,
                                       QString &stackId, QString &trnFrom,
                                       QString &trnTo) {
  moneyOutNum = 0;
  moneyOutSum = 0;
  ...
  if (data == "") {
    ...
    if (collId != "") {
      ...
      if (getCollectionInfo(collId, collectionIdNext, stackId, dateCreate,
                            xmlDenom)) {
        ...
        if (parsDenomilSlot(xmlDenom)) {
          ...
          emit emit_Loging(1, senderName,
                           QString("Ошибка при парсинге xml collect."));
          return "";
        }
      } else {
        emit emit_Loging(
  ...
  if (nominalData.coins.length() > 0) {
    ...
    for (auto &coin : nominalData.coins) {
      strCoins += QString("<tr bgcolor=\"#%5\">"
                      ...
                      .arg(coin.value)
                      .arg(getCoinTxt(coin.face * coin.value),
                           i % 2 == 0 ? "fff" : "eee");
  ...
  if (nominalData.coins.length() > 0) {
    ...
    auto totalSum =
        nominalData.billSum + nominalData.coinSum / nominalData.coinDivider;
...
QString CollectDaemons::getCoinTxt(int coin) {
  ...
  if (coin >= nominalData.coinDivider)
    coin_txt = QString("%1 %2")
                   .arg(coin / nominalData.coinDivider)
                   .arg(nominalData.billCurrency);
...
void CollectDaemons::getRequestParam() {
  ...
  if (collectionToSend == 0) {
    ...
    emit emit_Loging(0, senderName,
                     QString("Отключаем демон( нет новых инкассаций )"));
    return;
  }

  emit emit_Loging(0, senderName,
                   QString("Количество новых инкассаций в системе - %1.")
                       .arg(collectionToSend));
  ...
  if (collectionId == "") {
    ...
    emit emit_Loging(
        0, senderName,
        QString("В системе имеется новая инкассация под номером - %1.")
            .arg(collectionId));
  }

  if (getCollectionInfo(collectionId, collectionIdNext, trnId, dateCreate,
                        xmlDenom)) {
    // Теперь надо отпарсить xml
    if (parsDenomilSlot(xmlDenom)) {
      // Транзакция с, по,
      ...
      if (trnId != "") {
        ...
        requestXml = "";
        getXmlForSend(requestXml, collectionId, collectionIdNext, trnId,
                      dateCreate, xmlDenom, moneyOutNum, moneyOutSum, trnFrom,
                      trnTo);
...
QString CollectDaemons::getCollectIdByDate(QString date) {
  ...
  QString strCollect =
      QString("SELECT collect_id FROM terminal_collect WHERE date_create = "
  ...
  if (selectCollect.next()) {
    collectionId =
        selectCollect.value(recordCollect.indexOf("collect_id")).toString();
  }
...
bool CollectDaemons::parserCollectIntoOperation(const QString cid) {
  ...
  QSqlQuery selectCollect(db);
  QString strCollect =
      QString("SELECT operation_money_denom FROM terminal_operation WHERE "
  ...
  while (selectCollect.next()) {
    QString operation_money_denom =
        selectCollect.value(recordCollect.indexOf("operation_money_denom"))
            .toString();
...
bool CollectDaemons::getCheckText(QString &text, bool preCheck, QString dateP,
                                  QString cid) {
  QString trnId = "";
  QString collectId = "";
  ...
  if (!preCheck) {
    ...
    emit emit_Loging(0, senderName,
                     QString("Пришла команда на инкасирование платежей."));

    if (countNonCollectPayment > 0) {
      emit emit_Loging(0, senderName,
                       QString("Количество не инкасированных платежей %1.")
                           .arg(countNonCollectPayment));
      ...
      if (parserCollectIntoOperation(cid)) {
        ...
        if (sendCollection(trnId, cid, collectIdNext, dateCreate, xmlDenom)) {
          ...
          emit emit_Loging(2, senderName,
                           QString("Ошибка при создании инкассации."));
          return false;
        }
      } else {
        emit emit_Loging(2, senderName,
                         QString("Не возможно вытащить xml из платежей."));
        return false;
      }
    } else {
      //            if (remoteCollection) {
      ...
      //            } else {
      emit emit_Loging(0, senderName,
                       QString("В системе нет неинкассированных платежей."));
      //            }
      return false;
    }
  } else {
    ...
    if (collectId != "") {
      ...
      if (getCollectionInfo(collectId, collectIdNext, trnId, dateCreate,
                            xmlDenom)) {
        // Теперь надо отпарсить xml
        if (parsDenomilSlot(xmlDenom)) {
  ...
  text = QString("[p]Инкассация %1\n"
             ...
             .arg(collectId.left(8), numTrm, trnId, dateCreate, datePre,
                  trnFrom, trnTo, bills, coins, total)
             .arg(moneyOutNum)
             .arg(moneyOutSum);
  ...
  if (!preCheck && trnId != "") {
    // Тут надо создать XML с инкассацией
    requestXml = "";
    getXmlForSend(requestXml, collectId, collectIdNext, trnId, dateCreate,
...
void CollectDaemons::sendCollectRequest() {
  emit emit_Loging(0, senderName,
                   QString("Начинаем отправлять данные инкассации на сервер."));
  ...
  if (countAllRep < RealRepeet) {
    ...
    if (!sendRequest(requestXml, 30000)) {
      // Тут надо отправить XML на сервер
      emit emit_Loging(0, senderName,
                       QString("Тут надо отправить XML на сервер."));
...
void CollectDaemons::getXmlForSend(QString &xml, QString cid, QString cidNext,
                                   QString sid, QString dateCreate,
                                   QString collectDenom, int moneyOutCount,
                                   double moneyOutSum, QString trnFrom,
                                   QString trnTo) {
  ...
  QString center_xml =
      QString("<stacker cid=\"%1\" cid_next=\"%2\" sid=\"%3\" action=\"new\">\n"
              "<date>%4</date>\n"
              "%5\n"
              "<trn from=\"%6\" to=\"%7\"/>\n"
...
bool CollectDaemons::updateMoneyOut(QString collectionId,
...
bool CollectDaemons::getCollectionInfo(QString collectionId,
                                       QString &collectionIdNext,
                                       QString &idTrn, QString &dateCreate,
                                       QString &denomXml) {
  QSqlQuery selectCollect(db);

  QString strQuery;

  strQuery = QString("SELECT * FROM terminal_collect WHERE collect_id = '%1';")
                 .arg(collectionId);
  ...
  if (selectCollect.next()) {
    ...
    dateCreate =
        selectCollect.value(recordCollect.indexOf("date_create")).toString();
    denomXml = selectCollect.value(recordCollect.indexOf("denom")).toString();
    collectionIdNext =
        selectCollect.value(recordCollect.indexOf("collect_id_next"))
            .toString();
  }
...
bool CollectDaemons::getMoneyOut(const QString &collectionId, int &numOut,
                                 double &sumOut) {
  ...
  strQuery =
      QString("SELECT * FROM terminal_moneyout WHERE moneyout_collect_id = "
  ...
  while (selectMoneyOut.next()) {
    double sum = selectMoneyOut.value(recordCollect.indexOf("moneyout_value"))
                     .toDouble();
    sumMoney += sum;
    numMoney++;
  }
...
bool CollectDaemons::getTrnOperation(const QString collectId,
                                     const QString &cmd, QString &trn) {
  QSqlQuery selectCollect(db);
...
bool CollectDaemons::getDatePrevCollection(QString &date,
                                           QString collectionId) {
  QSqlQuery selectCollect(db);
...
QString CollectDaemons::getDenominalXml() {
  ...
  for (auto &bill : nominalData.bills) {
    xml +=
        QString("<bill face=\"%1\">%2</bill>").arg(bill.face).arg(bill.value);
  }

  for (auto &coin : nominalData.coins) {
    xml +=
        QString("<bill face=\"%1M\">%2</bill>").arg(coin.face).arg(coin.value);
  }
...
bool CollectDaemons::sendCollection(QString &trnId, const QString collectionId,
                                    QString &collectionIdNext,
                                    QString &dateCreate, QString xmlDenom) {
  ...
  QString strCollect =
      QString("UPDATE terminal_collect SET stack_id = %2, date_create = "
              "\"%3\", denom = '%4', "
...
int CollectDaemons::getCollectCount(bool new_collect) {
  ...
  QString strCollect =
      QString("SELECT count(*) AS count FROM terminal_collect %1 LIMIT 1;")
          .arg(param);
  if (!selectCollect.exec(strCollect)) {
    return 0;
  }
...
QString CollectDaemons::getCollectionId(QString status) {
  ...
  QString strQuery =
      QString("SELECT collect_id FROM terminal_collect WHERE status = '%1';")
          .arg(status);
...
int CollectDaemons::getCollectionCount(QString status) {
  ...
  QString strQuery =
      QString(
          "SELECT count(*) AS count FROM terminal_collect WHERE status='%1';")
          .arg(status);
