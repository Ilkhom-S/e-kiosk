...
void CollectDaemons::errResponse()
{
	...
	if (countAllRep >= RealRepeet)
	{
		...
		emit emit_Loging(0, senderName,
						 QString("Отключаем демон( превышено число отправок инкассации - %1 )").arg(countAllRep));
		demonTimer->stop();
	}
	else
	{
...
QString CollectDaemons::getHtmlInfoBox(QString &nonCollectPayment, int &moneyOutNum, double &moneyOutSum, QString data,
									   QString &collectionId, QString &stackId, QString &trnFrom, QString &trnTo)
{
	...
	QString strBills = "<table width = \"100%\" style=\"font-size: 12px; border-style:solid; border-width:1px; "
					   "border-color:#ffffff;\">"
	...
	strBills += QString("<tr>"
						"<td align=\"center\" width=\"50%\"><b>Купюры - %1</b></td> <td colspan=\"2\" align=\"center\" "
	...
	{
		strCoins += "<table width = \"100%\" style=\"font-size: 12px; border-style:solid; border-width:1px; "
					"border-color:#ffffff;\">"
					"<tr bgcolor=\"#aaaaaa\" style=\"font-size: 14px; color: #ffffff; \">"
					"<td align=\"center\" height=\"40px\">Монета</td><td align=\"center\">Количество</td><td "
					"align=\"center\">Сумма</td>"
					"</tr>";
		...
		strCoins += QString("<tr>"
							"<td align=\"center\" width=\"50%\"><b>Монеты - %1</b></td> <td colspan=\"2\" "
	...
	if (nominalData.coins.length() > 0)
	{
		...
		strTotal = QString("<br><td style='font-size: 14px;' colspan = \"3\" align=\"center\"><b><font "
						   "color=\"blue\">Общее количество</font> - %1 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <font "
...
QString CollectDaemons::getCoinTxt(int coin)
{
	...
	if (coin >= nominalData.coinDivider)
		coin_txt = QString("%1 %2").arg(coin / nominalData.coinDivider).arg(nominalData.billCurrency);
	...
	if (getCollectionInfo(collectionId, collectionIdNext, trnId, dateCreate, xmlDenom))
	{
		...
		if (parsDenomilSlot(xmlDenom))
		{
			...
			if (trnId != "")
			{
				...
				requestXml = "";
				getXmlForSend(requestXml, collectionId, collectionIdNext, trnId, dateCreate, xmlDenom, moneyOutNum,
							  moneyOutSum, trnFrom, trnTo);
...
QString CollectDaemons::getCollectIdByDate(QString date)
{
	...
	QString strCollect =
		QString("SELECT collect_id FROM terminal_collect WHERE date_create = \"%1\" and status != 'new' LIMIT 1;")
			.arg(date);
...
bool CollectDaemons::parserCollectIntoOperation(const QString cid)
{
	...
	QSqlQuery selectCollect(db);
	QString strCollect =
		QString("SELECT operation_money_denom FROM terminal_operation WHERE operation_collect_id = '%1';").arg(cid);
...
bool CollectDaemons::getCheckText(QString &text, bool preCheck, QString dateP, QString cid)
{
	...
	if (!preCheck)
	{
		...
		if (countNonCollectPayment > 0)
		{
			...
			if (parserCollectIntoOperation(cid))
			{
				...
				if (sendCollection(trnId, cid, collectIdNext, dateCreate, xmlDenom))
				{
					...
					emit emit_Loging(
						0, senderName,
						QString("Транзакции не инкассированных платежей с - %1 по - %2").arg(trnFrom, trnTo));
		...
		{
			//            if (remoteCollection) {
			//                emit emit_Loging(0, senderName, QString("В системе отсутствует инкассация с cid:
			//                %1").arg(cid));
			...
			//                auto trnId      = "1" +
			//                QDateTime::currentDateTime().toString("yyyyMMddHHmmsszzz").right(15); auto cidNext    =
			//                getCollectionId();
			...
			//                QString strQuery = QString("INSERT INTO terminal_collect(collect_id, stack_id,
			//                date_create, denom, status, collect_id_next)"
			//                                           " VALUES('%1', %2 , \"%3\", '%4', 'send', '%5');").arg(cid,
			//                                           trnId, dateCreate, xmlDenom, cidNext);
	...
	if (!preCheck && trnId != "")
	{
		// Тут надо создать XML с инкассацией
		requestXml = "";
		getXmlForSend(requestXml, collectId, collectIdNext, trnId, dateCreate, xmlDenom, moneyOutNum, moneyOutSum,
					  trnFrom, trnTo);
...
void CollectDaemons::getXmlForSend(QString &xml, QString cid, QString cidNext, QString sid, QString dateCreate,
								   QString collectDenom, int moneyOutCount, double moneyOutSum, QString trnFrom,
								   QString trnTo)
{
...
bool CollectDaemons::getCollectionInfo(QString collectionId, QString &collectionIdNext, QString &idTrn,
									   QString &dateCreate, QString &denomXml)
{
	QSqlQuery selectCollect(db);
...
bool CollectDaemons::getMoneyOut(const QString &collectionId, int &numOut, double &sumOut)
{
	...
	strQuery =
		QString(
			"SELECT * FROM terminal_moneyout WHERE moneyout_collect_id = '%1' AND moneyout_collect_status = 'send';")
			.arg(collectionId);
	// if(Debuger) qDebug() << strQuery;
...
bool CollectDaemons::getTrnOperation(const QString collectId, const QString &cmd, QString &trn)
{
	...
	strCollect = QString("SELECT operation_id FROM terminal_operation WHERE operation_collect_id = '%1' ORDER BY "
...
bool CollectDaemons::getDatePrevCollection(QString &date, QString collectionId)
{
	...
	QString strCollect =
		QString("SELECT date_create FROM terminal_collect WHERE collect_id_next = '%1' LIMIT 1;").arg(collectionId);
...
bool CollectDaemons::sendCollection(QString &trnId, const QString collectionId, QString &collectionIdNext,
									QString &dateCreate, QString xmlDenom)
{
	...
	QString strCollect = QString("UPDATE terminal_collect SET stack_id = %2, date_create = \"%3\", denom = '%4', "
...
bool CollectDaemons::confirmCollection(QString collectionId)
{
	...
	strUpdateCollect =
		QString("UPDATE terminal_collect SET status='confirmed' WHERE collect_id = '%1'").arg(collectionId);
...
int CollectDaemons::getNonCollectOperationCount(const QString cid)
{
	...
	QString query =
		QString("SELECT count(*) as count FROM terminal_operation WHERE operation_collect_id = '%1';").arg(cid);
