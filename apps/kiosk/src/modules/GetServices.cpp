...
void GetServices::parcerNote(const QDomNode &domElement) {
  ...
  while (!domNode.isNull()) {
    if (domNode.isElement()) {
      ...
      if (strTag == "name") {
        infoMap["name_agent"] =
            domElement.text().replace("\'", "").replace("\"", "");
      ...
      if (strTag == "prv") {
        ...
        providerList[index_prv]["category_id"] =
            domElement.attribute("cat", "");
        ...
        providerList[index_prv]["name_ru"] =
            domElement.attribute("nru", "").trimmed();
        providerList[index_prv]["name_en"] =
            domElement.attribute("nen", "").trimmed();
        ...
        providerList[index_prv]["return_max"] =
            domElement.attribute("hmax", "");
      ...
      if (strTag == "categories") {
        ...
        category["name_local"] =
            domElement.attribute("name_loc").replace("'", "''");
      ...
      if (strTag == "commission") {
        auto serviceId =
            domElement.parentNode().toElement().attribute("id", "");
        ...
        if (serviceId != "") {
          ...
          saveCommissionDB(cid.toInt(), sumFrom, sumTo, typeId, value,
                           serviceId.toInt(), index);
        }
      }
...
bool GetServices::saveCommissionDB(const int id, const double sumFrom,
                                   const double sumTo, const int type,
                                   const double value, const int serviceId,
                                   const int index) {
...
bool GetServices::saveCategoriesDB(const QVariantMap category) {
  ...
  QString query =
      QString("INSERT INTO terminal_categories(id, name_local, name_secondary, "
...
bool GetServices::saveInputsDB(const QVariantMap input) {
  ...
  QString query =
      QString(
          "INSERT INTO terminal_inputs(id, input_panel, regexp, mask, "
...
bool GetServices::saveServicesInputsDB() {
  ...
  sqlQuery.prepare(
      "INSERT INTO terminal_services_inputs(input_id, service_id, field, "
...
bool GetServices::insertTerminalExtra(QVariantMap map) {
  ...
  QString query =
      QString("INSERT INTO terminal_extra( id, id_agent, name, inn, phone, "
...
bool GetServices::getTerminalExtra(QVariantMap &map) {
  ...
  if (selectQuery.next()) {
    ...
    map["usd_curce"] =
        selectQuery.value(record.indexOf("currency_usd")).toString();
...
void GetServices::sendGetServicesQuery() {
  ...
  emit emit_Loging(0, senderName,
                   "Отправлен запрос на получение конфигурации.");
