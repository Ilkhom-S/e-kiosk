...
void CheckOnline::sendCheckOnlineRequest(QString trn, QString prvId,
                                         QString account, double amount,
                                         QVariantMap param) {
  ...
  QString json =
      QJsonDocument::fromVariant(param).toJson(QJsonDocument::Compact);
  QString paramStr =
      param.isEmpty() ? "" : QString("<param>%1</param>\n").arg(json);
  QString amountStr =
      amount > 0 ? QString("<amount>%1</amount>\n").arg(amount) : "";

  QString xml =
      QString(header_xml + "<trn>%1</trn>\n" + "<prv_id>%2</prv_id>\n" +
