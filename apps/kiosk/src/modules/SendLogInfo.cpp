...
void SendLogInfo::resendRequest() {
  if (countAllRep < 20) {
    // Повторная отправка
    QTimer::singleShot(20000 * this->countAllRep, this,
                       SLOT(sendRequestRepeet()));
  } else {
...
void SendLogInfo::sendRequestRepeet() {
  ...
  emit emit_Loging(0, "SEND_LOG_INFO",
                   QString("Попытка N-%1 отправки на сервер лога с id - %2")
                       .arg(countAllRep)
                       .arg(nowIdTrn));
...
void SendLogInfo::sendLogInfoToServer(QString trn, QString date) {
  ...
  // дата создания
  QString vrmDate =
      QDate::fromString(date, "yyyy-MM-dd").toString("dd.MM.yyyy");
  QString striper = "";
  bool result = false;
  ...
  QString center_xml =
      QString("<messages>\n"
              ...
              "</message>\n"
              "</messages>\n")
          .arg(idTrn, dateCreate == "" ? vrmDate : dateCreate, vrmDate, striper,
               trn);
...
void SendLogInfo::sendLogValidatorToServer(QString trn, QString date,
                                           QString account) {
  QString header_xml = getHeaderRequest(Request::Type::SendLogInfo);
  ...
  QString center_xml =
      QString("<messages>\n"
              ...
              "</message>\n"
              "</messages>\n")
          .arg(idTrn, dateCreate == "" ? vrmDate : dateCreate, account, striper,
               date, trn);
...
void SendLogInfo::getCompressLogData(QString date, bool &result,
                                     QString &strip) {
  QByteArray ba;
...
void SendLogInfo::getCompressValiatorLogData(QString date, QString account,
                                             bool &result, QString &strip) {
  QByteArray ba;
