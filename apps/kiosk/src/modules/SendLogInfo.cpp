...
void SendLogInfo::resendRequest()
{
	...
	else
	{
		emit emit_Loging(
			2, "SEND_LOG_INFO",
			QString("Невозможно отправить лог с id - %1 на сервер, количество повторов превысело 20").arg(nowIdTrn));
	}
}
...
void SendLogInfo::sendLogValidatorToServer(QString trn, QString date, QString account)
{
	...
	emit emit_Loging(
		0, "SEND_LOG_VALIDATOR_INFO",
		QString("Начинаем отправлять лог валидатора на сервер(id - %1, дата создания - %2, наименование - %3)")
