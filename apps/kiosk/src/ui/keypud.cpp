...
keyPud::keyPud(QWidget *parent) : QWidget(parent), ui(new Ui::keyPud) {
  ...
  //     connect(ui->KEY_CLEAR, SIGNAL(released()),     &signalMapper,
  //     SLOT(map()));
...
void keyPud::buttonClicked(QWidget *w) {
  ...
  if (shiftKEY) {
    ...
    if ((on == "KEY_CLEAR") || (on == "KEY_BACKSPACE") || (on == "KEY_TAB") ||
        (on == "KEY_ENTER"))
      chr = BcharMap[on];
    else {
...
keyPud::~keyPud() { delete ui; }

void keyPud::clickBackspace() { ui->KEY_BACKSPACE->click(); }
