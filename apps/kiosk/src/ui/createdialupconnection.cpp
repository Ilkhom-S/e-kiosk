...
CreateDialupConnection::CreateDialupConnection(QWidget *parent)
  ...
  connect(ui->btnDialupCreateNewCon, SIGNAL(clicked()),
          SLOT(btnCreateNewConClc()));
}
...
void CreateDialupConnection::btnCreateNewConClc() {
  ...
  messageBox.setText(
      QString("Вы действительно хотите создать<br/>"
          ...
          .arg(data["device"].toString(), data["connection"].toString(),
               data["phone"].toString(), data["login"].toString(),
...
CreateDialupConnection::~CreateDialupConnection() { delete ui; }
