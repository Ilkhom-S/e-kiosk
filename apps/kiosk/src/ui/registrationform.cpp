...
RegistrationForm::RegistrationForm(QWidget *parent)
  ...
  connect(ui->btnEditValidator, SIGNAL(clicked(bool)),
          SLOT(showValidatorData(bool)));
  connect(ui->btnEditCoinAcceptor, SIGNAL(clicked(bool)),
          SLOT(showCoinAcceptorData(bool)));
  connect(ui->btnEditPrinter, SIGNAL(clicked(bool)),
          SLOT(showPrinterData(bool)));
  connect(ui->btnEditWatchDog, SIGNAL(clicked(bool)),
          SLOT(showWatchdogData(bool)));
  connect(ui->btnEditModem, SIGNAL(clicked(bool)), SLOT(showModemData(bool)));

  connect(ui->chbValidatorSearch, SIGNAL(toggled(bool)),
          SLOT(searchValidatorToggle(bool)));
  connect(ui->chbCoinAcceptorSearch, SIGNAL(toggled(bool)),
          SLOT(searchCointAcceptorToggle(bool)));
  connect(ui->chbPrinterSearch, SIGNAL(toggled(bool)),
          SLOT(searchPrinterToggle(bool)));
  connect(ui->chbWatchDogSearch, SIGNAL(toggled(bool)),
          SLOT(searchWatchdogToggle(bool)));
  connect(ui->chbModemSearch, SIGNAL(toggled(bool)),
          SLOT(searchModemToggle(bool)));
  ...
  connect(ui->cbxCoinAcceptorName, SIGNAL(activated(int)),
          SLOT(coinAcceptorSave()));
  connect(ui->cbxCoinAcceptorPort, SIGNAL(activated(int)),
          SLOT(coinAcceptorSave()));
  ...
  connect(ui->radioDialUpConnection, SIGNAL(toggled(bool)),
          SLOT(showDialUpParams(bool)));
  connect(ui->listWidgetConnectionList, SIGNAL(currentTextChanged(QString)),
          SLOT(connectionSelect(QString)));

  QRegExpValidator *loginRegValidator = new QRegExpValidator(
      QRegExp("[\\S\\w\\W\\d\\D]{4,15}"), ui->editLoginReg);
  ui->editLoginReg->setValidator(loginRegValidator);

  QRegExpValidator *passRegValidator =
      new QRegExpValidator(QRegExp("[\\S\\w\\W\\d\\D]{4,15}"), ui->editOtpReg);
  ui->editOtpReg->setValidator(passRegValidator);

  QRegExpValidator *secretLoginValidator = new QRegExpValidator(
      QRegExp("[\\S\\w\\W\\d\\D]{1,15}"), ui->editSecretLoginReg);
  ui->editSecretLoginReg->setValidator(secretLoginValidator);

  QRegExpValidator *secretPassValidator = new QRegExpValidator(
      QRegExp("[\\S\\w\\W\\d\\D]{1,15}"), ui->editSecretPasswordReg);
  ui->editSecretPasswordReg->setValidator(secretPassValidator);

  QRegExpValidator *secretPassConfirmValidator = new QRegExpValidator(
      QRegExp("[\\S\\w\\W\\d\\D]{1,15}"), ui->editSecretPasswordConfirmReg);
  ui->editSecretPasswordConfirmReg->setValidator(secretPassConfirmValidator);

  connect(ui->editLoginReg, SIGNAL(textChanged(QString)),
          SLOT(checkAuthInput(QString)));
  connect(ui->editOtpReg, SIGNAL(textChanged(QString)),
          SLOT(checkAuthInput(QString)));
  connect(ui->editSecretLoginReg, SIGNAL(textChanged(QString)),
          SLOT(checkAuthInput(QString)));
  connect(ui->editSecretPasswordReg, SIGNAL(textChanged(QString)),
          SLOT(checkAuthInput(QString)));
  connect(ui->editSecretPasswordConfirmReg, SIGNAL(textChanged(QString)),
          SLOT(checkAuthInput(QString)));

  connect(ui->btnSecretPassword, SIGNAL(toggled(bool)),
          SLOT(secretPassView(bool)));
  connect(ui->btnSecretPasswordConfirm, SIGNAL(toggled(bool)),
          SLOT(secretPassConfirmView(bool)));
...
void RegistrationForm::btnNextClicked() {
  ...
  case Page::SelectConnection: {
    ...
    QString connectionName =
        ui->radioDialUpConnection->isChecked()
  ...
  if (resultCode != "" && resultCode.toInt() == 0 && token != "") {
    ...
    data["connection"] =
        ui->radioDialUpConnection->isChecked()
            ? ui->listWidgetConnectionList->currentItem()->text()
            : "Local Connection";
    data["sim_balance_req"] =
        ui->cbxRequestBalanceSimReg->currentText().trimmed();
    data["sim_balance_position"] =
        ui->cbxPositionBalanceSimReg->currentText().trimmed();
    data["sim_number_req"] =
        ui->cbxRequestNumberSimReg->currentText().trimmed();
...
void RegistrationForm::clearStatusText() { ui->lblStatus->clear(); }
...
void RegistrationForm::addPageData() {
  ...
  case Page::SelectTpl:
    ...
    btnNextEnabled =
        ui->radioTplTJK->isChecked() || ui->radioTplUZB->isChecked();
    break;
  case Page::SearchDevice: {
...
void RegistrationForm::searchValidatorToggle(bool value) {
  ...
  ui->labelDeviceValidator->setText(
      QString("<font color=\"%2\">%1</font>")
          .arg("Купюроприемник", value ? "black" : "gray"));
}
...
void RegistrationForm::searchCointAcceptorToggle(bool value) {
  ...
  ui->labelDeviceCoinAcceptor->setText(
      QString("<font color=\"%2\">%1</font>")
          .arg("Монетоприемник", value ? "black" : "gray"));
}
...
void RegistrationForm::searchWatchdogToggle(bool value) {
  ...
  ui->labelDeviceWatchDog->setText(
      QString("<font color=\"%2\">%1</font>")
          .arg("Сторожевой таймер", value ? "black" : "gray"));
}
...
void RegistrationForm::searchModemToggle(bool value) {
  ...
  ui->labelDeviceModem->setText(QString("<font color=\"%2\">%1</font>")
                                    .arg("Модем", value ? "black" : "gray"));
}
...
void RegistrationForm::deviceSearchResult(int device, int result,
                                          QString dev_name, QString dev_comment,
                                          QString dev_port) {
  switch (device) {
...
void RegistrationForm::showMe() {
  ...
  ui->cbxValidatorName->setCurrentText(
      devices.value("validator_name").toString());
  ui->cbxValidatorPort->setCurrentText(
      devices.value("validator_port").toString());

  ui->cbxCoinAcceptorName->setCurrentText(
      devices.value("coin_acceptor_name").toString());
  ui->cbxCoinAcceptorPort->setCurrentText(
      devices.value("coin_acceptor_port").toString());
  ...
  ui->cbxWinprinter->setCurrentText(
      devices.value("printer_comment").toString());
  ...
  ui->cbxWatchdogPort->setCurrentText(
      devices.value("watchdog_port").toString());
...
bool RegistrationForm::isValidAuthInput() {
  bool validAuthData = ui->editLoginReg->hasAcceptableInput() &&
                       ui->editOtpReg->hasAcceptableInput();
  bool validAuthAdminData =
      ui->editSecretLoginReg->hasAcceptableInput() &&
      ui->editSecretPasswordReg->hasAcceptableInput() &&
      ui->editSecretPasswordConfirmReg->hasAcceptableInput();
...
void RegistrationForm::closeMe() { this->close(); }
