...
MainPageLoader::MainPageLoader(QWidget *parent)
    : QWidget(parent), ui(new Ui::MainPageLoader) {
  ui->setupUi(this);
...
void MainPageLoader::populateJavaScriptWindowObject() {
  // QtWebEngine uses a different approach for exposing C++ objects
  // This will be set up through QWebEngineScript in production code
...
void MainPageLoader::inspectEnable() {
  // QtWebEngine developer tools can be enabled at runtime
  // This is typically handled by the environment or through QWebEngineProfile
...
void MainPageLoader::showHideReturnNominal(bool status) {
  ...
  QString jsFunction = status ? QString("showReturnNominal(%1)").arg(amountMax)
                              : "hideReturnNominal()";
...
void MainPageLoader::setPageStatus(PageIn::page page) {
  ...
  if (page == PageIn::InputNumber || page == PageIn::InputSum ||
      page == PageIn::PrintDialog || page == PageIn::AfterPrint) {
    updaterLock = true;
  }
...
void MainPageLoader::precheck(QString idPrv, QString account, double amount) {
  ...
  gblTrnOnlineCheck = QString("%1%2").arg(
  ...
  QString param =
      QJsonDocument::fromVariant(paramPrecheck).toJson(QJsonDocument::Compact);
  emit emit_toLoging(
      0, "MAIN",
      QString("Переходим на страницу пречека account- %1, param- %2")
          .arg(account, param));

  emit emit_checkOnline(gblTrnOnlineCheck, idPrv, account, amount,
                        paramPrecheck);
}
...
void MainPageLoader::orzuUserDefine(QString inn) {
  ...
  emit emit_sendJsonRequest(QJsonObject(), QString("orzu/user/%1").arg(inn),
                            "orzu_user_define", 0, 20);
}

void MainPageLoader::orzuOtpResend() {
  auto phone = orzuInfo["phone_number"].toString();
  emit emit_toLoging(0, "ORZU",
                     QString("Отправка отп на номер: %1").arg(phone));
...
void MainPageLoader::orzuOtpConfirm(QString inn, QString otp) {
  ...
  emit emit_toLoging(0, "ORZU",
                     QString("Подтверждение введенного otp: %1").arg(otp));

  emit emit_sendJsonRequest(json, "orzu/otp/confirm-otp", "orzu_otp_confirm", 1,
                            20);
}
...
void MainPageLoader::orzuConditionsGet(int amount) {
  ...
  auto url =
      QString("orzu/business/conditions?orzu_id=%1&service_id=%2&summa=%3")
          .arg(orzuId, "118")
          .arg(amount);
  ...
  emit emit_sendJsonRequest(QJsonObject(), url, "orzu_business_conditions", 0,
                            20, header);

  emit emit_toLoging(0, "ORZU",
                     QString("Отправляется запрос на получения условий "
                             "кредита. Введенная сумма: %1")
                         .arg(amount));
}
...
void MainPageLoader::orzuTranshCreate() {
  ...
  emit emit_sendJsonRequest(json, "orzu/business/transh",
                            "orzu_business_transh", 1, 30, header);
}

void MainPageLoader::jsonResponseSuccess(QVariantMap response,
                                         QString requestName) {
  QString jsFunc;
  ...
  if (requestName == "orzu_user_define") {
    ...
    emit emit_toLoging(0, "ORZU",
                       QString("Определен клиент: %1")
                           .arg(orzuInfo.value("client_name").toString()));
  ...
  if (requestName == "orzu_business_conditions") {
    ...
    emit emit_toLoging(0, "ORZU",
                       QString("Получены условия кредита - мин. сумма: %1, "
                               "макс. сумма: %2, срок кредита: %3")
                           .arg(min)
                           .arg(max)
                           .arg(terms.join(",")));
...
void MainPageLoader::otpSend(QString account) {
  ...
  emit emit_toLoging(0, "OTP",
                     QString("Отправка otp на номер: %1").arg(account));
...
void MainPageLoader::loadHtmlPage(PageIn::page page) {
  ...
  switch (page) {
  case PageIn::Main:
    currentWebPage = QString("file:///%1?lng=%2")
                         .arg(mainWebPage.absoluteFilePath(), _langDefault);
    break;
  case PageIn::LockTerminal:
    currentWebPage = QString("file:///%1?lng=%2")
                         .arg(lockWebPage.absoluteFilePath(), _langDefault);
    break;
  default:
    break;
  }
...
void MainPageLoader::gotoPageInsertNominal(QString account,
                                           QString accountDesign,
                                           int serviceId) {
...
void MainPageLoader::gotoPage(PageIn::page page) {
  ...
  case PageIn::InputSum: {
    emit emit_toLoging(
        0, "MAIN",
        QString("Номер абонента %1 на провайдер %2 подтвержден, переходим на "
                "страницу ввода денег.")
...
void MainPageLoader::setCheckOnlineResult(QString resultCode, QString status,
                                          QString message, QVariantList items) {
  Q_UNUSED(status)
...
void MainPageLoader::sendReceiptResult(QString resultCode, QString trn,
                                       QString status) {
  Q_UNUSED(trn)
  Q_UNUSED(status)
  ...
  if (resultCode == "1") {
    message =
        QString("Ошибка при отправке эл.чека (код ошибки %1)").arg(resultCode);
    jsFunc = QString("resultError(\"%1\")").arg(message);
    // Успешно
  } else if (resultCode == "0") {
...
void MainPageLoader::sendOtpResult(QString resultCode, QString otpId) {
  ...
  if (resultCode == "1") {
    message =
        QString("Ошибка при отправке смс кода (код ошибки %1)").arg(resultCode);
...
QVariantMap MainPageLoader::homeData() {
  ...
  QStringList images = directory.entryList(
      QStringList() << "*.png" << "*.jpg" << "*.svg", QDir::Files);
...
void MainPageLoader::favoriteServicesInit() {
  ...
  std::stable_sort(favorites.begin(), favorites.end(),
                   [=](const QVariant &d1, const QVariant &d2) -> bool {
                     return d1.toMap().value("favorite").toInt() <
                            d2.toMap().value("favorite").toInt();
                   });
  ...
  if (count_comis > 1) {
    ...
    for (int i = 1; i <= count_comis; i++) {
      if (amountFrom >= commissionMap[i]["sum_from"].toDouble() &&
        ...
        nominalAmount =
            getMoneyToFromAll(commissionMap[i]["type"].toInt(), amountFrom,
    ...
    nominalAmount =
        getMoneyToFromAll(commissionMap[1]["type"].toInt(), amountFrom,
...
double MainPageLoader::getMoneyToFromAll(int cmsType, double from,
                                         double value) {
  double sum_to_in = 0;
...
void MainPageLoader::getCommissionMap() {
  ...
  if (cid == 0) {
    commissionMap[1]["sum_from"] =
        _serviceCurrent.value("amount_min").toString();
    ...
    QString userQuery =
        QString("SELECT * FROM terminal_commission WHERE service_id = %1 AND "
    ...
    if (userSql.exec(userQuery)) {
      ...
      while (userSql.next()) {
        ...
        if (commissionMap[j]["sum_to"].toInt() <= 0) {
          commissionMap[j]["sum_to"] =
              _serviceCurrent.value("amount_max").toString();
        }
