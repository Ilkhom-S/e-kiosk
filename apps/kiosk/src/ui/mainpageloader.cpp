...
void MainPageLoader::precheck(QString idPrv, QString account, double amount)
{
	...
	gblTrnOnlineCheck = QString("%1%2").arg(QDateTime::currentDateTime().toString("yyMMddHHmmsszzz").left(14),
...
void MainPageLoader::orzuConditionsGet(int amount)
{
	...
	emit emit_toLoging(0, "ORZU",
					   QString("Отправляется запрос на получения условий кредита. Введенная сумма: %1").arg(amount));
}
...
void MainPageLoader::gotoPage(PageIn::page page)
{
	...
	case PageIn::SelectServices:
	{
		...
		emit emit_toLoging(
			0, "MAIN",
			QString("Переходим на страницу выбора оператора в группе %1").arg(_categoryCurrent.value("id").toInt()));
	}
	break;
	...
	{
		emit emit_toLoging(0, "MAIN",
						   QString("Номер абонента %1 на провайдер %2 подтвержден, переходим на страницу ввода денег.")
...
void MainPageLoader::inputNominal(int nominal, bool coin)
{
	...
	if (nominalInserted == 1)
	{
		...
		gblNowTranzaction = gblTrnOnlineCheck != ""
								? gblTrnOnlineCheck
...
void MainPageLoader::getSumToFromMinusCommis(double amountFrom)
{
	...
	else
	{
		...
		nominalAmount =
			getMoneyToFromAll(commissionMap[1]["type"].toInt(), amountFrom, commissionMap[1]["value"].toDouble());
		return;
	}
...
void MainPageLoader::getCommissionMap()
{
	...
	{
		QString userQuery =
			QString("SELECT * FROM terminal_commission WHERE service_id = %1 AND commission_id = %2 ORDER BY idx;")
				.arg(serviceId)
				.arg(cid);
