...
RegistrationDialog::RegistrationDialog(QWidget *parent)
  ...
  this->connect(ui->btnTabReg1, SIGNAL(clicked()), this,
                SLOT(tabToRegistration1()));
  this->connect(ui->btnTabReg2, SIGNAL(clicked()), this,
                SLOT(tabToRegistration2()));
  ...
  QRegExpValidator *loginRegValidator = new QRegExpValidator(
      QRegExp("[\\S\\w\\W\\d\\D]{1,30}"), ui->editLoginReg);
  ui->editLoginReg->setValidator(loginRegValidator);

  // Пароль пользователя
  QRegExpValidator *passRegValidator = new QRegExpValidator(
      QRegExp("[\\S\\w\\W\\d\\D]{1,30}"), ui->editPasswordReg);
  ui->editPasswordReg->setValidator(passRegValidator);

  QRegExpValidator *confPassRegValidator = new QRegExpValidator(
      QRegExp("[\\S\\w\\W\\d\\D]{1,30}"), ui->editPasswordReg);
  ...
  // Логин пользователя
  QRegExpValidator *secLoginRegValidator = new QRegExpValidator(
      QRegExp("[\\S\\w\\W\\d\\D]{1,30}"), ui->editSecretLoginReg);
  ui->editSecretLoginReg->setValidator(secLoginRegValidator);

  // Пароль пользователя
  QRegExpValidator *secPassRegValidator = new QRegExpValidator(
      QRegExp("[\\S\\w\\W\\d\\D]{1,30}"), ui->editSecretPasswordReg);
  ui->editSecretPasswordReg->setValidator(secPassRegValidator);

  QRegExpValidator *secConfPassRegValidator = new QRegExpValidator(
      QRegExp("[\\S\\w\\W\\d\\D]{1,30}"), ui->editSecretConfirmPasswordReg);
  ...
  connect(ui->btnAdminCreateNewConnection, SIGNAL(clicked()),
          SLOT(createNewConnection()));

  connect(ui->btnRegCloseThis, SIGNAL(clicked()), SLOT(closeForm()));

  connect(ui->btnAdminSaveConnectionParam, SIGNAL(clicked()),
          SLOT(save3BtnReg()));
  ...
  connect(ui->tbnAddTerminalData, SIGNAL(currentChanged(int)), this,
          SLOT(tabChanged(int)));
...
void RegistrationDialog::save1BtnReg() {
  ...
  if (ui->editNumTrmReg->hasAcceptableInput() &&
      ui->editLoginReg->hasAcceptableInput() &&
    ...
    if (ui->editConfirmPasswordReg->text() == "") {
      ui->editConfirmPasswordReg->setToolTip(
          "Пароль состоит из 1-30 цифр/букв/знаков.");
...
void RegistrationDialog::save2BtnReg() {
  ...
  if (ui->editSecretLoginReg->hasAcceptableInput() &&
      ...
      (ui->editSecretPasswordReg->text() ==
       ui->editSecretConfirmPasswordReg->text())) {
    // Запись в переменные
    ...
    if (!ui->editSecretLoginReg->hasAcceptableInput()) {
      ui->editSecretLoginReg->setToolTip(
          "Логин состоит из 1-30 цифр/букв/знаков.");
...
void RegistrationDialog::starttest() {
  ...
  emit emitStartTest(this->gblNumTrm, this->gblLogin, this->gblPass,
                     this->gblSecretLogin, this->gblSecretPass, conName,
                     balanceReq, positionReq, numberReq);
}
