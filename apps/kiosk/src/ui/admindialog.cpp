...
AdminDialog::AdminDialog(QWidget *parent)
  ...
  connect(KeyPud, SIGNAL(characterGenerated(QChar)), this,
  ...
  connect(ui->cbxAdminTextDateIncashment, SIGNAL(currentIndexChanged(QString)),
          this, SLOT(getCollectDate(QString)));

  // Выбор опций просмотра логов
  selectCategoryLogView = new SelectCategoryLogView(this);
  connect(selectCategoryLogView,
          SIGNAL(emit_SelectOptions(bool, bool, bool, bool, bool, bool, bool,
                                    bool, bool)),
          SLOT(SelectOptionsForSearch(bool, bool, bool, bool, bool, bool, bool,
                                      bool, bool)));

  // Создание нового соединения
  createDialupConnection = new CreateDialupConnection(this);
  connect(createDialupConnection, SIGNAL(emitDialupParam(QVariantMap)),
  ...
  // Сделать инкасацию
  connect(ui->btnAdminCreateIncashment, SIGNAL(clicked()),
          SLOT(doCollectExec()));
  connect(ui->btnAdminCreatePreIncashment, SIGNAL(clicked()),
          SLOT(doCollectDateExec()));
  connect(ui->btnAdminTestRestartValidator, SIGNAL(clicked()),
          SLOT(restartValidator()));
  connect(ui->btnAdminTestPrintCheck, SIGNAL(clicked()),
          SLOT(printTestCheck()));
  connect(ui->btnAdminTestRestartModem, SIGNAL(clicked()),
          SLOT(restartModem()));
  ...
  connect(ui->btnSaveChangeTypeDevices, SIGNAL(clicked()),
          SLOT(saveDeviceParam()));
  connect(ui->btnAdminCheckConnection, SIGNAL(clicked()),
          SLOT(checkConnection()));
  connect(ui->btnAdminGetDataFromModem, SIGNAL(clicked()),
          SLOT(getModemDataInfo()));
  connect(ui->btnAdminSaveTerminalAccept, SIGNAL(clicked()),
          SLOT(saveTrmAutorizationData()));
  connect(ui->btnAdminSelectLogParam, SIGNAL(clicked()),
          SLOT(openSelectCategory()));
  connect(ui->btnAdminUpdateLogData, SIGNAL(clicked()),
          SLOT(openLogInfoDate()));
  connect(ui->btnAdminNavigationUp, SIGNAL(clicked()), SLOT(go_to_up_log()));
  connect(ui->btnAdminNavigationDown, SIGNAL(clicked()),
          SLOT(go_to_doun_log()));
  connect(ui->btnAdminSearchKeyParam, SIGNAL(clicked()),
          SLOT(searchWithKeyParam()));
  connect(ui->btnAdminCreateNewConnections, SIGNAL(clicked()),
          SLOT(createNewConnection()));
  connect(ui->btnAdminGetActiveConnection, SIGNAL(clicked()),
          SLOT(getActiveRasCon()));
  connect(ui->btnAdminRestartNetwork, SIGNAL(clicked()), SLOT(cmdRestartNet()));
  connect(ui->btnAdminSaveConnectionParams, SIGNAL(clicked()),
          SLOT(saveConnectionParam()));
  connect(ui->btnAdminSavePrinterParam, SIGNAL(clicked()),
          SLOT(savePrinterParam()));
  connect(ui->btnAdminGetServicesData, SIGNAL(clicked()), SLOT(getServices()));
  connect(ui->btnAdminSaveTrmSettings2, SIGNAL(clicked()),
          SLOT(saveOtherSettings()));
  connect(ui->btnAdminSaveTerminaSecret, SIGNAL(clicked()),
          SLOT(saveUserAutorizationData()));
  connect(ui->btnAdminTestRestartModemReal, SIGNAL(clicked()),
          SLOT(restartModem()));
  connect(ui->cbxAdminNamePrinter, SIGNAL(currentIndexChanged(int)),
          SLOT(printerNameChanged(int)));
...
void AdminDialog::saveOtherSettings() {
  ...
  settings["status_validator_jam_in_box"] =
      ui->chbxAdminSendErrorCodValidatorJam->isChecked();
  ...
  settings["default_lang"] =
      ui->cbxAdminSelectMainLang->currentText().trimmed();
...
void AdminDialog::savePrinterParam() {
  ...
  settings["prt_win_width"] =
      ui->cbxAdminWidthWinprtCheck->currentText().trimmed().toInt();
  settings["prt_win_height"] =
      ui->cbxAdminHeightWinprtCheck->currentText().trimmed().toInt();
  settings["prt_win_font_size"] =
      ui->cbxAdminFontWinprtCheck->currentText().trimmed().toInt();
  ...
  settings["chek_small_beetwen_string"] =
      ui->chbxAdminSmallLineText->isChecked();
  settings["chek_small_text"] = ui->chbxAdminSmallCheck->isChecked();
  settings["chek_width"] =
      ui->cbxAdminWidthCheck->currentText().trimmed().toInt();
  settings["chek_left_size"] =
      ui->cbxAdminLeftMarginCheck->currentText().trimmed().toInt();
  ...
  settings["counter_ring_value"] =
      ui->editAdminLengthOfCounter->text().trimmed().toInt();
  settings["exist_counter_printer_chek"] =
      ui->chbxAdminCounterPrinterExist->isChecked();
  settings["printing_chek"] = ui->cbxPrintingChek->currentIndex();
...
void AdminDialog::saveConnectionParam() {
  ...
  settings["ussd_balance_sim"] =
      ui->cbxAdminRequestBalanceSim->currentText().trimmed();
  settings["index_check_balance"] =
      ui->cbxAdminPositionBalanceSim->currentText().trimmed();
  settings["ussd_number_sim"] =
      ui->cbxAdminRequestNumberSim->currentText().trimmed();
...
void AdminDialog::SelectOptionsForSearch(
    bool SelectValidatorJam, bool SelectMoneyOut, bool SelectERROR,
...
void AdminDialog::searchWithKeyParam() {
  ...
  QStringList lstLog;
  QString string;
  getLogDataFromFile(lstLog, string);
...
void AdminDialog::getModemDataInfo() {
  ...
  if (ui->cbxAdminConnectionList->currentText().toUpper() !=
      "LOCAL CONNECTION") {
    ...
    if (rr == QMessageBox::Yes) {
      ...
      auto data =
          QVariantMap({{"message", "Начинаем проверять данные SIM карты..."}});
...
void AdminDialog::saveDeviceParam() {
  ...
  settings["printer_comment"] = printerName == "WindowsPrinter"
                                    ? ui->cbxAdminWinprinter->currentText()
                                    : "";
  ...
  if (settings != deviceParam) {
    ...
    if (rr == QMessageBox::Yes) {
      ...
      ui->cbxAdminNamePrinter->setCurrentText(
          deviceParam.value("printer_name").toString());
      ui->cbxAdminPortPrinter->setCurrentText(
          deviceParam.value("printer_port").toString());

      ui->cbxAdminPortSpeed->setCurrentIndex(
          deviceParam["printer_comment"].toInt());
      ui->cbxAdminPortModem->setCurrentText(
          deviceParam.value("modem_port").toString());
      ui->cbxAdminPortWD->setCurrentText(
          deviceParam.value("watchdog_port").toString());

      ui->cbxAdminWinprinter->setCurrentText(
          deviceParam["printer_comment"].toString());
      settings.clear();
      return;
    }
  }
  ...
  settings["search_coin_acceptor"] =
      ui->chbxAdminSearchCoinAcceptor->isChecked();
...
void AdminDialog::printTestCheck() {
  ...
  if (ui->cbxAdminNamePrinter->currentText() != "") {
    ...
    printerComment = printerName == "WindowsPrinter"
                         ? ui->cbxAdminWinprinter->currentText()
                         : "";
...
void AdminDialog::setDataToAdmin(int cmd, QVariantMap data) {
  ...
  case AdminCommand::aCmdGetBalance: {
    ...
    // Порог отключения
    ui->lblAdminTextPorocOtclucheniya->setText(
        data.value("threshold").toString());
  } break;
  case AdminCommand::aCmdGetNewOperation: {
    // Новые платежи
    ui->lblTextAdminNumberNewPay->setText(
        data.value("payment_count").toString());
  } break;
  case AdminCommand::aCmdListAllIncash: {
    // Делаем список инкассаций по датам
    ui->cbxAdminTextDateIncashment->clear();
    ui->cbxAdminTextDateIncashment->addItems(
        data.value("encash_list").toStringList());
  } break;
  ...
  case AdminCommand::aCmdValidatorInform: {
    ...
    ui->lblAdminTextDeviceInf1->setText(
        data.value("validator_info").toString());

    // Список Валидаторов
    ui->cbxAdminNameValidator->clear();
    ui->cbxAdminNameValidator->addItems(
        data.value("validator_list").toStringList());

    // Устанавливаем текущий
    ui->cbxAdminNameValidator->setCurrentText(
        data.value("validator_name").toString());
    ...
    ui->cbxAdminPortValidator->addItems(
        data.value("validator_port_list").toStringList());

    // Текущий порт
    int index = ui->cbxAdminPortValidator->findText(
        data.value("validator_port").toString());
  ...
  case AdminCommand::aCmdCoinAcceptorInf: {
    ...
    ui->lblAdminTextAcceptor->setText(
        data.value("coin_acceptor_info").toString());

    // Список Валидаторов
    ui->cbxAdminNameAcceptor->clear();
    ui->cbxAdminNameAcceptor->addItems(
        data.value("coin_acceptor_list").toStringList());

    // Устанавливаем текущий
    ui->cbxAdminNameAcceptor->setCurrentText(
        data.value("coin_acceptor_name").toString());

    deviceParam["coin_acceptor_name"] = ui->cbxAdminNameAcceptor->currentText();

    // Список сом портов
    ui->cbxAdminPortAcceptor->clear();
    ui->cbxAdminPortAcceptor->addItems(
        data.value("coin_acceptor_port_list").toStringList());

    // Текущий порт
    int index = ui->cbxAdminPortAcceptor->findText(
        data.value("coin_acceptor_port").toString());
  ...
  case AdminCommand::aCmdPrinterInform: {
    ...
    ui->cbxAdminNamePrinter->addItems(
        data.value("printer_list").toStringList());

    // Win принтеры
    ui->cbxAdminWinprinter->clear();
    ui->cbxAdminWinprinter->addItems(
        data.value("winprinter_list").toStringList());
    ...
    ui->gbxAdminWindowsPrinterParam->setVisible(printerName ==
                                                "WindowsPrinter");
    ...
    ui->cbxAdminPortPrinter->addItems(
        data.value("printer_port_list").toStringList());

    // Текущий порт
    int index = ui->cbxAdminPortPrinter->findText(
        data.value("printer_port").toString());
  ...
  case AdminCommand::aCmdModemInform: {
    ...
    ui->cbxAdminPortModem->addItems(
        data.value("modem_port_list").toStringList());

    // Текущий порт
    int index =
        ui->cbxAdminPortModem->findText(data.value("modem_port").toString());
  ...
  case AdminCommand::aCmdWDInform: {
    // Показываем информацию
    ui->lblAdminTextDeviceInfWD1->setText(
        data.value("watchdog_comment").toString());
    ui->lblAdminTextDeviceInfWD2->setText(
        data.value("watchdog_name").toString());

    // Список сом портов
    ui->cbxAdminPortWD->clear();
    ui->cbxAdminPortWD->addItems(
        data.value("watchdog_port_list").toStringList());

    // Текущий порт
    int index =
        ui->cbxAdminPortWD->findText(data.value("watchdog_port").toString());
  ...
  case AdminCommand::aCmdSearchParamRef: {
    ui->chbxAdminSearchValidator->setChecked(
        data.value("search_validator").toBool());
    ui->chbxAdminSearchPrinter->setChecked(
        data.value("search_printer").toBool());
  ...
  case AdminCommand::aCmdSimInfoData: {
    ui->lblAdminTextPrvSim->setText(
        data.value("modem_sim_provider").toString());
    ui->lblAdminTextNumberSim->setText(
        data.value("modem_sim_number").toString());
    ui->lblAdminTextUrovanSignala->setText(
        data.value("modem_sim_rate").toString());
    ui->lblAdminTextBalanceSim->setText(
        data.value("modem_sim_balance").toString());
  } break;
  case AdminCommand::aCmdRasConnlist: {
    ...
    ui->cbxAdminConnectionList->setCurrentText(
        data.value("vpn_point").toString());
  } break;
  ...
  case AdminCommand::aCmdModemInfData: {
    ui->chbxAdminCheckBalanceSim->setChecked(
        data.value("check_balance_sim").toBool());
    ui->chbxAdminCheckNumberSim->setChecked(
        data.value("check_number_sim").toBool());
    ui->cbxAdminRequestBalanceSim->setCurrentText(
        data.value("ussd_balance_sim").toString());
    ui->cbxAdminPositionBalanceSim->setCurrentText(
        data.value("index_check_balance").toString());
    ui->cbxAdminRequestNumberSim->setCurrentText(
        data.value("ussd_number_sim").toString());
  } break;
  case AdminCommand::aCmdPrinterInfData: {
    ui->chbxAdminShowPrintDialog->setChecked(
        data.value("show_print_dialog").toBool());
    ui->chbxAdminSmallLineText->setChecked(
        data.value("chek_small_beetwen_string").toBool());
    ...
    ui->cbxAdminLeftMarginCheck->setCurrentText(
        data.value("chek_left_size").toString());
  ...
  case AdminCommand::aCmdSmsSending: {
    ...
    ui->chbxAdminSMSerrValidator->setChecked(
        data.value("sms_err_validator").toBool());
  } break;
  case AdminCommand::aCmdWinPrinterParam: {
    ui->cbxAdminWidthWinprtCheck->setCurrentText(
        data.value("prt_win_width").toString());
    ui->cbxAdminHeightWinprtCheck->setCurrentText(
        data.value("prt_win_height").toString());
    ui->cbxAdminFontWinprtCheck->setCurrentText(
        data.value("prt_win_font_size").toString());
  ...
  case AdminCommand::aCmdCounterCheckInf: {
    ui->chbxAdminCounterExist->setChecked(
        data.value("exist_counter_chek").toBool());
    ui->editAdminLengthOfRulon->setText(
        data.value("counter_len_rulon").toString());
    ui->editAdminLengthOfCheck->setText(
        data.value("counter_len_chek").toString());
    ui->editAdminLengthOfCounter->setText(
        data.value("counter_ring_value").toString());
  } break;
  case AdminCommand::aCmdCounterCheckVal: {
    ui->lblAdminResultCounterInfo->setText(
        data.value("counter_info").toString());
  } break;
  ...
  case AdminCommand::aCmdOtherSettings: {
    ...
    ui->cbxAdminSelectMainLang->setCurrentText(
        data.value("default_lang").toString());
    ui->chbxAdminTornOnAvtoUpdate->setChecked(
        data.value("auto_update_status").toBool());
    ui->chbxAdminCounterPrinterExist->setChecked(
        data.value("exist_counter_printer_chek").toBool());
    ui->chbxAdminNominalDuplicate->setChecked(
        !data.value("lock_duplicate_nominal").toBool());
  } break;
