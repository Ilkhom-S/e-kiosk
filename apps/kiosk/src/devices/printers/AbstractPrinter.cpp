...
QByteArray BasePrinterDevices::packetImage(const QString &aPixelString,
                                           uchar aWidth) {
  QByteArray result;
  uchar height = 0;
  if (aWidth)
    height = (uchar)(aPixelString.size() / aWidth);
  uchar horizontalSize =
      aWidth; // Размер по горизонтали, который мы передадим принтеру
  uchar verticalSize =
      height / 8; // Размер по вертикали, который мы передадим принтеру
  if (height % 8) {
    verticalSize++;
  }
...
void BasePrinterDevices::setChekSmall(bool smallChek) {
  qDebug() << QString("BasePrinterDevices::setChekSmall - small - %1")
                  .arg(smallChek);
  this->smallChek = smallChek;
  qDebug() << QString("BasePrinterDevices::setChekSmall - smallChek - %1")
                  .arg(smallChek);
}
...
bool BasePrinterDevices::sendCommand(QByteArray dataRequest, bool getResponse,
                                     int timeResponse, bool &respOk,
...
QByteArray BasePrinterDevices::encodingString(const QString &text,
                                              const QByteArray charCode) {
  // Меняем кодировку
