...
bool ClassPrinter::isItYou(QStringList &comList, QString &printer_name,
                           QString &com_str, QString &printer_coment) {
...
void ClassPrinter::clearListPrinterData(QString name) {
  ...
  if (numJobs == 0) {
    ...
    EnumJobs(hPrinter, 0, numJobs, 1, (LPBYTE)pJobInfo, 0, &bytesNeeded,
             &jobsReturned);
    pJobInfo = (JOB_INFO_1 *)malloc(bytesNeeded);
    EnumJobs(hPrinter, 0, numJobs, 1, (LPBYTE)pJobInfo, bytesNeeded,
             &bytesNeeded, &jobsReturned);

    // Loop and delete each waiting job
    for (int count = 0; count < int(jobsReturned); count++) {
      // qDebug() << "Deleting JobID  " << pJobInfo[count].JobId;

      if (SetJob(hPrinter, pJobInfo[count].JobId, 0, NULL,
                 JOB_CONTROL_DELETE) != 0) {
        // qDebug() << "...... Deleted OK";
      } else {
...
bool IsPrinterError(HANDLE hPrinter) {
  ...
  if (dwPrinterStatus == PRINTER_STATUS_ERROR ||
      dwPrinterStatus == PRINTER_STATUS_PAPER_JAM ||
      ...
      dwPrinterStatus == PRINTER_STATUS_DOOR_OPEN) {
    free(pJobs);
    return true;
  }
  ...
  for (i = 0; i < cJobs; i++) {
    if (pJobs[i].Status == JOB_STATUS_PRINTING) {
      ...
      if (pJobs[i].Status == JOB_STATUS_ERROR ||
          pJobs[i].Status == JOB_STATUS_OFFLINE ||
