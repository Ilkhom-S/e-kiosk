...
bool ClassPrinter::isItYou(QStringList &comList, QString &printer_name, QString &com_str,
...
void ClassPrinter::setCounterPrinterIndicator(bool sts_indicate) {
    if (systemModel == PrinterModel::Custom_VKP80) CustomVKP80->counterIndicate = sts_indicate;
    if (systemModel == PrinterModel::Citizen_PPU700) CitizenPPU700->counterIndicate = sts_indicate;
...
void ClassPrinter::setPrinterModel(const QString printerModel) {
    ...
    if (systemModel == PrinterModel::Custom_VKP80) CustomVKP80 = new CustomVKP80_PRINTER();
    if (systemModel == PrinterModel::Custom_TG2480) CustomTG2480 = new TG2480_PRINTER();
    if (systemModel == PrinterModel::AV_268) AV268 = new AV268_PRINTER();
    if (systemModel == PrinterModel::CitizenCBM1000) CitizenCBM1000 = new CitizenCBM1000_PRINTER();
    if (systemModel == PrinterModel::Citizen_PPU700) CitizenPPU700 = new CitizenPPU700_PRINTER();
    if (systemModel == PrinterModel::Citizen_CTS2000) CitizenCTS2000 = new CitizenCTS2000_PRINTER();
    if (systemModel == PrinterModel::Phoenix_model) Phoenix = new Phoenix_PRINTER();
    if (systemModel == PrinterModel::KM1X) KM1X = new KM1X_PRINTER();

    if (systemModel == PrinterModel::Windows_Printer) textBrowser = new QTextBrowser();
}

void ClassPrinter::setChekWidth(const int width) {
    chekWidth = width;
    if (systemModel == PrinterModel::Custom_VKP80) CustomVKP80->setChekWidth(width);
}

void ClassPrinter::setFirmPatern(const QString firm_name) {
    firmName = firm_name;
    if (systemModel == PrinterModel::Custom_VKP80) CustomVKP80->setFirmPatern(firmName);
    if (systemModel == PrinterModel::Citizen_PPU700) CitizenPPU700->setFirmPatern(firmName);
    if (systemModel == PrinterModel::Citizen_CTS2000) CitizenCTS2000->setFirmPatern(firmName);
}

void ClassPrinter::setSmallBeetwenString(bool beet) {
    smallBeetwenStr = beet;

    if (systemModel == PrinterModel::Custom_VKP80) CustomVKP80->setSmallBeetwenString(beet);
    if (systemModel == PrinterModel::Citizen_PPU700) CitizenPPU700->setSmallBeetwenString(beet);
    if (systemModel == PrinterModel::Citizen_CTS2000) CitizenCTS2000->setSmallBeetwenString(beet);
}

void ClassPrinter::setSmallText(bool small_i) {
    if (systemModel == PrinterModel::Custom_VKP80) CustomVKP80->setChekSmall(small_i);
    if (systemModel == PrinterModel::Citizen_PPU700) CitizenPPU700->setChekSmall(small_i);
    if (systemModel == PrinterModel::Citizen_CTS2000) CitizenCTS2000->setChekSmall(small_i);
}

void ClassPrinter::setComList(const QStringList list) { comList = list; }

void ClassPrinter::setLeftMargin(int left_margin) {
    leftMargin = left_margin;
    if (systemModel == PrinterModel::Custom_VKP80) CustomVKP80->setLeftMargin(leftMargin);
    if (systemModel == PrinterModel::Custom_TG2480) CustomTG2480->setLeftMargin(leftMargin);
    if (systemModel == PrinterModel::AV_268) AV268->setLeftMargin(leftMargin);
    if (systemModel == PrinterModel::CitizenCBM1000) CitizenCBM1000->setLeftMargin(leftMargin);
    if (systemModel == PrinterModel::Citizen_PPU700) CitizenPPU700->setLeftMargin(leftMargin);
    if (systemModel == PrinterModel::Citizen_CTS2000) CitizenCTS2000->setLeftMargin(leftMargin);
    if (systemModel == PrinterModel::Phoenix_model) Phoenix->setLeftMargin(leftMargin);
}

void ClassPrinter::setPortName(const QString port_name) { portName = port_name; }
...
void ClassPrinter::tarminateThis() { this->terminate(); }
...
bool ClassPrinter::CIsItYou(QString &comment) {
    if (systemModel == PrinterModel::Custom_VKP80) {
        if (CustomVKP80->isItYou()) {
            qDebug() << QString("ClassPrinter -- Availabled");
            if (CustomVKP80->comment != "") comment = "ROM v. " + CustomVKP80->comment;
            return true;
        } else {
            qDebug() << QString("ClassPrinter -- NotAvailabled");
            return false;
...
void ClassPrinter::winPrint(QString text) {
    if (this->winPrinterName != "") {
        ...
        if (printer.printerName() != "") textBrowser->print(&printer);
...
void ClassPrinter::CGetStatus() {
    ...
    if (systemModel == PrinterModel::Custom_VKP80) {
        ...
        if (getSts) {
            // qDebug() << QString("ClassPrinter -- get status true -- %1").arg(systemModel);
            ...
            // qDebug() << QString("ClassPrinter -- get status false -- %1").arg(systemModel);
            if (s_status.NotAvailabled) {
                // qDebug() << QString("ClassPrinter -- NotAvailabled");
            }
        }
...
bool ClassPrinter::printerOpen() {
    ...
    if (systemModel == PrinterModel::Custom_VKP80) {
        ...
        if (port_printer) {
            // qDebug() << QString("ClassPrinter -- port opened -- %1").arg(systemModel);
            open_port = true;
        } else {
            // qDebug() << QString("ClassPrinter -- error port opened -- %1").arg(systemModel);
            open_port = false;
        }
    }
...
void ClassPrinter::clearListPrinterData(QString name) {
    ...
    MultiByteToWideChar(0, 0, prtNameIn.toLatin1().data(), prtNameIn.length(), wStr,
                        prtNameIn.length());
    ...
    if (OpenPrinter(lpszStr, &hPrinter, NULL) == 0) {
    ...
    if (numJobs == 0) {
        ...
        EnumJobs(hPrinter, 0, numJobs, 1, (LPBYTE)pJobInfo, bytesNeeded, &bytesNeeded,
                 &jobsReturned);
...
bool IsPrinterError(HANDLE hPrinter) {
     ...
     */
    if (!GetJobs(hPrinter, &pJobs, &cJobs, &dwPrinterStatus)) return false;
