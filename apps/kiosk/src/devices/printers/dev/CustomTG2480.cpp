...
//                if (!devicePort->setFlowControl(AbstractSerial::FlowControlUndefined)) return false;
//                if (!serialPort->setCharIntervalTimeout(CMDTG2480::charTimeOut)) return false;
                if (!serialPort->setBaudRate(QSerialPort::Baud19200)) return false;

                //if(Debuger) qDebug() << "\nPrinter " << CMDTG2480::DeviceName << " to Port " << devicePort->deviceName() << " open in " << devicePort->openMode();
...
void TG2480_PRINTER::getSpecialCharecters(QByteArray& printText)
{
    ...
    printText.replace(QString(CScharsetParam::OpenTagDelimiter + CScharsetParam::FontTypeBold + CScharsetParam::CloseTagDelimiter).toUtf8(), asciiNull());
