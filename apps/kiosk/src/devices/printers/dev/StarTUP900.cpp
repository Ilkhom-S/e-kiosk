...
StarTUP900_PRINTER::StarTUP900_PRINTER(QObject *parent)
...
bool StarTUP900_PRINTER::cut() {
  ...
  return this->sendCommand(cmd, true, CMDStarTUP900::TimeOutAfterWriting,
                           respData, answer, 0);
}
...
bool StarTUP900_PRINTER::initialize() {
  ...
  if (!this->sendCommand(cmd, true, CMDStarTUP900::TimeOutAfterWriting,
                         respData, answer, 0))
    return false;
...
bool StarTUP900_PRINTER::feed(int aCount) {
  ...
  for (int i = 0; i < aCount; ++i) {
    if (!sendCommand(cmd, true, CMDStarTUP900::TimeOutAfterWriting, result,
                     answer, 0))
      return false;
  }
  return true;
