...
CitizenCTS2000_PRINTER::CitizenCTS2000_PRINTER(QObject *parent)
    : BasePrinterDevices(parent) {
  //    printer_name = "Custom-VKP80";
}

bool CitizenCTS2000_PRINTER::OpenPrinterPort() { return openPort(); }
...
bool CitizenCTS2000_PRINTER::isItYou() {
  ...
  cmd.push_back(
      CMDCitizenCTS2000::PrinterCommandModelParam); // Узнаем модель принтера
...
bool CitizenCTS2000_PRINTER::getStatus(int &aStatus) {
  ...
  if (status.size() > CMDCitizenCTS2000::StatusAnswerLength) {
    ...
    // if(Debuger) qDebug() << "getStatus(): Printer is not available, perhaps
  ...
  if ((offlineError == CMDCitizenCTS2000::PrinterIsNotAvailable) ||
      ...
      (!(positiveMasking(offlineError,
                         CMDCitizenCTS2000::Control::StatusMask) &&
         positiveMasking(printerError,
                         CMDCitizenCTS2000::Control::StatusMask) &&
         positiveMasking(paperDetectorError,
                         CMDCitizenCTS2000::Control::StatusMask)))) {

    // Printer is not available
    result |= PrinterState::PrinterNotAvailable;
    // if(Debuger) qDebug() << "CitizenCTS2000::getStatus(): Printer is not
    // available";
    aStatus = result;
    return true;
  }
  ...
  if (printerError != CMDCitizenCTS2000::PrinterIsOK) {
    ...
    if (code > 0) {
      // Unrecoverable error
      result |= PrinterState::PrinterError;
      // if(Debuger) qDebug() << "CitizenCTS2000::getStatus(): Unrecoverable
      // error";
    }
    ...
    if (code > 0) {
      // Recoverable error
      result |= PrinterState::PrinterError;
      // if(Debuger) qDebug() << "CitizenCTS2000::getStatus(): Recoverable
      // error";
    }
  }
...
QByteArray CitizenCTS2000_PRINTER::getState() {
  ...
  if (!this->sendCommand(cmd, true, CMDCitizenCTS2000::TimeOutAfterWriting,
                         resp_data, answer_1, 0)) {
    // if(Debuger) qDebug() << "CitizenCTS2000::getState(): error in
    // sendPacketInPort()";
    return answer;
  }
...
void CitizenCTS2000_PRINTER::getSpecialCharecters(QByteArray &printText) {
  ...
  if (smallChek) {
    ...
    fontTypeDoubleWidth_start.push_back(
        CMDCitizenCTS2000::PrinterCommandFirstByte);
    fontTypeDoubleWidth_start.push_back(
        CMDCitizenCTS2000::PrinterFontCommandSecondByte);
...
bool CitizenCTS2000_PRINTER::initialize() {
  ...
  this->sendCommand(cmd, true, CMDCitizenCTS2000::TimeOutAfterWriting, result,
                    answer, 0);
...
bool CitizenCTS2000_PRINTER::cut() {
  ...
  return sendCommand(cmd, true, CMDCitizenCTS2000::TimeOutAfterWriting, result,
                     answer, 0);
}
