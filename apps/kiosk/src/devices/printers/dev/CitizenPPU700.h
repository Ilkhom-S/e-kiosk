...
namespace CMDCitizenPPU700
{
    ...
    struct SPaper
    {
        ...
        SPaper():
            NearEndSensor1(false),
            NearEndSensor2(false),
            InPresenter(false),
            End(false)
        {}
    };
    ...
    const uchar PrinterCommandPaperStatusSecondByte  = 0x76; /// Вторая часть команды paper sensor Status.

    const uchar PrinterCommandStatusFirstByte        = 0x10; /// Первая часть команды Printer Status.
    const uchar PrinterCommandStatusSecondByte       = 0x04; /// Вторая часть команды Printer Status.
    const uchar PrinterCommandStatusThirdByte        = 5;    /// Третья часть команды Printer Status.
    const uchar PrinterCommandStatusThirdByte1       = 3;    /// Третья часть команды Printer Status(paper jam).
    ...
    const uchar PrinterCommandModelParam             = 67;   /// Параметр для команды получения ID принтера.
    const QString PrinterPPU700                      = "_PPU-700"; /// Идентификатор для принтера PPU 700.

    const uchar PrinterCommandImageSecondByte        = 0x2F; /// Второй байт команды печать картинки
    const uchar PrinterCommandRegImageSecondByte     = 0x2A; /// Второй байт команды регистрации картинки
    ...
    const uchar PrintingHeadTemperatureError         = 0x08; /// Температурная ошибка печатающей головки принтера
...
public:
    ...
    //        bool getControlInfo(char aInfoType, CMDCitizenPPU700::SControlInfo & aControlInfo);
    bool isItYou();
