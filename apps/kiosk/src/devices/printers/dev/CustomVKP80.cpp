...
bool CustomVKP80_PRINTER::isEnabled(CMDCustomVKP80::SStatus &s_status,
                                    int &state) {
...
bool CustomVKP80_PRINTER::getStatus(int &aStatus,
...
bool CustomVKP80_PRINTER::getState(char aStatusType,
                                   CMDCustomVKP80::SStatus &aStatus) {
  ...
  if (this->sendCommand(commandPacket, true, 50, respData, answerPacket, 0)) {
    ...
    case CMDCustomVKP80::Constants::Status::Printer: {
      aStatus.Offline =
          getBit(answer, CMDCustomVKP80::Positions::Answer::Offline);
      break;
    }
    case CMDCustomVKP80::Constants::Status::Offline: {
      aStatus.CoverOpen =
          getBit(answer, CMDCustomVKP80::Positions::Answer::CoverOpen);
      aStatus.PaperOut =
          getBit(answer, CMDCustomVKP80::Positions::Answer::PaperOut);
    ...
    case CMDCustomVKP80::Constants::Status::Errors: {
      ...
      aStatus.Failures.Unrecoverable = getBit(
          answer, CMDCustomVKP80::Positions::Answer::Failures::Unrecoverable);
      aStatus.Failures.AutoRecovery = getBit(
          answer, CMDCustomVKP80::Positions::Answer::Failures::AutoRecovery);
      break;
    }
    case CMDCustomVKP80::Constants::Status::Paper: {
      aStatus.Paper.End =
          getBit(answer, CMDCustomVKP80::Positions::Answer::Paper::End);
...
bool CustomVKP80_PRINTER::isItYou() {
  ...
  if (!getControlInfo(CMDCustomVKP80::Constants::ControlInfo::ModelID,
                      controlInfo)) {
    // if(Debuger) qDebug() << "Unable to get control info, type Model ID!";
  }

  if (!getControlInfo(CMDCustomVKP80::Constants::ControlInfo::Features,
                      controlInfo)) {
    // if(Debuger) qDebug() << "Unable to get control info, type Features!";
  }

  if (!getControlInfo(CMDCustomVKP80::Constants::ControlInfo::ROM,
                      controlInfo)) {
...
bool CustomVKP80_PRINTER::getControlInfo(
    char aInfoType, CMDCustomVKP80::SControlInfo &aControlInfo) {
  ...
  if (this->sendCommand(commandPacket, true, 200, resp, answerPacket, 0)) {
    ...
    case CMDCustomVKP80::Constants::ControlInfo::ModelID: {
      if (answerPacket.size() ==
          CMDCustomVKP80::AnswersLength::ControlInfo::ModelID) {
        if (answerPacket[0] ==
            CMDCustomVKP80::Constants::ModelIDs::PrinterID200Dpi) {
      ...
      if (aControlInfo.wrongAnswer) {
        ...
        // if(Debuger) qDebug() << "getControlInfo, request modelID: wrong
        // answer from printer";
      }
    ...
    case CMDCustomVKP80::Constants::ControlInfo::Features: {
      if (answerPacket.size() ==
          CMDCustomVKP80::AnswersLength::ControlInfo::Features) {
        aControlInfo.isUnicodeSupported =
            getBit(answerPacket[0], CMDCustomVKP80::Positions::Answer::
                                        GetFeaturesInfo::UnicodeSupported);
        aControlInfo.isAutoCutterSupplied =
            getBit(answerPacket[0], CMDCustomVKP80::Positions::Answer::
                                        GetFeaturesInfo::AutoCutterSupplied);
      } else {
        // if(Debuger) qDebug() << "getControlInfo, request Features: wrong
        // answer from printer";
        aControlInfo.wrongAnswer = true;
      }
      break;
    ...
    case CMDCustomVKP80::Constants::ControlInfo::ROM: {
      if (answerPacket.size() ==
          CMDCustomVKP80::AnswersLength::ControlInfo::ROM) {
        aControlInfo.ROMVersion = answerPacket;
      } else {
        // if(Debuger) qDebug() << "getControlInfo, request ROM version: wrong
        // answer from printer";
        aControlInfo.wrongAnswer = true;
      }
      break;
    ...
    default: {
      // if(Debuger) qDebug() << QString("getControlInfo, unknown request = 0x
...
void CustomVKP80_PRINTER::getSpecialCharecters(QByteArray &printText) {
  ...
  fontTypeDoubleHeight_start.push_back(
      CMDCustomVKP80::PrinterCommandGetIDFirstByte);
  fontTypeDoubleHeight_start.push_back(
      CMDCustomVKP80::PrinterFontDoubleWidthHeight);
  fontTypeDoubleHeight_start.push_back(1);

  QByteArray fontTypeDoubleHeight_end;
  fontTypeDoubleHeight_end.push_back(
      CMDCustomVKP80::PrinterCommandGetIDFirstByte);
  fontTypeDoubleHeight_end.push_back(
      CMDCustomVKP80::PrinterFontDoubleWidthHeight);
  ...
  fontTypeDoubleWidth_start.push_back(
      CMDCustomVKP80::PrinterCommandGetIDFirstByte);
  fontTypeDoubleWidth_start.push_back(
      CMDCustomVKP80::PrinterFontDoubleWidthHeight);
  ...
  fontTypeDoubleWidth_end.push_back(
      CMDCustomVKP80::PrinterCommandGetIDFirstByte);
  fontTypeDoubleWidth_end.push_back(
      CMDCustomVKP80::PrinterFontDoubleWidthHeight);
...
bool CustomVKP80_PRINTER::printImageI(const QString &aPixelString, uchar aWidth,
...
bool CustomVKP80_PRINTER::registerLogo(const QString &aPixelString,
                                       uchar aWidth) {
