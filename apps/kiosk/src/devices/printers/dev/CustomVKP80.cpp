    ...
    if (!controlInfo.wrongAnswer) {
        if (!controlInfo.noAnswer) {
            /*                         if(Debuger) qDebug() << QString("Control info:\
                                    \nResolution = %1,\
                                    \nFeatures: Unicode symbols is %2supported, Autocutter is
               %3supplied\
                                    \nROM version = %4")
        ...
        switch (aInfoType) {
            case CMDCustomVKP80::Constants::ControlInfo::ModelID: {
                ...
                if (aControlInfo.wrongAnswer) {
                    ...
                    // if(Debuger) qDebug() << "getControlInfo, request modelID: wrong answer from
                    // printer";
                }
            ...
            case CMDCustomVKP80::Constants::ControlInfo::Features: {
                if (answerPacket.size() == CMDCustomVKP80::AnswersLength::ControlInfo::Features) {
                    aControlInfo.isUnicodeSupported = getBit(
                        answerPacket[0],
                        CMDCustomVKP80::Positions::Answer::GetFeaturesInfo::UnicodeSupported);
                    aControlInfo.isAutoCutterSupplied = getBit(
                        answerPacket[0],
                        CMDCustomVKP80::Positions::Answer::GetFeaturesInfo::AutoCutterSupplied);
                } else {
                    // if(Debuger) qDebug() << "getControlInfo, request Features: wrong answer from
                    // printer";
                    aControlInfo.wrongAnswer = true;
                }
                break;
            ...
            case CMDCustomVKP80::Constants::ControlInfo::ROM: {
                if (answerPacket.size() == CMDCustomVKP80::AnswersLength::ControlInfo::ROM) {
                    ...
                    // if(Debuger) qDebug() << "getControlInfo, request ROM version: wrong answer
                    // from printer";
                    aControlInfo.wrongAnswer = true;
                }
                break;
...
void CustomVKP80_PRINTER::getSpecialCharecters(QByteArray& printText) {
    ...
    printText.replace(
        QString(CScharsetParam::OpenTagDelimiter + CScharsetParam::FontTypeDoubleHeight +
                CScharsetParam::CloseTagDelimiter)
            .toUtf8(),
        fontTypeDoubleHeight_start);
    printText.replace(
        QString(CScharsetParam::OpenTagDelimiter + CScharsetParam::CloseTagSymbol +
    ...
    printText.replace(
        QString(CScharsetParam::OpenTagDelimiter + CScharsetParam::FontTypeDoubleWidth +
                CScharsetParam::CloseTagDelimiter)
            .toUtf8(),
    ...
    printText.replace(QString(CScharsetParam::OpenTagDelimiter + CScharsetParam::ProbelCount +
                              CScharsetParam::CloseTagDelimiter)
                          .toUtf8(),
                      probel);
    ...
    QByteArray star;
    for (int j = 1; j <= col_z; j++) star.append("*");
