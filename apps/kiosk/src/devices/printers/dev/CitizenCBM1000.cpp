...
bool CitizenCBM1000_PRINTER::openPort() {
    if (devicesCreated) {
        ...
        if (serialPort->open(QIODevice::ReadWrite)) {
            ...
            //                if (!devicePort->setFlowControl(AbstractSerial::FlowControlUndefined))
            //                return false; if
            ...
            // if(Debuger) qDebug() << "\nPrinter " << CMDCitizenCBM1000::DeviceName << " to Port "
            ...
            // if(Debuger) qDebug() << "Char timeout, msec     : " <<
...
bool CitizenCBM1000_PRINTER::getStatus(int& aStatus) {
    ...
    if (answer.size() < 1) {
        // if(Debuger) qDebug() << QString("AV268::getStatus(): wrong size of buffer. Buffer is:
        // %1").arg(answer.data());
        return false;
    }
...
bool CitizenCBM1000_PRINTER::printCheck(const QString& aCheck) {
    ...
    if (!this->sendCommand(printText, false, 0, respData, answer, 0)) return false;
...
bool CitizenCBM1000_PRINTER::feed(int aCount) {
    ...
    for (int i = 0; i < aCount; ++i) {
        if (!this->sendCommand(cmd, true, 50, respData, answer, 0)) return false;
    }
