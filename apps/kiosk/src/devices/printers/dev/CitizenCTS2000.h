...
namespace CMDCitizenCTS2000
{
    ...
    const char PrinterCommandModelParam           = '\x01'; /// Параметр для команды, чтобы получить ID устройства.
    const char PrinterCTS2000                     = '\x51'; /// Если такой ответ принтера на получение ID, то модель CTS2000.
    ...
    const char PrinterCommandImageFirstByte       = '\x1D'; /// Первый байт команд печати и регистрации логотипа
