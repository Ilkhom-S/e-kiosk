...
struct SControlInfo {
  ...
  SControlInfo()
      : noAnswer(false), wrongAnswer(false), Resolution(0),
        isUnicodeSupported(false), isAutoCutterSupplied(false), ROMVersion("") {
  }
};
...
const uchar PrinterCommandAnotherFeedSecondByte =
    0x4A; /// Вторая часть команды Feed.
...
const uchar PrinterCommandCharacterSetSecondByte =
    0x21; /// Вторая часть команды international character set.
const uchar PrinterCommandCharacterSetThirdByte =
    0x01; /// Третья часть команды international character set.
...
const uchar PrinterCommandModelParam =
    49; /// Третья часть команды получения ID принтера.

const uchar PrinterCommandClrDispenserSecondByte =
    0x65; /// Второй байт команды clear dispenser
const uchar PrinterCommandClrDispenserThirdByte =
    0x05; /// Третий байт команды clear dispenser

const uchar PrinterCommandDispenseThirdByte =
    0x03; /// Третий байт команды dispense
const uchar PrinterCommandDispenseForthByte =
    0x1E; /// Четвертый байт команды dispense

const uchar PrinterCommandPaperSizeSecondByte =
    0x33; /// Второй байт параметра размера чека

const uchar PrinterCommandPaperSizeThirdByteSmall =
    0x28; /// Третий байт параметра размера укароченного чека
const uchar PrinterCommandPaperSizeThirdByteNorm =
    0x32; /// Третий байт параметра размера нормального чека

const uchar PrinterCommandLogoRegSecondByte =
    0x2A; /// Второй байт команды регистрации логотипа

const uchar PrinterCommandLogoPrintSecondByte =
    0xFA; /// Второй байт команды печати логотипа
const uchar PrinterCommandLogoPrintThirdByte =
    0x64; /// Третий байт команды печати логотипа
const uchar PrinterCommandLogoPrintFothByte =
    0x6E; /// Третий байт команды печати логотипа
