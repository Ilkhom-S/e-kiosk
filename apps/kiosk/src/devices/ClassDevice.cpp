...
void SearchDevices::run() {
  ...
  if (searchValidator) {
    emit emitDeviceSearch(SearchDev::search_validator, SearchDev::start_search,
                          v_name, v_comment, c_port);

    // Даем выбранные устройства на проверку
    if (searchDeviceMethod(SearchDev::search_validator, v_name, c_port,
                           v_comment)) {
      // Устройство получило результат true
      // Тут будет отпрален сигнал устройство найдено...
      emit emitDeviceSearch(SearchDev::search_validator,
                            SearchDev::device_found, v_name, v_comment, c_port);
      this->setDeviseInBase(SearchDev::search_validator, v_name, c_port,
                            v_comment, 1);
      ...
      emit emitDeviceSearch(SearchDev::search_validator,
                            SearchDev::device_notfound, v_name, v_comment,
                            c_port);
      this->setDeviseInBase(SearchDev::search_validator, v_name, c_port,
                            v_comment, 0);
    }
  ...
  if (searchCoinAcceptor) {
    emit emitDeviceSearch(SearchDev::search_coin_acceptor,
                          SearchDev::start_search, v_name, v_comment, c_port);

    // Даем выбранные устройства на проверку
    if (searchDeviceMethod(SearchDev::search_coin_acceptor, v_name, c_port,
                           v_comment)) {
      // Устройство получило результат true
      // Тут будет отпрален сигнал устройство найдено...
      emit emitDeviceSearch(SearchDev::search_coin_acceptor,
                            SearchDev::device_found, v_name, v_comment, c_port);
      this->setDeviseInBase(SearchDev::search_coin_acceptor, v_name, c_port,
                            v_comment, 1);
  ...
  if (v_name != PrinterModel::Windows_Printer) {
    ...
    if (searchPrinter) {
      emit emitDeviceSearch(SearchDev::search_printer, SearchDev::start_search,
                            v_name, v_comment, c_port);
      ...
      if (v_name == PrinterModel::KM1X) {
        ...
        emit emitDeviceSearch(SearchDev::search_printer,
                              SearchDev::device_found, v_name, v_comment,
                              c_port);
        ...
        // Даем выбранные устройства на проверку
        if (searchDeviceMethod(SearchDev::search_printer, v_name, c_port,
                               v_comment)) {
          // Устройство получило результат true
          // Тут будет отпрален сигнал устройство найдено...
          emit emitDeviceSearch(SearchDev::search_printer,
                                SearchDev::device_found, v_name, v_comment,
                                c_port);
          setDeviseInBase(SearchDev::search_printer, v_name, c_port, v_comment,
                          1);
  ...
  if (searchWD) {
    emit emitDeviceSearch(SearchDev::search_watchdog, SearchDev::start_search,
                          v_name, v_comment, c_port);

    // Даем выбранные устройства на проверку
    if (searchDeviceMethod(SearchDev::search_watchdog, v_name, c_port,
                           v_comment)) {
      // Устройство получило результат true
      // Тут будет отпрален сигнал устройство найдено...
      emit emitDeviceSearch(SearchDev::search_watchdog, SearchDev::device_found,
                            v_name, v_comment, c_port);
      ...
      // Устройство не найдено
      emit emitDeviceSearch(SearchDev::search_watchdog,
                            SearchDev::device_notfound, v_name, v_comment,
                            c_port);
  ...
  if (searchModem) {
    emit emitDeviceSearch(SearchDev::search_modem, SearchDev::start_search,
                          v_name, v_comment, c_port);
    ...
    // Даем выбранные устройства на проверку
    if (!modemConUp) {
      if (searchDeviceMethod(SearchDev::search_modem, v_name, c_port,
                             v_comment)) {
        // Устройство получило результат true
        // Тут будет отпрален сигнал устройство найдено...
        emit emitDeviceSearch(SearchDev::search_modem, SearchDev::device_found,
                              v_name, v_comment, c_port);
        ...
        // Устройство не найдено
        emit emitDeviceSearch(SearchDev::search_modem,
                              SearchDev::device_notfound, v_name, v_comment,
                              c_port);
      ...
      // Устройство не найдено
      emit emitDeviceSearch(SearchDev::search_modem, SearchDev::device_notfound,
                            v_name, v_comment, c_port);
      setDeviseInBase(SearchDev::search_modem, v_name, c_port, v_comment, 0);
...
bool SearchDevices::setDeviseInBase(int id_device, const QString &name_device,
  ...
  strUpdate = QString("UPDATE terminal_devices SET name = \"%1\", port = "
                  ...
                  .arg(name_device, port, comment, QString::number(state),
                       QString::number(id_device));
...
bool SearchDevices::insertDeviseInBase(int id_device, QString &name_device,
                                       QString &port) {
  ...
  strSelect =
      QString("INSERT INTO terminal_devices VALUES('',%1,'null','null',0);")
          .arg(id_device);
...
bool SearchDevices::getDeviseInBase(int id_device, QString &name_device,
                                    QString &port) {
  QSqlQuery selectDevices(this->db);
  QString strSelect;
  id_device++;
  ...
  strSelect =
      QString("SELECT * FROM terminal_devices WHERE id = %1").arg(id_device);
...
bool SearchDevices::searchDeviceMethod(int device, QString &dev_name,
                                       QString &com_str, QString &dev_coment,
