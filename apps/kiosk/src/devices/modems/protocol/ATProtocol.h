...
const auto regExpBalance =
    "([0-9]{1,5}[.][0-9]{1,2}|[0-9]{1,5}[,][0-9]{1,2}|[-][0-9]{1,5}[.][0-9]{1,2}|[-][0-9]{1,5}[,]["
    "0-9]{1,2})";
...
enum Enum {
    ...
    /// Это сообщение модем передает компьютеру каждый раз, когда по телефонной линии
    ...
    /// Ошибка в командной строке, командный буфер переполнен или ошибка в контрольной сумме
    /// (команда I2)
    Error,
...
class ATProtocol : public QThread {
  ...
  private:
    ...
    bool sendCommand(QByteArray dataRequest, bool getResponse, int timeResponse,
    ...
    /// Возвращает:
    /// - успешность выполнения.
    bool getAnswer(QByteArray& aData, bool& codecError, int aAddRepeatCount = 0,
                   int aPauseTime = 0);
