...
/*!
    ...
    \brief The QAtUtils class provides several utility functions that assist with interfacing to
   AT-based modems.
...
QString QCodePage850Codec::convertToUnicode(const char *in, int length, ConverterState *) const {
    QString str;
    while (length-- > 0) str += QChar((unsigned int)cp850ToUnicode[*in++ & 0xFF]);
    return str;
}
...
QTextCodec *QAtUtils::codec(const QString &gsmCharset) {
    ...
    if (cs == "gsm") {
        ...
        if (!hex) hex = new QGsmHexCodec();
        codec = hex;
    } else if (cs == "ucs2") {
        // Hex-encoded UCS2 character set.
        static QTextCodec *ucs2 = 0;
        if (!ucs2) ucs2 = new QUcs2HexCodec();
        codec = ucs2;
    } else if (cs == "ira") {
        ...
        if (!cp437) cp437 = new QCodePage437Codec();
        codec = cp437;
