...
/*!
    ...
    \brief The QAtResult class provides access to the results of AT modem
   commands and unsolicited notifications
    \ingroup telephony::serial

    AT commands that are sent to a modem with QAtChat::chat() result in a
   QAtResult object being made available to describe the result of the command
   when it completes.
...
/*!
    ...
    \value PhoneAdapterLinkReserved GSM 27.07 code 2: phone-adaptor link
   reserved
    ...
    \value InvalidCharsInTextString GSM 27.07 code 25: invalid characters in
   text string
    \value DialStringTooLong GSM 27.07 code 26: dial string too long
    \value InvalidCharsInDialString GSM 27.07 code 27: invalid characters in
   dial string
    \value NoNetworkService GSM 27.07 code 30: no network service
    \value NetworkTimeout GSM 27.07 code 31: network timeout
    \value NetworkNotAllowed GSM 27.07 code 32: network not allowed - emergency
   calls only
    \value NetPersPinRequired GSM 27.07 code 40: network personalization PIN
   required
    \value NetPersPukRequired GSM 27.07 code 41: network personalization PUK
   required
    \value NetSubsetPersPinRequired GSM 27.07 code 42: network subset
   personalization PIN required
    \value NetSubsetPersPukRequired GSM 27.07 code 43: network subset
   personalization PUK required
    \value ServProvPersPinRequired GSM 27.07 code 44: service provider
   personalization PIN required
    \value ServProvPersPukRequired GSM 27.07 code 45: service provider
   personalization PUK required
    \value CorpPersPinRequired GSM 27.07 code 46: corporate personalization PIN
   required
    \value CorpPersPukRequired GSM 27.07 code 47: corporate personalization PUK
   required
    ...
    \value RoamingNotAllowed GSM 27.07 code 113: Roaming not allowed in this
   location area
    \value ServiceOptionNotSupported GSM 27.07 code 132: service option not
   supported
    \value ServiceOptionNotSubscribed GSM 27.07 code 133: service option not
   subscribed
    \value ServiceOptionOutOfOrder GSM 27.07 code 134: service option
   temporarily out of order
    \value UnspecifiedGPRSError GSM 27.07 code 148: unspecified GPRS error
    \value PDPAuthenticationFailure GSM 27.07 code 149: PDP authentication
   failure
    \value InvalidMobileClass GSM 27.07 code 150: invalid mobile class
    \value VBSVGCSNotSupported GSM 27.07 code 151: VBS/VGCS not supported by the
   network
    \value NoServiceSubscriptionOnSim GSM 27.07 code 152: No service
   subscription on SIM
    \value NoSubscriptionForGroupId GSM 27.07 code 153: No subscription for
   group ID
    \value GroupIdNotActivatedOnSim GSM 27.07 code 154: Group Id not activated
   on SIM
    \value NoMatchingNotification GSM 27.07 code 155: No matching notification
    \value VBSVGCSCallAlreadyPresent GSM 27.07 code 156: VBS/VGCS call already
   present
    ...
    \value NoAccessRightsForSimFile GSM 27.07 code 160: No access rights for SIM
   file
    \value NoSubscriptionForPriority GSM 27.07 code 161: No subscription for
   priority
    \value OperationNotApplicable GSM 27.07 code 162: operation not applicable
   or not possible
    ...
    \value InvalidPDUModeParameter GSM 27.05 code 304: invalid PDU mode
   parameter
    \value InvalidTextModeParameter GSM 27.05 code 305: invalid text mode
   parameter
    ...
    \value NoCNMAAckExpected GSM 27.05 code 340: no +CNMA acknowledgment
   expected
...
/*!
    ...
    \brief The UserData class provides a mechanism to add user data to a result
   object via QAtChat::chat().

    Usually this class will be inherited, with extra fields added to hold the
   user data.
...
static QAtCodeInfo const ext_codes[] = {
    ...
    {QAtResult::PhoneAdapterLinkReserved,
     "phone-adaptor link reserved"},                               // no tr
    ...
    {QAtResult::InvalidCharsInTextString,
     "invalid characters in text string"},                  // no tr
    {QAtResult::DialStringTooLong, "dial string too long"}, // no tr
    {QAtResult::InvalidCharsInDialString,
     "invalid characters in dial string"},               // no tr
    {QAtResult::NoNetworkService, "no network service"}, // no tr
    {QAtResult::NetworkTimeout, "network timeout"},      // no tr
    {QAtResult::NetworkNotAllowed,
     "network not allowed - emergency calls only"}, // no tr
    {QAtResult::NetPersPinRequired,
     "network personalization PIN required"}, // no tr
    {QAtResult::NetPersPukRequired,
     "network personalization PUK required"}, // no tr
    {QAtResult::NetSubsetPersPinRequired,
     "network subset personalization PIN required"}, // no tr
    {QAtResult::NetSubsetPersPukRequired,
     "network subset personalization PUK required"}, // no tr
    {QAtResult::ServProvPersPinRequired,
     "service provider personalization PIN required"}, // no tr
    {QAtResult::ServProvPersPukRequired,
     "service provider personalization PUK required"}, // no tr
    {QAtResult::CorpPersPinRequired,
     "corporate personalization PIN required"}, // no tr
    {QAtResult::CorpPersPukRequired,
     "corporate personalization PUK required"},            // no tr
    ...
    {QAtResult::RoamingNotAllowed,
     "Roaming not allowed in this location area"}, // no tr
    {QAtResult::ServiceOptionNotSupported,
     "service option not supported"}, // no tr
    {QAtResult::ServiceOptionNotSubscribed,
     "requested service option not subscribed"}, // no tr
    {QAtResult::ServiceOptionOutOfOrder,
     "service option temporarily out of order"},                 // no tr
    {QAtResult::UnspecifiedGPRSError, "unspecified GPRS error"}, // no tr
    {QAtResult::PDPAuthenticationFailure,
     "PDP authentication failure"},                          // no tr
    {QAtResult::InvalidMobileClass, "invalid mobile class"}, // no tr
    // no tr
    {QAtResult::VBSVGCSNotSupported,
     "VBS/VGCS not supported by the network"}, // no tr
    {QAtResult::NoServiceSubscriptionOnSim,
     "No service subscription on SIM"}, // no tr
    {QAtResult::NoSubscriptionForGroupId,
     "No subscription for group ID"}, // no tr
    {QAtResult::GroupIdNotActivatedOnSim,
     "Group Id not activated on SIM"},                               // no tr
    {QAtResult::NoMatchingNotification, "No matching notification"}, // no tr
    {QAtResult::VBSVGCSCallAlreadyPresent,
     "VBS/VGCS call already present"},              // no tr
    ...
    {QAtResult::NoAccessRightsForSimFile,
     "No access rights for SIM file"}, // no tr
    {QAtResult::NoSubscriptionForPriority,
     "No subscription for priority"}, // no tr
    {QAtResult::OperationNotApplicable,
     "operation not applicable or not possible"}, // no tr
    ...
    {QAtResult::InvalidTextModeParameter,
     "invalid text mode parameter"},                          // no tr
    ...
    {QAtResult::NoCNMAAckExpected,
     "no +CNMA acknowledgement expected"},     // no tr
