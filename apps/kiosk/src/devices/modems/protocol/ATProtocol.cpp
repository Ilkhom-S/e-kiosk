...

{
    ...
    smsTextInit << SmsTextIndex::txtErrorValidator << SmsTextIndex::txtErrorPrinter
                << SmsTextIndex::txtErrorBalanceAgent << SmsTextIndex::txtErrorSimBalance
...
bool ATProtocol::openPort() {
    if (devicesCreated) {
        ...
        if (serialPort->open(QIODevice::ReadWrite)) {
            ...
            //                if(Debuger) qDebug() << "\nModem " << " to Port " <<
            //                devicePort->deviceName() << " open in " << devicePort->openMode();

            //                        if(Debuger) qDebug() << "\n= Defaults parameters =";
            //                        if(Debuger) qDebug() << "Device name            : " <<
            //                        devicePort->deviceName(); if(Debuger) qDebug() << "Baud rate
            //                        : " << devicePort->baudRate(); if(Debuger) qDebug() << "Data
            //                        bits              : " << devicePort->dataBits(); if(Debuger)
            //                        qDebug() << "Parity                 : " <<
            //                        devicePort->parity(); if(Debuger) qDebug() << "Stop bits : "
            //                        << devicePort->stopBits(); if(Debuger) qDebug() << "Flow : "
            //                        << devicePort->flowControl(); if(Debuger) qDebug() << "Char
            //                        timeout, msec     : " << devicePort->charIntervalTimeout();
...
bool ATProtocol::sendCommand(QByteArray dataRequest, bool getResponse, int timeResponse,
...
//--------------------------------------------------------------------------------
bool ATProtocol::getAnswer(QByteArray& aData, bool& codecError, int aAddRepeatCount,
                           int aPauseTime) {
    ...
    for (int i = 0; i < aAddRepeatCount + 1; ++i) {
        ...
        if (!readPort(tempAnswer)) return false;
        ...
        if (aPauseTime) this->msleep(aPauseTime);
    }
...
bool ATProtocol::getCommandPacket(ModemProtocolCommands::Enum aCommand,
                                  const QByteArray& aCommandData, QByteArray& aPacketCommand) {
    ...
    switch (aCommand) {
        case ModemProtocolCommands::GetOperator: {
            ...
            if ((indexFirstQuote != -1) && (indexSecondQuote != -1))
                ...
                //                                if(Debuger) qDebug() << "Protocol AT: Invalid
                //                                format of operator";
                return false;
            }
        ...
        case ModemProtocolCommands::GetSignalQuality: {
            ...
            int indexSecond = -1;
            if (indexFirst != -1) indexSecond = signalString.indexOf(",");
...
bool ATProtocol::getStatusInfo(SModemStatusInfo& aStatusInfo) {
    ...
    // Статус пока будем определять только по идентификации, чтобы определить наличие коннекта с
    // модемом TO DO в дальнейшем необходимо доделать
    bool isConnect = true;
