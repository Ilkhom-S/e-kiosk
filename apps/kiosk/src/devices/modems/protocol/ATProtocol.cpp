...
void ATProtocol::setPortName(const QString com_Name) { comName = com_Name; }
...
bool ATProtocol::openPort() {
  if (devicesCreated) {
    ...
    //        if(Debuger) qDebug() << "After set devicePort->DeviceName(); " <<
    ...
    if (serialPort->open(QIODevice::ReadWrite)) {
      ...
      //                        devicePort->deviceName(); if(Debuger) qDebug()
      //                        << "Baud rate              : " <<
      //                        devicePort->baudRate(); if(Debuger) qDebug() <<
      //                        "Data bits              : " <<
      //                        devicePort->dataBits(); if(Debuger) qDebug() <<
      //                        "Parity                 : " <<
      //                        devicePort->parity(); if(Debuger) qDebug() <<
      //                        "Stop bits              : " <<
      //                        devicePort->stopBits(); if(Debuger) qDebug() <<
      //                        "Flow                   : " <<
      //                        devicePort->flowControl(); if(Debuger) qDebug()
      //                        << "Char timeout, msec     : " <<
...
bool ATProtocol::sendSMSParam(QString text) {
  ...
  // at+cmgf=1
  if (this->processCommand(ModemProtocolCommands::CmdStateSMS, request,
                           response)) {

    request.clear();
    response.clear();

    this->processCommand(ModemProtocolCommands::CmdSetNumberSMS, request,
                         response);
    ...
    // Сам текст сообщения
    if (this->processCommand(ModemProtocolCommands::CmdTextIntersetSMS, request,
                             response)) {
...
//--------------------------------------------------------------------------------
void ATProtocol::packetData(const QByteArray &aCommandPacket,
                            QByteArray &aPacket) {
  // записываем команду
...
bool ATProtocol::processCommand(ModemProtocolCommands::Enum aCommand,
  ...
  qDebug()
      << "PRE if (!getCommandPacket(aCommand, commandData, commandPacket))";
  ...
  if (!execCommand(packet)) {
    //                if(Debuger) qDebug() << "Protocol AT: Error executing
    //                command";
    return false;
  }
  ...
  if (!prepareAnswer(aCommand, aAnswerData)) {
    //                if(Debuger) qDebug() << "Protocol AT: Error
    //                prepareAnswer";
    return false;
  }
  return true;
...
bool ATProtocol::sendCommand(QByteArray dataRequest, bool getResponse,
                             int timeResponse, QByteArray &dataResponse,
                             int timeSleep) {
  bool respOk = false;
  if (this->isOpened()) {
    // Если девайс открыт
...
//--------------------------------------------------------------------------------
bool ATProtocol::getAnswer(QByteArray &aData, bool &codecError,
                           int aAddRepeatCount, int aPauseTime) {
...
//--------------------------------------------------------------------------------
bool ATProtocol::prepareAnswer(ModemProtocolCommands::Enum aCommand,
                               QByteArray &aAnswer) {
  switch (aCommand) {
  case ModemProtocolCommands::GetOperator: {
    //                        if(Debuger) qDebug() << "Protocol AT:
    //                        GetOperator...";
    ...
    if ((indexFirstQuote != -1) && (indexSecondQuote != -1))
      operatorString = operatorString.mid(
          indexFirstQuote + 1, indexSecondQuote - indexFirstQuote - 1);
    else {
      //                                if(Debuger) qDebug() << "Protocol AT:
  ...
  case ModemProtocolCommands::GetSignalQuality: {
    ...
    if ((indexFirst != -1) && (indexSecond != -1))
      signalString =
          signalString.mid(indexFirst + 1, indexSecond - indexFirst - 1);
    else {
      //                                if(Debuger) qDebug() << "Protocol AT:
      //                                Invalid format of operator";
      return false;
  ...
  case ModemProtocolCommands::GetSimNumber: {
    ...
    responseString =
        responseString
            .mid(indexFirstQuote + 10, indexSecondQuote - indexFirstQuote - 10)
            .trimmed();
...
bool ATProtocol::getStatusInfo(SModemStatusInfo &aStatusInfo) {
  ...
  bool isConnect = true;
  if (!processCommand(ModemProtocolCommands::Identification, commandData,
                      answerData)) {
    // if(Debuger) qDebug() << "Protocol AT: error processing command
    // \"Identification\"";

    // ошибка, возможно произошла из-за дисконнекта.
    // проверим данные на пустой байт
    if (answerData.size() &&
        answerData.at(0) == CATProtocolConstants::EmptyByte) {
