...
void ClassModem::setPort(QString port_name) { ATDevice->setPortName(port_name); }
...
bool ClassModem::e_Data_Execute() {
    ...
    if (command == ModemCmd::GetBalance) {
        this->setBalanceRequest(this->ussdRequestBalanseSim, CModemConstants::regExpBalance,
...
bool ClassModem::isItYou(QStringList &comList, QString &modem_name, QString &com_str,
...
bool ClassModem::isItYou(QString &modem_coment) {
    ...
    if (ATDevice->openPort()) {
        ...
        if (!ATDevice->processCommand(protocolCommand, cmdData, answerData)) {
            nowSimPresent = false;
            // qDebug() << ": Error processing protocol command = ISPIN, may be SIM is absent";
            //             return false;
        }
