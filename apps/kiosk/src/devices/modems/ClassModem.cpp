...
void ClassModem::sendSmsToAgentFew(QStringList smsId) { smsIdIn = smsId; }

void ClassModem::setPort(QString port_name) {
  ATDevice->setPortName(port_name);
}

void ClassModem::close() { ATDevice->closePort(); }
...
bool ClassModem::e_Data_Execute() {
  ...
  if (command == ModemCmd::GetOperator) {
    if (ATDevice->processCommand(ModemProtocolCommands::GetOperator, request,
                                 response)) {
      // qDebug() << "\n<<======Answer true";
      resp = true;
    } else {
  ...
  if (command == ModemCmd::GetSignalQuality) {
    if (ATDevice->processCommand(ModemProtocolCommands::GetSignalQuality,
                                 request, response)) {
      // qDebug() << "\n<<======Answer true";
      resp = true;
    } else {
  ...
  if (command == ModemCmd::CmdRestart) {
    if (ATDevice->processCommand(ModemProtocolCommands::CmdRestart, request,
                                 response)) {
      // qDebug() << "\n<<======Answer true";
      resp = true;
    } else {
  ...
  if (command == ModemCmd::GetBalance) {
    this->setBalanceRequest(this->ussdRequestBalanseSim,
                            CModemConstants::regExpBalance,
                            this->indexBalanceParse);
    if (ATDevice->processCommand(ModemProtocolCommands::GetBalance, request,
                                 response)) {
  ...
  if (command == ModemCmd::GetSimNumber) {
    this->setSimNumberRequest(this->ussdRequestNumberSim,
                              CModemConstants::regExpNumberSim);
    if (ATDevice->processCommand(ModemProtocolCommands::GetSimNumber, request,
                                 response)) {
...
void ClassModem::run() { this->e_Data_Execute(); }

bool ClassModem::isItYou(QStringList &comList, QString &modem_name,
                         QString &com_str, QString &modem_coment) {
...
bool ClassModem::execCommand(ModemProtocolCommands::Enum aCommand,
                             bool thread) {
  switch (aCommand) {
