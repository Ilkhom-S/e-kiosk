...
#include "CashPayment.h"
        ...
        int CCNetSm::readPollInfo(QByteArray byte) {
                ...
                switch (byte[4]) {
                    ...
                    case CCNetConstruct::Rejects::IncorrectBarcode:
                        this->sendStatusTo(
                            ...
                            QString(
                                "Rejecting due to incorrect number of characters in barcode.(93)"));
                        return 0;
                        break;
            ...
            switch (byte[3]) {
                ...
                // Validator but has not been stacked. Thus Bill Validator stops motion of the
                // second bill until the second bill is removed.
                ...
                case CCNetConstruct::Errors::Failure:
                    ...
                    switch (byte[4]) {
                        ...
                        case CCNetConstruct::Failures::BillToBillTransport:
                            this->sendStatusTo(
                                VStatus::Errors::BillToBillTransport,
        ...
        bool CCNetSm::firmwareUpdate(const QByteArray fw) {
            ...
            for (int page = 0; page < 112; page++) {
                for (int i = 0; i < 512; i++) packet[i] = fw[i + page * 512];
            ...
            if (state > 0) {
                ...
                while (true) {
                    ...
                    GetDataStatusFunc getDataStatus =
                        (GetDataStatusFunc)lib.resolve("GetDataStatus");
                    if (!getDataStatus) {
                        emit emitLog(2, "FIRMWARE_CCNET",
                                     QString("Не найдена функция: %1").arg(lib.errorString()));
                        fwCancelC100();
