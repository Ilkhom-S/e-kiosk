...
bool CCNetSm::isItYou() {
  ...
  if (this->isOpened()) {
    ...
    if (respData[0] == '\x02' && respData[1] == '\x03' &&
        respData[2] == '\x06' && respData[5] != '\x81') {
...
QByteArray CCNetSm::makeCustomRequest(int adr, int cmd,
...
bool CCNetSm::execCommand(ValidatorCommands::Enum cmdType,
  ...
  try {
    if (is_open) {
      ...
      case ValidatorCommands::SetEnabled: {
        ...
        cmdRequest = this->makeCustomRequest(CCNetConstruct::PABillValidator,
                                             CCNetConstruct::CCEnableBillTypes,
                                             btm_data);
      } break;
...
TResult CCNetSm::processCommand(const QByteArray &aCommandData,
  ...
  return (checkingCounter <= CCNetConstruct::MaxRepeatPacket)
...
QString CCNetSm::check(const QByteArray &aAnswer) {
  ...
  if (length != aAnswer.size()) {
    return QString("CCNet: Invalid length = %1, need %2")
        .arg(aAnswer.size())
        .arg(length);
  }
...
bool CCNetSm::sendACK() {
  QByteArray cmdRequest = this->makeCustomRequest(
  ...
  while (!stopPoll) {
    ...
    if (respData[3] == CCNetConstruct::States::Escrow) {
      ...
      emit emitLog(0, "CCNET",
                   QString("Определена купюра %1 смн").arg(escrowNominal));

      if (escrowNominal > 0) {
        if (hasDBError) {
          this->execCommand(ValidatorCommands::Return, respData);
          emit emitLog(0, "CCNET",
                       QString("Возвращаем купюру %1 смн, из за ошибки БД")
                           .arg(escrowNominal));
          ...
          this->execCommand(ValidatorCommands::Stack, respData);
          emit emitLog(0, "CCNET",
                       QString("Отправляем команду на укладку (Stack)"));
        }
      } else {
        this->execCommand(ValidatorCommands::Return, respData);
      }
    ...
    if (nominal > 0) {
      ...
      if (sicPoint > 500 && escrowed) {
        toValidatorLog(1, " ",
                       QString("Вставлена купюра %1 смн.").arg(nominal));
        escrowed = false;
...
int CCNetSm::readPollInfo(QByteArray byte) {
  ...
  if (byte[3] == CCNetConstruct::States::Rejecting) {
    ...
    case CCNetConstruct::Rejects::UV:
      this->sendStatusTo(VStatus::Warning::UV,
                         QString("Rejecting due to UV.(6D)"));
      return 0;
      break;
    ...
    case CCNetConstruct::Rejects::UnknownBarcode:
      this->sendStatusTo(
          VStatus::Warning::UnknownBarcode,
    ...
    case CCNetConstruct::Rejects::CorruptedTrailingData:
      this->sendStatusTo(
          VStatus::Warning::CorruptedTrailingData,
  ...
  case CCNetConstruct::Errors::ValidatorJammed:
    this->sendStatusTo(
        VStatus::Errors::ValidatorJammed,
  ...
  case CCNetConstruct::Errors::Cheated:
    this->sendStatusTo(VStatus::Warning::Cheated,
                       QString("Попытка мошенничество (45)"));
    this->setBoolingDlgState(false);
    return 0;
    break;
  ...
  case CCNetConstruct::States::Initialize:
    this->sendStatusTo(VStatus::Success::Ok,
                       QString("Идет инициализация купюрника.(13)"));
    this->setBoolingDlgState(false);
    return 0;
    break;
...
QByteArray CCNetSm::fwPacketRequest(quint8 command, quint8 address,
                                    const QByteArray &data) {
...
bool CCNetSm::fwCmdExec(const QByteArray aCommandData) {
  ...
  } while ((clockTimer.elapsed() < 2000) &&
           QString::fromUtf8(aAnswerData) != "OK");
...
bool CCNetSm::unlockValidator() {
  ...
  auto bloader =
      QByteArray(reinterpret_cast<const char *>(CCNetFirmware::bloader),
  ...
  auto ldr1A00 =
      QByteArray(reinterpret_cast<const char *>(CCNetFirmware::ldr1A00),
  ...
  auto ldr1C00 =
      QByteArray(reinterpret_cast<const char *>(CCNetFirmware::ldr1C00),
  ...
  auto ldr1E00 =
      QByteArray(reinterpret_cast<const char *>(CCNetFirmware::ldr1E00),
  ...
  auto ldrFE00 =
      QByteArray(reinterpret_cast<const char *>(CCNetFirmware::ldrFE00),
...
bool CCNetSm::CmdFirmwareUpdate(QString version) {
  ...
  if (version.isEmpty()) {
    ...
    version =
        firmwareList.length() > 0 ? firmwareList.last().remove(".txt") : "";
  }
  ...
  for (int page = 0; page < 112; page++) {
    ...
    if (res) {
      ...
      emit emitLog(0, "FIRMWARE_CCNET",
                   QString("Обновление прошивки %1%").arg(qRound(prg)));
    } else {
      firmwareUpdating = false;
...
bool CCNetSm::fwUpdateC100(QString version) {
  ...
  if (!m_lib.load()) {
    emit emitLog(
        2, "FIRMWARE_CCNET",
        QString("Не удалось загрузить DLL: %1").arg(m_lib.errorString()));
    emit emitFirmwareUpdate("cancel");
    return false;
  }
  ...
  if (state > 0) {
    ...
    while (true) {
      ...
      GetDataStatusFunc getDataStatus =
          (GetDataStatusFunc)m_lib.resolve("GetDataStatus");
      if (!getDataStatus) {
        emit emitLog(
            2, "FIRMWARE_CCNET",
            QString("Не найдена функция: %1").arg(m_lib.errorString()));
  ...
  if (state == -1099) {
    emit emitLog(2, "FIRMWARE_CCNET",
                 QString("Отсутствует файл прошивки %1").arg(path));
  } else if (state == -1100) {
