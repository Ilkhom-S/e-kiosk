...
/// Размер используемой части блока данных 1 номинала в ответе на запрос
/// номиналов.
...
const QByteArray Reset =
    QByteArray::fromRawData("\x60\x7F\x7F\x7F", 4); /// Сброс.
const QByteArray GetType = QByteArray::fromRawData(
    "\x60\x00\x00\x04",
    4); /// Тип купюроприемника (что это такое - только в MEI знают).
const QByteArray GetSerialNumber =
    QByteArray::fromRawData("\x60\x00\x00\x05", 4); /// Серийник.
const QByteArray GetBootSoftVersion = QByteArray::fromRawData(
    "\x60\x00\x00\x06", 4); /// Софт загрузчика интерфейсной платы.
const QByteArray GetAppSoftVersion =
    QByteArray::fromRawData("\x60\x00\x00\x07", 4); /// Софт приложения головы.
const QByteArray GetVariantName = QByteArray::fromRawData(
    "\x60\x00\x00\x08", 4); /// Название билл-сета (вариант, по-канадски).
...
private:
  ...
  TResult processCommand(QByteArray &aCommandData, QByteArray &aAnswerData,
                         bool aNeedAnswer);
