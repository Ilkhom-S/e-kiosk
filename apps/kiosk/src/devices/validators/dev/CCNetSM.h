...
/// After a RESET command from the Controller Bill Validator returns the bill and continues
/// initializing.
...
/// Bill Validator transports a bill from Escrow position to drop cassette and remains
/// in this state until the bill is stacked or jammed.
const char Stacking = '\x17';

/// Bill Validator transports a bill from Escrow position back to customer and remains
/// in this state until customer removes the bill or the bill is jammed.
...
/// The state, in which the bill is held in Escrow position after the HOLD command of the
/// Controller.
...
/// Controller to stack or to return bill. If during 10 sec command will not be sent bill will be
/// returned.
...
/// The unloading of the recycling cassette is carried out, and if it is necessary, reprogramming
/// EEPROM.
...
/// but has not been stacked. Thus Bill Validator stops motion of the second bill until the second
/// bill is removed.
