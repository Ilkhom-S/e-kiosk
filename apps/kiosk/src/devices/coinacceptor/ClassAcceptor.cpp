...
void ClassAcceptor::setValidator(QString name) {
    ...
    if (validatorName == AcceptorModel::CCTalk) {
        ...
        connect(CCTalkAcceptor, SIGNAL(emitNominalDuplicate(int)), this,
                SIGNAL(eNominalDuplicate(int)));
        connect(CCTalkAcceptor, SIGNAL(emitAnimateStatus(bool)), this,
                SIGNAL(showHideDialogAnimate(bool)));
        connect(CCTalkAcceptor, SIGNAL(emitStatus(int, QString)), this,
                SLOT(getStatusFromAcceptor(int, QString)));
        connect(CCTalkAcceptor, SIGNAL(emitLoging(int, QString, QString)),
                SIGNAL(emitLoging(int, QString, QString)));
        connect(CCTalkAcceptor, SIGNAL(emitBillTable(QString)), this,
                SIGNAL(emitBillTable(QString)));
    }
}
...
void ClassAcceptor::setPortListInfo(QStringList port_list) { portList = port_list; }
...
bool ClassAcceptor::isItYou(QStringList &comList, QString &validator_name, QString &com_str,
