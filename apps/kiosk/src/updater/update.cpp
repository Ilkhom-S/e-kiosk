...
DownloadManager::DownloadManager() : QThread() {
  ...
  connect(this->copyFile, SIGNAL(emit_Loging(int, QString, QString)), this,
          SIGNAL(emit_Loging(int, QString, QString)));
  connect(this->copyFile, SIGNAL(copyComplite()), this,
          SLOT(afterCopyAllFiles()));
  connect(this->copyFile, SIGNAL(emit_FilesUpdated(QVariantMap)), this,
...
int DownloadManager::getAppFileSize() {
  ...
  QString strQuery = QString(
      "SELECT size FROM local_files WHERE name = \"EKiosk.exe\" LIMIT 1;");
...
void DownloadManager::setUpdatePointName(QString name) { FolderName = name; }

void DownloadManager::setUrlServerIp(QString ip) { IpServer = ip; }

void DownloadManager::setXmlFileName(QString name) { XmlName = name; }
...
void DownloadManager::startToUpdate() {
  emit this->emit_Loging(0, this->senderName,
                         QString("Начинаем закачку файлов с сервера..."));
  ...
  emit this->emit_Loging(
      0, this->senderName,
      QString("Начинаем качать XML файл %1").arg(nowDownloadFileName));
...
void DownloadManager::startNextDownload() {
  ...
  if (fileWrite) {
    ...
    if (!output.open(QIODevice::Append)) {
      ...
      emit this->emit_Loging(
          2, this->senderName,
          QString("Не возможно открыть файл для до записи %1")
    ...
    request.setRawHeader(QByteArray("Range"),
                         QString("bytes=%1-").arg(size).toLatin1());
...
void DownloadManager::downloadFileResponse() {
  emit this->emit_Loging(
      0, this->senderName,
      QString("Файл %1 успешно закачен на локальный компьютер...")
          .arg(nowDownloadFileName));
      ...
      if (hash != nowDownloadFileHash) {
        if (count_download < 20) {
          emit this->emit_Loging(
              0, this->senderName,
              QString(
                  "Хеш файла %1 не совпадает (делаем докачку через 20 сек...)")
                  .arg(nowDownloadFileName));
          this->resendTimer->start(20000);
          return;
        } else {
...
void DownloadManager::openDocumentXml() {
  ...
  if (!file.open(QIODevice::ReadOnly)) {
    ...
    emit this->emit_Loging(1, this->senderName,
                           QString("Не возможно открыть %1 файл для чтения.")
                               .arg(tmpDirectory + this->XmlName));
    return;
  }
  // Файл открыт успешно
  emit this->emit_Loging(0, this->senderName,
                         QString("Файл %1 открыт успешно.").arg(this->XmlName));
  qDebug() << QString(" =====> File %1%2 OPENED;")
                  .arg(tmpDirectory)
                  .arg(this->XmlName);
  ...
  if (!doc.setContent(&file)) {
    ...
    emit this->emit_Loging(
        1, this->senderName,
        QString("Не возможно отпарсить %1 файл.").arg(this->XmlName));
    return;
  } else {
    ...
    emit this->emit_Loging(
        0, this->senderName,
        QString("Начинаем парсить %1 файл.").arg(this->XmlName));
  ...
  qDebug() << QString(" =====> File %1%2 CLOSED;")
                  .arg(tmpDirectory)
                  .arg(this->XmlName);
  emit this->emit_Loging(
      0, this->senderName,
      QString("Файл %1 успешно отпарсен.").arg(this->XmlName));
  ...
  qDebug() << QString(" =====> File %1%2 REMOVED;")
                  .arg(tmpDirectory)
                  .arg(this->XmlName);
...
void DownloadManager::run() {
  ...
  for (int i = 1; i <= countFileTag; i++) {
    if (!DataQuery.exec(
            strQuery.arg(ServerXmlMap[i]["path"], ServerXmlMap[i]["name"]))) {
      ...
      emit this->emit_Loging(
          2, this->senderName,
          QString("Ошибка выполнения запроса к базе с файлами."));
      return;
    }
    ...
    if (DataQuery.next()) {
      ...
      if (ServerXmlMap[i]["hash"] != hash || sts == 1) {
        ...
        if (this->updateRecordFile(id, QString("status"), QString("1"))) {
          ...
          /// данные
          if (this->updateRecordForMore(id, ServerXmlMap[i]["hash"],
                                        ServerXmlMap[i]["size"].toInt())) {
      ...
      if (this->insertNewRecordFile(
              ServerXmlMap[i]["path"], ServerXmlMap[i]["name"],
              ServerXmlMap[i]["size"], ServerXmlMap[i]["hash"])) {
        qDebug() << QString("ADD NEW FILE %1 FOR DOWNLOAD ")
                        .arg(ServerXmlMap[i]["name"]);
        updateFile = true;
...
void DownloadManager::addToDatabaseFile() { this->start(); }
...
void DownloadManager::downloadOneByOneFile() {
  ...
  QString strQuery =
      QString("SELECT * FROM local_files WHERE status = 1 LIMIT 1;");
  ...
  if (DataQuery.next()) {
    ...
    this->nowDownloadFileName =
        DataQuery.value(record.indexOf("name")).toString();
    this->nowDownloadFileSize = DataQuery.value(record.indexOf("size")).toInt();
    this->nowDownloadFileHash =
        DataQuery.value(record.indexOf("hash")).toString();
    this->nowDownloadFileId = DataQuery.value(record.indexOf("id")).toInt();
    this->nowDownloadFilePath =
        DataQuery.value(record.indexOf("path")).toString();
    ...
    QString vrmUrl =
        QString("%1/%2/%3")
            .arg(this->IpServer, this->FolderName, nowDownloadFileAll);
...
void DownloadManager::afterCopyAllFiles() {
  ...
  if (infoNamePath.exists()) {
    ...
    QDir dir;
    if (dir.rename("tmp",
                   QString("tmp_%1").arg(QDateTime::currentDateTime().toString(
                       "yyyyMMddHHmmss"))))
...
void DownloadManager::reCopyAllFiles() {
  try {
    ...
    int count_i_files = 0;
    for (QStringList::Iterator iter = allFilesList.begin();
         iter != allFilesList.end(); ++iter) {
      ...
      QFile fileCopy;
      if (infoIn.fileName() != "EKiosk.exe" &&
          infoIn.fileName() != "h-sheller.exe") {
        copyXmlMap[count_i_files]["pathFrom"] = "tmp/" + fileFrom;
        copyXmlMap[count_i_files]["pathTo"] = fileFrom;
        qDebug() << "--- FILE-PATH FROM --- "
                 << copyXmlMap[count_i_files]["pathFrom"];
...
QStringList DownloadManager::getDirFiles(const QString &dirName) {
  ...
  for (QStringList::Iterator dit = dirList.begin(); dit != dirList.end();
       ++dit) {
...
void DownloadManager::updateGlobalHash() {
  ...
  QString strUpdate =
      QString("UPDATE global_hash SET hash = \"%1\" WHERE id = 1")
          .arg(this->gblNewHash);
...
bool DownloadManager::updateRecordFile(int id, QString fieldName,
                                       QString value) {
  QSqlQuery updateQuery(this->db_);

  QString strUpdate =
      QString("UPDATE local_files SET %1 = \"%2\" WHERE id = %3")
...
bool DownloadManager::updateRecordForMore(int id, QString hash, int size) {
  ...
  QString strUpdate =
      QString("UPDATE local_files SET hash = \"%1\", size = %2 WHERE id = %3")
          .arg(hash)
          .arg(size)
          .arg(id);
...
bool DownloadManager::insertNewRecordFile(QString path, QString name,
                                          QString size, QString hash) {
  QSqlQuery insertQuery(this->db_);

  QString strUpdate =
      QString("INSERT INTO local_files (id,path,name,size,hash,status)"
...
void DownloadManager::setDbName(QSqlDatabase &db) { this->db_ = db; }

QString DownloadManager::getOldHash() {
  QSqlQuery selectHash(this->db_);

  QString strQuery =
      QString("SELECT hash FROM global_hash WHERE id = 1 LIMIT 1;");
...
QString
DownloadManager::fileChecksum(const QString &fileName,
