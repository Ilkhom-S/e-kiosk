...
bool DownloadManager::checkHashMonitor(QString hash)
{
	...
	if (hash != oldHash)
	{
		...
		emit this->emit_Loging(0, this->senderName,
							   QString("Hash_update_interface отличаются(старый - %1, новый - %2)").arg(oldHash, hash));
		// Присваиваем хеш
		gblNewHash = hash;
	}
		...
		if (currentDownload->error())
		{
			...
			else
			{
				emit this->emit_Loging(
					1, this->senderName,
					QString("Превышено число попыток для закачки файла %1 (повторная закачка через 5 мин.)")
						.arg(nowDownloadFileName));
...
void DownloadManager::run()
{
	...
	QString strQuery =
		QString("SELECT id,hash,name,status FROM local_files WHERE path = \"%1\" AND name = \"%2\" LIMIT 1;");
		...
		if (domNode.isElement())
		{
			...
			if (strTag == "file")
			{
				...
				ServerXmlMap[countFileTag]["path"] =
					domElement.attribute("path", "").right(domElement.attribute("path", "").length() - 1);
