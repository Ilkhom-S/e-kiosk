...
void DownloadManager::startNextDownload() {
    ...
    if (this->nowDownloadFilePath != "") this->createPathDirIfNotExist(this->nowDownloadFilePath);
    ...
    if (currentDownload->error() == 203) {
        ...
        if (currentDownload->error()) {
            ...
            if (nowDownloadFileName == this->XmlName) {
                emit this->emit_Loging(
                    1, this->senderName,
                    QString("Ошибка при закачки XML файла %1, повторная закачка через 5 мин.")
            ...
            if (count_download < 20) {
                emit this->emit_Loging(
                    0, this->senderName,
                    QString("Попытка №%1 для закачки файла %2 (повторная закачка через 20 сек.)")
                        .arg(count_download)
                        .arg(nowDownloadFileName));
                ...
                emit this->emit_Loging(1, this->senderName,
                                       QString("Превышено число попыток для закачки файла %1 "
...
void DownloadManager::run() {
    ...
    QString strQuery = QString(
        "SELECT id,hash,name,status FROM local_files WHERE path = \"%1\" AND name = \"%2\" LIMIT "
        "1;");
    ...
    for (int i = 1; i <= countFileTag; i++) {
        ...
        if (DataQuery.next()) {
            ...
            if (this->insertNewRecordFile(ServerXmlMap[i]["path"], ServerXmlMap[i]["name"],
    ...
    if (updateFile) {
        ...
        emit emit_Loging(
            0, this->senderName,
            QString("Нет новых файлов на сервере (обновляем hash и закрываем UPDATER)"));
...
void DownloadManager::afterCopyAllFiles() {
    ...
    if (infoNamePath.exists()) {
        ...
        /// Удаление папки tmp
        QDir dir;
        if (dir.rename("tmp", QString("tmp_%1").arg(
...
void DownloadManager::compliteUpdateIn() {
    ...
    emit this->emit_Loging(
        0, this->senderName,
        QString("База не содержит новых файлов (обновляем hash и закрываем UPDATER)"));
...
bool DownloadManager::insertNewRecordFile(QString path, QString name, QString size, QString hash) {
    ...
    QString strUpdate = QString(
                            "INSERT INTO local_files (id,path,name,size,hash,status)"
