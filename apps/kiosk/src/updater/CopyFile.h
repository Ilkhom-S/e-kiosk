            ...
            for (QStringList::Iterator iter = allFilesList.begin(); iter != allFilesList.end(); ++iter) {
                ...
                if (fileNameIn == "h-sheller.exe") {
                    ...
                    if (fileCopy.copy("tmp/h-sheller.exe", QDir::currentPath() + "/h-sheller.exe")) {
                        ...
                        emit emit_Loging(0, senderName,QString("Файл %1 успешно перемещен в директорию root...").arg(infoIn.fileName()));
                    } else {
                        if (debugger) qDebug() << "--- COPY FILE ERROR ---sheler";
                        emit emit_Loging(1, senderName,QString("Файл %1 не удалось переместить в директорию root...").arg(infoIn.fileName()));
                    }
                    continue;
                }
                ...
                if (fileCopy.copy(copyXmlMap[idx]["pathFrom"],copyXmlMap[idx]["pathTo"])) {
                    ...
                    emit emit_Loging(0, senderName, QString("Файл %1 успешно перемещен в директорию assets...").arg(infoIn.fileName()));
                }else{
                    if (debugger) qDebug() << "--- COPY FILE ERROR --- " << infoIn.fileName();
                    emit emit_Loging(1, senderName, QString("Файл %1 не удалось переместить в директорию assets...").arg(infoIn.fileName()));
                }
