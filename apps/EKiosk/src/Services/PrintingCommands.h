...
const QStringList FFDataList = QStringList()
                               << CPrintConstants::OpPhone << CPrintConstants::DealerSupportPhone
...
public:
    PrintFiscalCommand(const QString &aReceiptType,
...
//---------------------------------------------------------------------------
// Команды, реализующие проверку принтера и печать определенного вида чека. В зависимости от типа
// чека, также могут выполняться вспомогательные действия (например сохранение чека в текстовом
// файле, добавление определенных полей и т.д.)
