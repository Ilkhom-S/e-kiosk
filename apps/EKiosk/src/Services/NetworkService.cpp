...
NetworkService::NetworkService(IApplication *aApplication)
    : ILogable("Connection"), mDeviceService(0), mEventService(0), mConnection(0), mEnabled(true),
      mDontWatchConnection(false), mApplication(aApplication), mFails(0) {
    ...
    connect(&mNetworkTaskManager, SIGNAL(networkTaskStatus(bool)), this, SLOT(onNetworkTaskStatus(bool)),
            Qt::QueuedConnection);
...
const QSet<QString> &NetworkService::getRequiredServices() const {
    static QSet<QString> requiredServices = QSet<QString>() << CServices::DeviceService << CServices::EventService
...
void NetworkService::updateModemParameters() {
    ...
    if (modemDevice) {
        ...
        if (getConnectionTemplate(getConnection().name, connectionTemplate)) {
            ...
            toLog(LogLevel::Error, QString("Connection template '%1' not found. USSD request for balance undefined.")
        ...
        mDatabaseUtils->setDeviceParam(mDeviceService->getDeviceConfigName(modemDevice),
                                       PP::CDatabaseConstants::Parameters::ConnectionName, mOperator.get());
        ...
        if (modemDevice->getSignalQuality(signalLevel)) {
            ...
            mDatabaseUtils->setDeviceParam(mDeviceService->getDeviceConfigName(modemDevice),
                                           PP::CDatabaseConstants::Parameters::SignalLevel, mSignalLevel.get());
        } else {
        ...
        if (modemDevice->getInfo(modemInfo)) {
            ...
            mDatabaseUtils->setDeviceParam(mDeviceService->getDeviceConfigName(modemDevice),
                                           PP::CDatabaseConstants::Parameters::DeviceInfo, modemInfo);
        }
        ...
        if (modemDevice->takeMessages(messages)) {
            foreach (auto sms, messages) {
                toLog(LogLevel::Normal,
                      QString("SMS [%1] at %2: %3.").arg(sms.from).arg(sms.date.toString()).arg(sms.text));
            }
        }
    } else {
...
void NetworkService::doConnect(const SDK::PaymentProcessor::SConnection &aConnection) {
    ...
    try {
        toLog(LogLevel::Normal,
              QString("Attempt %2: establishing connection '%1'...").arg(aConnection.name).arg(mFails));

        mConnection =
            QSharedPointer<IConnection>(IConnection::create(aConnection.name, aConnection.type, &mNetworkTaskManager,
        ...
        if (isConnected()) {
            ...
            if (getConnection().type == EConnectionTypes::Dialup) {
                ...
                toLog(LogLevel::Normal, QString("Using proxy: host = %1, port = %2.")
        ...
        if (isConnected() && getConnection().type == EConnectionTypes::Dialup) {
            auto modem = getModem();
            if (modem) {
                DeviceService::instance(mApplication)
...
void NetworkService::onConnectionLost() {
    ...
    if (mFails >= CNetworkService::FailsBeforeReboot) {
        toLog(
            LogLevel::Warning,
            QString("Generating system reboot event after %1 unsuccessful tries to establish connection.").arg(mFails));
    ...
    if (getConnection().type == EConnectionTypes::Dialup) {
        ...
        if (modem) {
            DeviceService::instance(mApplication)
                ->overwriteDeviceStatus(modem, SDK::Driver::EWarningLevel::Warning, "Disconnected", 0);
        }
    }
    ...
    if (getConnection().type == EConnectionTypes::Dialup) {
        ...
        if (!watchdogConfigName.empty()) {
            watchdogDevice =
                dynamic_cast<SDK::Driver::IWatchdog *>(mDeviceService->acquireDevice(watchdogConfigName.first()));
        }
...
SDK::Driver::IModem *NetworkService::getModem() {
    ...
    SDK::Driver::IModem *modemDevice =
        !modemConfigName.empty()
            ? dynamic_cast<SDK::Driver::IModem *>(mDeviceService->acquireDevice(modemConfigName.first()))
            : nullptr;
        ...
        if (getConnectionTemplate(aConnectionName, connectionTemplate)) {
            if (!aModemDevice->setInitString(connectionTemplate.initString)) {
                toLog(LogLevel::Error,
                      QString("Failed to set modem initialization string: %1.").arg(connectionTemplate.initString));
            }
        }
    }
...
void NetworkService::onNetworkTaskStatus(bool aFailure) {
    ...
    } else if (qAbs(mNetworkTaskFailureStamp.secsTo(QDateTime::currentDateTime()) / 60.) >
               CNetworkService::NetworkFailureTimeout) {
        toLog(LogLevel::Error, QString("Errors network calls for %1 minutes. Connection lost.")
