...
void HIDService::updateHardwareConfiguration() {
    ...
    foreach (
        ...
        if (device) {
            ...
            device->subscribe(
                SDK::Driver::IHID::DataSignal, this, SIGNAL(data(const QVariantMap &)));
            toLog(LogLevel::Normal, QString("Device '%1' added...").arg(device->getName()));
        } else {
            toLog(LogLevel::Error,
                  QString("Failed to acquire HID device %1.").arg(configurationName));
        }
    }
    ...
    foreach (const QString &configurationName,
        ...
        if (device) {
            ...
            // Подписываемся на сигнал от устройства.
            device->subscribe(
                SDK::Driver::ICardReader::InsertedSignal,
                this,
                SLOT(onCardInserted(SDK::Driver::ECardType::Enum, const QVariantMap &)));
            ...
            toLog(LogLevel::Error,
                  QString("Failed to acquire CardReader device %1.").arg(configurationName));
        }
    }
}

//---------------------------------------------------------------------------
void HIDService::onStatusChanged(DSDK::EWarningLevel::Enum /*aLevel*/,
...
bool HIDService::setEnable(bool aEnable, const QString &aDevice) {
    ...
    toLog(LogLevel::Debug,
          QString("Update HID device state: name [%1], state [%2], result: [%3]")
              .arg(aDevice)
...
void HIDService::onCardInserted(SDK::Driver::ECardType::Enum aCardType, const QVariantMap &aData) {
    ...
    if (cardReader) {
        toLog(
            LogLevel::Normal,
            QString("Card type %1 inserted into '%2'.").arg(aCardType).arg(cardReader->getName()));
        ...
        if (!aData.isEmpty()) {
            ...
            foreach (auto track, aData.keys()) {
                toLog(
                    LogLevel::Debug,
                    QString("Card data %1 : '%2'.").arg(track).arg(aData.value(track).toString()));
            }
#endif
            emit inserted(aData);
        }
