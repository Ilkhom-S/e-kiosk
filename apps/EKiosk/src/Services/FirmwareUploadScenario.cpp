...
void FirmwareUploadScenario::abortScenario(const QString &aErrorMessage)
{
    ...
    if (--mRetryCount > 0)
    {
        toLog(LogLevel::Normal,
              QString("Retry upload throw %1 sec").arg(CFirmwareUploadScenario::UploadRetryTimeout / 1000));
    ...
    if (mDevice)
    {
        disconnect(dynamic_cast<QObject *>(mDevice), SDK::Driver::IDevice::InitializedSignal, this,
...
void FirmwareUploadScenario::acquireDevice()
{
    ...
    {
        toLog(LogLevel::Normal,
              QString("Device '%1' have status: %2.").arg(device->getName()).arg(status->description()));
    ...
    {
        if (!connect(dynamic_cast<QObject *>(device), SDK::Driver::IDevice::InitializedSignal, this,
                     SLOT(onDeviceInitialized())))
        {
