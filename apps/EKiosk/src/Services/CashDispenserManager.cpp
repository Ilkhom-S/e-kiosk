...
CashDispenserManager::CashDispenserManager(IApplication *aApplication)
    : ILogable(CFundsService::LogName), mApplication(aApplication), mDeviceService(nullptr), mDatabase(nullptr),
...
void CashDispenserManager::updateHardwareConfiguration()
{
    ...
    foreach (const QString &configurationName, deviceList)
    {
        ...
        if (device)
        {
            ...
            device->subscribe(SDK::Driver::IDevice::StatusSignal, this,
...
//---------------------------------------------------------------------------
void CashDispenserManager::onStatusChanged(DSDK::EWarningLevel::Enum aLevel, const QString &aTranslation,
                                           int /*aStatus*/)
{
...
PPSDK::SCashUnit *CashDispenserManager::checkSignal(QObject *aSender, const QString &aSignalName, int aUnit)
{
    ...
    if (!dispenser)
    {
        toLog(LogLevel::Error,
              QString("Receive %1 signal, but sender not have DSDK::IDispenser interface.").arg(aSignalName));
        return nullptr;
    }
...
//---------------------------------------------------------------------------
bool CashDispenserManager::handleSignal(QObject *aSender, const QString &aSignalName, int aUnit, int aItems,
                                        PPSDK::TPaymentAmount &aAmount)
{
    ...
    toLog(LogLevel::Normal, QString("%1 %2 notes (nominal %3) AMOUNT = %4")
                                .arg(aSignalName)
                                .arg(aItems)
                                .arg(nominal)
                                .arg(aAmount, 0, 'f', 2));
...
void CashDispenserManager::onUnitsDefined()
{
    ...
    foreach (DSDK::IDispenser *dispenser, dispensers)
    {
        ...
        if (units)
        {
            ...
            {
                PPSDK::TCashUnitList cashUnitList =
                    PPSDK::TCashUnitList(units - currentUnits, PPSDK::SCashUnit(mCurrencyName, 0, 0));
...
void CashDispenserManager::saveCashCount()
{
    foreach (auto configName, mCurrencyCashList.keys())
    {
        ...
        mDatabase->setDeviceParam(configName, PPSDK::CDatabaseConstants::Parameters::CashUnits,
...
//---------------------------------------------------------------------------
bool CashDispenserManager::getItemData(SDK::PaymentProcessor::TPaymentAmount aAmount, TItemDataSet &aItemData,
...
PPSDK::TPaymentAmount CashDispenserManager::canDispense(PPSDK::TPaymentAmount aRequiredAmount)
{
    ...
    while (getItemData(aRequiredAmount - dispensingAmount, itemDataSet, itemDataSetIt))
    {
        ...
        int availableCount =
            std::accumulate(itemDataSetIt->begin(), itemDataSetIt->end(), 0,
                            [](int aCount, const SItemData &data) -> int { return aCount + data.count; });
...
bool CashDispenserManager::storeNotes(QObject *aSender, int aUnit, int aItems)
{
    ...
    {
        notes.push_back(
            PPSDK::SNote(PPSDK::EAmountType::Bill, cashUnit->nominal, settings->getCurrencySettings().id, ""));
    }
