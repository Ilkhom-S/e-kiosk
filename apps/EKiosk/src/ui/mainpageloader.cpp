...
void MainPageLoader::orzuUserDefine(QString inn) {
    ...
    emit emit_sendJsonRequest(
        QJsonObject(), QString("orzu/user/%1").arg(inn), "orzu_user_define", 0, 20);
}
...
void MainPageLoader::jsonResponseSuccess(QVariantMap response, QString requestName) {
    ...
    if (requestName == "check_pan") {
        emit emit_toLoging(
            0, "ORZU", "Карта успешно подтвеждена, отправляется запрос на получение кредита");
...
void MainPageLoader::setCheckOnlineResult(QString resultCode,
                                          QString status,
                                          QString message,
                                          QVariantList items) {
    Q_UNUSED(status)
...
void MainPageLoader::getCommissionMap() {
    ...
    if (cid == 0) {
        ...
        QString userQuery = QString("SELECT * FROM terminal_commission WHERE service_id = %1 AND "
