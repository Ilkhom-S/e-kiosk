...
QMap<qint64, TPaymentParameters> DatabaseUtils::getPaymentParameters(const QList<qint64> &aIds) {
    ...
    QString strQuery = QString("SELECT `create_date`, `last_update`, `type`, `initial_session`, "
                               "`session`, `server_status`, "
                               "`server_error`, `number_of_tries`, `operator`, `status`, "
                               "`priority`, `signature`, `receipt_printed`, "
...
bool DatabaseUtils::savePayment(PPSDK::IPayment *aPayment, const QString &aSignature) {
    ...
    try {
        QString strQuery = "UPDATE `payment` SET `create_date` = :creation_date, `last_update` = "
                           ":last_update, `type` = :type, "
                           "`initial_session` = :initial_session, `session` = :session, "
                           "`server_status` = :server_result, "
                           "`server_error` = :server_error, `number_of_tries` = :try_count, "
                           "`next_try_date` = :next_try_date, "
                           "`operator` = :provider, "
                           "`status` = :status, `priority` = :priority, `signature` = :signature, "
                           "`receipt_printed` = "
        ...
        query->bindValue(":creation_date",
                         aPayment->getCreationDate().toString(CIDatabaseProxy::DateFormat));
        query->bindValue(":last_update",
        ...
        query->bindValue(":next_try_date",
                         aPayment->getParameter(PPSDK::CPayment::Parameters::NextTryDate)
                             .value.toDateTime()
        ...
        query->bindValue(
            ":receipt_printed",
            aPayment->getParameter(PPSDK::CPayment::Parameters::ReceiptPrinted).value.toBool() ? 1
                                                                                               : 0);
        ...
        savedParameters << PPSDK::CPayment::Parameters::ID
        ...
        strQuery = "INSERT OR REPLACE INTO `payment_param` (`name`, `value`, `type`, "
                   "`fk_payment_id`, `external`) "
        ...
        foreach (const PPSDK::IPayment::SParameter &parameter, aPayment->serialize()) {
            if (!savedParameters.contains(parameter.name)) {
                ...
                if (!query->exec()) {
                    throw QString("failed to save parameter %2")
                        .arg(aPayment->getID())
                        .arg(parameter.name);
                }
            }
...
QList<qint64> DatabaseUtils::getPaymentQueue() {
    ...
    QString strQuery = "SELECT `id` FROM `payment` WHERE (`status` IN (:status_init, "
                       ":status_check, :status_error)) AND "
                       "(`next_try_date` IS NULL OR strftime('%s%f', `next_try_date`) IS NULL OR "
                       "strftime('%s%f', `next_try_date`) <= "
                       "strftime('%s%f', :date)) "
...
PPSDK::SBalance DatabaseUtils::getBalance() {
    ...
    try {
        ...
        queryStr =
            QString("SELECT DISTINCT(payment.[id]) FROM payment, payment_note "
        ...
        {
            queryStr = QString("SELECT `type`, `nominal`, COUNT(*), GROUP_CONCAT(`serial`, ',') "
                               "FROM `payment_note` "
            ...
            for (query->first(); query->isValid(); query->next()) {
                PPSDK::EAmountType::Enum type =
                    static_cast<PPSDK::EAmountType::Enum>(query->value(0).toInt());
                ...
                amounts[type].amounts
                    << PPSDK::SBalance::SAmounts::SAmount(query->value(1).toDouble(),
                                                          query->value(2).toInt(),
        ...
        queryStr = "SELECT `id`, `receipt_printed`, `status` FROM `payment` WHERE `id` >= (SELECT "
                   "P.`id` FROM "
                   "`payment` AS P, `payment_note` "
                   "AS PN WHERE PN.`fk_payment_id` = P.`id` AND ((NOT PN.`ejection`) OR "
        ...
        if (query->first()) {
            do {
                ...
                if (paymentStatus != PPSDK::EPaymentStatus::LostChange &&
            ...
            queryStr = QString("SELECT SUM(`value`) FROM `payment_param` WHERE `name` = :name AND "
                               "`fk_payment_id` >= "
                               ":first_payment "
            ...
            // 5. Посчитаем сумму проведенных платежей. Не можем считать по notes'ам, так как нужно
            // учитывать платежи, оплаченные сдачей.
            queryStr =
                "SELECT SUM(`value`) FROM `payment_param` WHERE `name` = :name AND `fk_payment_id` "
                "IN "
            ...
            result.processed = query->value(0).toString().isEmpty() ? QString::number(0)
...
//---------------------------------------------------------------------------
QList<qint64>
DatabaseUtils::getPayments(const QSet<SDK::PaymentProcessor::EPaymentStatus::Enum> &aStates) {
    QList<qint64> result;
...
QList<qint64> DatabaseUtils::findPayments(const QDate &aDate, const QString &aPhoneNumber) {
    ...
    if (aDate.isValid() && !aDate.isNull()) {
        queryStr += QString(" AND p.create_date between '%1 00:00:00' AND '%1 59:59:59'")
...
void DatabaseUtils::fillEncashmentReport(PPSDK::SEncashment &aEncashment) {
    foreach (qint64 payment, aEncashment.balance.payments) {
        ...
        report << PPSDK::IPayment::parameterByName(PPSDK::CPayment::Parameters::ID, parameters)
                      .value.toString()
               << PPSDK::IPayment::parameterByName(PPSDK::CPayment::Parameters::CreationDate,
                                                   parameters)
                      .value.toString()
               << PPSDK::IPayment::parameterByName(PPSDK::CPayment::Parameters::InitialSession,
                                                   parameters)
                      .value.toString()
               << PPSDK::IPayment::parameterByName(PPSDK::CPayment::Parameters::Session, parameters)
                      .value.toString()
               << PPSDK::IPayment::parameterByName(PPSDK::CPayment::Parameters::Provider,
                                                   parameters)
                      .value.toString()
               << PPSDK::IPayment::parameterByName(PPSDK::CPayment::Parameters::Amount, parameters)
                      .value.toString()
               << PPSDK::IPayment::parameterByName(PPSDK::CPayment::Parameters::AmountAll,
                                                   parameters)
                      .value.toString()
               << PPSDK::IPayment::parameterByName(PPSDK::CPayment::Parameters::Status, parameters)
                      .value.toString();
        ...
        for (auto &field : provider.fields) {
            ...
            providerFields << QString("%1:%2").arg(field.id).arg(
                parameter.crypted ? "**CRYPTED**" : parameter.value.toString());
        }
...
QList<SDK::PaymentProcessor::SEncashment> DatabaseUtils::getLastEncashments(int aCount) {
    ...
    // 1. Вытаскиваем дату последней инкассации, она будет у нас начальной.
    QString queryStr =
        QString("SELECT `id`, `date`, `amount`, `fee`, `processed`, `report`, `notes`, `coins`, "
    ...
    for (; query->isValid(); query->next()) {
        ...
        {
            queryStr = "SELECT `type`, `nominal`, COUNT(*), GROUP_CONCAT(`serial`, ',') FROM "
                       "`payment_note` WHERE "
                       "`ejection` = :ejection "
                       "GROUP BY `type`, `nominal` ORDER BY `type` ASC, `nominal` DESC";
            QScopedPointer<IDatabaseQuery> q(mDatabase.createQuery(queryStr));
            ...
            if (q->exec() && q->first()) {
                ...
                for (; q->isValid(); q->next()) {
                    PPSDK::EAmountType::Enum type =
                        static_cast<PPSDK::EAmountType::Enum>(q->value(0).toInt());
...
bool DatabaseUtils::backupOldPayments() {
    ...
    strQuery = "SELECT `id`, `create_date`, `last_update`, `type`,	`initial_session`, `session`, "
               "`server_status`, `server_error`, `number_of_tries`, `next_try_date`, `operator`, "
               "`status`, `on_monitoring`, "
               "`priority`, `step`, `currency`, `signature`, `receipt_printed` FROM `payment` "
               "WHERE strftime('%s%f', "
