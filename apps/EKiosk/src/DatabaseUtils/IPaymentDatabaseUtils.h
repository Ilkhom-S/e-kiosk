...
public:
    ...
    virtual bool addPaymentNote(qint64 aPayment,
                                const QList<SDK::PaymentProcessor::SNote> &aNotes) = 0;

    /// Добавим в БД информацию о выданных купюрах
    virtual bool addChangeNote(const QString &aSession,
                               const QList<SDK::PaymentProcessor::SNote> &aNotes) = 0;

    /// Получить информацию по всем купюрам в контексте платежа.
    ...
    getPayments(const QSet<SDK::PaymentProcessor::EPaymentStatus::Enum> &aStates) = 0;
