...
/*!
        ...
        \sa toHex()
*/
QByteArray QAtUtils::from_Hex(const QString &hex) {
    QByteArray bytes;
    uint ch;
    int posn;
...
static int From_HexDigit(uint ch) {
    if (ch >= '0' && ch <= '9') {
        return (int)(ch - '0');
    } else if (ch >= 'A' && ch <= 'F') {
        return (int)(ch - 'A' + 10);
...
static int From_OctalDigit(uint ch) {
    if (ch >= '0' && ch <= '7') {
        return (int)(ch - '0');
    } else {
        return -1;
    ...
    while (posn < (uint)(buf.length()) && (ch = buf[posn].unicode()) != '"') {
        ...
        if (ch == '\\') {
            if (!octalEscapesFlag) {
                ...
                if (posn >= (uint)buf.length())
                    break;
                digit = From_HexDigit(buf[posn].unicode());
                if (digit == -1) {
                    result += (QChar)'\\';
                    continue;
                }
                if ((posn + 1) >= (uint)buf.length()) {
                    ...
                    digit2 = From_HexDigit(buf[posn + 1].unicode());
                    if (digit2 == -1) {
                        ch = (uint)digit;
                        ++posn;
                    } else {
                ...
                if (posn >= (uint)buf.length())
                    break;
                digit = From_OctalDigit(buf[posn].unicode());
                if (digit == -1) {
                    result += (QChar)'\\';
                    continue;
                }
                if ((posn + 1) >= (uint)buf.length()) {
                    ...
                    digit2 = From_OctalDigit(buf[posn + 1].unicode());
                    if (digit2 == -1) {
                        ch = (uint)digit;
                        ++posn;
                    } else {
                        if ((posn + 2) >= (uint)buf.length()) {
                            ...
                            digit3 = From_OctalDigit(buf[posn + 2].unicode());
                            if (digit3 == -1) {
                                ch = (uint)(digit * 8 + digit2);
                                posn += 2;
                            } else {
...
class QGsm_HexCodec : public QTextCodec {
public:
    QGsm_HexCodec();
    ~QGsm_HexCodec();
...
QByteArray QGsm_HexCodec::name() const {
    return QByteArray("gsm-hex");
}

int QGsm_HexCodec::mibEnum() const {
    return 61239;
}

QString QGsm_HexCodec::convertToUnicode(const char *in, int length, ConverterState *) const {
    QString str;
    int nibble = 0;
    int value = 0;
    int digit;
    ...
    while (length-- > 0) {
        ...
        if (!nibble) {
            ...
            if (!secondByte) {
                if (value != 0x1B) {
                    str += QGsm_Codec::twoByteToUnicode((unsigned short)value);
                } else {
                    secondByte = true;
                }
            } else {
...
static unsigned char const cp437From_Unicode[256] = {
    ...
    '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',
    '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',  '?',
    0xff, 0xad, 0x9b, 0x9c, '?',  0x9d, '?',  0x15, '?',  '?',  0xa6, 0xae, 0xaa, '?',  '?',  '?',
    0xf8, 0xf1, 0xfd, '?',  '?',  '?',  0x14, 0xfa, '?',  '?',  0xa7, 0xaf, 0xac, 0xab, '?',  0xa8,
...
QByteArray
QCodePage437Codec::convertFrom_Unicode(const QChar *in, int length, ConverterState *) const {
    QByteArray result;
    unsigned int ch;
    char *out;
    ...
    for (int posn = 0; !non437 && posn < length; ++posn) {
        ...
        if (ch >= 0x0100)
            non437 = true;
        else if (cp437From_Unicode[ch] == '?' && ch != '?')
            non437 = true;
    }
    if (non437) {
    ...
    out = result.data();
    while (length-- > 0) {
        *out++ = (char)cp437From_Unicode[in->unicode()];
        ++in;
    }
    return result;
}
...
static unsigned char const cp850From_Unicode[256] = {
    0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
    0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x7f, 0x1b, 0x1a, 0x1d, 0x1e, 0x1f,
    0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f,
    0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3f,
...
static ushort const cp850MappingInput[] = {
    0x0110, 0x0131, 0x0192, 0x2017, 0x2022, 0x203c, 0x203e, 0x2190, 0x2191, 0x2192, 0x2193, 0x2194,
    0x2195, 0x21a8, 0x221f, 0x2302, 0x2500, 0x2502, 0x250c, 0x2510, 0x2514, 0x2518, 0x251c, 0x2524,
    0x252c, 0x2534, 0x253c, 0x2550, 0x2551, 0x2554, 0x2557, 0x255a, 0x255d, 0x2560, 0x2563, 0x2566,
    0x2569, 0x256c, 0x2580, 0x2584, 0x2588, 0x2591, 0x2592, 0x2593, 0x25a0, 0x25ac, 0x25b2, 0x25ba,
    0x25bc, 0x25c4, 0x25cb, 0x25d8, 0x25d9, 0x263a, 0x263b, 0x263c, 0x2640, 0x2642, 0x2660, 0x2663,
    0x2665, 0x2666, 0x266a, 0x266c, 0xff01, 0xff02, 0xff03, 0xff04, 0xff05, 0xff06, 0xff07, 0xff08,
    ...
    0xff5d, 0xff5e, 0xffe8, 0xffe9, 0xffea, 0xffeb, 0xffec, 0xffed, 0xffee};
static unsigned char const cp850MappingOutput[] = {
    0xd1, 0xd5, 0x9f, 0xf2, 0x07, 0x13, 0xee, 0x1b, 0x18, 0x1a, 0x19, 0x1d, 0x12, 0x17, 0x1c,
    0x7f, 0xc4, 0xb3, 0xda, 0xbf, 0xc0, 0xd9, 0xc3, 0xb4, 0xc2, 0xc1, 0xc5, 0xcd, 0xba, 0xc9,
    0xbb, 0xc8, 0xbc, 0xcc, 0xb9, 0xcb, 0xca, 0xce, 0xdf, 0xdc, 0xdb, 0xb0, 0xb1, 0xb2, 0xfe,
    0x16, 0x1e, 0x10, 0x1f, 0x11, 0x09, 0x08, 0x0a, 0x01, 0x02, 0x0f, 0x0c, 0x0b, 0x06, 0x05,
    0x03, 0x04, 0x0d, 0x0e, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b,
    ...
    0x77, 0x78, 0x79, 0x7a, 0x7b, 0x7c, 0x7d, 0x7e, 0xb3, 0x1b, 0x18, 0x1a, 0x19, 0xfe, 0x09};
...
QByteArray
QCodePage850Codec::convertFrom_Unicode(const QChar *in, int length, ConverterState *) const {
    QByteArray result;
    unsigned int ch;
    char *out;
    result.resize(length);
...
/*!
   ...
   gsm_Charset. The returned object should not be deleted.
   ...
   QTextCodec::codecForName().
        ...
        \sa QTextCodec, QGsm_Codec
*/
QTextCodec *QAtUtils::codec(const QString &gsm_Charset) {
    QString cs = gsm_Charset.toLower();
    QTextCodec *codec = 0;
    ...
    if (cs == "gsm") {
        ...
        static QTextCodec *gsm_NoLoss = 0;
        if (!gsm_NoLoss)
            gsm_NoLoss = new QGsm_Codec(true);
        codec = gsm_NoLoss;
    } else if (cs == "hex") {
        ...
        codec = QTextCodec::codecForName(gsm_Charset.toLatin1());
    }
