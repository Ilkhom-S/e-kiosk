...
SkinProvider::SkinProvider(const QString &aInterfacePath, const QString &aContentPath, const QString &aUserPath,
                           const Skin *aSkin)
...
QImage SkinProvider::requestImage(const QString &aId, QSize *aSize, const QSize &aRequestedSize)
{
    ...
    else
    {
        ...
        if (bimage == mBackgrounds.end() && !background.isEmpty())
        {
            ...
            {
                Log(Log::Error) << QString("SkinProvider: failed to load logo background '%1' from '%2'.")
                                       .arg(background)
                                       .arg(path);
            }
        }
