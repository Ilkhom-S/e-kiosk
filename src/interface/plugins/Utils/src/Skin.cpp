...
Skin::Skin(const QObject *aApplication, const QString &aInterfacePath, const QString &aUserPath)
    ...
    mProviderSkinConfig =
        userProviderSkinConfig.isEmpty() ? mProviderSkinConfig : userProviderSkinConfig;
    mProviderSkinConfig.insert("-1", mName);

    // Зарегистрируем шрифты всех скинов
    QDirIterator dirEntry(QString("%1/skins").arg(mInterfacePath),
        ...
        if (QJsonParseError::NoError == error.error) {
            ...
            foreach (QString key, config.keys()) {
                if (!key.startsWith("color.") && !key.startsWith("font.")) {
                    QString path = mInterfacePath + QDir::separator() + "skins" +
                                   QDir::separator() + mName + QDir::separator() +
                    ...
                    if (!QFile::exists(path)) {
                        Log(Log::Debug) << QString("SkinProvider: failed to load texture '%1' from "
                                               ...
                                               .arg(key)
                                               .arg(path);
                        path = mInterfacePath + QDir::separator() + "skins" + QDir::separator() +
