...
DownloadManager::DownloadManager() : QThread()
{
    ...
    connect(this->copyFile,SIGNAL(emit_Loging(int,QString,QString)),this,SIGNAL(emit_Loging(int,QString,QString)));
    connect(this->copyFile,SIGNAL(copyComplite()),this,SLOT(afterCopyAllFiles()));
    connect(this->copyFile,SIGNAL(emit_FilesUpdated(QVariantMap)),this,SIGNAL(emit_FilesUpdated(QVariantMap)));
...
bool DownloadManager::checkHashMonitor(QString hash)
{
    ...
    if(hash != oldHash){
        ...
        result = true;
        emit this->emit_Loging(0,this->senderName,QString("Hash_update_interface отличаются(старый - %1, новый - %2)").arg(oldHash, hash));
        //Присваиваем хеш
        gblNewHash  = hash;
    }
...
//    this->createPathDirIfNotExist(tmpDirectory);
    ...
    emit this->emit_Loging(0, this->senderName,QString("Начинаем качать XML файл %1").arg(nowDownloadFileName));
...
void DownloadManager::startNextDownload()
{
    ...
    this->createPathDirIfNotExist(tmpDirectory);
//    emit this->emit_Loging(0,this->senderName,QString("TMP PATH - %1").arg(tmpDirectory));
    ...
    if(fileWrite){
        /// Если надо тупа перезаписать или создать файл открываем с параметром (WriteOnly)
        if (!output.open(QIODevice::WriteOnly)) {
            fprintf(stderr, "Problem opening save file '%s' for download '%s': %s\n",
                    qPrintable(filename), now_url.toEncoded().constData(),
                    qPrintable(output.errorString()));

            //Не возможно создать файл
            emit this->emit_Loging(2,this->senderName,QString("Не возможно создать файл %1").arg(nowDownloadFileName));
        ...
        /// Если надо дозаписать файл открываем с параметром (Append)
            if (!output.open(QIODevice::Append)){
                ...
                emit this->emit_Loging(2,this->senderName,QString("Не возможно открыть файл для до записи %1").arg(nowDownloadFileName));
                return;                 // Пропускаем закачку
            }else{
        ...
        /// Готовим параметры для заголовка файла закачки
            ...
            //if(Debuger) qDebug() << "----- request.setRawHeader ----- size - " << size;
    }

    //Необходимо создать abort так как закачка может подвиснуть
    /// задаем время
    int timeAbort = 480; //sec
    if(this->nowDownloadFileName == this->XmlName) timeAbort = 180; // 3 min
    if(this->nowDownloadFileName == "EKiosk.exe") timeAbort = 900; // 8 min
...
{
    emit this->emit_Loging(0,this->senderName,QString("Файл %1 успешно закачен на локальный компьютер...").arg(nowDownloadFileName));
...
void DownloadManager::downloadFinished()
{
    ...
    if (currentDownload->error() == 203){
        ...
        if (currentDownload->error()){
            ...
            //Можно отрубить чтобы главная прога перезапустила сеть и вновь запустила апдейтер
            if(nowDownloadFileName == this->XmlName){
                emit this->emit_Loging(1,this->senderName,QString("Ошибка при закачки XML файла %1, повторная закачка через 5 мин.").arg(nowDownloadFileName));
            }
            /// Если число закачек не превышает 5
            if(count_download < 20){
                emit this->emit_Loging(0,this->senderName,QString("Попытка №%1 для закачки файла %2 (повторная закачка через 20 сек.)").arg(count_download).arg(nowDownloadFileName));
                //if(Debuger) qDebug() << "----- count_download < 5 -----";
                this->resendTimer->start(20000);
                return;
            }else{
                emit this->emit_Loging(1,this->senderName,QString("Превышено число попыток для закачки файла %1 (повторная закачка через 5 мин.)").arg(nowDownloadFileName));
    ...
    //            this->deleteErrorDownloadFile(nowDownloadFileId);
            ...
            if(hash != nowDownloadFileHash){
                if(count_download < 20){
                    emit this->emit_Loging(0,this->senderName,QString("Хеш файла %1 не совпадает (делаем докачку через 20 сек...)").arg(nowDownloadFileName));
                    this->resendTimer->start(20000);
                    return;
                }else{
                    this->reDownloadFile();
...
void DownloadManager::openDocumentXml()
{
    ...
    if (!file.open(QIODevice::ReadOnly)){
        ...
        emit this->emit_Loging(1,this->senderName,QString("Не возможно открыть %1 файл для чтения.").arg(tmpDirectory + this->XmlName));
        return;
    }
    //Файл открыт успешно
    emit this->emit_Loging(0,this->senderName,QString("Файл %1 открыт успешно.").arg(this->XmlName));
    ...
    emit this->emit_Loging(0,this->senderName,QString("Файл %1 успешно отпарсен.").arg(this->XmlName));
...
void DownloadManager::run()
{
    ...
    QString strQuery = QString("SELECT id,hash,name,status FROM local_files WHERE path = \"%1\" AND name = \"%2\" LIMIT 1;");
        ...
        if(DataQuery.next()){
            ...
            if(ServerXmlMap[i]["hash"] != hash || sts == 1)
            {
                ...
                if(this->updateRecordFile(id,QString("status"),QString("1"))){
                    /// Если строка состояния обновлена успешно то обновляем остальны данные
            ...
            if(this->insertNewRecordFile(ServerXmlMap[i]["path"], ServerXmlMap[i]["name"], ServerXmlMap[i]["size"], ServerXmlMap[i]["hash"])){
    ...
    if(updateFile){
        ...
        updateGlobalHash();
        emit emit_Loging(0,this->senderName,QString("Нет новых файлов на сервере (обновляем hash и закрываем UPDATER)"));
...
void DownloadManager::downloadOneByOneFile()
{
        ...
        if(DataQuery.next()){
            ...
            emit this->emit_Loging(0,this->senderName,QString("Начинаем качать файл %1 объемом %2 Byte в директорию %3").arg(nowDownloadFileName).arg(nowDownloadFileSize).arg(this->tmpDirectory));
...
//    this->copyFile->terminate();
    ...
    if(infoNamePath.exists()){
        ...
        /// Удаление папки tmp
        QDir dir;
        if(dir.rename("tmp",QString("tmp_%1").arg(QDateTime::currentDateTime().toString("yyyyMMddHHmmss"))))
...
//    this->dirCont.remove("tmp");
    ...
    emit this->emit_Loging(0,this->senderName,QString("База не содержит новых файлов (обновляем hash и закрываем UPDATER)"));
...
//            qDebug() << "--- FILES PATH --- " << fileFrom;
            ...
            if(infoIn.fileName() != "EKiosk.exe" && infoIn.fileName() != "h-sheller.exe"){
                ...
                if(fileCopy.copy(copyXmlMap[count_i_files]["pathFrom"], copyXmlMap[count_i_files]["pathTo"])){
                    ...
                    emit this->emit_Loging(0,this->senderName,QString("Файл %1 успешно перемещен в директорию assets...").arg(infoIn.fileName()));
                }else{
                    qDebug() << "--- COPY FILE ERROR --- " << infoIn.fileName();
                    emit this->emit_Loging(1,this->senderName,QString("Файл %1 не удалось переместить в директорию assets...").arg(infoIn.fileName()));
                }
    ...
    {
           if(Debuger) qDebug() << "Error recucrciyo " << QString(e.what());
           return ;
    }
}
...
bool DownloadManager::updateRecordFile(int id, QString fieldName, QString value)
{
    ...
    QString strUpdate = QString("UPDATE local_files SET %1 = \"%2\" WHERE id = %3").arg(fieldName, value, QString("%1").arg(id));
...
bool DownloadManager::updateRecordForMore(int id,QString hash,int size)
{
    ...
    QString strUpdate = QString("UPDATE local_files SET hash = \"%1\", size = %2 WHERE id = %3").arg(hash).arg(size).arg(id);
...
bool DownloadManager::insertNewRecordFile(QString path, QString name, QString size, QString hash)
{
    ...
    QString strUpdate = QString("INSERT INTO local_files (id,path,name,size,hash,status)"
                                "VALUES (NULL,\"%1\",\"%2\",%3,\"%4\",1);").arg(path, name, size, hash);
    ...
    while(!domNode.isNull())
    {
        if(domNode.isElement()){
            ...
            if(strTag == "file"){
                ...
                ServerXmlMap[countFileTag]["path"] = domElement.attribute("path","").right(domElement.attribute("path","").length() - 1);
