...
Response *Payment::sendRequest(const QUrl &aUrl, Request &aRequest) {
    ...
    toLog(LogLevel::Normal,
          QString("Payment %1. Response from url %2 received.").arg(getID()).arg(aUrl.toString()) +
              QString(" Fields: %1.").arg(response->toLogString()));

    if (!response->isOk()) {
        toLog(LogLevel::Warning,
              QString("Payment %1. Request was sent, but server response has non "
    ...
    setParameter(
        SParameter(PPSDK::CPayment::Parameters::ServerResult, response->getResult(), true));
    setParameter(
        SParameter(PPSDK::CPayment::Parameters::ErrorMessage, response->getErrorMessage(), true));
...
void Payment::setProcessError() {
    ...
    if (tryCount > CPayment::TriesLimit) {
        ...
        toLog(LogLevel::Normal,
              QString("Payment %1. Next try will be at %2.")
                  .arg(getID())
...
void Payment::perform_Transaction() {
    ...
    if (isCriticalError(serverError)) {
        toLog(LogLevel::Normal,
              QString("Payment %1. Server error %2 is critical. Payment marked as bad.")
                  .arg(getID())
                  .arg(serverError));
    ...
    if (serverError == EServerError::SessionAlreadyExist && status()) {
        ...
        if (serverResult >= EServerResult::Error3 && serverError == EServerError::Ok) {
            toLog(LogLevel::Normal,
                  QString("Payment %1. Already complete. STATUS=%2 ERROR=%3.")
                      .arg(getID())
                      .arg(serverResult)
                      .arg(serverError));
            ...
            if (!getPaymentFactory()->savePayment(this)) {
                toLog(LogLevel::Error,
                      QString("Payment %1. Failed to save data before pay request.").arg(getID()));

                setParameter(SParameter(
                    PPSDK::CPayment::Parameters::ServerError, ELocalError::DatabaseError, true));
            }
        }
    }
}
...
void Payment::perform_TransactionPay() {
    ...
    if (!getPaymentFactory()->savePayment(this)) {
        ...
        setParameter(
            SParameter(PPSDK::CPayment::Parameters::ServerError, ELocalError::DatabaseError, true));
...
void Payment::updateNumberOfTries() {
    ...
    case 2:
        ...
        toLog(LogLevel::Normal,
              QString("Payment %1. Number rejected, next try date: %2.")
                  .arg(getID())
                  .arg(getNextTryDate().toString(CPayment::DateLogFormat)));
    ...
    default:
        ...
        toLog(LogLevel::Normal,
              QString("Payment %1. Number rejected 3 times. Payment marked as bad.").arg(getID()));
    }
}
...
void Payment::process() {
    ...
    if (getCreationDate().addDays(60) < QDateTime::currentDateTime()) {
        toLog(LogLevel::Normal,
              QString("Payment %1. Old payment. Cannot be processed.").arg(getID()));
    }
    ...
    while (!processed) {
        ...
        case PPSDK::EPaymentStatus::ProcessError: {
            ...
            if (step == CPayment::Steps::Init) {
                if (getSession().isEmpty()) {
                    setParameter(SParameter(
                        PPSDK::CPayment::Parameters::Session, getInitialSession(), true));
                } else {
                    setParameter(SParameter(
                ...
                if (!status()) {
                    ...
                    int serverResult =
                        getParameter(PPSDK::CPayment::Parameters::ServerResult).value.toInt();
                    int serverError =
                        getParameter(PPSDK::CPayment::Parameters::ServerError).value.toInt();
                    ...
                    case EServerResult::Empty: {
                        if (serverError == EServerError::SessionNotExist) {
                            processed = false;
                            setParameter(SParameter(
                                PPSDK::CPayment::Parameters::Step, CPayment::Steps::Init, true));
                        } else {
                            setProcessError();
                        }
                    ...
                    case EServerResult::Error6: {
                        ...
                        toLog(LogLevel::Normal,
                              QString("Payment %1. Waiting for the payment to complete.")
                                  .arg(getID()));
                    ...
                    case EServerResult::StatusCheckOk: {
                        if (serverError == EServerError::Ok) {
                            ...
                            toLog(LogLevel::Normal,
                                  QString("Payment %1. Already complete.").arg(getID()));
                        } else {
                            processed = false;
                            setParameter(SParameter(
                                PPSDK::CPayment::Parameters::Step, CPayment::Steps::Init, true));
                    ...
                    default: {
                        toLog(LogLevel::Error,
                              QString("Payment %1. Unknown server result: %2.")
                                  .arg(getID())
                                  .arg(serverResult));
        ...
        case PPSDK::EPaymentStatus::Cheated: {
            toLog(LogLevel::Error,
                  QString("Payment %1. Cannot handle cheated payment.").arg(getID()));
        ...
        default: {
            toLog(LogLevel::Error,
                  QString("Payment %1. Cannot handle payment status %2.")
                      .arg(getID())
                      .arg(getStatus()));
...
bool Payment::remove() {
    ...
    case EServerResult::Error1:
        ...
        toLog(LogLevel::Normal,
              QString("Payment %1. Deleted. (server result:%2)").arg(getID()).arg(serverResult));
        return true;
    ...
    default:
        toLog(LogLevel::Warning,
              QString("Payment %1. Failed to delete. (server result:%2, server error:%3)")
                  .arg(getID())
                  .arg(serverResult)
                  .arg(serverError));
        return false;
