                    ...
                    else
                    {
                        ...
                        switch (serverResult)
                        {
                            ...
                            case EServerResult::Error6:
                            {
                                ...
                                toLog(LogLevel::Normal,
                                      QString("Payment %1. Waiting for the payment to complete.").arg(getID()));
...
bool Payment::remove()
{
    ...
    switch (serverResult)
    {
        ...
        default:
            toLog(LogLevel::Warning, QString("Payment %1. Failed to delete. (server result:%2, server error:%3)")
                                         .arg(getID())
                                         .arg(serverResult)
                                         .arg(serverError));
            return false;
