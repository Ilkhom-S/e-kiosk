...
void PaymentRequest::addProviderParameters(const QString &aStep) {
    ...
    foreach (auto parameter, m_Payment->getParameters()) {
        if (parameter.external) {
            addParameter(parameter.name, parameter.value);
        }
    }

    auto provider = m_Payment->getProviderSettings();
    ...
    if (provider.processor.requests.contains(aStep.toUpper())) {
        foreach (const SProvider::SProcessingTraits::SRequest::SField &field,
            ...
            while (match.capturedStart() != -1) {
                auto pp = m_Payment->getParameter(match.captured(1));
            ...
            if (field.crypted) {
                ...
                default: {
                    ICryptEngine *cryptEngine = m_Payment->getPaymentFactory()->getCryptEngine();
                ...
                PPSDK::SecurityFilter filter(provider,
                                             PPSDK::SProviderField::SecuritySubsystem::Log);
