...
{
	QString qmlPath = QDir::toNativeSeparators(
		QDir::cleanPath(aInfo.directory + QDir::separator() + aInfo.parameters[CQMLGraphicsItem::ItemKey]));
...
{
	if (mItem->metaObject()->indexOfMethod(QMetaObject::normalizedSignature(CQMLGraphicsItem::ResetHandlerSignature)) !=
		-1)
	{
		QVariant error;
