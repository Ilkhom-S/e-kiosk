            ...
            if (processPath.toLower().contains("ucs_")) {
                ...
                if (hProcess != 0) {
                    if (!TerminateProcess(hProcess, 0)) {
                        toLog(LogLevel::Error,
                              QString("Error TerminateProcess(%1, 0) (%2): #%3.")
                                  .arg(processes[i])
                                  .arg(processPath)
                    ...
                    toLog(LogLevel::Error,
                          QString("Error OpenProcess(PROCESS_ALL_ACCESS, 0, %1) (%2): #%3.")
                              .arg(processes[i])
                              .arg(processPath)
                              .arg(GetLastError(), 0, 16));
                }
...
bool API::isErrorResponse(BaseResponsePtr aResponse) {
    ...
    if (errorResponse) {
        QString e = QTextCodec::codecForName("windows-1251")
                        ->toUnicode(errorResponse->getErrorMessage().toLatin1());
...
bool API::isLoginResponse(BaseResponsePtr aResponse) {
    ...
    if (loginResponse) {
        toLog(LogLevel::Normal,
              QString("< Login. TID: %1. Encashment state: %2")
