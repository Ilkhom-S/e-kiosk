...
namespace Ucs
{
		  ...
		  mNeedPrintAllEncashmentReports(false), mDatabase(mCore->getDatabaseService(), ILog::getInstance(Ucs::LogName))
	{
		...
		connect(mEncashmentTask, SIGNAL(finished(const QString&, bool)), this,
	...
	void API::onResponseFinished()
	{
		...
		responseHandlers << std::mem_fun_ref(&API::isErrorResponse) << std::mem_fun_ref(&API::isConsoleResponse)
						 << std::mem_fun_ref(&API::isLoginResponse) << std::mem_fun_ref(&API::isEnchashmentResponse)
						 << std::mem_fun_ref(&API::isInitialResponse) << std::mem_fun_ref(&API::isPINRequiredResponse)
	...
	void API::sale(double aAmount)
	{
		...
		mResponseWatcher.setFuture(QtConcurrent::run(
			this, &API::send,
			aAmount ? makeRequest(Ucs::Class::AuthRequest, Ucs::Sale::CodeRequest, buffer) : QByteArray(),
			mLastLineReceived));
	}
	...
	void API::breakSale()
	{
		...
		mResponseWatcher.setFuture(
			QtConcurrent::run(this, &API::send, makeRequest(Ucs::Class::Session, Ucs::Break::CodeRequest), true));
	}
		...
		if (printLineResponse)
		{
			...
			else
			{
				if (printLineResponse->isLast())
				{
					...
					if (mAuthResponse)
					{
						emit saleComplete(mAuthResponse->mTransactionSumm / 100.0, mAuthResponse->mCurrency,
	...
	API::TResponse API::send(const QByteArray& aRequest, bool aWaitOperationComplete /*= true*/)
	{
		...
		int result =
			mEftpDo(mPySelf, aRequest.isEmpty() ? 0 : (char*)aRequest.data(), responseBuffer.data(), NULL, NULL);
