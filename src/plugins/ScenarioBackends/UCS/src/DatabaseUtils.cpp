...
bool DatabaseUtils::save(const Encashment &aEncashment) {
    ...
    if (!query) {
        toLog(LogLevel::Error,
              QString("Failed to save aEncashment %1.").arg(aEncashment.date.toString()));
...
bool DatabaseUtils::markAsPrinted(const Encashment &aEncashment) {
    ...
    if (!query) {
        toLog(LogLevel::Error,
              QString("Failed to update aEncashment %1.").arg(aEncashment.date.toString()));
...
bool DatabaseUtils::initTables() {
    ...
    sqls
        << "CREATE TABLE IF NOT EXISTS [ucs_encashments] ([create_date] DATETIME PRIMARY KEY, "
           "[receipt] TEXT NOT "
