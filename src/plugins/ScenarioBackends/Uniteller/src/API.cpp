...
namespace Uniteller {
    ...
    void API::sell(double aAmount, const QString &aOrderID, const QString &aCategory) {
        ...
        toLog(
            LogLevel::Normal,
            QString("> Sell: amount=%1, order=%2, category=%3.").arg(aAmount, 0, 'f', 2).arg(aOrderID).arg(aCategory));
    ...
    void API::onReadyRead() {
        ...
        responseHandlers << std::mem_fun_ref(&API::isErrorResponse) << std::mem_fun_ref(&API::isLoginResponse)
                         << std::mem_fun_ref(&API::isPrintLineResponse) << std::mem_fun_ref(&API::isGetStateResponse)
                         << std::mem_fun_ref(&API::isInitialResponse) << std::mem_fun_ref(&API::isDeviceEventResponse)
