...
bool UnitellerChargeProvider::enable(PPSDK::TPaymentAmount aMaxAmount)
{
	...
	QString initialSession =
		mCore->getPaymentService()
			->getPaymentField(paymentId, SDK::PaymentProcessor::CPayment::Parameters::InitialSession)
			.value.toString();
											 ...
											 const QString& aConfirmationCode)
{
	...
	mCore->getPaymentService()->updatePaymentField(mCore->getPaymentService()->getActivePayment(),
