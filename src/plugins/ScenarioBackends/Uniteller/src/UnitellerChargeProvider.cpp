...
//------------------------------------------------------------------------------
    ...
    void UnitellerChargeProvider::setConfiguration(const QVariantMap &aParameters) {
        ...
        m_Api->setupRuntimePath(
            aParameters.value(CUnitellerChargeProvider::RuntimePath).toString());
        m_Api->enable();
    }
    ...
    //------------------------------------------------------------------------------
    bool
    UnitellerChargeProvider::subscribe(const char *aSignal, QObject *aReceiver, const char *aSlot) {
    ...
    bool UnitellerChargeProvider::enable(PPSDK::TPaymentAmount aMaxAmount) {
        ...
        // Откусываем от номера сессии первые 2 символа, т.к. EFTPOS 3.0 такой большой номер заказа
        // не осилит.
