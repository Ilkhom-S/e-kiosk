...
Response *AdPayment::sendRequest(const QUrl &aUrl, Request &aRequest) {
    ...
    toLog(
        LogLevel::Normal,
        QString("AdPayment %1. Response from url %2 received.").arg(getID()).arg(aUrl.toString()) +
            QString(" Fields: %1.").arg(response->toLogString()));

    if (!response->isOk()) {
        toLog(LogLevel::Warning,
              QString("AdPayment %1. Request was sent, but server response has non "
...
bool AdPayment::pay() {
    ...
    if (!request) {
        toLog(LogLevel::Error,
              QString("AdPayment %1. Failed to create request for operation.").arg(getID()));
...
void AdPayment::setProcessError() {
    ...
    if (tryCount > CPayment::TriesLimit) {
        ...
        if (serverError != ELocalError::NetworkError) {
            toLog(LogLevel::Normal,
                  QString("AdPayment %1. Next try will be at %2.")
                      .arg(getID())
                      .arg(getNextTryDate().toString(CPayment::DateLogFormat)));
        } else {
            toLog(LogLevel::Normal,
                  QString("AdPayment %1. Waiting for connection is established.").arg(getID()));
        }
...
void AdPayment::perform_Transaction() {
    ...
    if (!getPaymentFactory()->savePayment(this)) {
        toLog(LogLevel::Error,
              QString("AdPayment %1. Failed to save data before pay request.").arg(getID()));
...
void AdPayment::process() {
    ...
    if (getCreationDate().addDays(60) < QDateTime::currentDateTime()) {
        toLog(LogLevel::Normal,
              QString("AdPayment %1. Old payment. Cannot be processed.").arg(getID()));
    }
    ...
    while (!processed) {
        ...
        case PPSDK::EPaymentStatus::Cheated: {
            toLog(LogLevel::Error,
                  QString("AdPayment %1. Cannot handle cheated payment.").arg(getID()));
        ...
        default: {
            toLog(LogLevel::Error,
                  QString("AdPayment %1. Cannot handle payment status %2.")
                      .arg(getID())
                      .arg(getStatus()));
