...
protected:
    ...
    template <> void addConfiguration<PrimSeriesType>(QVariantMap &aParameters) {
        ...
        aParameters.insert(
            Payment, QCoreApplication::translate("ChequeParameters", "#cheque_payment")); // Платеж.
        aParameters.insert(
            Depositing,
            QCoreApplication::translate("ChequeParameters", "#cheque_depositing")); // Внесено.
        aParameters.insert(
            SerialNumber,
            QCoreApplication::translate("ChequeParameters",
                                        "#cheque_fr_serial_number")); // Серийный номер.
        aParameters.insert(
            DocumentNumber,
            QCoreApplication::translate("ChequeParameters", "#cheque_number")); // Номер документа.
        aParameters.insert(INN,
                           QCoreApplication::translate("ChequeParameters", "#cheque_inn")); // ИНН.
        aParameters.insert(
            Date, QCoreApplication::translate("ChequeParameters", "#cheque_date")); // Дата.
        aParameters.insert(
            Time, QCoreApplication::translate("ChequeParameters", "#cheque_time")); // Время.
        aParameters.insert(
            Amount, QCoreApplication::translate("ChequeParameters", "#cheque_sum")); // Сумма.
        aParameters.insert(
            Session, QCoreApplication::translate("ChequeParameters", "#cheque_session")); // Смена.
        aParameters.insert(
            Cashier, QCoreApplication::translate("ChequeParameters", "#cheque_cashier")); // Кассир.
        aParameters.insert(
            ReceiptNumber,
            QCoreApplication::translate("ChequeParameters", "#cheque_receipt_number")); // Чек.
        aParameters.insert(
            Total,
            QCoreApplication::translate("ChequeParameters", "#cheque_total")); // Итоговая сумма.
    }
};
