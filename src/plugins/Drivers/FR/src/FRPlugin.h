...
template <class T> class FRPluginBase : public DevicePluginBase<T> {
  ...
  protected:
    ...
    template <> void addConfiguration<PrimSeriesType>(QVariantMap &aParameters) {
        ...
        aParameters.insert(Depositing,
                           QCoreApplication::translate("ChequeParameters", "#cheque_depositing")); // Внесено.
        aParameters.insert(SerialNumber, QCoreApplication::translate("ChequeParameters",
                                                                     "#cheque_fr_serial_number")); // Серийный номер.
        aParameters.insert(DocumentNumber,
        ...
        aParameters.insert(ReceiptNumber,
