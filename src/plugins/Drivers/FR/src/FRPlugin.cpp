...
template <class T> TParameterList ShtrihParameters(const QStringList &aModels) {
    ...
    return defaultParameters<T>(aModels, CComponents::FiscalRegistrator)
           << setProtocol(ProtocolNames::FR::Shtrih) << setModifiedValues(CHardwareSDK::ModelName, modelNames)
           << setWeightSensorsEnabled() << setNullingSumInCash();
}
...
#define SINGLE_ATOL_PLUGIN(aClassName, aParameters, aName, aProtocol)                                                  \
    COMMON_DRIVER(aClassName,                                                                                          \
                  std::bind(&aParameters<aClassName>, QStringList() << #aName, ProtocolNames::FR::aProtocol))
#define COMMON_ATOL_PLUGIN(aClassName, aDeviceType, aParameters, aProtocol)                                            \
    COMMON_DRIVER(aClassName, std::bind(&aParameters<aClassName>, aClassName::getModelList(),                          \
                                        CComponents::aDeviceType, ProtocolNames::FR::aProtocol))
