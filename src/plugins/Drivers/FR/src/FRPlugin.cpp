...
template <class T> TParameterList EjectorPrimParameters(const QStringList &aModels) {
    return PrimParametersBase<T>(aModels)
           ...
           << setLeftReceiptAction(
                  PrinterSettings::PreviousReceipt, true, true, PrinterValues::Retract, false)
           << setLeftReceiptAction(
                  PrinterSettings::NotTakenReceipt, true, true, PrinterValues::Retract, false)
           << setLeftReceiptTimeout() << setPrinterModel("Custom VKP-80");
}
...
template <class T> TParameterList ShtrihParameters(const QStringList &aModels) {
    ...
    modelNames.insert("Shtrih-M Shtrih Kiosk-FR-K based on Nippon NP-325",
                      "Shtrih-M Kiosk-FR-K (Nippon)");
...
template <class T>
TParameterList
AtolLSParameters(const QStringList &aModels, const QString &aDeviceType, const QString &aProtocol) {
    return AtolParameters<T>(aModels, aDeviceType, aProtocol) << setLineSpacing(1, 9, 3, 3);
}

//------------------------------------------------------------------------------
template <class T>
TParameterList AtolOnlineParameters(const QStringList &aModels,
                                    const QString &aDeviceType,
...
template <class T> TParameterList PayVKP80Parameters(const QStringList &aModels) {
    return PayParameters<T>(aModels) << setLoopEnabled(PPT::ForFiscalDocuments, false)
                                     << setLeftReceiptAction(PrinterSettings::PreviousReceipt,
                                                             true,
                                                             true,
                                                             PrinterValues::Retract,
                                                             false,
                                                             PPT::ForFiscalDocuments)
...
template <class T> TParameterList PayPPU700Parameters(const QStringList &aModels) {
    return PayParameters<T>(aModels)
           << SPluginParameter(CHardware::FR::EjectorParameters,
                               false,
                               PPT::EjectorParameters,
                               QString(),
                               FRValues::LoopAndPushNotTakenOnTimeout,
                               QStringList() << FRValues::LoopAndPushNotTakenOnTimeout
                                             << FRValues::NoLoopAndPushNotTakenOnTimeout
                                             << FRValues::NoLoopAndRetractNotTakenOnTimeout,
                               false)
           << setLeftReceiptTimeout();
}
...
TParameterList PaymasterParameters(const QStringList &aModels, const QString &aProtocol) {
    return AtolLSParameters<T>(aModels, CComponents::FiscalRegistrator, aProtocol)
           ...
           << setLeftReceiptAction(PrinterSettings::PreviousReceipt,
                                   ...
                                   false,
                                   PPT::ForFiscalDocuments)
           << setPresentationLength("", 2, 14) << setNotPrinting()
           << setPrinterModel(CAtolOnlinePrinters::CModels().getNames(),
...
template <class T> TParameterList SparkParameters(const QStringList &aModels) {
    return defaultParameters<T>(aModels, CComponents::FiscalRegistrator)
           ...
           << setLeftReceiptAction(
                  PrinterSettings::PreviousReceipt, true, false, PrinterValues::Retract)
           << setLeftReceiptAction(
                  PrinterSettings::NotTakenReceipt, true, false, PrinterValues::Retract)
           << setLeftReceiptTimeout() << setFiscalChequeCreation();
}
...
//------------------------------------------------------------------------------
#define SINGLE_FR_PLUGIN(aClassName, aParameters, aName)                                           \
    COMMON_DRIVER(aClassName, std::bind(&aParameters<aClassName>, QStringList() << #aName))
#define COMMON_FR_PLUGIN(aClassName, aParameters)                                                  \
    COMMON_DRIVER(aClassName, std::bind(&aParameters<aClassName>, aClassName::getModelList()))

#define SINGLE_ATOL_PLUGIN(aClassName, aParameters, aName, aProtocol)                              \
    COMMON_DRIVER(aClassName,                                                                      \
                  std::bind(&aParameters<aClassName>,                                              \
                            QStringList() << #aName,                                               \
                            ProtocolNames::FR::aProtocol))
#define COMMON_ATOL_PLUGIN(aClassName, aDeviceType, aParameters, aProtocol)                        \
    COMMON_DRIVER(aClassName,                                                                      \
                  std::bind(&aParameters<aClassName>,                                              \
                            aClassName::getModelList(),                                            \
                            CComponents::aDeviceType,                                              \
                            ProtocolNames::FR::aProtocol))
#define COMMON_ATOL2_PLUGIN(aClassName, aDeviceType, aParameters)                                  \
    COMMON_ATOL_PLUGIN(aClassName, aDeviceType, aParameters, ATOL2)
#define COMMON_ATOL3_PLUGIN(aClassName, aDeviceType, aParameters)                                  \
    COMMON_ATOL_PLUGIN(aClassName, aDeviceType, aParameters, ATOL3)
