...
TParameterList CAParameters() {
    return modifyPriority(createNamedList<VirtualCashAcceptor>("Virtual cash acceptor"),
           ...
           << SPluginParameter(CHardware::VirtualCashAcceptor::NotesPerEscrow,
                               true,
                               VDPPT::NotesPerEscrow,
                               QString(),
                               "1",
                               QStringList() << "1" << "2" << "10" << "100" << "250" << "500"
                                             << "1000" << "4000",
                               false);
}
...
TParameterList dispenserParameters() {
    ...
    return modifyPriority(createNamedList<VirtualDispenser>("Virtual dispenser"),
                          EDetectingPriority::Fallback)
           // Количество кассет.
           << SPluginParameter(
                  CHardware::Dispenser::Units, true, VDPPT::Units, QString(), 2, getList(1, 4))

           // Номер предмета, когда кассета почти пуста.
           << SPluginParameter(CHardware::Dispenser::NearEndCount,
                               true,
                               VDPPT::NearEndCount,
                               QString(),
                               10,
           ...
           << SPluginParameter(CHardware::Dispenser::JammedItem,
                               true,
                               VDPPT::JammedItem,
                               QString(),
                               5,
