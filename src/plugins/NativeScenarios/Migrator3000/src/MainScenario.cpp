            ...
            BOOST_FOREACH (ptree::value_type const &v, pt.get_child("terminal")) {
                if (v.first == "connection") {
                    ...
                    if (proxy.type() != QNetworkProxy::NoProxy) {
                        ...
                        proxy.setPassword(
                            QString::fromStdString(v.second.get<std::string>("proxy.<xmlattr>.password", "")));
                        proxy.setHostName(
                            QString::fromStdString(v.second.get<std::string>("proxy.<xmlattr>.host", "")));
                        proxy.setPort(
                            QString::fromStdString(v.second.get<std::string>("proxy.<xmlattr>.port", "0")).toUShort());
                    }
