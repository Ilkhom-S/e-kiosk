...
void WizardFrame::showPage(const QString &aContext, WizardPageBase *aPage)
{
    ...
    if (mCurrentPage)
    {
        mCurrentPage->activate();
        wPage->layout()->addWidget(mCurrentPage);
        mCurrentPage->setProperty(CWizardFrame::ContextProperty, aContext);
        mCurrentPage->show();
    }
}
...
//----------------------------------------------------------------------------
void WizardFrame::setupControl(Control aControl, bool aEnabled, const QString &aTitle, const QString &aContext,
                               bool aCanCache)
{
    ...
    if (button)
    {
        ...
        if (aCanCache)
        {
            mSignalMapper.setMapping(button, aContext);
        }
    }
}
...
void WizardFrame::onChangePage(const QString &aContext)
{
    ...
    else if (aContext == CWizardContext::SetupHardware)
    {
        ...
        setupControl(WizardFrame::BackButton, true, tr("#to_start_page"), CWizardContext::StartPage);
        setupControl(WizardFrame::ForwardButton, true, tr("#to_network_setup"), CWizardContext::SetupNetwork);
    ...
    else if (aContext == CWizardContext::SetupNetwork)
    {
        ...
        setupControl(WizardFrame::BackButton, true, tr("#to_hardware_setup"), CWizardContext::SetupHardware);
