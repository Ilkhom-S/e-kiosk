...
public:
    LogsServiceWindow(HumoServiceBackend *aBackend, QWidget *aParent = 0);
...
private:
    template <class T> void scrollPgUp(T *aListWidget) {
        ...
        if (firstVisibleIndex.isValid()) {
            ...
            QCoreApplication::postEvent(
                aListWidget, new QKeyEvent(QEvent::KeyPress, Qt::Key_PageUp, Qt::NoModifier));
        }
    }

    template <class T> void scrollPgDown(T *aListWidget) {
        ...
        if (lastVisibleIndex.isValid()) {
            ...
            QCoreApplication::postEvent(
                aListWidget, new QKeyEvent(QEvent::KeyPress, Qt::Key_PageDown, Qt::NoModifier));
        }
    }
