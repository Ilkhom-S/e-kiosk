	...
	: EncashmentWindow(aBackend, aParent), mBackend(aBackend)
{
	...
	PPSDK::TerminalSettings* s = static_cast<PPSDK::TerminalSettings*>(
		mBackend->getCore()->getSettingsService()->getAdapter(SDK::PaymentProcessor::CAdapterNames::TerminalAdapter));

	mPayloadSettingsPath = QString("%1/%2")
...
void EncashmentServiceWindow::updateUI()
{
	...
	ui.btnPrintZReport->setEnabled(isFiscalPrinter &&
								   mBackend->getPaymentManager()->canPrint(PPSDK::CReceiptType::ZReport));
...
//------------------------------------------------------------------------
void EncashmentServiceWindow::onDeviceStatusChanged(const QString& aConfigName, const QString& aStatusString,
