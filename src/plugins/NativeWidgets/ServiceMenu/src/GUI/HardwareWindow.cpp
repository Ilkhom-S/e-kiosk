	...
	: QWidget(aParent), mBackend(aBackend), mCreationMode(Default)
{
	...
	connect(mBackend->getHardwareManager(), SIGNAL(deviceDetected(const QString&)), this,
			SLOT(onDeviceFound(const QString&)));

	connect(mBackend->getHardwareManager(), SIGNAL(detectionStopped()), this, SLOT(onDetectionFinished()));
...
void HardwareWindow::checkDeviceSlotConcurrent(DeviceSlot* aSlot)
{
	...
	if (it != mDeviceList.get<ModelTag>().end())
	{
		...
		QMetaObject::invokeMethod(mBackend->getHardwareManager(), "createDevice", Qt::BlockingQueuedConnection,
								  Q_RETURN_ARG(QString, result), Q_ARG(const QString&, (*it).driver),
