	...
	: QFrame(aParent), ServiceWindowBase(aBackend), mSpacerItem(0)
{
	...
	connect(
		...
		SIGNAL(deviceStatusChanged(const QString&, const QString&, const QString&, SDK::Driver::EWarningLevel::Enum)),
		this,
		SLOT(onDeviceStatusChanged(const QString&, const QString&, const QString&, SDK::Driver::EWarningLevel::Enum)));
...
//------------------------------------------------------------------------
void DiagnosticsServiceWindow::onDeviceStatusChanged(const QString& aConfigurationName, const QString& aNewStatus,
...
void DiagnosticsServiceWindow::updateInfoPanel()
{
	...
	lbRejectedBills->setText(
		QString::number(result[SDK::PaymentProcessor::CServiceParameters::Funds::RejectCount].toInt()));
	lbUnprocessedPayments->setText(
		QString::number(result[SDK::PaymentProcessor::CServiceParameters::Payment::UnprocessedPaymentCount].toInt()));
	lbPrintedReceipts->setText(
		QString::number(result[SDK::PaymentProcessor::CServiceParameters::Printing::ReceiptCount].toInt()));
	lbRestartPerDay->setText(
		QString::number(result[SDK::PaymentProcessor::CServiceParameters::Terminal::RestartCount].toInt()));
	lbPaymentsPerDay->setText(
		QString::number(result[SDK::PaymentProcessor::CServiceParameters::Payment::PaymentsPerDay].toInt()));
