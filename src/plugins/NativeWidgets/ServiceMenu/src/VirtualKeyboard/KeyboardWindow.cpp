...
void KeyboardWindow::onButtonClicked() {
    ...
    VirtualButton button =
        mAltMode ? mAltKeyMap[sender()->objectName()] : mKeyMap[sender()->objectName()];

    QKeyEvent keyPress(QEvent::KeyPress,
                       button.getKey(mShifted),
                       mShifted ? Qt::ShiftModifier : Qt::NoModifier,
...
void KeyboardWindow::updateKeys() {
    ...
    foreach (QToolButton *button, padButtons) {
        if (button->objectName() == "KEY_LANG") {
            ...
            button->setText(
                (mAltMode ? mAltKeyMap[button->objectName()] : mKeyMap[button->objectName()])
                    .getText(mShifted));
        }
    }
}
