...
bool CitizenPPU700_PRINTER::getStatus(int& aStatus, CMDCitizenPPU700::SStatus& s_status)
{
    ...
    }else if (s_status.Failures.Unrecoverable || s_status.Failures.CRC || s_status.Failures.DetectionPresenter || s_status.Error){
...
bool CitizenPPU700_PRINTER::getState(char aStatusType, CMDCitizenPPU700::SStatus & aStatus)
{
    ...
    if (this->sendCommand(commandPacket,true,200,respData,answerPacket,0))
    {
        ...
        {
            aStatus.Failures.DetectionPresenter = getBit(answer, CMDCitizenPPU700::Positions::Answer::Errors::DetectionPresenter);
        ...
        case CMDCitizenPPU700::Constants::Status::ErrorDetails1 :
        {
            ...
            aStatus.Failures.HeadOverheat = getBit(answer, CMDCitizenPPU700::Positions::Answer::ErrorDetails1::HeadOverheat);
            aStatus.Failures.LowVoltage   = getBit(answer, CMDCitizenPPU700::Positions::Answer::ErrorDetails1::LowVoltage);
            aStatus.Failures.HighVoltage  = getBit(answer, CMDCitizenPPU700::Positions::Answer::ErrorDetails1::HighVoltage);
            break;
        }
...
//    QByteArray packet;
    ...
    if (answer.lastIndexOf(CMDCitizenPPU700::PrinterPPU700.toLatin1()) >= 0)
    {
        if(answer.contains(CMDCitizenPPU700::Response_d::Resp_Mod_Name))
            //if(Debuger) qDebug() << "CitizenPPU700::isItYou(): response - " << CMDCitizenPPU700::Response_d::Resp_Mod_Name;
            result = true;
    }
