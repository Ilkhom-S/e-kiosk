...
{
    Printer_List << PrinterModel::Custom_VKP80 << PrinterModel::CitizenCBM1000 << PrinterModel::Citizen_CTS2000 << PrinterModel::Custom_TG2480 << PrinterModel::Citizen_PPU700 << PrinterModel::AV_268 << PrinterModel::Phoenix_model;
...
void ClassPrinter::setPrinterModel(const QString printerModel)
{
    ...
    if(systemModel == PrinterModel::Custom_VKP80)
         CustomVKP80 = new CustomVKP80_PRINTER();
    if(systemModel == PrinterModel::Custom_TG2480)
         CustomTG2480 = new TG2480_PRINTER();
...
void ClassPrinter::setChekWidth(const int width)
{
    chekWidth = width;
    if(systemModel == PrinterModel::Custom_VKP80)
        CustomVKP80->setChekWidth(width);
}
...
void ClassPrinter::setFirmPatern(const QString firm_name)
{
    firmName = firm_name;
    if(systemModel == PrinterModel::Custom_VKP80)
        CustomVKP80->setFirmPatern(firmName);
    if(systemModel == PrinterModel::Citizen_PPU700)
        CitizenPPU700->setFirmPatern(firmName);
    if(systemModel == PrinterModel::Citizen_CTS2000)
...
void ClassPrinter::setSmallBeetwenString(bool beet)
{
    ...
    if(systemModel == PrinterModel::Custom_VKP80)
        CustomVKP80->setSmallBeetwenString(beet);
    if(systemModel == PrinterModel::Citizen_PPU700)
        CitizenPPU700->setSmallBeetwenString(beet);
    if(systemModel == PrinterModel::Citizen_CTS2000)
         CitizenCTS2000->setSmallBeetwenString(beet);
}

void ClassPrinter::setSmallText(bool small_i)
{
    if(systemModel == PrinterModel::Custom_VKP80)
          CustomVKP80->setChekSmall(small_i);
    if(systemModel == PrinterModel::Citizen_PPU700)
          CitizenPPU700->setChekSmall(small_i);
    if(systemModel == PrinterModel::Citizen_CTS2000)
          CitizenCTS2000->setChekSmall(small_i);
}
...
void ClassPrinter::setLeftMargin(int left_margin)
{
    ...
    if(systemModel == PrinterModel::Custom_TG2480)
          CustomTG2480->setLeftMargin(leftMargin);
    if(systemModel == PrinterModel::AV_268)
...
void ClassPrinter::CPrint()
{
    ...
    if(systemModel == PrinterModel::KM1X){
        textToPrint = textToPrint.replace("[p]","").replace("[b]","").replace("[/b]","").replace("[u]","").replace("[/u]","");
...
void ClassPrinter::winPrint(QString text)
{
    if(this->winPrinterName != ""){
        ...
        text.replace("\n","<br>");
        text.replace("[p]","");
        text.replace("[b]","<b>"); text.replace("[/b]","</b>");
        text.replace("[u]","<u>"); text.replace("[/u]","</u>");
        text.replace("[i]","<i>"); text.replace("[/i]","</i>");
...
bool ClassPrinter::printerOpen()
{
    ...
    if(systemModel == PrinterModel::Custom_VKP80){
        ...
        bool port_printer = false; port_printer = CustomVKP80->OpenPrinterPort();
        if(port_printer){
    ...
    if(systemModel == PrinterModel::Custom_TG2480)
    {
        ...
        bool port_printer = false; port_printer = CustomTG2480->OpenPrinterPort();
        if(port_printer){
            //qDebug() << QString("ClassPrinter -- port opened -- %1").arg(systemModel);
            open_port = true;
        }else{
    ...
    if(systemModel == PrinterModel::AV_268)
    {
        ...
        bool port_printer = false; port_printer = AV268->OpenPrinterPort();
        if(port_printer){
            //qDebug() << QString("ClassPrinter -- port opened -- %1").arg(systemModel);
            open_port = true;
        }else{
    ...
    if(systemModel == PrinterModel::CitizenCBM1000)
    {
        ...
        bool port_printer = false; port_printer = CitizenCBM1000->OpenPrinterPort();
        if(port_printer){
            //qDebug() << QString("ClassPrinter -- port opened -- %1").arg(systemModel);
            open_port = true;
        }else{
    ...
    if(systemModel == PrinterModel::Citizen_PPU700)
    {
        ...
        bool port_printer = false; port_printer = CitizenPPU700->OpenPrinterPort();
        if(port_printer){
            //qDebug() << QString("ClassPrinter -- port opened -- %1").arg(systemModel);
            open_port = true;
        }else{
    ...
    if(systemModel == PrinterModel::Citizen_CTS2000)
    {
        ...
        bool port_printer = false; port_printer = CitizenCTS2000->OpenPrinterPort();
        if(port_printer){
            //qDebug() << QString("ClassPrinter -- port opened -- %1").arg(systemModel);
            open_port = true;
        }else{
    ...
    if(systemModel == PrinterModel::Phoenix_model)
    {
        ...
        bool port_printer = false; port_printer = Phoenix->OpenPrinterPort();
        if(port_printer){
            //qDebug() << QString("ClassPrinter -- port opened -- %1").arg(systemModel);
            open_port = true;
        }else{
    ...
    if(systemModel == PrinterModel::KM1X)
    {
        ...
        bool port_printer = false; port_printer = KM1X->OpenPrinterPort();
        if(port_printer){
            //qDebug() << QString("ClassPrinter -- port opened -- %1").arg(systemModel);
            open_port = true;
        }else{
...
bool IsPrinterError(HANDLE hPrinter)
   {
        ...
        */
       if (dwPrinterStatus == PRINTER_STATUS_ERROR || dwPrinterStatus == PRINTER_STATUS_PAPER_JAM || dwPrinterStatus == PRINTER_STATUS_PAPER_OUT || dwPrinterStatus == PRINTER_STATUS_PAPER_PROBLEM || dwPrinterStatus == PRINTER_STATUS_OUTPUT_BIN_FULL || dwPrinterStatus == PRINTER_STATUS_NOT_AVAILABLE || dwPrinterStatus == PRINTER_STATUS_NO_TONER || dwPrinterStatus == PRINTER_STATUS_OUT_OF_MEMORY || dwPrinterStatus == PRINTER_STATUS_OFFLINE || dwPrinterStatus == PRINTER_STATUS_DOOR_OPEN)
       {
           free( pJobs );
           return true;
       }
       ...
       for (i=0; i < cJobs; i++)
       {
           if (pJobs[i].Status == JOB_STATUS_PRINTING)
           {
                ...
                */
               if (pJobs[i].Status == JOB_STATUS_ERROR || pJobs[i].Status == JOB_STATUS_OFFLINE || pJobs[i].Status == JOB_STATUS_PAPEROUT || pJobs[i].Status == JOB_STATUS_BLOCKED_DEVQ)
               {
                   free( pJobs );
                   return true;
               }
