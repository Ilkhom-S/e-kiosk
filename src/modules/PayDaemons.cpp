    ...
    while(!domNode.isNull())
    {
        if (domNode.isElement()) {
            ...
            if (strTag == "resultCode") {
                ...
                if (sts.toInt() > 0) {
                   emit emit_Loging(2, senderName, QString("Пришел ответ от сервера resultCode: %1").arg(sts));
                }
            }
            ...
            if (strTag == "payment") {
                ...
                if (!update_sts){
                    emit emit_Loging(1, senderName, QString("Не могу обновить платеж с транзакцией - %1.").arg(vrmTrn));
                }else{
                    emit emit_Loging(0, senderName, QString("Платеж с транзакцией - %1: обновлен на статус - %2 в %3 часов.").arg(vrmTrn, strSts, vrmDataConfirm));
...
void PayDaemons::get_new_pay(QVariantMap payment)
{
    ...
    QString extra = extraInfo.isEmpty() ? "" : QJsonDocument::fromVariant(extraInfo).toJson(QJsonDocument::Compact);

    if (GetOperationCount(trnId)) {
        emit emit_Loging(1, senderName,QString("Такая транзакция уже есть в базе operation_id - %1.").arg(trnId));
        return;
    }
    ...
    QString strCreateNewOperation = QString("INSERT INTO terminal_operation"
    " (operation_ratio_number, operation_ratio_persent, operation_account_design, operation_account"
    ", operation_collect_id, operation_date_create, operation_date_confirm, operation_id"
    ", operation_is_send, operation_money_amount, operation_money_cash, operation_money_denom"
    ", operation_services_id, operation_status, operation_print_status, operation_services_name, notify_route, extra_info)"
    " VALUES(%1,%2,'%3','%4','%5','%6','%6',%7,0,%8,%9,'%10',%11,54,0,'%12','%13','%14');")
    .arg(ratioSum).arg(ratioPercent).arg(accountMasked, account, collectionId, dateCreate, trnId)
    .arg(sumTo).arg(sumFrom).arg(denomination, prvId, prvName, notifyRoute, extra);
    ...
    if (!createNewOperation.exec(strCreateNewOperation)){
        ...
        emit emit_Loging(2, senderName, QString("Ошибка Базы Данных... Невозможно создать платёж (номер - %1, сумма - %2). Error: %3. Query: %4")
                                           .arg(account).arg(sumFrom).arg(createNewOperation.lastError().text(), strCreateNewOperation));
        emit emit_errorDB();
        return ;
    }

    emit emit_Loging(0, senderName, QString("Платеж с номером- %1, на провайдер- %2,"
                               " транзакцией- %3, суммой от- %4, суммой к- %5,"
                               " занесен в базу %6 числа.")
                                .arg(account, prvName, trnId).arg(sumFrom).arg(sumTo).arg(dateCreate));
}
...
void PayDaemons::get_update_pay(QVariantMap payment)
{
    ...
    QString strUpdateOperation = QString("UPDATE terminal_operation SET"
                                         ...
                                         .arg(ratioSum).arg(ratioPercent)
                                         .arg(dateCreate).arg(sumTo).arg(sumFrom).arg(denomination, trnId);

    if (!updateOperation.exec(strUpdateOperation)){
        //if(Debuger) qDebug() << "Error Update Operation for status LOCK in function get_update_pay";
        emit emit_Loging(2, senderName, QString("Ошибка Базы Данных... Невозможно обновить платёж. Error: %1. Query: %2")
                                           .arg(updateOperation.lastError().text(), strUpdateOperation));
        emit emit_errorDB();
        return ;
    }

    emit emit_Loging(0, senderName, QString("Платеж с"
                               " транзакцией- %1, суммой от- %2, суммой к- %3,"
                               " обновил параметры в базе %4 числа.")
                                .arg(trnId).arg(sumFrom)
                                .arg(sumTo).arg(dateCreate));
}
...
bool PayDaemons::GetOperationCount(const QString &id_trn)
{
    ...
    QString operationQuery = QString("SELECT count(*) AS count FROM terminal_operation WHERE operation_id = %1 LIMIT 1;").arg(id_trn);
...
{
    //if(Debuger) qDebug() <<  "update NewOperation-->  with status confirm_create";
    ...
    if(print){
        strUpdateOperation = QString("UPDATE terminal_operation SET"
                                                 ...
                                                 " WHERE operation_id = %1").arg(tranz_id);
    }else{
        strUpdateOperation = QString("UPDATE terminal_operation SET"
                                                 " operation_status = 55"
                                                 " WHERE operation_id = %1 AND operation_status = 54;").arg(tranz_id);
    }


    if (!updateOperation.exec(strUpdateOperation)) {
        //if(Debuger) qDebug() << "Error Update Operation for status LOCK in function get_confirm_pay)";
        return ;
    }

    if (print) {
        emit emit_Loging(0, senderName, QString("К платежу с транзакцией- %1 чек был распечатан %2 числа.").arg(tranz_id, vrmDateCreate));
    } else {
        emit emit_Loging(0, senderName, QString("Платеж с транзакцией- %1 полностью оплачен %2 числа.").arg(tranz_id, vrmDateCreate));
...
QString PayDaemons::getReceiptInfo(QString tranz_id)
{
    ...
    //Данные о платеже
    QString date_create; QString prv_name; QString account; QString sum_from; QString sum_to; QString ratio_sum; QString ratio_persent;

    if(!getPayData(tranz_id, date_create, prv_name, account, sum_from, sum_to, ratio_sum, ratio_persent)){
...
bool PayDaemons::getPayData(QString id_trn, QString &date_create, QString &prv_name, QString &account, QString &sum_from,QString &sum_to, QString &ratio_sum,QString &ratio_persent)
{
    QSqlQuery selectOperation(db);

    QString strOperation = QString("SELECT * FROM terminal_operation WHERE operation_id = '%1' LIMIT 1;").arg(id_trn);
    ...
    if(selectOperation.next()){
        date_create = selectOperation.value(recordOperation.indexOf("operation_date_create")).toString();
        prv_name = selectOperation.value(recordOperation.indexOf("operation_services_name")).toString();
        account = selectOperation.value(recordOperation.indexOf("operation_account_design")).toString();
        sum_from = selectOperation.value(recordOperation.indexOf("operation_money_cash")).toString();
        sum_to = selectOperation.value(recordOperation.indexOf("operation_money_amount")).toString();

        ratio_sum = selectOperation.value(recordOperation.indexOf("operation_ratio_number")).toString();
        ratio_persent = selectOperation.value(recordOperation.indexOf("operation_ratio_persent")).toString();
        ...
        if (count_pay > 0) {
            ...
            QString request =   QString(header_xml
                              + "<auth count=\"%1\" toAmount=\"%2\">\n"
                                    "%3"
                                "</auth>\n"
                              + footer).arg(count_pay).arg(allSum).arg(paymentXml);
...
bool PayDaemons::getPaymentMap(QString &payment, int &count_pay, double &all_sum)
{
    ...
    if (!selectOperationForSend.exec(strQueryOperation)){
        ...
        //if(Debuger) qDebug() << "Error Select Operation Data for Send in function bool PayDaemons::SendNewOperation()";
        return false;
    }
    ...
    while(selectOperationForSend.next()){
        ...
        QString vrmIdTrn = selectOperationForSend.value(recordOperationForSend.indexOf("operation_id")).toString();
        QString vrmFSum = selectOperationForSend.value(recordOperationForSend.indexOf("operation_money_cash")).toString();
        double vrmTSum = selectOperationForSend.value(recordOperationForSend.indexOf("operation_money_amount")).toDouble();
        QString billInfo = selectOperationForSend.value(recordOperationForSend.indexOf("operation_money_denom")).toString();
        QString vrmPrvID = selectOperationForSend.value(recordOperationForSend.indexOf("operation_services_id")).toString();
        QString vrmAccount = selectOperationForSend.value(recordOperationForSend.indexOf("operation_account")).toString();
        QString vrmDateCreate = selectOperationForSend.value(recordOperationForSend.indexOf("operation_date_create")).toString();
        QString stsPrint = selectOperationForSend.value(recordOperationForSend.indexOf("operation_print_status")).toString();
        QString collectionId = selectOperationForSend.value(recordOperationForSend.indexOf("operation_collect_id")).toString();
        QString vrmNotifyRoute = selectOperationForSend.value(recordOperationForSend.indexOf("notify_route")).toString();
        QString extraInfo = selectOperationForSend.value(recordOperationForSend.indexOf("extra_info")).toString();
        ...
        payment += QString("<payment trn=\"%1\" fsum=\"%2\" tsum=\"%3\" prv_id=\"%4\" account=\"%5\" receipt=\"%6\" time=\"%7\" receipt_sts=\"%8\" bill_info=\"%9\" collection_id=\"%10\" notify_route=\"%11\" %12/>\n")
                   .arg(vrmIdTrn, vrmFSum).arg(vrmTSum).arg(vrmPrvID, vrmAccount, vrmIdTrn, vrmDateCreate, stsPrint, billInfo, collectionId, vrmNotifyRoute, extra);

        count_pay ++;
        all_sum += vrmTSum;

        emit emit_Loging(0, senderName, QString("Платеж с номером- %1 суммой на счет- %2 и транзакцией- %3 в PAY_REQUEST_XML сформирован.").arg(vrmAccount).arg(vrmTSum).arg(vrmIdTrn));
    }
    return true;
}

bool PayDaemons::updateOperationStatus(const QString &id_trm, const QString &status, const QString &dateConfirm)
{    
    QSqlQuery updateOperation(db);
    QString strUpdateOperation;
    QString strSend = ""; if(dateConfirm != "0") strSend = ", operation_is_send = 1";
    strUpdateOperation = QString("UPDATE terminal_operation SET operation_status = %2, operation_date_confirm = '%3'%4 WHERE operation_id = %1").arg(id_trm, status, dateConfirm, strSend);

    if (!updateOperation.exec(strUpdateOperation)){
        //if(Debuger) qDebug() << updateOperation.lastError();
        //if(Debuger) qDebug() << "Error Update Operation for status UNKNOWN in function bool PayDaemons::UpdateOperationForUnknown(const QString &id_trm)";
        return false;
    }
    return true;
}
...
bool PayDaemons::getCountPayment(int &count)
{
    ...
    QString operationQuery = QString("SELECT count(*) AS count FROM terminal_operation WHERE operation_status = 55 LIMIT 1;");
...
void PayDaemons::checkPayStatus54(){
    ...
    QString operationQuery = QString("SELECT count(*) AS count FROM terminal_operation WHERE operation_status = 54 OR operation_status = 3;");
    //if(Debuger) qDebug() << operationQuery;
    ...
    if (selectOperationQuery.next()) {
        unconfrm_pay_count = selectOperationQuery.value(record.indexOf("count")).toInt();
    }
...
void PayDaemons::confirmPayments()
{
    ...
    // TODO remove after update operation_status = 3
    strUpdateOperation = QString("UPDATE terminal_operation SET operation_status = 55  WHERE operation_status = 54 OR operation_status = 3;");
...
QString PayDaemons::getCollectionId() {
    ...
    if (collectId == "") {
        ...
        strQuery = QString("INSERT INTO terminal_collect(collect_id, stack_id, date_create, denom, status)"
                           " VALUES('%1', %2 , \"%3\", '%4', 'new');").arg(colId, trnId, dateCreate, xmlDenom);
