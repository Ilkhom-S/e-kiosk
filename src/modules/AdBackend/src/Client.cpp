...
namespace Ad {
    ...
    Client::Client(SDK::PaymentProcessor::ICore *aCore, ILog *aLog, int aKeyPair)
        ...
        connect(mCore->getRemoteService(), SIGNAL(commandStatusChanged(int, int, QVariantMap)),
    ...
    Ad::Campaign Client::getAdInternal(const QString &aType) const {
        ...
        camp.expired =
            QDateTime::fromString(mSettings->value(Settings::Expired).toString(), Ad::Parameters::DateTimeFormat);
        ...
        if (aCampaign.isValid() && !aCampaign.isExpired() && !aCampaign.expired.isNull()) {
            ...
            if (mExpirationTime.isNull() || mExpirationTime > aCampaign.expired) {
                ...
                if (interval > 0) {
                    ...
                    toLog(
                        LogLevel::Normal,
                        QString("Start expiration timer on '%1'").arg(mExpirationTime.toString("yyyy-MM-dd hh:mm:ss")));
