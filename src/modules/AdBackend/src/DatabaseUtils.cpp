...
const QString AddStatRecordQuery = "UPDATE ad_statistics SET quantity = quantity + 1 WHERE id = ? "
                                   "AND channel = ? AND `date` = CURRENT_DATE;";
const QString SetStatRecordQuery =
    "UPDATE ad_statistics SET quantity = ? WHERE id = ? AND channel = ? AND `date` = CURRENT_DATE;";
...
DatabaseUtils::DatabaseUtils(const QString &aWorkingDirectory, ILog *aLog) : ILogable(aLog) {
    ...
    if (!m_Database.open()) {
        ...
        if (script.open(QIODevice::ReadOnly)) {
            ...
            toLog(LogLevel::Error,
                  QString("Failed to open database sql script: %1").arg(script.errorString()));
        }
    }
}
...
bool DatabaseUtils::addStatisticRecord(qint64 aId, const QString &aChannel) {
    ...
    if (!result) {
        toLog(LogLevel::Error,
              QString("Failed to execute query '%1': %2")
                  .arg(query.lastQuery())
...
bool DatabaseUtils::getUnsentStatisticRecords(QList<SStatisticRecord> &aRecords, int aLimit) {
    ...
    if (!query.prepare(
        ...
        toLog(LogLevel::Error,
              QString("Cannot get unsent statistic records. Error: %1.")
    ...
    while (query.isValid()) {
        aRecords << SStatisticRecord(query.value(0).toLongLong(),
...
bool DatabaseUtils::deleteStatisticRecords(const QList<SStatisticRecord> &aRecords) {
    ...
    if (!query.prepare("UPDATE ad_statistics SET quantity_reported = ? WHERE record_id = ?")) {
        toLog(LogLevel::Error,
              QString("Cannot delete unsent statistic records. Error: %1.")
                  .arg(query.lastError().databaseText()));
