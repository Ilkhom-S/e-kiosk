...
const int DecryptMaxBufferSize = 4096; // Максимальный размер для расшифровывания данных.
...
QSet<CCrypt::ETypeEngine> CryptEngine::availableEngines() {
    ...
    if (CRYPT_IS_SUCCESS(Crypt_Ctrl_Null(IPRIV_ENGINE_PKCS11_RUTOKEN, IPRIV_ENGCMD_IS_READY)) &&
        ...
        Crypt_Ctrl_String(
            IPRIV_ENGINE_PKCS11_RUTOKEN, IPRIV_ENGCMD_SET_PIN, getRootPassword()[0].data());
...
CCrypt::TokenStatus CryptEngine::getTokenStatus(CCrypt::ETypeEngine aEngine) {
    ...
    if (result.available) {
        ...
        result.initialized = CRYPT_IS_SUCCESS(Crypt_Ctrl(
            aEngine, IPRIV_ENGCMD_ENUM_PKCS11_KEYS, keys, sizeof(keys) / sizeof(keys[0])));
    }
...
//---------------------------------------------------------------------------
bool CryptEngine::createKeyPair(const QString &aPath,
                                const QString &aKeyPairName,
    ...
    int res = Crypt_GenKey2(CCrypt::ETypeEngine::File,
                            aSerialNumber,
                            aUserId.toLatin1().data(),
                            &secretKey,
                            &publicKey,
...
//---------------------------------------------------------------------------
bool CryptEngine::exportSecretKeyToFile(int aKeyPair,
...
//---------------------------------------------------------------------------
bool CryptEngine::exportPublicKeyToFile(int aKeyPair,
                                        const QString &aFilePath,
                                        ulong &aSerialNumber) {
    QByteArray keyData;
...
//---------------------------------------------------------------------------
bool CryptEngine::createKeyPair(int aKeyPair,
    ...
    int result = Crypt_GenKey(
        aEngine, aKeyCard.data(), aKeyCard.size(), &pair.first, &pair.second, aKeySize);
    if (result != 0) {
...
//---------------------------------------------------------------------------
bool CryptEngine::loadKeyPair(int aKeyPair,
                              CCrypt::ETypeEngine aEngine,
                              const QString &aSecretKeyPath,
                              const QString &aPassword,
                              const QString &aPublicKeyPath,
                              const ulong aSerialNumber,
...
bool CryptEngine::sign(int aKeyPair,
    ...
    int res = ::Crypt_SignEx(aRequest.data(),
                             aRequest.size(),
                             aSignature.data(),
                             aSignature.size(),
                             &keyPair.value().first,
                             CCryptEngine::IprivHashAlg);
...
//---------------------------------------------------------------------------
bool CryptEngine::signDetach(int aKeyPair,
                             const QByteArray &aRequest,
                             QByteArray &aSignature,
                             QString &aErrorDescription) {
    QMutexLocker locker(&m_Mutex);
    ...
    int res = ::Crypt_Sign2Ex(aRequest.data(),
                              aRequest.size(),
                              aSignature.data(),
                              aSignature.size(),
                              &keyPair.value().first,
...
//---------------------------------------------------------------------------
bool CryptEngine::verify(int aKeyPair,
    ...
    int res = ::Crypt_Verify(aResponseString.data(),
                             aResponseString.size(),
                             &originalResponseData,
                             &originalResponseDataSize,
...
//---------------------------------------------------------------------------
bool CryptEngine::verifyDetach(int aKeyPair,
                               const QByteArray &aResponseString,
    ...
    int res = ::Crypt_Verify3(aResponseString.data(),
                              aResponseString.size(),
                              aSignature.data(),
                              aSignature.size(),
                              &keyPair.value().second);
...
//---------------------------------------------------------------------------
bool CryptEngine::encrypt(int aKeyPair,
    ...
    IPRIV_KEY key =
        (aKey == CCrypt::ETypeKey::Public) ? keyPair.value().second : keyPair.value().first;
...
//---------------------------------------------------------------------------
bool CryptEngine::encryptLong(int aKeyPair,
                              const QByteArray &aData,
                              QByteArray &aResult,
                              CCrypt::ETypeKey aKey,
                              QString &aErrorDescription) {
...
//---------------------------------------------------------------------------
bool CryptEngine::decrypt(int aKeyPair,
                          const QByteArray &aData,
                          QByteArray &aResult,
                          CCrypt::ETypeKey aKey,
                          QString &aErrorDescription) {
...
//---------------------------------------------------------------------------
bool CryptEngine::decryptLong(int aKeyPair,
                              const QByteArray &aData,
                              QByteArray &aResult,
                              CCrypt::ETypeKey aKey,
                              QString &aErrorDescription) {
...
bool CryptEngine::setData(const QString &aName, const QByteArray &aData) {
    if (getTokenStatus(CCrypt::ETypeEngine::RuToken).isOK()) {
        return CRYPT_IS_SUCCESS(Crypt_Ctrl(IPRIV_ENGINE_PKCS11_RUTOKEN,
                                           IPRIV_ENGCMD_PKCS11_SET_DATA,
                                           aName.toLatin1().data(),
                                           aData.data(),
                                           aData.size()));
    }
...
bool CryptEngine::getData(const QString &aName, QByteArray &aData) {
    if (getTokenStatus(CCrypt::ETypeEngine::RuToken).isOK()) {
        ...
        do {
            ...
            size = Crypt_Ctrl(IPRIV_ENGINE_PKCS11_RUTOKEN,
                              IPRIV_ENGCMD_PKCS11_GET_DATA,
                              aName.toLatin1().data(),
                              aData.data(),
                              aData.size());
...
QString CryptEngine::errorToString(int aError) const {
    ...
    case CRYPT_ERR_OUT_OF_MEMORY:
        return QString("bad memory allocation (%1)").arg(aError);
    case CRYPT_ERR_INVALID_FORMAT:
        return QString("bad document format (%1)").arg(aError);
    case CRYPT_ERR_NO_DATA_FOUND:
        return QString("unexpected end of document (%1)").arg(aError);
    case CRYPT_ERR_INVALID_PACKET_FORMAT:
        return QString("wrong document structure (%1)").arg(aError);
    case CRYPT_ERR_UNKNOWN_ALG:
        return QString("unknown algorithm (%1)").arg(aError);
    case CRYPT_ERR_INVALID_KEYLEN:
        return QString("key length and document length are different (%1)").arg(aError);
    case CRYPT_ERR_INVALID_PASSWD:
        return QString("wrong secret password (%1)").arg(aError);
    case CRYPT_ERR_DOCTYPE:
        return QString("wrong document type (%1)").arg(aError);
    case CRYPT_ERR_RADIX_DECODE:
        return QString("failed to ASCII decode (%1)").arg(aError);
    case CRYPT_ERR_RADIX_ENCODE:
        return QString("failed to ASCII encode (%1)").arg(aError);
    case CRYPT_ERR_INVALID_ENG:
        return QString("unknown provider (%1)").arg(aError);
    ...
    case CRYPT_ERR_CANT_READ_FILE:
        return QString("failed to read file (%1)").arg(aError);
    case CRYPT_ERR_INVALID_KEY:
        return QString("invalid key (%1)").arg(aError);
    case CRYPT_ERR_SEC_ENC:
        return QString("failed to build sign (%1)").arg(aError);
    case CRYPT_ERR_PUB_KEY_NOT_FOUND:
        return QString("public key not found (%1)").arg(aError);
    ...
    case CRYPT_ERR_INVALID_KEYCARD:
        return QString("wrong keycard format (%1)").arg(aError);
    case CRYPT_ERR_GENKEY:
        return QString("failed to generate key (%1)").arg(aError);
    case CRYPT_ERR_PUB_ENC:
        return QString("encode error (%1)").arg(aError);
    case CRYPT_ERR_SEC_DEC:
        return QString("decode error (%1)").arg(aError);
    case CRYPT_ERR_UNKNOWN_SENDER:
        return QString("unknown sender (%1)").arg(aError);

    default:
        return QString("unknown error (%1)").arg(aError);
    }
}
