    ...
    namespace PaymentProcessor {
        namespace Scripting {
            ...
            PaymentService::PaymentService(ICore *aCore)
                ...
                mDirectory =
                    static_cast<PPSDK::Directory *>(aCore->getSettingsService()->getAdapter(CAdapterNames::Directory));
            ...
            void PaymentService::checkStatus() {
                ...
                QString queryStr = QString("SELECT `create_date` FROM `device_status` WHERE `fk_device_id` = 1 AND "
                ...
                foreach (SDK::PaymentProcessor::SBlockByNote note, notes) {
                    QString queryStr =
                        QString(
                            ...
                            "`type` <> 2 AND `nominal` = %1 AND `date` >= DATETIME('now', 'localtime', '-%2 seconds')")
                            .arg(note.nominal)
                            .arg(deltaTime ? qMin(deltaTime, note.interval) : note.interval);
                    ...
                    if (query && query->first() && query->value(0).toUInt() > note.repeat) {
                        ...
                        mCore->getEventService()->sendEvent(
                            PPSDK::Event(SDK::PaymentProcessor::EEventType::Critical, "Payment",
