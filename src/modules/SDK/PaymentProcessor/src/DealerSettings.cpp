...
namespace SDK
{
    namespace PaymentProcessor
    {
        ...
        bool DealerSettings::loadProviders()
        {
            ...
            toLog(LogLevel::Normal, QString("Total providers loaded: %1, elapsed %2 ms.")
        ...
        bool DealerSettings::loadProvidersFromBuffer(const std::string &aBuffer, SProvider &aProvider)
        {
            ...
            BOOST_FOREACH (const auto &value, operators.get_child("", emptyTree))
            {
                try
                {
                    ...
                    aProvider.processor.feeType = processorBranchAttr.get<QString>("fee_type", "amount_all") == "amount"
                    ...
                    for (auto requestIt = requests.first; requestIt != requests.second; ++requestIt)
                    {
                        ...
                        aProvider.processor.requests[requestIt->second.get<QString>("<xmlattr>.name").toUpper()] =
                            request;
                    }
