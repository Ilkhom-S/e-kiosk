        ...
        namespace Humo {
            ...
            class RequestSender : public QObject {
              ...
              public:
                ...
                typedef std::function<bool(const QByteArray &, QByteArray &, ESignatureType, QByteArray &)>
                    TRequestSigner;

                /// Метод, проверяющий подпись ответа и возвращающий данные, освобождённые от подписи.
                typedef std::function<bool(const QByteArray &, QByteArray &, ESignatureType, const QByteArray &)>
                    TResponseVerifier;
                ...
                /// Устанавливает модификатор заголовков запроса.
                ...
                Response *get(const QUrl &aUrl, Request &aRequest, ESignatureType aSignatureType, ESendError &aError,
                              int aTimeout = 60 * 1000);

                /// Отправляет запрос методом POST на адрес aUrl.
                Response *post(const QUrl &aUrl, Request &aRequest, ESignatureType aSignatureType, ESendError &aError,
                               int aTimeout = 60 * 1000);

              protected:
                /// Отправляет запрос методом aType на адрес aUrl.
                Response *request(NetworkTask::Type aType, const QUrl &aUrl, Request &aRequest,
                ...
                bool defaultRequestSigner(const QByteArray &aRequest, QByteArray &aSignedRequest,
                ...
                bool defaultRequestModifier(Request &aRequest, NetworkTask::TByteMap &aHeaders,
                                            QByteArray &aEncodedAndSignedData);

                /// Метод, проверяющий запросы по умолчанию.
                bool defaultResponseVerifier(const QByteArray &aSignedResponse, QByteArray &aResponse,
