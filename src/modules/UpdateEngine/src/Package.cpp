    ...
    if (!aFiles.isEmpty()) {
        ...
        if (optional() && QFile(QDir::toNativeSeparators(getTemporaryFolder() + "/" + getId() + ".zip")).size() <= 0) {
            ...
            if (!unzip.unpack(QDir::toNativeSeparators(getTemporaryFolder() + "/" + getId() + ".zip"),
                ...
                throw Exception(QString("Failed to unzip archive %1.zip error %2. Output: %3")
                                    .arg(getId())
                                    .arg(unzip.exitCode())
...
void Package::applyPostActions(const QString &aWorkingDir) throw(std::exception) {
    ...
    if (!postActions.empty()) {
        ...
        if (!unzip.unpack(QDir::toNativeSeparators(getTemporaryFolder() + "/" + getId() + ".zip"),
            ...
            throw Exception(QString("Failed to unzip files from archive %1.zip error %2. Output: %3.")
                                .arg(getId())
                                .arg(unzip.exitCode())
                                .arg(QString(unzip.messages())));
        }
    ...
    foreach (auto action, postActions) {
        ...
        if (runAction.exitCode() != 0) {
            throw Exception(QString("Process %1 exited with error. Return code: %2, output: %3.")
                                .arg(action)
