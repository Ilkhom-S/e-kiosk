...
DL_DECLARE_FUN_THROW(WinVerifyTrustEx, HRESULT,
                     (IN HWND)(IN GUID *)(IN WINTRUST_DATA *))
DL_USE_MODULE_END()

//--------------------------------------------------------------------------------
DL_USE_MODULE_BEGIN(crypt, "crypt32.dll")
DL_DECLARE_FUN_THROW(
    CryptDecodeObject, BOOL,
    (IN DWORD)(IN LPCSTR)(IN const BYTE *)(IN DWORD)(IN DWORD)(OUT void
                                                                   *)(DWORD *))
DL_DECLARE_FUN_THROW(CryptMsgClose, BOOL, (IN HCRYPTMSG))
DL_DECLARE_FUN_THROW(CryptMsgGetParam, BOOL,
                     (IN HCRYPTMSG)(IN DWORD)(IN DWORD)(OUT void *)(DWORD *))
DL_DECLARE_FUN_THROW(CertCloseStore, BOOL, (IN HCERTSTORE)(IN DWORD))
DL_DECLARE_FUN_THROW(
    CertFindCertificateInStore, PCCERT_CONTEXT,
    (IN HCERTSTORE)(IN DWORD)(IN DWORD)(IN DWORD)(IN const void
                                                      *)(IN PCCERT_CONTEXT))
DL_DECLARE_FUN_THROW(CertFreeCertificateContext, BOOL, (IN PCCERT_CONTEXT))
DL_DECLARE_FUN_THROW(
    CertGetNameString, BOOL,
    (IN PCCERT_CONTEXT)(IN DWORD)(IN DWORD)(IN void *)(OUT LPTSTR)(IN DWORD))
DL_DECLARE_FUN_THROW(
    CryptQueryObject, BOOL,
    (IN DWORD)(IN const void
                   *)(IN DWORD)(IN DWORD)(IN DWORD)(OUT DWORD
                                                        *)(OUT DWORD
                                                               *)(OUT DWORD
                                                                      *)(OUT HCERTSTORE
                                                                             *)(OUT HCRYPTMSG
    ...
    for (DWORD n = 0; n < pSignerInfo->AuthAttrs.cAttr; n++) {
      if (lstrcmpA(SPC_SP_OPUS_INFO_OBJID,
        ...
        fResult = crypt::CryptDecodeObject(
            ...
            &dwData);
        if (!fResult) {
          qDebug() << "CryptDecodeObject failed with"
    ...
    for (DWORD n = 0; n < pSignerInfo->AuthAttrs.cAttr; n++) {
      if (lstrcmpA(szOID_RSA_signingTime,
        ...
        fResult = crypt::CryptDecodeObject(
            ...
            pSignerInfo->AuthAttrs.rgAttr[n].rgValue[0].pbData,
            pSignerInfo->AuthAttrs.rgAttr[n].rgValue[0].cbData, 0, (PVOID)&ft,
            &dwData);
        if (!fResult) {
          qDebug() << "CryptDecodeObject failed with"
                   << QString::number(GetLastError(), 16);
    ...
    *pCounterSignerInfo = NULL;
    ...
    for (DWORD n = 0; n < pSignerInfo->UnauthAttrs.cAttr; n++) {
      if (lstrcmpA(pSignerInfo->UnauthAttrs.rgAttr[n].pszObjId,
        ...
        fResult = crypt::CryptDecodeObject(
            ENCODING, PKCS7_SIGNER_INFO,
            pSignerInfo->UnauthAttrs.rgAttr[n].rgValue[0].pbData,
            pSignerInfo->UnauthAttrs.rgAttr[n].rgValue[0].cbData, 0, NULL,
            &dwSize);
        if (!fResult) {
          qDebug() << "CryptDecodeObject failed with"
                   << QString::number(GetLastError(), 16);
...
bool ISysUtils::getSignerInfo(const QString &aFile, SSignerInfo &aSigner) {
  ...
  try {
    ...
    fResult = crypt::CryptQueryObject(
    ...
    if (!fResult) {
      qDebug() << "CryptQueryObject failed with"
               << QString::number(GetLastError(), 16);
      throw false;
    }

    // Get signer information size.
    fResult = crypt::CryptMsgGetParam(hMsg, CMSG_SIGNER_INFO_PARAM, 0, NULL,
                                      &dwSignerInfo);
    if (!fResult) {
      qDebug() << "CryptMsgGetParam failed with"
               << QString::number(GetLastError(), 16);
      throw false;
    }
    ...
    fResult = crypt::CryptMsgGetParam(hMsg, CMSG_SIGNER_INFO_PARAM, 0,
    ...
    pCertContext = crypt::CertFindCertificateInStore(
        hStore, ENCODING, 0, CERT_FIND_SUBJECT_CERT, (PVOID)&CertInfo, NULL);
    if (!pCertContext) {
      qDebug() << "CertFindCertificateInStore failed with"
               << QString::number(GetLastError(), 16);
      throw false;
    }
    ...
    if ((dwData = crypt::CertGetNameString(
    ...
    // Get Issuer name.
    if (!(crypt::CertGetNameString(pCertContext, CERT_NAME_SIMPLE_DISPLAY_TYPE,
                                   CERT_NAME_ISSUER_FLAG, NULL, szSignerName,
                                   dwData))) {
  ...
  if (pCertContext != NULL)
    crypt::CertFreeCertificateContext(pCertContext);
  if (hStore != NULL)
    crypt::CertCloseStore(hStore, 0);
  if (hMsg != NULL)
    crypt::CryptMsgClose(hMsg);

  if (szSignerName != NULL)
    LocalFree(szSignerName);
