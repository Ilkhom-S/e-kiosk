...
DL_DECLARE_FUN_THROW(
    CryptDecodeObject,
    BOOL,
    (IN DWORD)(IN LPCSTR)(IN const BYTE *)(IN DWORD)(IN DWORD)(OUT void *)(DWORD *))
DL_DECLARE_FUN_THROW(CryptMsgClose, BOOL, (IN HCRYPTMSG))
DL_DECLARE_FUN_THROW(CryptMsgGetParam,
                     BOOL,
...
DL_DECLARE_FUN_THROW(CertGetNameString,
                     BOOL,
    ...
    try {
        ...
        for (DWORD n = 0; n < pSignerInfo->AuthAttrs.cAttr; n++) {
            if (lstrcmpA(SPC_SP_OPUS_INFO_OBJID, pSignerInfo->AuthAttrs.rgAttr[n].pszObjId) == 0) {
                ...
                fResult =
                    crypt::CryptDecodeObject(ENCODING,
...
bool ISysUtils::getSignerInfo(const QString &aFile, SSignerInfo &aSigner) {
    ...
    try {
        ...
        fResult = crypt::CryptMsgGetParam(
        ...
        if ((dwData = crypt::CertGetNameString(
                 pCertContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, NULL, 0)) == 0) {
        ...
        // Get subject name.
        if (!(crypt::CertGetNameString(
                pCertContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, szSignerName, dwData))) {
