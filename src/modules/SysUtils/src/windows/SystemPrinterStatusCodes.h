/* @file Спецификация статусов системного принтера. */
...
#define ADD_SYSTEM_CODE(aCodeType, aCodeName, aStatusCodeType,                 \
                        aStatusCodeName)                                       \
  appendStatus(aCodeType##aCodeName,                                           \
               aStatusCodeType##StatusCode::aStatusCodeName,                   \
               QString(#aCodeName).replace("_", " ").toLower());

#define ADD_SYSTEM_STATUS(aCodeName, aStatusCodeType, aStatusCodeName)         \
  ADD_SYSTEM_CODE(PRINTER_STATUS, aCodeName, aStatusCodeType, aStatusCodeName)
#define ADD_SYSTEM_JOB_STATUS(aCodeName, aStatusCodeType, aStatusCodeName)     \
