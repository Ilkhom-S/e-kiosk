...
QString ISysUtils::getErrorMessage(ulong aError, bool aNativeLanguage) {
  LPVOID data = nullptr;
  DWORD languageId = aNativeLanguage
                         ? MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT)
                         : MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US);

  FormatMessageW(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,
                 nullptr, aError, languageId, (LPWSTR)&data, 0, nullptr);
...
bool ISysUtils::bringWindowToFront(WId aWindow) {
  ...
  SetWindowPos((HWND)aWindow, HWND_TOPMOST, NULL, NULL, NULL, NULL,
