...
bool getPrinterStatusData(const QString &aPrinterName, TJobsStatus &aJobsStatus,
  ...
  if (!GetPrinter(printer, 2, NULL, 0, &byteNeeded) &&
  ...
  PRINTER_INFO_2 *printerInfo =
      reinterpret_cast<PRINTER_INFO_2 *>(printerInfoBuffer.data());
  ...
  if (!EnumJobs(printer, 0, printerInfo->cJobs, 2, NULL, 0,
                (LPDWORD)&byteNeeded, (LPDWORD)&returned) &&
      GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
    ClosePrinter(printer);
  ...
  JOB_INFO_2 *jobStorage =
      reinterpret_cast<JOB_INFO_2 *>(jobsInfoBuffer.data());

  bool enumJobsOK =
      EnumJobs(printer, 0, printerInfo->cJobs, 2, (LPBYTE)jobStorage,
               byteNeeded, (LPDWORD)&byteUsed, (LPDWORD)&returned);
...
//--------------------------------------------------------------------------------
bool ISysUtils::setPrintingQueuedMode(const QString &aPrinterName,
  ...
  if (OpenPrinter((LPWSTR)aPrinterName.toStdWString().data(), &printer,
    ...
    if (GetPrinter(printer, 2, reinterpret_cast<LPBYTE>(printerInfo),
                   byteNeeded, &byteUsed)) {
