...
bool getPrinterStatusData(const QString &aPrinterName, TJobsStatus &aJobsStatus, ulong &aStatus, ulong &aAttributes)
{
    ...
    if (!OpenPrinterW((LPWSTR)aPrinterName.toStdWString().c_str(), &printer, &defaults))
    {
        return false;
    }
    ...
    if (!EnumJobs(printer, 0, printerInfo->cJobs, 2, NULL, 0, (LPDWORD)&byteNeeded, (LPDWORD)&returned) &&
    ...
    bool enumJobsOK = EnumJobs(printer, 0, printerInfo->cJobs, 2, (LPBYTE)jobStorage, byteNeeded, (LPDWORD)&byteUsed,
                               (LPDWORD)&returned);
...
QVariantMap ISysUtils::getPrinterData(const QString &aPrinterName)
{
    ...
    result.insert(CDeviceData::Name, QString::fromWCharArray((const wchar_t *)printerInfo->pPrinterName));
    result.insert(CDeviceData::Port, QString::fromWCharArray((const wchar_t *)printerInfo->pPortName));
    result.insert(CDeviceData::Driver, QString::fromWCharArray((const wchar_t *)printerInfo->pDriverName));
    result.insert(CDeviceData::Printers::Location, QString::fromWCharArray((const wchar_t *)printerInfo->pLocation));
    result.insert(CDeviceData::Printers::Comment, QString::fromWCharArray((const wchar_t *)printerInfo->pComment));
    result.insert(CDeviceData::Printers::Server, QString::fromWCharArray((const wchar_t *)printerInfo->pServerName));
    result.insert(CDeviceData::Printers::Share, QString::fromWCharArray((const wchar_t *)printerInfo->pShareName));
                                 ...
                                 TStatusGroupNames &aGroupNames)
{
    ...
    if (attributes & PRINTER_STATUS_NOT_AVAILABLE || attributes & PRINTER_ATTRIBUTE_WORK_OFFLINE ||
...
bool ISysUtils::setPrintingQueuedMode(const QString &aPrinterName, QString &aErrorMessage)
{
    ...
    if (OpenPrinterW((LPWSTR)aPrinterName.toStdWString().c_str(), &printer, &defaultsAdmin))
    {
