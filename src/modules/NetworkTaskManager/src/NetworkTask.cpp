...
NetworkTask::NetworkTask()
    : mType(Get), mTimeout(0), mError(NotReady), mHttpError(0),
      mProcessing(false), mParentThread(QThread::currentThread()), mFlags(None),
      mSize(0), mCurrentSize(0) {
...
//------------------------------------------------------------------------
NetworkTaskManager *NetworkTask::getManager() const { return mManager; }
...
//------------------------------------------------------------------------
void NetworkTask::setProcessing(NetworkTaskManager *aManager,
                                bool aProcessing) {
    mManager = aManager;
...
//------------------------------------------------------------------------
void NetworkTask::setType(Type aType) { mType = aType; }

//------------------------------------------------------------------------
NetworkTask::Type NetworkTask::getType() const { return mType; }

//------------------------------------------------------------------------
void NetworkTask::setUrl(const QUrl &aUrl) { mUrl = aUrl; }

//------------------------------------------------------------------------
const QUrl &NetworkTask::getUrl() const { return mUrl; }

//------------------------------------------------------------------------
void NetworkTask::setTimeout(int aMsec) { mTimer.setInterval(aMsec); }

//------------------------------------------------------------------------
int NetworkTask::getTimeout() const { return mTimeout; }

//------------------------------------------------------------------------
void NetworkTask::setFlags(Flags aFlags) { mFlags = aFlags; }

//------------------------------------------------------------------------
NetworkTask::Flags NetworkTask::getFlags() const { return mFlags; }
...
//------------------------------------------------------------------------
int NetworkTask::getError() const { return mError; }

//------------------------------------------------------------------------
void NetworkTask::setHttpError(int aError) { mHttpError = aError; }

//------------------------------------------------------------------------
int NetworkTask::getHttpError() const { return mHttpError; }
...
//------------------------------------------------------------------------
IVerifier *NetworkTask::getVerifier() const { return mVerifier.data(); }
...
//------------------------------------------------------------------------
DataStream *NetworkTask::getDataStream() const { return mDataStream.data(); }
...
//------------------------------------------------------------------------
qint64 NetworkTask::getSize() const { return mSize; }

//------------------------------------------------------------------------
qint64 NetworkTask::getCurrentSize() const { return mCurrentSize; }

//------------------------------------------------------------------------
void NetworkTask::setTag(const QVariant &aTag) { mTag = aTag; }

//------------------------------------------------------------------------
const QVariant &NetworkTask::getTag() const { return mTag; }
