...
class NetworkTask : public QObject {
  ...
  public:
    ...
    enum Error {
        NoError = 0,
        StreamWriteError =
            -1, /// Не удалось произвести запись в переданный поток данных.
        ...
        BadTask = -4, /// Ошибочный запрос к серверу. Пример: пытаемся скачать
                      /// кусок файла с неправильным смещением.
        VerifyFailed = -5, /// Запрос к серверу завершён без ошибок, но данные
                           /// не прошли верификацию.
        TaskFailedButVerified =
            -6, /// Запрос к серверу завершился с ошибкой, но полученные данные
                /// успешно прошли верификацию. Пример: пытаемся до качать уже
                /// полностью скачанный файл.
        NotReady = -7 /// Задача была создана, но не выполнена.
    };
    ...
    enum Flags {
        ...
        IgnoreErrors = 0x4  /// При возникновении ошибок продолжать закачку до
                            /// тех пор, пока не скачаем весь файл. Закачка
                            /// прекращается при достижении таймаута.
    };
    ...
    /// Возвращает размер скачиваемого контента в байтах. Будет не нулевым после
    /// получения первого пакета с данными.
  ...
  signals:
    ...
    /// Сигнал срабатывает при изменении прогресса выполнения задачи. aCurrent,
    /// aTotal - кол-во байт.
