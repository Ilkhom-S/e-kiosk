...
void NetworkTaskManager::setProxy(const QNetworkProxy &aProxy) {
    metaObject()->invokeMethod(
        this, "onSetProxy", Qt::QueuedConnection, Q_ARG(QNetworkProxy, aProxy));
}

//------------------------------------------------------------------------
void NetworkTaskManager::setDownloadSpeedLimit(int aPercent) {
    metaObject()->invokeMethod(
        this, "onSetDownloadSpeedLimit", Qt::QueuedConnection, Q_ARG(int, aPercent));
}
...
void NetworkTaskManager::removeTask(NetworkTask *aTask) {
    metaObject()->invokeMethod(
        this, "onRemoveTask", Qt::AutoConnection, Q_ARG(NetworkTask *, aTask));
}
...
void NetworkTaskManager::onAddTask(NetworkTask *aTask) {
    ...
    default: {
        toLog(LogLevel::Error,
              QString("Failed to process task. Unknown operation type: %1.").arg(aTask->getType()));
    ...
    connect(
        reply, SIGNAL(uploadProgress(qint64, qint64)), SLOT(onTaskUploadProgress(qint64, qint64)));
    ...
    if (reply && mTasks.contains(reply) && !mTasks.value(reply).isNull()) {
        ...
        if (task) {
            ...
            if (httpStatusCode.isValid()) {
                ...
                {
                    if (task->getSize() == 0) // выполняем этот код, только в момент получения
                                              // первого пакета данных
                    {
                        ...
                        if (contentRange.isEmpty()) {
                            ...
                            if (match.hasMatch()) {
                                ...
                                toLog(LogLevel::Error,
                                      QString("Can't parse Content-Range: %1.").arg(contentRange));
    ...
    for (TTaskMap::iterator it = mTasks.begin(); it != mTasks.end(); ++it) {
        if (it.key() == reply && !it.value().isNull()) {
            ...
            task->getResponseHeader().insert(
                "Content-Type", reply->header(QNetworkRequest::ContentTypeHeader).toByteArray());
...
QSslCertificate NetworkTaskManager::loadCertResource(const QString &aPath) {
    ...
    QSslCertificate cert(buffer,
                         res.fileName().split(".").takeLast() == "cer" ? QSsl::Der : QSsl::Pem);

    // Replacement for isValid()
    bool isExpiredOrNotYetValid = QDateTime::currentDateTime() < cert.effectiveDate() ||
                                  QDateTime::currentDateTime() > cert.expiryDate();

    if (cert.isNull() || isExpiredOrNotYetValid || cert.isBlacklisted()) {
        toLog(LogLevel::Error,
              QString("Failed load cert: %1. (Null, Expired, or Blacklisted)").arg(aPath));
    } else {
        toLog(LogLevel::Normal,
