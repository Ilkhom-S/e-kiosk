...
void NetworkTaskManager::setProxy(const QNetworkProxy &aProxy) {
    metaObject()->invokeMethod(this, "onSetProxy", Qt::QueuedConnection,
                               Q_ARG(QNetworkProxy, aProxy));
}

//------------------------------------------------------------------------
void NetworkTaskManager::setDownloadSpeedLimit(int aPercent) {
    metaObject()->invokeMethod(this, "onSetDownloadSpeedLimit",
                               Qt::QueuedConnection, Q_ARG(int, aPercent));
}

//------------------------------------------------------------------------
void NetworkTaskManager::addTask(NetworkTask *aTask) {
    aTask->setProcessing(this, true);
    metaObject()->invokeMethod(this, "onAddTask", Qt::AutoConnection,
                               Q_ARG(NetworkTask *, aTask));
}

//------------------------------------------------------------------------
void NetworkTaskManager::removeTask(NetworkTask *aTask) {
    metaObject()->invokeMethod(this, "onRemoveTask", Qt::AutoConnection,
                               Q_ARG(NetworkTask *, aTask));
}
...
void NetworkTaskManager::onAddTask(NetworkTask *aTask) {
    ...
    if (aTask->getFlags() & NetworkTask::Continue) {
        ...
        if (offset > 0) {
            request.setRawHeader("Range",
                                 QString("bytes=%1-").arg(offset).toLatin1());

            toLog(LogLevel::Normal,
                  QString("Downloading data. Url: %1. Offset: %2.")
    ...
    default: {
        toLog(LogLevel::Error,
              QString("Failed to process task. Unknown operation type: %1.")
                  .arg(aTask->getType()));
    ...
    connect(reply, SIGNAL(downloadProgress(qint64, qint64)),
            SLOT(onTaskProgress(qint64, qint64)));
    connect(reply, SIGNAL(uploadProgress(qint64, qint64)),
            SLOT(onTaskUploadProgress(qint64, qint64)));
    connect(reply, SIGNAL(readyRead()), SLOT(onTaskReadyRead()));
    connect(reply, SIGNAL(error(QNetworkReply::NetworkError)),
            SLOT(onTaskError(QNetworkReply::NetworkError)));
    connect(reply, SIGNAL(sslErrors(const QList<QSslError> &)),
            SLOT(onTaskSslErrors(const QList<QSslError> &)));
            ...
            if (aTask->getError()) {
                toLog(LogLevel::Error,
                      QString("< Error: %1. HttpError: %2. Request URL: %3.")
                          ...
                          .arg(aTask->getHttpError())
                          .arg(aTask->getUrl().toString()));
            }
...
void NetworkTaskManager::onTaskProgress(qint64 aReceived, qint64 aTotal) {
    TTaskMap::iterator it =
        mTasks.find(dynamic_cast<QNetworkReply *>(sender()));
    ...
    if (reply && mTasks.contains(reply) && !mTasks.value(reply).isNull()) {
        ...
        if (task) {
            ...
            if (httpStatusCode.isValid()) {
                ...
                case 416: {
                    toLog(LogLevel::Error, "Request range header is wrong, "
                ...
                {
                    if (task->getSize() ==
                        0) // выполняем этот код, только в момент получения
                           // первого пакета данных
                    {
                        QString contentRange = QString::fromLatin1(
                            reply->rawHeader("Content-Range"));

                        if (contentRange.isEmpty()) {
                            // Если запрашивали кусок данных, а пришел файл
                            // целиком - нужно начинать писать поток с 0-го
                            // байта
                            ...
                            if (rx.indexIn(contentRange) > 0) {
                                ...
                                if (!task->getDataStream()->seek(pos)) {
                                    toLog(
                                        LogLevel::Error,
                                        QString("Content-Range: %1. Error seek "
                                                "stream to position: %2.")
                                            .arg(contentRange)
                                            .arg(pos));
                                }
                            } else {
                                toLog(LogLevel::Error,
                                      QString("Can't parse Content-Range: %1.")
                                          .arg(contentRange));
                    ...
                    toLog(LogLevel::Debug,
                          QString("< receive %1%2 bytes.")
                              .arg(statusCode == 206 ? "(partial) " : "")
                              .arg(replyData.size()));

                    if (!task->getDataStream()->write(replyData)) {
                        toLog(LogLevel::Error,
                              "Cannot save received data to the stream.");
                ...
                default:
                    toLog(LogLevel::Error,
                          QString("Data is ready for read, but response code "
                                  "is incorrect: %1")
                              .arg(httpStatusCode.toString()));
                    reply->abort();
                }
            }
        }
...
void NetworkTaskManager::onTaskSslErrors(const QList<QSslError> &aErrors) {
    ...
    toLog(LogLevel::Warning,
          QString("One or more SSL errors has occurred: %1").arg(errorString));
    ...
    for (TTaskMap::iterator it = mTasks.begin(); it != mTasks.end(); ++it) {
        if (it.key() == reply && !it.value().isNull()) {
            ...
            foreach (QByteArray header, headers) {
                task->getResponseHeader().insert(header,
                                                 reply->rawHeader(header));
            }

            task->getResponseHeader().insert(
                "Content-Type",
            ...
            if (reply->error() != QNetworkReply::OperationCanceledError) {
                ...
                if (statusCode && ((statusCode == 301) || (statusCode == 416) ||
                                   (statusCode == 404))) {
                    toLog(LogLevel::Warning,
                          QString("Set bad task error, because statusCode=%1.")
                              .arg(statusCode));
                    ...
                    toLog(LogLevel::Warning,
                          QString("Set bad task error, because taskSize != "
                                  "size: (%1 != %2).")
                              .arg(task->getCurrentSize())
                              .arg(task->getSize()));
...
QSslCertificate NetworkTaskManager::loadCertResource(const QString &aPath) {
    ...
    QSslCertificate cert(buffer, res.fileName().split(".").takeLast() == "cer"
                                     ? QSsl::Der
                                     : QSsl::Pem);

    // Replacement for isValid()
    bool isExpiredOrNotYetValid =
        QDateTime::currentDateTime() < cert.effectiveDate() ||
        QDateTime::currentDateTime() > cert.expiryDate();

    if (cert.isNull() || isExpiredOrNotYetValid || cert.isBlacklisted()) {
        toLog(LogLevel::Error,
              QString("Failed load cert: %1. (Null, Expired, or Blacklisted)")
                  .arg(aPath));
    } else {
        toLog(
            LogLevel::Normal,
            QString("Load CA cert: %1")
...
void NetworkTaskManager::loadCerts() {
    ...
    caCerts << loadCertResource(":/CA/GlobalSignRootCA.pem")
            << loadCertResource(
                   ":/CA/GlobalSignDomainValidationCA-SHA256-G2.pem")
...
//------------------------------------------------------------------------
QString NetworkTaskManager::getUserAgent() const { return mUserAgent; }
