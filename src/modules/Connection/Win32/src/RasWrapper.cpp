    ...
    */
    ...
    void PhonebookEntryEnumerator::reset(const std::wstring &aPhonebookPath) {
        ...
        if (mLastError == ERROR_BUFFER_TOO_SMALL) {
            ...
            mLastError = RasEnumEntries(0, aPhonebookPath.empty() ? 0 : aPhonebookPath.data(), mEntries,
                                        &mRequestedBufSize, &mEntryCount);
        }
    }
    ...
    void ConnectionEnumerator::reset() {
        ...
        mLastError = RasEnumConnections(GetOSVersion() < EOSVersion::WindowsVista ? &dummy : 0, // различие(баг) в апи
    ...
    DWORD ValidatePhonebookEntryName(PhonebookEntryName &aEntry) {
        DWORD error = RasValidateEntryName(aEntry.phonebookPath().empty() ? 0 : aEntry.phonebookPath().data(),
                                           aEntry.name().data());
    ...
    DWORD RemovePhonebookEntry(const PhonebookEntryName &aEntryName) {
        ...
        if (error == ERROR_BUFFER_TOO_SMALL) {
            error = RasDeleteEntry(aEntryName.phonebookPath().empty() ? 0 : aEntryName.phonebookPath().data(),
    ...
    DWORD GetEntryProperties(const PhonebookEntryName &aEntryName, PhonebookEntry &aEntry) {
        ...
        DWORD error = RasGetEntryProperties(aEntryName.phonebookPath().empty() ? 0 : aEntryName.phonebookPath().data(),
                                            aEntryName.name().data(), (RASENTRY *)aEntry, &size, 0, 0);
    ...
    DWORD GetEntryDialParams(const PhonebookEntryName &aEntryName, DialParams &aParameters) {
        ...
        DWORD error = RasGetEntryDialParams(aEntryName.phonebookPath().empty() ? 0 : aEntryName.phonebookPath().data(),
    ...
    DWORD SetEntryDialParams(const PhonebookEntryName &aEntryName, DialParams &aParams) {
        ...
        DWORD error = RasSetEntryDialParams(aEntryName.phonebookPath().empty() ? 0 : aEntryName.phonebookPath().data(),
    ...
    std::wstring getAttachedTo(const std::wstring &aDeviceName) {
        ...
        for (int index = 0; index < 20 && result.empty(); ++index) {
            ...
            swprintf(regPath, MAX_PATH,
                     L"SYSTEM\\CurrentControlSet\\Control\\Class\\{4D36E96D-E325-11CE-BFC1-08002BE10318}\\%.4d", index);
            ...
            if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, regPath, NULL, KEY_READ, &modem) == ERROR_SUCCESS) {
                if (getRegValue(modem, L"FriendlyName") == aDeviceName ||
                    getRegValue(modem, L"DriverDesc") == aDeviceName || getRegValue(modem, L"Model") == aDeviceName) {
