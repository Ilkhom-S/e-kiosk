...
TResult Creator::checkAnswer(const QByteArray &aAnswer) {
    ...
    if (CRC != dataCRC) {
        toLog(LogLevel::Error, QString("Creator: CRC = %1, need = 0x%2")
...
TResult Creator::processCommand(const QByteArray &aCommandData, QByteArray &aUnpackedAnswer, bool aIOLogsDebugMode) {
    ...
    do {
        ...
        if (commandNAKIndex) {
            toLog(LogLevel::Warning, "Creator: Failed to send the command, trying to repeat after NAK, attempt " +
    ...
    toLog(LogLevel::Error, QString("Creator: Maximum quantity attempts of sending the command = %1 is exceeded.")
                               .arg(CCreator::MaxRepeatCommandNAK));
...
TResult Creator::receiveAnswer(QByteArray &aAnswer) {
    ...
    do {
        if (answerNAKIndex) {
            toLog(LogLevel::Warning, "Creator: Failed to check the answer, trying to repeat using NAK, attempt " +
    ...
    toLog(LogLevel::Error, QString("Creator: Maximum quantity attempts of reading the answer = %1 is exceeded.")
                               .arg(CCreator::MaxRepeatCommandNAK));
