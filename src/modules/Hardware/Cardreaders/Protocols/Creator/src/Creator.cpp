            ...
            if (aData.size() >= (aBase + 4)) {
                ...
                if (aData[postfixIndex] == ASCII::NUL) {
                    for (int i = 0; i < packetAmount; ++i) {
                        ...
                        if (aData.mid(basePacketIndex, CCreator::USBPacketSize) ==
                            CCreator::EmptyUSBPacket) {
...
TResult Creator::checkAnswer(const QByteArray &aAnswer) {
    ...
    if (aAnswer.size() < CCreator::MinPacketAnswerSize) {
        toLog(LogLevel::Error,
              QString("Creator: total answer length < %1").arg(CCreator::MinPacketAnswerSize));
    ...
    if (length != dataLength) {
        toLog(LogLevel::Error,
              QString("Creator: length = %1, need = %2").arg(length).arg(dataLength));
...
TResult Creator::processCommand(const QByteArray &aCommandData,
    ...
    toLog(LogLevel::Error,
          QString("Creator: Maximum quantity attempts of sending the command = %1 is exceeded.")
...
TResult Creator::receiveAnswer(QByteArray &aAnswer) {
    ...
    toLog(LogLevel::Error,
          QString("Creator: Maximum quantity attempts of reading the answer = %1 is exceeded.")
              .arg(CCreator::MaxRepeatCommandNAK));
