...
bool CreatorReader::updateParameters() {
    if (!processCommand(CCreatorReader::Commands::UnLockInitialize) ||
        !processCommand(CCreatorReader::Commands::SetMCReadingMode)) {
        return false;
    }
...
//--------------------------------------------------------------------------------
TResult CreatorReader::processCommand(const QByteArray &aCommand, const QByteArray &aCommandData, QByteArray *aAnswer,
...
bool CreatorReader::checkAnswer(const QByteArray &aCommand, const QByteArray &aAnswer) {
    ...
    if (command != dataCommand) {
        toLog(LogLevel::Error, QString("%1: command = {%2}, need = {%3}")
                                   .arg(mDeviceName)
...
bool CreatorReader::getStatus(TStatusCodes &aStatusCodes) {
    ...
    if (!OK1 || !OK0 || (ST1 && (ST1 != CCreatorReader::CardPosition::ST1)) ||
        (ST0 < CCreatorReader::CardPosition::Ejected) || (ST0 > CCreatorReader::CardPosition::Inserted)) {
        ...
        if (mCardPosition == CCreatorReader::CardPosition::Inserted) {
            ...
            if (typeMS) {
                toLog(LogLevel::Normal, QString("%1: Inserted %2 card type.")
                                            .arg(mDeviceName)
                                            .arg(CCreatorReader::CardTypes::Description[ECardType::MS]));
            }

            if (typeIC && !cardData.contains(CHardware::CardReader::Track1) &&
                !cardData.contains(CHardware::CardReader::Track2)) {
                QByteArray track2;
            ...
            } else if (typeMS && typeIC && typeRF)
                emit inserted(ECardType::MSICRF, cardData);
            else if (typeMS && typeIC)
                emit inserted(ECardType::MSIC, cardData);
            else if (typeIC)
                emit inserted(ECardType::IC, cardData);
            else if (typeMS)
                emit inserted(ECardType::MS, cardData);
            else if (typeRF)
...
bool CreatorReader::readMSData(QVariantMap &aData) {
    ...
    if (listingData.size() != 3) {
        toLog(LogLevel::Error,
              QString("%1: Wrong number of blocks = %2, need = 3").arg(mDeviceName).arg(listingData.size()));
        return false;
    }
