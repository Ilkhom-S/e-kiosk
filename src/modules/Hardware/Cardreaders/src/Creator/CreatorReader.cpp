...
//--------------------------------------------------------------------------------
TResult CreatorReader::processCommand(const QByteArray &aCommand,
                                      QByteArray *aAnswer,
...
bool CreatorReader::checkAnswer(const QByteArray &aCommand, const QByteArray &aAnswer) {
    ...
    if ((answerMarker != CCreatorReader::Markers::Error) &&
...
bool CreatorReader::getStatus(TStatusCodes &aStatusCodes) {
    ...
    if (!OK1 || !OK0 || (ST1 && (ST1 != CCreatorReader::CardPosition::ST1)) ||
        ...
        if (m_CardPosition == CCreatorReader::CardPosition::Inserted) {
            ...
            if (processCommand(CCreatorReader::Commands::IdentifyIC, &answer) &&
                (answer.size() >= 4)) {
                QByteArray buffer = answer.mid(2, 2);
                typeIC = buffer.toInt();
            ...
            if ((!typeMS && !typeIC && !typeRF) || cardData.isEmpty()) {
                emitStatusCode(CardReaderStatusCode::Warning::NeedReloading,
                               ECardReaderStatus::NeedReloading);
...
bool CreatorReader::readMSData(QVariantMap &aData) {
    ...
    if (processCommand(CCreatorReader::Commands::ReadMSData, &answer, true) ||
        (answer.size() < 13)) {
        return false;
    }
