...
bool ATGSMModem::getSiemensCellList(QString &aValue) {
    ...
    for (int i = 0; i < items.size(); i += 9) {
        ...
        if (items[i].trimmed().toInt() > 0 && (i + 6) < items.size()) {
            QStringList info;
            info << items[i].trimmed() << items[i + 1].trimmed() << QString::number(items[i + 3].toInt(0, 16)).trimmed()
                 << QString::number(items[i + 2].toInt(0, 16)).trimmed() << items[i + 6].trimmed();
...
bool ATGSMModem::takeMessages(TMessages &aMessages) {
    ...
    qStableSort(aMessages.begin(), aMessages.end(),
