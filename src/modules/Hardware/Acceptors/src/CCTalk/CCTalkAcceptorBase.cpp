...
template <class T> bool CCTalkAcceptorBase<T>::getStatus(TStatusCodes &aStatusCodes) {
    ...
    int statusCode = (fault && (fault == mModelData.fault)) ? BillAcceptorStatusCode::Busy::Unknown
        ...
        if (credit) {
            ...
            if (error == mModelData.error) {
                ...
                if (!localDescription.isEmpty()) {
                    ...
                    toLog(logLevel,
                          mDeviceName + QString(": %1 -> %2").arg(localDescription).arg(codeSpecification.description));
                }
