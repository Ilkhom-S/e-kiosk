...
public:
    DeviceCodeSpecification() {
        ...
        addStatus(2,
                  ...
                  "Inhibit state by communication time-out");
        addStatus(2, 6, DeviceStatusCode::Error::Unknown);
        addStatus(2,
                  5,
                  BillAcceptorStatusCode::OperationError::Communication,
                  "Communication error due to answer absence to IRQ request");
    ...
    bool isBusy(const QByteArray &aBuffer) {
        ...
        return std::find_if(specifications.begin(),
                            specifications.end(),
