...
bool SSPCashAcceptor::processReset() {
    ...
    if (!PollingExpector().wait<bool>(
            poll,
            [&]() -> bool {
                return !statusCodes.isEmpty() && !statusCodes.contains(DeviceStatusCode::OK::Initialization);
            },
            CSSP::ReadyWaiting)) {
...
void SSPCashAcceptor::processDeviceData() {
    ...
    if (!containsDeviceParameter(CDeviceData::Firmware) && processCommand(CSSP::Commands::GetSetupData, &answer) &&
        (answer.size() >= 5)) {
...
bool SSPCashAcceptor::loadParTable() {
    ...
    if (answer.size() < size) {
        toLog(LogLevel::Error,
              QString("%1: Failed to get setup data due to not enough data in the answer = %2, need %3.")
                  .arg(mDeviceName)
                  .arg(answer.size())
                  .arg(size));
        return false;
...
bool SSPCashAcceptor::performBaudRateChanging(bool aUp) {
    CSSP::EBaudRate::Enum upBaudRate =
        (mDeviceName == CSSP::Models::NV200Spectral) ? CSSP::EBaudRate::BR115200 : CSSP::EBaudRate::BR38400;
