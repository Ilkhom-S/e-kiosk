    ...
    class DeviceCodeSpecification : public CommonDeviceCodeSpecification {
      public:
        DeviceCodeSpecification() {
            ...
            addStatus('\xE1', DeviceStatusCode::OK::Initialization,
                      "Note cleared from front (power up with note in validator)");
            addStatus('\xE2', DeviceStatusCode::OK::Initialization,
            ...
            addStatus('\xEB', BillAcceptorStatusCode::Normal::Enabled,
                      "stacking completed"); // как бы финальный Stacked, выдается одновременно с последним Stacked-ом.
