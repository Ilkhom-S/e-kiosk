...
class CBaudRateData
    : public CSpecification<EBaudRate::Enum, SDK::Driver::IOPort::COM::EBaudRate::Enum> {
public:
    CBaudRateData() {
...
public:
    DeviceCodeSpecification() {
        ...
        addStatus('\xE1',
                  ...
                  "Note cleared from front (power up with note in validator)");
        addStatus('\xE2',
                  DeviceStatusCode::OK::Initialization,
                  "Note cleared into stacker (power up with note about in stacker)");
        ...
        addStatus('\xB5',
                  ...
                  "all note channels have been inhibited");
        ...
        addStatus('\xEB',
                  ...
                  "stacking completed"); // как бы финальный Stacked, выдается одновременно с
                                         // последним Stacked-ом.
        ...
        addStatus(
            '\xEA', BillAcceptorStatusCode::MechanicFailure::JammedInStacker, "safely jammed");
        addStatus(
            '\xE9', BillAcceptorStatusCode::MechanicFailure::JammedInValidator, "unsafely jammed");
    ...
    virtual void getSpecification(const QByteArray &aBuffer,
        ...
        if (aBuffer == EnabledStatus) {
            aSpecifications.insert(
                "", SDeviceCodeSpecification(BillAcceptorStatusCode::Normal::Enabled, ""));
        } else if (aBuffer == DisabledStatus) {
            aSpecifications.insert(
                "", SDeviceCodeSpecification(BillAcceptorStatusCode::Normal::Disabled, ""));
        } else {
            ...
            if (!aSpecifications.isEmpty()) {
                ...
                if ((statusCode == BillAcceptorStatusCode::BillOperation::Escrow) &&
                    (aBuffer.size() >= 2) && !aBuffer[1]) {
