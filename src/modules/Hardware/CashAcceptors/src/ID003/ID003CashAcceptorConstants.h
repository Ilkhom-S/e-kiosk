...
const char Stack1 = '\x41'; /// Уложить в стекер. Статус VendValid выставляется, когда купюра
                            /// пройдет рычаг соленоида стекера.
const char Stack2 = '\x42'; /// Уложить в стекер. Статус VendValid выставляется, когда полностью
                            /// зайдет в стекер (за этим следит сенсор).
...
public:
    DeviceCodeSpecification() {
        ...
        addStatus(
            '\xAB', BillAcceptorStatusCode::Busy::Unknown, "Cashbox not ready"); // TODO: ворнинг?
