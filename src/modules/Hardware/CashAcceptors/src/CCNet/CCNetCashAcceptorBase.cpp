...
bool CCNetCashAcceptorBase::waitNotBusyPowerUp()
{
    ...
    if (!PollingExpector().wait<bool>(poll, std::bind(&CCNetCashAcceptorBase::isNotBusyPowerUp, this),
                                      CCCNet::NotBusyPowerUpWaiting))
    {
        toLog(LogLevel::Error,
              mDeviceName + ": Failed to wait not busy and power-up status from the cash acceptor after reset command");
        return false;
    }
...
//---------------------------------------------------------------------------------
TResult CCNetCashAcceptorBase::performCommand(const QByteArray &aCommand, const QByteArray &aCommandData,
                                              QByteArray *aAnswer)
{
    QByteArray answer;
...
bool CCNetCashAcceptorBase::checkConnection(QByteArray &aAnswer)
{
    ...
    if (expector.wait<bool>(
            statusPoll, [&]() -> bool { return !result && !mLastAnswer.isEmpty(); }, CCCNet::FalseAutoDetectionWaiting))
    {
    ...
    auto isNotBusy = [&]() -> bool
    {
        return !mDeviceCodeBuffers.isEmpty() &&
               std::find_if(mDeviceCodeBuffers.begin(), mDeviceCodeBuffers.end(), [&](const QByteArray &aBuffer) -> bool
                            { return specification->isBusy(aBuffer); }) == mDeviceCodeBuffers.end();
    };
...
bool CCNetCashAcceptorBase::isConnected()
{
    ...
    if (!answer.simplified().isEmpty())
    {
        ...
        if (mFirmware)
        {
            ...
            {
                if (aSpecification.contains(mDeviceName) && aSpecification[mDeviceName].contains(mCurrencyCode) &&
            ...
            checkVersion(CCCNet::FimwareVersions.data(),
                         [&](int aVersion) -> bool { return (aVersion / 100 == base) && (aVersion % 100 > index); });
        }
    }
    else if (!isAutoDetecting())
    {
        ...
        auto it = std::find_if(modelData.data().begin(), modelData.data().end(),
...
bool CCNetCashAcceptorBase::enableMoneyAcceptingMode(bool aEnabled)
{
    ...
    bool isCoinsEnabled =
        std::find_if(mEscrowParTable.data().begin(), mEscrowParTable.data().end(), [&](const SPar &par) -> bool
                     { return (par.cashReceiver == ECashReceiver::CoinAcceptor) && par.enabled && !par.inhibit; }) !=
        mEscrowParTable.data().end();

    for (auto it = mEscrowParTable.data().begin(); it != mEscrowParTable.data().end(); ++it)
    {
        if (aEnabled && !it->inhibit &&
            (it->enabled || (isCoinsEnabled && (it->cashReceiver == ECashReceiver::CoinAcceptor))))
        {
...
bool CCNetCashAcceptorBase::performUpdateFirmware(const QByteArray &aBuffer)
{
    ...
    {
        toLog(LogLevel::Error,
              mDeviceName + ": Failed to wait any available status from the cash acceptor to updating");
        return false;
    }
    ...
    {
        toLog(LogLevel::Error,
              mDeviceName + ": Cashacceptor is not ready, status = " + ProtocolUtils::toHexLog(status));
        return false;
    }
    ...
    if (answerData.warningLevel == EWarningLevel::Warning)
    {
        toLog(LogLevel::Warning,
              mDeviceName + ": Firmware is updated correctly, but result = " + answerData.description);
    }
    else
    {
...
bool CCNetCashAcceptorBase::processBlockUpdating(uint aAddress, const QByteArray &aBuffer, int &aRepeat, int &aIndex)
{
    ...
    else if (++aRepeat < CCCNet::WriteFirmwareDataMaxRepeats)
    {
        toLog(
            LogLevel::Warning,
            mDeviceName +
                QString(": No answer for writing data block %1, trying attempt #%2").arg(aIndex + 1).arg(aRepeat + 1));
    ...
    {
        toLog(LogLevel::Error, mDeviceName + QString(": Failed to write data block %1%2")
                                                 .arg(aIndex + 1)
...
bool CCNetCashAcceptorBase::processUpdating(const QByteArray &aBuffer, int aSectionSize)
{
    ...
    toLog(LogLevel::Normal,
          mDeviceName +
              QString(": section size for updating the firmware = %1, buffer size = %2, amount of sections = %3")
                  .arg(aSectionSize)
                  .arg(aBuffer.size())
...
void CCNetCashAcceptorBase::cleanSpecificStatusCodes(TStatusCodes &aStatusCodes)
{
    if (mDeviceName == CCCNet::Models::CashcodeSL)
    {
        TStatusCodesHistoryList baseHistoryList = TStatusCodesHistoryList()
                                                  << (TStatusCodesHistory()
                                                      << BillAcceptorStatusCode::MechanicFailure::JammedInValidator)
                                                  << (TStatusCodesHistory()
                                                      << BillAcceptorStatusCode::MechanicFailure::JammedInValidator
                                                      << DeviceStatusCode::Error::NotAvailable);

        TStatusCodesHistory replaceableHistory = TStatusCodesHistory() << DeviceStatusCode::OK::Initialization
            ...
            if (aStatusCodes.contains(replaceableHistory[i]))
            {
                foreach (const TStatusCodesHistory &history, baseHistoryList)
                {
                    TStatusCodesHistory extraHistory =
                        TStatusCodesHistory() << history << BillAcceptorStatusCode::MechanicFailure::StickInExitChannel;
