...
/// Ожидание получения полла от купюроприёмника перед перепрошивкой.
...
public:
    Specifications() {
        ...
        append('\xE3',
               SData(EWarningLevel::Error, "Access denied (group or sequrity error) or CRC error"));
...
public:
    DeviceCodeSpecification() {
        ...
        addStatus('\x51',
                  BillAcceptorStatusCode::MechanicFailure::TransportMotor,
                  "Transport motor speed error");
        ...
        addStatus('\x54',
                  BillAcceptorStatusCode::MechanicFailure::Stacker,
                  "Initial stacker status is not correct");
        ...
        addStatus(
            '\x59', DeviceStatusCode::Error::Electronic, "Upper peripheral sensor board error");
        addStatus('\x5F', BillAcceptorStatusCode::SensorError::Dielectric);
        addStatus(
            '\x60', DeviceStatusCode::Error::Electronic, "Lower peripheral sensor board error");
        addStatus(
            '\xF4', BillAcceptorStatusCode::MechanicFailure::Stacker, "Unknown stacker failure");
        addStatus('\xFE', DeviceStatusCode::Error::CoverIsOpened);
    }

    // TODO: при рефакторинге - завязать на статус-код
    bool isBusy(const QByteArray &aBuffer) { return !aBuffer.isEmpty() && (aBuffer[0] == '\x1B'); }
    ...
    bool isPowerUp(const QByteArray &aBuffer) {
        return !aBuffer.isEmpty() &&
               ((aBuffer[0] == '\x10') || (aBuffer[0] == '\x11') || (aBuffer[0] == '\x12'));
    }
};
} // namespace CCCNet
