...
namespace CICTBase
{
	...
	public:
		DeviceCodeSpecification()
		{
			...
			appendStatus('\x26', DeviceStatusCode::Error::Unknown,
						 "Communication error - terminal hasn`t answer for powerUp status");
		...
		bool isPowerUp(const QByteArray& aBuffer) { return !aBuffer.isEmpty() && aBuffer.contains('\x80'); }
	};
} // namespace CICTBase
