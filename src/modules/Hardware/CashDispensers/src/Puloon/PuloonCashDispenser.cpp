...
//--------------------------------------------------------------------------------
TResult PuloonLCDM::processCommand(char aCommand,
    ...
    if (answerData.size() < CPuloonLCDM::MinAnswerDataSize) {
        toLog(LogLevel::Error,
              QString("%1: Answer data size = %2, need min %3")
                  .arg(mDeviceName)
                  .arg(answerData.size())
                  .arg(CPuloonLCDM::MinAnswerDataSize));
        return CommandResult::Answer;
    }
    ...
    if (commandInfo.error != -1) {
        if (answerData.size() < (commandInfo.error + 1)) {
            toLog(LogLevel::Error,
                  QString("%1: Answer data size = %2, need min %3 for error handling")
                      .arg(mDeviceName)
                      .arg(answerData.size())
                      .arg(commandInfo.error));
            return CommandResult::Answer;
        }

        char lastDeviceCode = answerData[commandInfo.error];
        SDeviceCodeSpecification deviceCodeData =
            CPuloonLCDM::DeviceCodeSpecification[lastDeviceCode];
        EWarningLevel::Enum warningLevel =
            mStatusCodesSpecification->value(deviceCodeData.statusCode).warningLevel;

        if (aCommand != CPuloonLCDM::Commands::GetStatus) {
            if (warningLevel == EWarningLevel::Error) {
                toLog(LogLevel::Error,
                      QString("%1: %2").arg(mDeviceName).arg(deviceCodeData.description));
                return CommandResult::Answer;
...
bool PuloonLCDM::getStatus(TStatusCodes &aStatusCodes) {
    ...
    SDeviceCodeSpecification statusSpecification =
        CPuloonLCDM::DeviceCodeSpecification[deviceStatusCodes[0]];
    aStatusCodes.insert(statusSpecification.statusCode);

    TDeviceCodeSpecifications sensorSpecifications;
    CPuloonLCDM::SensorSpecification.getSpecification(deviceStatusCodes.mid(1),
...
void PuloonLCDM::logStatusChanging(const SDeviceCodeSpecification &aStatusSpecification,
    ...
    foreach (const SDeviceCodeSpecification &specification, aSensorSpecifications) {
        ...
        warningLevel = qMax(
            warningLevel, mStatusCodesSpecification->value(specification.statusCode).warningLevel);
    }
...
void PuloonLCDM::performDispense(int aUnit, int aItems) {
    ...
    for (int i = 0; i < starts; ++i) {
        ...
        if (result || (result == CommandResult::Answer)) {
            if ((answer.size() > 4) && ((answer[4] == CPuloonLCDM::UpperUnitEmpty) ||
                                        (answer[4] == CPuloonLCDM::LowerUnitEmpty))) {
                emitUnitEmpty(aUnit);
            }
            ...
            if (rejectedItems) {
                toLog(LogLevel::Warning,
                      mDeviceName + QString(": emit rejected %1 notes from %2 unit")
                                        .arg(rejectedItems)
                                        .arg(aUnit));
