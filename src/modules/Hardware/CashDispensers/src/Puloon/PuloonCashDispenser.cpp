...
PuloonLCDM::PuloonLCDM() {
    ...
    // данные устройства
    m_DeviceName = CPuloonLCDM::Models::Data.getDefault().name;
}
...
TResult PuloonLCDM::processCommand(char aCommand,
    ...
    if (commandInfo.error != -1) {
        ...
        EWarningLevel::Enum warningLevel =
            m_StatusCodesSpecification->value(deviceCodeData.statusCode).warningLevel;
...
void PuloonLCDM::logStatusChanging(const SDeviceCodeSpecification &aStatusSpecification,
    ...
    foreach (const SDeviceCodeSpecification &specification, aSensorSpecifications) {
        ...
        warningLevel = qMax(
            warningLevel, m_StatusCodesSpecification->value(specification.statusCode).warningLevel);
    }
