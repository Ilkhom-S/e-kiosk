...
/// Максимальный таймаут для действий купюроприёмника по осознанию пустоты кассеты, [мс].
...
public:
    CDeviceCodeSpecification() {
        ...
        addStatus('\x3A',
                  ...
                  "Counting error (between CHK 3,4 sensor and DIV sensor)");
        addStatus('\x3B', DeviceStatusCode::Error::Unknown, "Note request error");
        addStatus('\x3C',
                  DeviceStatusCode::Error::Unknown,
                  "Counting Error(between DIV Sensor and EJT Sensor)");
        addStatus('\x3D',
                  DeviceStatusCode::Error::Unknown,
                  "Counting Error(between EJT Sensor and EXIT Sensor)");
        ...
        addStatus(
            '\x49', DeviceStatusCode::Error::Unknown, "Diverter solenoid or SOL Sensor error");
        addStatus('\x4A', DeviceStatusCode::Error::Unknown, "SOL Sensor error");
        addStatus('\x4B',
                  DeviceStatusCode::Error::Unknown,
                  "Different quantity of the detected biil between check sensor and divert sensor");
        ...
        addStatus('\x4F',
                  DeviceStatusCode::Error::Unknown,
                  "Bill being detected by a sensor in another cash box");
        addStatus(
            '\x50', DeviceStatusCode::OK::Unknown, "Timeout between CHK sensor and DIV Sensor)");
    }
};
...
public:
    CRejectingSpecification() {
        ...
        append('\x03',
               "The banknote are not normally passing on the check sensor because of fast "
               "consequential pickup");
        append('\x00',
               "The banknote are not normally passing on the check sensor because of fast "
               "consequential pickup");
