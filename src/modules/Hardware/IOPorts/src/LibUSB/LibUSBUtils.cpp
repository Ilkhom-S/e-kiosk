...
namespace LibUSBUtils {
    ...
    void getDeviceDescriptorData(libusb_device *aDevice, const libusb_device_descriptor &aDeviceDescriptor,
        ...
        QString deviceDescription = USB::ClassData.getData(
            aDeviceDescriptor.bDeviceClass, aDeviceDescriptor.bDeviceSubClass, aDeviceDescriptor.bDeviceProtocol);
        ...
        if (getDataFromMap(data, DeviceUSBData::ConfigData) &&
            ...
            for (int i = 0; i < endpointAmounts; ++i) {
                ...
                if (getDataFromMap(data, InterfaceData::EndpointData, false)) {
                    ...
                    CLibUSB::SEndPoint &deviceEP = (EP[0] & LIBUSB_ENDPOINT_DIR_MASK) ? aDeviceProperties.deviceToHost
                                                                                      : aDeviceProperties.hostToDevice;
                    deviceEP = CLibUSB::SEndPoint(transferType, EP[0], maxPacketSize, pollingInterval);
                    deviceEP.processIO = (transferType == LIBUSB_TRANSFER_TYPE_BULK) ? &libusb_bulk_transfer
    ...
    TLibUSBDataList getConfigData(libusb_device *aDevice, const libusb_device_descriptor &aDeviceDescriptor) {
        ...
        for (uint8_t i = 0; i < aDeviceDescriptor.bNumConfigurations; ++i) {
            ...
            if (LIB_USB_CALL_DEBUG(libusb_get_config_descriptor, aDevice, i, &config)) {
                ...
                // TODO: уточнить MaxPower по поводу единиц измерения. Сила тока тут обозначается как power, это не
                // мощность.
                /*
                // Extra descriptors. If libusb encounters unknown configuration descriptors, it will store them here,
                should you wish to parse them. const unsigned char *extra;

                // Length of the extra descriptors, in bytes. Must be non-negative.
                int extra_length;

                // Maximum power consumption of the USB device from this bus in this configuration when the device is
                fully operation. Expressed
    ...
    TLibUSBDataList getInterfaceData(libusb_config_descriptor *config) {
        ...
        for (int i = 0; i < config->bNumInterfaces; ++i) {
            ...
            for (int j = 0; j < configInterface.num_altsetting; ++j) {
                ...
                QString interfaceDescription = USB::ClassData.getData(
                ...
                // TODO: добавить параметры
                /*
                // Extra descriptors. If libusb encounters unknown interface descriptors, it will store them here,
                should you wish to parse them. const unsigned char *extra;
