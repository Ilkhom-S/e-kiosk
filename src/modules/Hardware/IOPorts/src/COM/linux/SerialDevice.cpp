...
SerialDeviceSettings::SerialDeviceSettings()
    : baudRate(CBaudRateType::Baud115200), dataBits(CDataBitsType::Data8), parity(CParityType::None),
      stopBits(CStopBitsType::Stop1), flowControl(CFlowType::Hardware), timeout(500) {
}
...
bool SerialDevice::doSetDataBits(CDataBitsType::Enum aDataBits) {
    const bool status = (m_settings.stopBits == CStopBitsType::Stop2 && aDataBits == CDataBitsType::Data5) ||
                        (m_settings.parity == CParityType::Space && aDataBits == CDataBitsType::Data8);
    if (status) {
        return false;
    } else {
...
bool SerialDevice::doSetParity(CParityType::Enum aParity) {
    const bool status = (aParity == CParityType::Space && m_settings.dataBits == CDataBitsType::Data8);
...
bool SerialDevice::doSetStopBits(CStopBitsType::Enum aStopBits) {
    const bool status = (m_settings.dataBits == CDataBitsType::Data5 && aStopBits == CStopBitsType::Stop2);
