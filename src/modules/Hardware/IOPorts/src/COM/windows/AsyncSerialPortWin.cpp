...
bool AsyncSerialPortWin::open() {
    ...
    data.changePerformingTimeout = std::bind(&AsyncSerialPortWin::changePerformingTimeout, this, _1, _2, _3);
...
bool AsyncSerialPortWin::write(const QByteArray &aData) {
    ...
    if (result || (mLastError == ERROR_IO_PENDING) || (mLastError == ERROR_MORE_DATA)) {
        ...
        DWORD expectedTimeout =
            qMax(DWORD(IIOPort::DefaultWriteTimeout), DWORD(requiredTime * CAsyncSerialPortWin::KSafety));
        result =
                ...
                if (index != -1) {
                    EPortTypes::Enum portType =
                        isMatched(it->data, CAsyncSerialPortWin::Tags::Virtual())
                            ...
                            : (isMatched(it->data, CAsyncSerialPortWin::Tags::Emulator()) ? EPortTypes::COMEmulator
                                                                                          : EPortTypes::Unknown);
