...
QStringList SystemDeviceUtils::enumerateCOMPorts() {
    ...
    foreach (auto port, data) {
        if (regExp.match(" " + port + " ").capturedStart() != -1) {
            result << // TODO: // TODO: // TODO: // TODO: regExp.cap(0) needs manual migration to
                      // match.captured(0) needs manual migration to match.captured(0) needs manual
                      // migration to match.captured(0) needs manual migration to
                      // match.captured(0).simplified();
        }
    }
...
QString SystemDeviceUtils::getRelevantPortName(const QString &aPortName,
    ...
    auto getRegexData = [](const QString &aData) -> QString {
        return QString("%2%1%2")
            .arg(getScreenedData(aData.simplified().toLower()))
            .arg("[^a-z0-9]+");
    };
    ...
    auto it =
        std::find_if(aPortNames.begin(), aPortNames.end(), [&](const QString &portName) -> bool {
...
QString SystemDeviceUtils::getRelevantWinProperty(const QString &aWinProperty,
    ...
    auto it = std::find_if(
        aWinProperties.begin(), aWinProperties.end(), [&](const QString &aProperty) -> bool {
            return (winProperty == getData(aProperty));
        });
    ...
    foreach (const QString &key, allKeys) {
        ...
        if (!getRelevantPortName(pathPropertyValue, portNames).isEmpty()) {
            ...
            if (properties.contains(CRegistrySerialPort::PathProperty)) {
                result[pathPropertyValue].path = QString("%1\\%2")
                                                     .arg(CRegistrySerialPort::InitialPath)
                                                     .arg(group.replace("/", "\\"));
...
QString SystemDeviceUtils::getProperty(const HDEVINFO &aHDevInfo,
    ...
    while (!SetupDiGetDeviceRegistryProperty(
        aHDevInfo, &aDeviceInfoData, aProperty, &DataT, (PBYTE)buffer, buffersize, &buffersize)) {
    ...
    for (DWORD i = 0; OK; i++) {
        ...
        if (OK) {
            ...
            SetupDiGetDeviceInterfaceDetail(
                deviceInfo, &interfaceData, NULL, 0, &detailedDataSize, &deviceData);
            ...
            if (!SetupDiGetDeviceInterfaceDetail(deviceInfo,
                                                 ...
                                                 NULL,
                                                 &deviceData)) {
                qCritical("SetupDiGetDeviceInterfaceDetail failed. Error code: %ld",
                          GetLastError());
                return false;
            }
            ...
            if (!aQuick) {
                ...
                foreach (const QString &winProperty, deviceWinProperties.data().values()) {
                    aDeviceProperties[pathProperty].data.insert(
                        winProperty,
                        getProperty(deviceInfo, deviceData, deviceWinProperties.key(winProperty)));
                }
            }
        ...
        if (!portName.isEmpty()) {
            ...
            for (auto jt = it->data.begin(); jt != it->data.end(); ++jt) {
                ...
                if (propertyName.isEmpty()) {
                    ...
                    bool contains =
                        propertyData.contains(registryData) || registryData.contains(propertyData);
    ...
    foreach (const QString &key, aKeys) {
        if (maxNameSize == CRegistrySerialPort::UuidSize) {
            ...
            if (CRegistrySerialPort::MaxUuidSize > maxNameSize) {
                ...
                result += QString("\n%1 %2 %3")
                              .arg(QString(line1Size, '-'))
                              .arg(key)
                              .arg(QString(line2Size, '-'));
