...
bool TCPPort::performOpen() {
    if (!mSocket) {
        ...
        connect(mSocket.data(), SIGNAL(stateChanged(QAbstractSocket::SocketState)),
    ...
    if (QRegExp(CTCPPort::AddressMask).indexIn(IP) == -1) {
        writeLog(
            QString("Failed to open the TCP socket with IP = %1, need like %2").arg(IP).arg(CTCPPort::AddressMaskLog));
        return false;
    }
...
bool TCPPort::performClose() {
    ...
    if (!mSocket->waitForDisconnected(CTCPPort::CloseningTimeout)) {
        toLog(LogLevel::Error,
              QString("Failed to close the TCP socket due to timeout = %1 is expired").arg(CTCPPort::CloseningTimeout));
        return false;
    }
