...
void OPOSMetrologicScanner::initializeResources()
{
    ...
    {
        connect(mDriver.data(), SIGNAL(signal(const QString &, int, void *)),
                SLOT(onGotData(const QString &, int, void *)), Qt::UniqueConnection);
    }
}
...
bool OPOSMetrologicScanner::enable(bool aReady)
{
    ...
    {
        toLog(LogLevel::Error, QString("%1: Cannot set possibility of data receiving to %2")
                                   .arg(mDeviceName)
    ...
    else
    {
        ...
        if (BOOL_CALL_OPOS(DataEventEnabled) != aReady)
        {
            toLog(
                LogLevel::Normal,
                QString("%1: set possibility of data receiving to %2").arg(mDeviceName).arg(aReady ? "true" : "false"));
        ...
        if (BOOL_CALL_OPOS(DecodeData) != aReady)
        {
            toLog(LogLevel::Normal,
                  QString("%1: set data decoding to %2").arg(mDeviceName).arg(aReady ? "true" : "false"));
