...
bool AV268::getStatus(TStatusCodes &aStatusCodes)
{
    ...
    if (mModelType != Simple)
    {
        ...
        if (((DIPSettings & CAV268::DIPSwitches::HalfHeight) && (~DIPSettings & CAV268::DIPSwitches::DoubleHeight)) ||
        ...
        if (processCommand(CAV268::Commands::GetPresenterStatus, &answer) && (answer.size() <= 10) &&
            (answer.size() >= 2) && answer.startsWith(CAV268::Answers::GetPresenterStatus))
        {
