...
template <class T> bool CustomVKP80<T>::printReceipt(const Tags::TLexemeReceipt &aLexemeReceipt) {
    if (mOverflow) {
        ...
        if (!expector.wait([&]() -> bool { return getAnswer(data, 10) && data.contains(ASCII::XOn); },
    ...
    if (!getConfigParameter(CHardware::Printer::OutCall).toBool()) {
        auto condition = [&]() -> bool {
            ...
            return getStatus(statusCodes) && !statusCodes.contains(PrinterStatusCode::OK::MotorMotion);
        };
