...
{
	return mIOPort->write(CCustomVKP80III::Command::GetModelId) && getAnswer(aAnswer, CPOSPrinter::Timeouts::Info) &&
...
char CustomVKP80III<T>::parseModelId(QByteArray& aAnswer)
{
	...
	{
		toLog(LogLevel::Error, QString("%1: Wrong answer {%2}, need {%3}")
								   .arg(mDeviceName)
...
bool CustomVKP80III<T>::receiptProcessing()
{
	...
	QString ejectorActionParameter =
		getConfigParameter(CHardware::Printer::Settings::PreviousAndNotTakenReceipts).toString();
	char ejectorAction = (ejectorActionParameter == CHardware::Printer::Values::Retract) ? CCustomVKP80III::Retraction
