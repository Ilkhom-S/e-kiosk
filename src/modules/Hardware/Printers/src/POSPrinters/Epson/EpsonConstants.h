...
namespace CEpsonEUT400
{
    namespace Command
    {
        ...
        const QByteArray GetMemorySwitch =
            QByteArray::fromRawData("\x1D\x28\x45\x02\x00\x04", 6); /// Получение memory-switch-а, основная часть
        const QByteArray SetMemorySwitch =
            QByteArray::fromRawData("\x1D\x28\x45\x0A\x00\x03", 6); /// Установка memory-switch-а, основная часть
    } // namespace Command
