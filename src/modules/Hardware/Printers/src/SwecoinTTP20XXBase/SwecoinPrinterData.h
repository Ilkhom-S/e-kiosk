...
const int MaxReadIdTimeout =
    10 * 1000; /// Максимальный таймаут для чтения ответа на команду получения Id.
const int MinReadIdTimeout =
    2 * 1000; /// Минимальный таймаут для чтения данных на команду получения Id.

extern const char UnknownModelName[]; /// Имя принтера по умолчанию.

/// Команды.
namespace Commands {
extern const char Initialize[];       /// Инициализация.
extern const char SetParameter[]; /// Часть команды установки параметра.
extern const char GetData[];          /// Часть команды получения данных об устройстве.
extern const char GetModelId[];   /// Получение Id устройства.
extern const char GetStatus[];    /// Статус.
extern const char GetPaperNearEndData[];
extern const char GetSensorData[];
} // namespace Commands
...
public:
    CStatuses() {
        // TODO: влияет ли BM error на печать, если BM сенсор включен? Разобраться с index error -
        // что это и зачем это.
...
public:
    CDeviceParameters() {
        ...
        auto processDigit = [&](const QByteArray &aAnswer) -> QString {
            return processData(aAnswer, 3);
        };
