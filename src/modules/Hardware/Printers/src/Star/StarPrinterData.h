...
namespace CSTAR {
/// Минимальная "версия" прошивки (число, характеризующее прошивку а Header 2 в ответе на запрос
/// статуса).
...
inline QByteArray getMemorySwitch(char aNumber) {
    return ProtocolUtils::getBufferFrom_String(
...
inline QByteArray setMemorySwitch(char aNumber, ushort aValue) {
    return ProtocolUtils::getBufferFrom_String(QString("1B 1D 23 2C %1 %2 0A 00")
                                                  .arg(int2ASCII(aNumber, sizeof(aNumber)))
