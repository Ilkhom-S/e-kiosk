...
namespace CShtrihFR
{
	namespace Models
	{
		...
		CData::CData()
		{
			...
			addNew(-1, "PAYONLINE", "PayOnline-01-FA", true, true, 1000,
				   0); // определяется размером SD-карты, если она есть
			addNew(-2, "PAY VKP-80K-ФА", "PayVKP-80K-FA", true, true, 1000,
				   0); // определяется размером SD-карты, если она есть
						   ...
						   int aZReportQuantity, int aFeed, const QDate& aDate, int aLinePrintingTimeout)
		{
			...
			append(aNumber, SModelData(QString::fromUtf8(aId), aName, aVerified, aEjector, aZReportQuantity, aFeed,
									   FWDate, 0, aLinePrintingTimeout));
		}
