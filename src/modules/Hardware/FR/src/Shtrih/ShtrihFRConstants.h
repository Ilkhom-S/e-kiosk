...
/// Налоги на закрытии чека по количеству налоговых  групп. Фиктивные, т.к. используются налоги на
/// позицию.
...
const ushort ControlLeverNotDropped =
    0x0100; /// Рычаг термоголовки контрольной ленты (0 – поднят, 1 – опущен).
const ushort PaperLeverNotDropped =
    0x0200;                           /// Рычаг термоголовки чековой ленты (0 – поднят, 1 – опущен).
...
/// Режим работы ККМ. Приходит в ответе на запрос FCh (получить тип устройства). Изменяться не
/// может.
namespace Types {
const int NoType = -1;
...
public:
    ...
    SData getInfo(const TRegisterId &aRegister) {
        ...
        else if (aRegister.second == ERegisterType::Operational)
            typeDescription = "Operational";
