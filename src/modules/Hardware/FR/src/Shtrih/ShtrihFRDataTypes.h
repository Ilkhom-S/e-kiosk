...
struct SModelData {
    ...
    SModelData(const QString &aId,
        ...
        : id(aId), name(aName), ejector(aEjector), ZBufferSize(aZBufferSize), verified(aVerified),
          feed(aFeed), date(aDate), build(aBuild), linePrintingTimeout(aLinePrintingTimeout) {}

    SModelData()
        : verified(false), ejector(false), ZBufferSize(0), feed(0), date(QDate::currentDate()),
          build(0), linePrintingTimeout(0) {}
};
...
#define ADD_SHTRIH_FIELD(aIndex, aName)                                                            \
    ...
    if ((aData.size() > aIndex) && (aName.field == NA))                                            \
        ...
        ADD_SHTRIH_FIELD(
            2, printOnControlTape); // Печать текстовых строк на ленте операционного журнала
        ...
        ADD_SHTRIH_FIELD(
            9,
            weightSensorEnable); // Использование весовых датчиков при контроле отсутствия бумаги
