...
void PayOnline<T>::setDeviceConfiguration(const QVariantMap& aConfiguration)
{
	...
	if (aConfiguration.contains(CHardware::FR::PrinterModel) && (printerModel != CPayPrinters::Default) &&
		!isNotPrinting())
	{
...
bool PayOnline<T>::setNotPrintDocument(bool aEnabled, bool /*aZReport*/)
{
	...
	auto it = std::find_if(CPayPrinters::Models.data().begin(), CPayPrinters::Models.data().end(),
	...
	uchar printerModelId = it.key();
	QByteArray data;
	bool result = getFRParameter(CShtrihOnlineFR::FRParameters::PrinterModel, data) && !data.isEmpty() &&
				  (uchar(data[0]) == printerModelId);
