...
const SData LastFiscalDT =
    SData(5,
          TAnswerTypes() << EAnswerTypes::Date << EAnswerTypes::Time,
          "date and time of last fiscal document"); /// Дата и время последней фискальной операции.
const SData LastRegDate =
    SData(6, EAnswerTypes::Date, "last registration date"); /// Дата регистрации/перерегистрации.
const SData TotalCash =
    SData(7, EAnswerTypes::Double, "total sum in cash"); /// Сумма наличных в денежном ящике.
const SData NextFiscalNumber =
    SData(8, EAnswerTypes::FInt, "number of next fiscal document"); /// Номер следующего документа.
const SData TotalPaySum =
    SData(12, EAnswerTypes::Double, "total sum of payments");       /// Нарастающий итог.
...
const SData FFDFS = SData(
    16, TAnswerTypes() << EAnswerTypes::Int << EAnswerTypes::Int, "FFD FS"); /// Код версии ФФД ФН.
...
const SData NotPrintDocument = SData(
    2, EAnswerTypes::Int, "cheque parameters: not print document", 0); /// Непечать  документов.
const SData PrintingOnClose = SData(2,
                                    EAnswerTypes::Int,
                                    "cheque parameters: print document on closing",
                                    6); /// Печать документов по закрытию чека.
...
const char GetLastFiscalizationData =
    '\xC7'; /// Получить данные последней фискализации из архива ФН.
} // namespace Commands
...
public:
    CData() {
        ...
        add(GetPrinterStatus,
            TAnswerTypes() << Int,
            7 * 1000); // TODO: если нет бумаги - запрос статуса идет 5000..5500 мс
        add(GetFRStatus, TAnswerTypes() << Int << Int);
        add(GetFRParameter, TAnswerTypes() << Unknown);
        add(GetFRDateTime, TAnswerTypes() << Date << Time);
        add(GetFSStatus,
            TAnswerTypes() << Int << Int << Int << Int << Int << Date << Time << String << Int
                           << String << Int << Date << Int << Int);
        add(GetOFDStatus, TAnswerTypes() << Int << Int << Int << Int << Date << Time, 3 * 1000);
        add(GetFiscalizationTotal,
            TAnswerTypes() << Date << Time << FInt << FInt << FInt << Int << Int << FInt << FInt);
...
public:
    Data() {
        ...
        add('\x0B',
            "Разница во времени, внутренних часов и указанной в команде начала работы, больше 8 "
            "минут");
