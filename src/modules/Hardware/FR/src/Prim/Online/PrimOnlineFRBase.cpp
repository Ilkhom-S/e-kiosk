...
bool PrimOnlineFRBase::updateParameters()
{
	...
	if (mFFDFR > EFFD::F10)
	{
		...
		bool canCheckAgentFlags =
			(mLastError != CPrimOnlineFR::Errors::NoRequiedData) && !FSVersion.contains(CPrimOnlineFR::FSNoAgentFlags);
		uchar FFData;
...
bool PrimOnlineFRBase::checkPayTypes()
{
	...
	for (int i = 0; i < CPrimOnlineFR::PayTypeAmount; ++i)
	{
		...
		if (!processCommand(CPrimOnlineFR::Commands::GetPayTypeData, CPrimFR::TData() << int2String(i).toLatin1(),
							&data))
		{
			return false;
		}
...
bool PrimOnlineFRBase::getRegTLVData(int aField)
{
	...
	toLog(LogLevel::Normal,
		  mDeviceName + QString(": Add %1 = %2 to config data").arg(mFFData.getTextLog(aField)).arg(FFData));
...
bool PrimOnlineFRBase::getStatus(TStatusCodes& aStatusCodes)
{
	...
	TResult result = processCommand(CPrimOnlineFR::Commands::GetOFDNotSentCount, 5,
									"OFD not sent fiscal documents count", OFDNotSentCount);
...
void PrimOnlineFRBase::processDeviceData()
{
	...
	if (processCommand(CPrimFR::Commands::GetStatus, &data))
	{
		...
		if (data.size() > 8)
		{
			...
			if (dateTime.isValid())
			{
				setDeviceParameter(CDeviceData::FR::OpeningDate, dateTime.toString(CFR::DateTimeLogFormat),
...
bool PrimOnlineFRBase::performFiscal(const QStringList& aReceipt, const SPaymentData& aPaymentData, quint32* aFDNumber)
{
	...
	if (aFDNumber &&
		!processCommand(CPrimOnlineFR::Commands::GetFSStatus, 12, "last document number in FS", *aFDNumber))
	{
		aFDNumber = 0;
	}
...
//--------------------------------------------------------------------------------
bool PrimOnlineFRBase::getFiscalFields(quint32 aFDNumber, TFiscalPaymentData& aFPData,
									   TComplexFiscalPaymentData& aPSData)
{
	CPrimFR::TData commandData =
		CPrimFR::TData() << QString("%1").arg(qToBigEndian(aFDNumber), 8, 16, QLatin1Char(ASCII::Zero)).toLatin1()
	...
	for (int i = 5; i < answer.size(); ++i)
	{
		...
		else
		{
			toLog(LogLevel::Warning,
				  QString("%1: Failed to add fiscal field %2 due to it is unknown").arg(mDeviceName).arg(TLV.field));
		}
	}
...
void PrimOnlineFRBase::setFPData(TFiscalPaymentData& aFPData, const CFR::STLV& aTLV)
{
	...
	else
	{
		...
		{
			toLog(LogLevel::Warning,
				  mDeviceName + QString(": Failed to parse money data %1 for field %2").arg(data).arg(aTLV.field));
		}
	}
}

//--------------------------------------------------------------------------------
void PrimOnlineFRBase::setFiscalData(CPrimFR::TData& aCommandData, CPrimFR::TDataList& aAdditionalAFDData,
									 const SPaymentData& aPaymentData, int aReceiptSize)
{
	...
	auto getAmountY = [&](const TSum& aSum) -> int
	{
		return getDataY(getAmountData(aSum).size());
	};
	...
	aAdditionalAFDData << addArbitraryFieldToBuffer(startX + 1, 1, serialNumber, mAFDFont)
					   << addArbitraryFieldToBuffer(startX + 1, documentNumberY - (documentNumber.size() + 1),
													documentNumber, mAFDFont)
	...
	for (int i = 0; i < aPaymentData.unitDataList.size(); ++i)
	{
		...
		QStringList data =
			QStringList() << unitData.name										// 1059 (товар)
...
#define ADD_AFD_TAG(aX, aY, aField, ...) \
	aAdditionalAFDData << addFiscalField(lastX + aX, aY, mAFDFont, CFR::FiscalFields::aField * 100 + i + 1, __VA_ARGS__)
#define ADD_AFD_TAG_MULTI(aX, aY, aField, aData) \
	...
	lastX += int(newLine)
		...
		{
			aAdditionalAFDData << addFiscalField(lastX + 1, 1, mAFDFont, CFR::FiscalFields::UnitName,
	...
	if (aPaymentData.taxSystem != ETaxSystems::None)
	{
		...
		aAdditionalAFDData << addFiscalField(lastX + 1, taxSystemY, mAFDFont, CFR::FiscalFields::TaxSystem,
											 int2String(index));
	}

#define ADD_PRIM_FF_DATA(aField, aData) \
	aAdditionalAFDData.append(addFiscalField(++lastX, 1, mAFDFont, CFR::FiscalFields::aField, aData))
#define ADD_PRIM_FF(aField) ADD_PRIM_FF_DATA(aField, mFFEngine.getConfigParameter(CFiscalSDK::aField).toString())
	...
	if (!userContact.isEmpty())
	{
		...
		{
			toLog(LogLevel::Warning,
				  mDeviceName + QString(": Failed to transmit %1 due to FFD version = %2, need %3 min")
...
TResult PrimOnlineFRBase::doZReport(bool aAuto)
{
	CPrimFR::TData commandData = CPrimFR::TData()
								 << CPrimFR::OperatorID << "" << ""; // + сообщение для ОФД и доп. реквизит
