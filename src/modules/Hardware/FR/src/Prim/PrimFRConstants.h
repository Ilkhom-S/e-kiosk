...
namespace CPrimFR
{
	...
	namespace Commands
	{
		...
		/// Дата и время в команде.
		const QSet<char> DateTimeIn = QSet<char>() << OpenSession << OpenFRSession << GetStatus << CancelDocument
	...
	namespace Errors
	{
		...
		public:
			Data()
			{
				...
				add('\x1B', "Неверный пароль доступа к ФП. Необходимо выполнить команду, например, \"Фискальный "
				...
				add('\x31', "Дата последней существующей записи в фискальной памяти позже, чем дата операции, которую "
							"пытались выполнить");
				add('\x32', "Необходима инициализация фискальной памяти");
				add('\x33', "Заполнена вся фискальная память. Блокируются все команды, кроме снятия фискальных отчетов "
		...
		public:
			ExtraData()
			{
				...
				mEKLZIncorrectStateReason.append('\x40',
												 "Ошибка, связанная с ЭКЛЗ (подробности в трассовой информации)");
