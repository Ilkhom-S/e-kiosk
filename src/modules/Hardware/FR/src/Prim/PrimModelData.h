...
inline TModels CommonModels() {
    return TModels() << Models::Unknown << Models::PRIM_07K << Models::PRIM_08TK
                     << Models::PRIM_09TK << Models::PRIM_88TK << Models::AZIMUT_TMU950PK;
}
...
struct SModelParameters {
    ...
    SModelParameters(const QString &aName,
                     bool aVerified,
                     bool aHasBuffer,
                     int aFeed,
                     const TStatusData &aStatusData)
        : name(aName), hasBuffer(aHasBuffer), feed(aFeed), statusData(aStatusData),
          verified(aVerified) {}
};
...
public:
    CModelData() {
        ...
        // Не онлайн
        data().insert(Models::PRIM_07K,
                      SModelParameters("Iskra PRIM-07K", true, false, 8, statusDataControlPaper));
        data().insert(Models::PRIM_08TK,
                      SModelParameters("Iskra PRIM-08TK", true, true, 4, statusData));
        data().insert(Models::PRIM_09TK,
                      SModelParameters("Iskra PRIM-09TK", false, false, 4, statusData));
        data().insert(Models::PRIM_88TK,
                      SModelParameters("Iskra PRIM-88TK", true, false, 4, statusData));
        data().insert(Models::PRIM_21K_01,
                      SModelParameters("Iskra PRIM-21K 01", true, true, 5, statusData21V0102));
        data().insert(Models::PRIM_21K_02,
                      SModelParameters("Iskra PRIM-21K 02", true, true, 5, statusData21V0102));
        data().insert(Models::PRIM_21K_03,
                      SModelParameters("Iskra PRIM-21K 03", true, true, 0, statusData21V03));

        data().insert(
            Models::AZIMUT_TMU950PK,
            SModelParameters("Iskra AZIMUT TM-U950PK", false, false, 4, statusDataControlPaper));

        // Онлайн
        data().insert(Models::PRIM_06F,
                      SModelParameters("Iskra PRIM 06-F", true, true, 4, statusData));
        data().insert(Models::PRIM_08F,
                      SModelParameters("Iskra PRIM 08-F", true, true, 4, statusData));
        data().insert(Models::PRIM_21FA_Epson,
                      SModelParameters("Iskra PRIM 21-FA", true, true, 5, statusData21V0102));
        data().insert(Models::PRIM_21FA_Custom,
                      SModelParameters("Iskra PRIM 21-FA", true, true, 0, statusData21V03));

        // default
        data().insert(Models::Unknown,
                      SModelParameters(DefaultModelName, false, false, 8, statusData));
        data().insert(Models::OnlineUnknown,
                      SModelParameters(DefaultOnlineModelName, false, true, 8, statusData));
