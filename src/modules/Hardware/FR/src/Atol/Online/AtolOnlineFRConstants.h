...
/// Максимальное количество повторов при ожидании оживания ФР при получении фискальных данных после
/// закрытия чека.
...
/// конкретную позицию.
const char Saling = CAtolFR::FiscalFlags::ExecutionMode | CAtolFR::FiscalFlags::CashChecking |
                    CAtolFR::FiscalFlags::TaxForPosition;
} // namespace FiscalFlags
...
#define PROCESS_ATOL_FD_DATA(aCommand, ...)                                                        \
    processDataWaiting([&]() -> TResult {                                                          \
        return processCommand(CAtolOnlineFR::Commands::FS::aCommand, __VA_ARGS__);                 \
    })
...
public:
    Data() {
        ...
        add('\x0D',
            "Сторно по коду невозможно (в чеке зарегистрировано меньшее количество товаров с "
            "указанным кодом)");
        ...
        add('\xB3',
            "Невозможно закрыть чек данным типом оплаты (в чеке присутствуют операции без контроля "
            "наличных)");
        ...
        add('\xDF',
            "Сумма налога больше суммы регистраций по чеку и/или итога или больше суммы "
            "регистрации");
...
public:
    CExtraData() {
        ...
        append(0x0102,
               "Недопустимо передавать более 10 регистраций при наличии в чеке кода товарной "
        ...
        append(
            0x0202,
            "Программирование реквизитов запрещено в данном режиме работы ФН (ФН отсутствует или "
            "закрыт архив)");
        ...
        append(0x0208,
               "Невозможно записать реквизит чека, начато формирование позиции (была подана "
               "команда EAh)");
        ...
        append(0x0301,
               "Печать фискального отчета прервана, необходим повтор команды печати отчета (есть "
               "доп. данные)");
