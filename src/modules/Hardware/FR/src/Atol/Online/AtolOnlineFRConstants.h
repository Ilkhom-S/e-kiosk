...
#define PROCESS_ATOL_FD_DATA(aCommand, ...)                                                                            \
    processDataWaiting([&]() -> TResult { return processCommand(CAtolOnlineFR::Commands::FS::aCommand, __VA_ARGS__); })
        ...
        class CExtraData : public CDescription<ushort>
        {
          public:
            CExtraData()
            {
                ...
                append(0x0202, "Программирование реквизитов запрещено в данном режиме работы ФН (ФН отсутствует или "
                               "закрыт архив)");
                ...
                append(0x0208,
                       "Невозможно записать реквизит чека, начато формирование позиции (была подана команда EAh)");
                ...
                append(0x0301,
                       "Печать фискального отчета прервана, необходим повтор команды печати отчета (есть доп. данные)");
