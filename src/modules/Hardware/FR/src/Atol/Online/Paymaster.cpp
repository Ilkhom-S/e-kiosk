...
template <class T> void Paymaster<T>::setDeviceConfiguration(const QVariantMap &aConfiguration)
{
    ...
    if (aConfiguration.contains(CHardware::FR::PrinterModel) && (printerModel != CAtolOnlinePrinters::Default) &&
        !isNotPrinting())
    {
...
#define SET_LCONFIG_FISCAL_FIELD(aName)                                                                                \
    ...
    if (!automaticNumber.isEmpty())
    {
        if (getTLV(CFR::FiscalFields::AutomaticNumber, data))
        {
            ...
            {
                toLog(LogLevel::Warning, QString("%1: The automatic number in FR = %2 is different from AP number = %3")
                                             .arg(mDeviceName)
                                             .arg(data.data())
                                             .arg(automaticNumber));
...
template <class T> bool Paymaster<T>::enterExtendedMode()
{
    ...
    QByteArray data;
    bool result = getFRParameter(CAtolOnlineFR::FRParameters::DocumentPerforming, data) && !data.isEmpty() &&
                  (data[0] == CAtolOnlineFR::ZReportInBuffer);
...
template <class T> bool Paymaster<T>::setNotPrintDocument(bool aEnabled, bool /*aZReport*/)
{
    ...
    QByteArray data;
    bool result = getFRParameter(CAtolOnlineFR::FRParameters::PrinterModel, data) && !data.isEmpty() &&
                  (data[0] == printingValue);
...
template <class T> bool Paymaster<T>::performZReport(bool /*aPrintDeferredReports*/)
{
    ...
    // ошибки наполнения и переполнения буфера Z-Отчетов можно будет сбросить, если будет доработан функционал запроса
    // свободного места в Z-буфере
    // checkZBufferState();
...
//--------------------------------------------------------------------------------
template <class T> void Paymaster<T>::parseShortStatusFlags(char aFlags, TStatusCodes &aStatusCodes)
{
