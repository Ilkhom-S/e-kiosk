...
bool AtolVKP80BasedFR<T>::getCommonStatus(TStatusCodes& aStatusCodes)
{
	...
	do
	{
		...
		// 3. В ответе на запрос статуса - перегрев и при этом нет сопутствующих мех. ошибок - нет бумаги, мех. ошибка и
		// ошибка отрезчика
...
bool AtolVKP80BasedFR<T>::reInitPrinter()
{
	...
	SleepHelper::msleep(
		CAtolVKP80BasedFR::ReInitialization::Timeout); // необходима при любом результате переинициализации
