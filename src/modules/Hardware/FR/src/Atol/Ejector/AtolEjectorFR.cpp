...
template <class T> bool AtolEjectorFR<T>::setEjectorMode(char aEjectorMode) {
    ...
    if (!enterInnerMode(CAtolFR::InnerModes::Programming)) {
        toLog(LogLevel::Error,
              "AtolEjectorFR: Failed to set document processing parameters for receipts");
        return false;
    }
...
template <class T> bool AtolEjectorFR<T>::clearImageCounter() {
    if (!processCommand(CAtolFR::Commands::PrinterAccess,
                        CAtolEjectorFR::ImageProcessing::ClearCounterData)) {
...
bool AtolEjectorFR<T>::printImage(const QImage &aImage, const Tags::TTypes & /*aTags*/) {
    ...
    if (width > CAtolEjectorFR::ImageProcessing::MaxWidth) {
        toLog(LogLevel::Warning,
              QString("AtolEjectorFR: Image width > %1, the excess part will be truncated")
    ...
    int linesInPart =
        qMin(lines, int((CAtolEjectorFR::ImageProcessing::MaxPartSize * 8.0) / pixelsInLine));
    int blocksInPart =
        qCeil(linesInPart * widthInPixels / double(CAtolEjectorFR::ImageProcessing::MaxBlockSize));
    int parts = qCeil(double(lines) / linesInPart);

    for (int i = 0; i < parts; ++i) {
        int partheightInBytes =
            qCeil(linesInPart * CAtolEjectorFR::ImageProcessing::LineHeight / 8.0);
        ...
        if (!processCommand(CAtolFR::Commands::PrinterAccess, initCommandData)) {
            toLog(LogLevel::Error,
                  "AtolEjectorFR: Failed to initialize image processing for part " +
                      QString::number(i));
            return false;
        }
        ...
        for (int j = 0; j < blocksInPart; ++j) {
            ...
            if (!processCommand(CAtolFR::Commands::PrinterAccess,
                                CAtolFR::Commands::PrinterAccess + commandData)) {
                toLog(LogLevel::Error, "AtolEjectorFR: Failed to load data image block");
                clearImageCounter();
        ...
        if (!processCommand(CAtolFR::Commands::PrinterAccess,
                            CAtolEjectorFR::ImageProcessing::WriteData)) {
