...
namespace CEjectorAtolFR
{
    ...
    struct SData
    {
        ...
        SData() : receipt(InitReceipt), autoZReport(InitReceipt), ZReport(InitReceipt), nextMask(0)
        {
        } // разнести по моделям
        SData(char aReceipt, char aAutoZReport, char aZReport, char aNextMask)
            : receipt(aReceipt), autoZReport(aAutoZReport), ZReport(aZReport), nextMask(aNextMask)
        {
        }
    };
} // namespace CEjectorAtolFR
