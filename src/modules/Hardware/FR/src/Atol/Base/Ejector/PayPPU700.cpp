...
void PayPPU700::setDeviceConfiguration(const QVariantMap &aConfiguration) {
    ...
    if (!configuration.contains(CHardware::FR::EjectorParameters) &&
        ...
        QString previousReceipt =
            configuration[CHardware::Printer::Settings::PreviousReceipt].toString();
...
bool PayPPU700::updateParameters() {
    ...
    int leftReceiptTimeout =
        getConfigParameter(CHardware::Printer::Settings::LeftReceiptTimeout).toInt();
    char leftReceiptTimeoutValue = (leftReceiptTimeout % ~CEjectorAtolFR::SpecialSettingMask) / 10;

    char ejectorMode = CPayPPU700::EjectorParameters::ZReportPush | ejectorParametersValue |
                       leftReceiptTimeoutValue;
