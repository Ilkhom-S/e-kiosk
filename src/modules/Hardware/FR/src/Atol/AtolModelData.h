...
namespace CAtolFR
{
	...
	private:
		...
		void addOldTrade(int aModelId, int aMaxStringSize, QString aName, bool aCutter, bool aLineSpacing,
						 bool aVerified = false, int aFeedingAmount = 0);

		/// Добавить кассовый ФР серии FPrint и/или следующих.
		void addTrade(int aModelId, int aMaxStringSize, QString aName, bool aCutter = true, bool aVerified = false,
					  int aFeedingAmount = 0);

		/// Добавить терминальный ФР.
		void addTerminal(int aModelId, int aMaxStringSize, const QString& aName, int aBuild, bool aEjector,
						 int aFeedingAmount = 0, int aZBufferSize = 0);

		/// Добавить онлайновый кассовый ФР.
		void addOnlineTrade(int aModelId, int aMaxStringSize, QString aName, bool aCutter, int aFeedingAmount = 0,
							int aBuild = OnlineTradeBuild, bool aVerified = true);

		/// Добавить онлайновый терминальный ФР.
		void addOnlineTerminal(int aModelId, int aMaxStringSize, const QString& aName, bool aEjector,
							   int aFeedingAmount, int aZBufferSize = 0, int aBuild = OnlineTerminalBuild,
							   bool aVerified = true);
	};
} // namespace CAtolFR
