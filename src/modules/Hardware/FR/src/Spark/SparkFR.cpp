...
//--------------------------------------------------------------------------------
template <class T> T SparkFR::fromBCD(const QByteArray &aData) {
    T result = 0;
    int size = aData.size();
...
//--------------------------------------------------------------------------------
template <> char SparkFR::fromBCD(const QByteArray &aData) {
...
bool SparkFR::checkSystemFlag(const QByteArray &aFlagBuffer, int aNumber) {
    auto dataIt =
        std::find_if(mSystemFlags.begin(), mSystemFlags.end(),
                     [&](const CSparkFR::SystemFlags::SData &aData) -> bool { return aData.number == aNumber; });

    if (dataIt == mSystemFlags.end()) {
        toLog(LogLevel::Error,
              QString("%1: Failed to find system flag %2 in device one").arg(mDeviceName).arg(aNumber));
        return false;
    }
    ...
    if (flag != newFlag) {
        ...
        if (!processCommand(CSparkFR::Commands::SetFlag, commandData, &answer)) {
            toLog(LogLevel::Error,
                  QString("%1: Failed to set system flag %2 (%3)").arg(mDeviceName).arg(aNumber).arg(dataIt->name));
            return false;
        }
    }
...
bool SparkFR::getSystemFlags(QByteArray &aData, TTaxData *aTaxData) {
    ...
    if (data.size() < 5) {
        toLog(LogLevel::Error, QString("%1: Failed to check system flags due to size of data list = %2, need %3")
                                   .arg(mDeviceName)
                                   .arg(data.size())
                                   .arg(5));
        return false;
...
bool SparkFR::checkSystemFlags(QByteArray &aFlagData) {
    auto dataIt =
        std::find_if(mSystemFlags.begin(), mSystemFlags.end(), [&](const CSparkFR::SystemFlags::SData &aData) -> bool {
            return aData.number == CSparkFR::SystemFlags::ZReportsAndFiscal;
        });

    if (dataIt == mSystemFlags.end()) {
        toLog(LogLevel::Error, QString("%1: Failed to find system flag %2 in device one")
                                   .arg(mDeviceName)
...
bool SparkFR::checkTaxFlags(const TTaxData &aTaxData) {
    ...
    if (!absentVATs.isEmpty()) {
        ...
        for (int i = 0; i < CSparkFR::TaxRateCount; ++i) {
            commandData += (i < mTaxes.size()) ? QString("%1").arg(mTaxes[i], 2, 10, QChar(ASCII::Zero)).toLatin1()
                                               : CSparkFR::NoTaxes;
        }
...
TResult SparkFR::execCommand(const QByteArray &aCommand, const QByteArray &aCommandData, QByteArray *aAnswer) {
    ...
    if (data.password) {
        ...
        if (!result) {
            toLog(LogLevel::Error,
                  mDeviceName + ": Failed to enter control password, unable to perform current action therefore");
            return result;
        }
    }
        ...
        case CSparkFR::Errors::TimeOff: {
            ...
            if (!mStatusCollection.contains(FRStatusCode::Error::ZBufferOverflow)) {
                if (mOperatorPresence) {
                    toLog(LogLevel::Error,
                          mDeviceName + ": Failed to process auto-Z-report due to presence of the operator.");
...
bool SparkFR::setFiscalParameters(const QStringList &aReceipt) {
    ...
    for (int i = 0; i < aReceipt.size(); ++i) {
        if (!aReceipt[i].isEmpty()) {
            commandData.clear();
            commandData +=
                QString("%1").arg(CSparkFR::TextProperties::Numbers[i], 2, 10, QChar(ASCII::Zero)).right(2).toLatin1() +
...
bool SparkFR::performFiscal(const QStringList &aReceipt, const SPaymentData &aPaymentData, quint32 * /*aFDNumber*/) {
    ...
    if (result) {
        ...
        result =
            result && processCommand(CSparkFR::Commands::CloseFiscal, QByteArray::number(CSparkFR::CashPaymentType));
...
bool SparkFR::sale(const SUnitData &aUnitData) {
    ...
    QByteArray commandData = QByteArray::number(qRound64(aUnitData.sum * 100.0)).rightJustified(8, ASCII::Zero) +
...
bool SparkFR::getKKMData(TKKMInfoData &aData) {
    ...
    if (aData.size() < 11) {
        toLog(
            LogLevel::Error,
            QString("%1: Too small sections in KKM info answer = %2, need 11 min").arg(mDeviceName).arg(aData.size()));
        return false;
    }
...
ESessionState::Enum SparkFR::getSessionState() {
    ...
    */
    ...
    // Т.е. если дата и время начала смены валидны, то она открыта. Возможно - особенность нефискализированного
    // аппарата.
