...
struct SData {
    ...
    SData() : number(0) {}
    SData(int aNumber, const QString &aMask, const QString &aName)
...
public:
    CData() {
        ...
        flags110.add(
            13,
            "x0000000",
            "format and protection of fiscal document"); /// Формат и защита полей фискального чека

        flags110.add(3,
                     "xxxxxx0x",
                     "сashbox and EKLZ extra options in Z-report"); /// Денежный ящик и доп. инфо
                                                                    /// ЭКЛЗ на Z-отчете
        flags110.add(4, "xx000000", "format of fiscal document"); /// Формат полей фискального чека
        flags110.add(
            7,
            "10xxxxxx",
            "format of fiscal document and Z-report"); /// Формат полей фискального чека и Z-отчета
        flags110.add(
            9, "1xxxxxxx", "inhibition of payment instrument"); /// Запреты средств платежей
        flags110.add(10, "xxxxx0xx", "KKM working features");   /// Особенности работы ККМ
        flags110.add(14,
                     "00xxxxxx",
                     "paper heating and extra options"); /// Прогрев бумаги м другие настройки
        flags110.add(18, "11xxxxxx", "barcode options"); /// Настройки печати штрих-кодов
        flags110.add(19,
                     "xx01xxxx",
                     "taxes and other options"); /// Дополнительные возможности по использованию
                                                 /// нулевой ставки налогов
        ...
        flags110.add(
            25,
            "00001000",
            "backfeed"); /// Величина обратного движения бумаги после обрезки чека (только 110)
        flags110.add(26,
                     "01xxxxxx",
                     "X,Z-report and paper spending"); /// X- и Z-отчеты и расход бумаги после
                                                       /// отрезки чека (только 110)
