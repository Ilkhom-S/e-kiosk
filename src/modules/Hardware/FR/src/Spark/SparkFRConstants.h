...
public:
    Data() {
        ...
        addData(135,
                "Разные итоги документа в ОЗУ и ЭКЛЗ. Невозможно выполнить аварийную отмену чека");
        ...
        addData(
            139,
            "Нет ни одного наличного платёжного средства, из которого можно было бы выдать сдачу");
        ...
        addData(
            161,
            "Биты 4..6 флага 4 защищены PF13:0 от модификации или бит 5 флага 3 защищен от сброса");
        ...
        addData(168,
                "Глубина дерева налогов слишком велика, дерево содержит цикл или обратную ссылку");
        addData(169,
                "Предупреждение: ставки налогов во включенном режиме цепочек не программируются");
...
private:
    void addData(uchar aKey, const char *aDescription) { add(char(aKey), aDescription); }
};
} // namespace Errors
