...
template QByteArray
PortFRBase<SerialPrinterBase<PrinterBase<SerialDeviceBase<PortPollingDeviceBase<ProtoFR>>>>>::performStatus(
    TStatusCodes &, char, int);
template QByteArray
PortFRBase<SerialPrinterBase<PrinterBase<SerialDeviceBase<PortPollingDeviceBase<ProtoFR>>>>>::performStatus(
    TStatusCodes &, const char *, int);
template QByteArray
PortFRBase<PortPrinterBase<PrinterBase<TCPDeviceBase<PortPollingDeviceBase<ProtoFR>>>>>::performStatus(TStatusCodes &,
                                                                                                       char, int);
template QByteArray
PortFRBase<PortPrinterBase<PrinterBase<TCPDeviceBase<PortPollingDeviceBase<ProtoFR>>>>>::performStatus(TStatusCodes &,
                                                                                                       const char *,
...
//--------------------------------------------------------------------------------
template <class T> PortFRBase<T>::PortFRBase() : mLastError('\x00'), mLastCommandResult(CommandResult::OK)
{
    setInitialData();
...
//--------------------------------------------------------------------------------
template <class T> void PortFRBase<T>::makeReceipt(const QStringList &aReceipt, QStringList &aBuffer)
{
...
//--------------------------------------------------------------------------------
template <class T> void PortFRBase<T>::makeReceipt(const QStringList &aReceipt, TReceiptBuffer &aBuffer)
{
    QStringList buffer;
...
//--------------------------------------------------------------------------------
template <class T>
bool PortFRBase<T>::checkFiscalFields(quint32 aFDNumber, TFiscalPaymentData &aFPData,
...
//--------------------------------------------------------------------------------
template <class T>
bool PortFRBase<T>::processFiscalTLVData(const TGetFiscalTLVData &aGetFiscalTLVData, TFiscalPaymentData *aFPData,
                                         TComplexFiscalPaymentData *aPSData)
{
    ...
    do
    {
        ...
        else if (!mFFData.data().contains(TLV.field))
        {
            toLog(LogLevel::Warning,
                  mDeviceName + QString(": Failed to add fiscal field %1 due to it is unknown").arg(TLV.field));
        }
