...
//--------------------------------------------------------------------------------
template <class T> FRBase<T>::FRBase() : mFFEngine(mLog)
{
    ...
    mUnnecessaryErrors.insert(EFiscalPrinterCommand::Encashment, TStatusCodes()
                                                                     << FRStatusCode::Error::ZBufferOverflow
                                                                     << FRStatusCode::Error::NeedCloseSession);
    mUnnecessaryErrors.insert(EFiscalPrinterCommand::XReport, TStatusCodes() << FRStatusCode::Error::ZBufferOverflow
                                                                             << FRStatusCode::Error::NeedCloseSession
    ...
    mOFDFiscalFields << CFR::FiscalFields::Cashier << CFR::FiscalFields::CashierINN << CFR::FiscalFields::UserContact;
...
template <class T> void FRBase<T>::initializeZReportByTimer()
{
    ...
    if (ZT.isValid())
    {
        ...
        else if (OT.isValid())
        {
            ...
            {
                logZReport =
                    QString("(CT <= OT) && (OT < ZT) -> %1, session opening time and Z-report time < 0").arg(log);
            }
        }
...
template <class T> void FRBase<T>::checkZReportByTimer()
{
    ...
    toLog(LogLevel::Normal,
          mDeviceName + ": Timer for Z-report is scheduled for " + ZReportTime.toString(CFR::TimeLogFormat));
...
template <class T> bool FRBase<T>::checkNotPrinting(bool aEnabled, bool aZReport)
{
    ...
    if (!WorkingThreadProxy(&mThread).invokeMethod<bool>(
...
//---------------------------------------------------------------------------
template <class T> void FRBase<T>::checkOFDNotSentCount(int aOFDNotSentCount, TStatusCodes &aStatusCodes)
{
...
//--------------------------------------------------------------------------------
template <class T> bool FRBase<T>::checkOFDData(const QByteArray &aAddressData, const QByteArray &aPortData)
{
    ...
    auto it = std::find_if(
        OFDData.begin(), OFDData.end(),
        [&](const CHardware::SOFDData &aOFDData) -> bool
        {
    ...
    it = std::find_if(OFDData.begin(), OFDData.end(),
                      [&](const CHardware::SOFDData &aOFDData) -> bool
                      {
                          bool testURL =
                              URL.valid && aOFDData.address.URL.valid && (URL.value == aOFDData.address.URL.value);
                          bool testIP =
...
//--------------------------------------------------------------------------------
template <class T> bool FRBase<T>::isFiscalReady(bool aOnline, EFiscalPrinterCommand::Enum aCommand)
{
...
bool FRBase<T>::printFiscal(const QStringList &aReceipt, const SPaymentData &aPaymentData, quint32 *aFDNumber)
{
    ...
    return processNonReentrant(
        std::bind(&FRBase::processFiscal, this, std::ref(aReceipt), std::ref(aPaymentData), aFDNumber));
}
...
bool FRBase<T>::processFiscal(const QStringList &aReceipt, const SPaymentData &aPaymentData, uint *aFDNumber)
{
    ...
    if (!checkAmountsOnPayment(paymentData) || !checkSumInCash(paymentData) || !checkPayTypeOnPayment(paymentData) ||
        !checkNotPrinting() || !mFFEngine.checkTaxSystemOnPayment(paymentData) ||
        !mFFEngine.checkAgentFlagOnPayment(paymentData) || !taxesOK)
    {
        return false;
    }
...
bool FRBase<T>::checkFiscalFields(quint32 aFDNumber, TFiscalPaymentData &aFPData, TComplexFiscalPaymentData &aPSData)
{
    ...
    return WorkingThreadProxy(&mThread).invokeMethod<bool>(
        std::bind(&FRBase<T>::processFiscalFields, this, aFDNumber, std::ref(aFPData), std::ref(aPSData)));
}
...
{
    double amountInCash =
        WorkingThreadProxy(&mThread).invokeMethod<double>(std::bind(&FRBase<T>::getAmountInCash, this));
...
template <class T> bool FRBase<T>::complexFiscalDocument(TBoolMethod aMethod, const QString &aLog)
{
    ...
    if (!result)
    {
        if (!mNextReceiptProcessing)
        {
            ...
            TStatusCodes nonFiscalErrors =
                mStatusCollection.value(EWarningLevel::Error) - getFiscalStatusCodes(EWarningLevel::Error);
...
template <class T> ESessionState::Enum FRBase<T>::checkSessionState()
{
    ...
    if (sessionState == ESessionState::Error)
        toLog(LogLevel::Error, mDeviceName + ": Failed to get session state");
    if (sessionState == ESessionState::Expired)
        toLog(LogLevel::Normal, mDeviceName + ": Session is expired");
    if (sessionState == ESessionState::Closed)
        toLog(LogLevel::Normal, mDeviceName + ": Session is closed");
    if (sessionState == ESessionState::Opened)
        toLog(LogLevel::Normal, mDeviceName + ": Session is opened");
...
template <class T> EDocumentState::Enum FRBase<T>::checkDocumentState()
{
    ...
    if (documentState == EDocumentState::Error)
        toLog(LogLevel::Error, mDeviceName + ": Failed to get document state");
    if (documentState == EDocumentState::Closed)
        toLog(LogLevel::Normal, mDeviceName + ": Document is closed");
    if (documentState == EDocumentState::Opened)
        toLog(LogLevel::Normal, mDeviceName + ": Document is opened");
...
template <class T> bool FRBase<T>::processStatus(TStatusCodes &aStatusCodes)
{
    ...
    if (mWrongTaxOnPayment)
        aStatusCodes.insert(FRStatusCode::Warning::WrongTaxOnPayment);
    if (mNeedTimeSynchronization)
        aStatusCodes.insert(FRStatusCode::Warning::NeedTimeSynchronization);

    if (mIsOnline)
    {
        TAgentFlags mainAgentFlags = TAgentFlags() << EAgentFlags::BankAgent << EAgentFlags::BankSubagent
        ...
        {
            if (!mStatusCollection.contains(FRStatusCode::Warning::DealerSupportPhone) &&
        ...
        bool compliance =
            (mFFDFR == mFFDFS) || ((mFFDFS == EFFD::F10) && ((mFFDFR == EFFD::F105) || (mFFDFR == EFFD::F10Beta)));
        ...
        if (!mFiscalServerPresence &&
            (mWrongFiscalizationSettings || (mOperatorPresence && !automaticNumber.isEmpty())))
        {
...
template <class T> void FRBase<T>::checkTaxes2019()
{
    ...
    bool canZReportWithoutPrinting = getConfigParameter(CHardwareSDK::FR::CanWithoutPrinting, false).toBool() ||
...
{
    return std::accumulate(aPaymentData.unitDataList.begin(), aPaymentData.unitDataList.end(), TSum(0),
    ...
    foreach (const SUnitData &unitData, aPaymentData.unitDataList)
    {
        if (unitData.sum <= 0)
        {
            toLog(LogLevel::Error,
                  mDeviceName +
                      QString(": Failed to process %1 for sum = %2").arg(payOffType).arg(unitData.sum, 0, 'f', 2));
            return false;
        }
    }
...
template <class T> bool FRBase<T>::checkSumInCash(const SPaymentData &aPaymentData)
{
    ...
    {
        toLog(LogLevel::Error, mDeviceName + QString(": Failed to process %1 for %2 > %3 in cash")
                                                 .arg(payOffType)
...
template <class T> bool FRBase<T>::checkTaxesOnPayment(const SPaymentData &aPaymentData)
{
    ...
    foreach (auto unitData, aPaymentData.unitDataList)
    {
        ...
        {
            toLog(LogLevel::Error,
                  mDeviceName + QString(": The taxes specification does not contain VAT = %1").arg(unitData.VAT));
            return false;
        }
    }
...
{
    checkFFExistingOnPayment(CFR::FiscalFields::AgentFlagsReg, aPaymentData.agentFlag != EAgentFlags::None,
                             mAgentFlags.size() != 1);
    checkFFExistingOnPayment(CFR::FiscalFields::TaxSystem, aPaymentData.taxSystem != ETaxSystems::None,
                             mTaxSystems.size() != 1);
    ...
    QString agentOperation =
        QString::fromUtf8(aPaymentData.back() ? CFR::AgentOperation::Payout : CFR::AgentOperation::Payment);
...
template <class T> bool FRBase<T>::checkPayTypeOnPayment(const SPaymentData &aPaymentData)
{
    ...
    {
        toLog(LogLevel::Error, mDeviceName + QString(": The pay types specification does not contain type = %1 (%2)")
...
template <class T> bool FRBase<T>::checkFiscalFieldsOnPayment()
{
    ...
    {
        toLog(LogLevel::Error, mDeviceName + ": Failed to process fiscal document due to wrong fiscal field(s): " +
                                   absentFields.join(", "));
        return false;
    }
