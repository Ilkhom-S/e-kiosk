...
#define ADD_KASBI_FF(aField, aName, aType)                                                                             \
    ADD_STATIC_DATA(CKasbiFR::FiscalFields::Data, int, aName, aField,                                                  \
        ...
        class Data : public FRError::Data
        {
          public:
            Data()
            {
                ...
                add('\x34', "Данные команды уже были переданы ранее. (Применительно к данным агента, данным оплаты, "
                            "данным автоматического устройства расчетов)");
                add('\x35', "Аппаратный сбой ККТ");
                add('\x36', "Неверно указан признак расчета, возможные значения: приход, расход, возврат прихода, "
                            "возврат расхода");
                ...
                add('\x39', "Сумма расчета чека не равна сумме следующих значений по чеку: сумма наличными, сумма "
                ...
                add('\x3F', "Невозможно передать данные агента, допустимы данные агента либо для всего чека, либо "
                ...
                add('\x45',
                    "Сумма по чеку коррекции всеми типами оплаты не равна полной сумме для расчетов по ставкам НДС");
                ...
                add('\x52', "В чеке не должны присутствовать иные предметы расчета помимо предмета расчета с признаком "
                            "способа расчета «Оплата кредита»");
                add('\x53', "Переданы не все необходимые данные для агента");
                add('\x54', "Итоговая сумма расчета(в рублях без учета копеек) не равна сумме стоимости всех предметов "
                ...
                add('\x3F', "Передать данные автоматического устройства расчетов для кассового чека (БСО) коррекции");
            }
        };
    } // namespace Errors
