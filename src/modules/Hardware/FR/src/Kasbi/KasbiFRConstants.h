...
struct SFSData {
    ...
    SFSData()
        : documentOpened(false), sessionOpened(false), flags(ASCII::NUL), number(0),
          lastFDNumber(0) {}
    SFSData(bool aDocumentOpened,
            bool aSessionOpened,
            char aFlags,
            qulonglong aNumber,
            uint aLastFDNumber)
        : documentOpened(aDocumentOpened), sessionOpened(aSessionOpened), flags(aFlags),
          number(aNumber), lastFDNumber(aLastFDNumber) {}
};
...
const char StartFiscalTLVData =
    '\x35'; /// Начать получение данных фискального документа в TLV-формате.
...
public:
    QString getTextLog(int aField) const {
        return QString("fiscal field %1 (%2)")
            .arg(aField)
            .arg(value(aField).textKey.replace("_", " "));
    }
};

#define ADD_KASBI_FF(aField, aName, aType)                                                         \
    ADD_STATIC_DATA(CKasbiFR::FiscalFields::Data,                                                  \
                    int,                                                                           \
                    aName,                                                                         \
                    aField,                                                                        \
...
public:
    Data() {
        ...
        add('\x07', "Дата и время операции не соответствуют логике работы ФН", EType::FS);
        ...
        add('\x12', "Исчерпан ресурс КС. Требуется закрытие фискального режима", EType::FS);
        ...
        add('\x30', "Нет связи с ФН", EType::FS);
        ...
        add('\x34',
            "Данные команды уже были переданы ранее. (Применительно к данным агента, данным "
            "оплаты, "
            "данным автоматического устройства расчетов)");
        add('\x35', "Аппаратный сбой ККТ");
        add('\x36',
            "Неверно указан признак расчета, возможные значения: приход, расход, возврат прихода, "
            "возврат расхода");
        ...
        add('\x39',
            "Сумма расчета чека не равна сумме следующих значений по чеку: сумма наличными, сумма "
            "электронными, сумма предоплатой, сумма постоплатой, сумма встречным предоставлением");
        add('\x3A',
            "Сумма оплаты соответствующими типами (за исключением наличных) превышает итог чека");
        ...
        add('\x3E',
            "Превышено максимально допустимое количество предметов расчета c данными агента в "
            "чеке");
        add('\x3F',
            "Невозможно передать данные агента, допустимы данные агента либо для всего чека, либо "
            "данные агента по предметам расчета");
        ...
        add('\x45',
            "Сумма по чеку коррекции всеми типами оплаты не равна полной сумме для расчетов по "
            "ставкам НДС");
        add('\x46',
            "Сумма по чеку коррекции всеми типами оплаты не равна итоговой сумме чека коррекции");
        ...
        add('\x52',
            "В чеке не должны присутствовать иные предметы расчета помимо предмета расчета с "
            "признаком "
            "способа расчета «Оплата кредита»");
        add('\x53', "Переданы не все необходимые данные для агента");
        add('\x54',
            "Итоговая сумма расчета(в рублях без учета копеек) не равна сумме стоимости всех "
            "предметов "
            "расчета(в рублях без учета копеек)");
        add('\x55',
            "Неверно указан признак расчета для чека коррекции, возможные значения: приход, "
            "расход");
        ...
        add('\x59', "Не указано или неверно указано значение тега \"Признак платежного агента\"");
        ...
        add('\x5C',
            "Некорректный формат или длина в массиве переданных строк нефискальной информации");
        ...
        add('\xF3', "Подключенный ФН не соответствует данным регистрации ККТ", EType::FS);
        add('\xF4', "ФН еще не был активирован", EType::FS);
        add('\xF5', "ФН был закрыт", EType::FS);
        add('\x1F', "Передать данные автоматического устройства расчетов для кассового чека (БСО)");
        add('\x3F',
            "Передать данные автоматического устройства расчетов для кассового чека (БСО) "
            "коррекции");
    }
};
} // namespace Errors
