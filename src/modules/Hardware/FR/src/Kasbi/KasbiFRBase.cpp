...
KasbiFRBase::KasbiFRBase() {
    ...
    mUnprocessedErrorData.add(CKasbiFR::Commands::GetFiscalTLVData,
...
void KasbiFRBase::processDeviceData() {
    ...
    if (processCommand(CKasbiFR::Commands::GetOFDData, &data)) {
        ...
        if (TLVs.contains(CKasbiFR::FiscalFields::OFDAddress) &&
            ...
            mOFDDataError = !checkOFDData(TLVs[CKasbiFR::FiscalFields::OFDAddress],
...
TResult KasbiFRBase::execCommand(const QByteArray &aCommand,
    ...
    TResult result =
        mProtocol.processCommand(commandData, answer, CKasbiFR::Commands::Data[aCommand[0]]);
    ...
    if (answer.size() < CKasbiFR::MinUnpackedAnswerSize) {
        toLog(LogLevel::Error,
              mDeviceName + ": Answer data is less than " +
    ...
    if (!answer[0]) {
        ...
        toLog(LogLevel::Error,
              mDeviceName + ": Answer data in packet is less than " +
...
bool KasbiFRBase::sale(const SUnitData &aUnitData) {
    ...
    QByteArray commandData = QByteArray() +
                             ...
                             mFFEngine.getTLVData(CFR::FiscalFields::PayOffSubjectUnitPrice,
                             ...
                             mFFEngine.getTLVData(CFR::FiscalFields::PayOffSubjectMethodType,
                                                  aUnitData.payOffSubjectMethodType);

    return processCommand(CKasbiFR::Commands::Sale,
                          mFFEngine.getTLVData(CFR::FiscalFields::PayOffSubject, commandData));
}
...
bool KasbiFRBase::performFiscal(const QStringList &aReceipt,
    ...
    QByteArray commandData =
        ...
        mFFEngine.getTLVData(CFR::FiscalFields::CardFiscalTotal,
                             totalPayTypeSum(EPayTypes::EMoney)) +
        mFFEngine.getTLVData(CFR::FiscalFields::PrePaymentFiscalTotal,
                             totalPayTypeSum(EPayTypes::PrePayment)) +
        mFFEngine.getTLVData(CFR::FiscalFields::PostPaymentFiscalTotal,
                             totalPayTypeSum(EPayTypes::PostPayment)) +
        mFFEngine.getTLVData(CFR::FiscalFields::CounterOfferFiscalTotal,
    ...
    if (!cashier.isEmpty())
        commandData += mFFEngine.getTLVData(CFR::FiscalFields::Cashier);
    if (!cashierINN.isEmpty())
        commandData += mFFEngine.getTLVData(CFR::FiscalFields::CashierINN);
    if (!userContact.isEmpty())
        commandData += mFFEngine.getTLVData(CFR::FiscalFields::UserContact);
    ...
    if (aPaymentData.agentFlag != EAgentFlags::None) {
        ...
        if (isBankAgent) {
            commandData += mFFEngine.getTLVData(CFR::FiscalFields::TransferOperatorAddress) +
        ...
        if (isBankAgent || isPaymentAgent) {
            commandData += mFFEngine.getTLVData(CFR::FiscalFields::AgentPhone) +
...
bool KasbiFRBase::setNotPrintDocument(bool aEnabled, bool /*aZReport*/) {
    ...
    */
    ...
    if (!printerModelKey) {
        toLog(LogLevel::Error,
              mDeviceName + ": Cannot set auto printer model due to it is not allowed");
        return false;
    }
