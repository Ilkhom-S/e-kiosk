...
bool Puloon::check(const QByteArray &aAnswer, const QByteArray &aRequest) const {
    if (aAnswer.size() < CPuloon::MinPacketAnswerSize) {
        toLog(LogLevel::Error,
              QString("Puloon: The length of the packet is less than %1 byte")
                  .arg(CPuloon::MinPacketAnswerSize));
        return false;
    }

    if (aAnswer[0] != CPuloon::AnswerMark) {
        toLog(LogLevel::Error,
              QString("Puloon: Invalid answer mark = %1, need %2")
    ...
    if (aAnswer[1] != aRequest[1]) {
        toLog(LogLevel::Error,
              QString("Puloon: Invalid communication Id = %1, need %2")
    ...
    if (aAnswer[2] != CPuloon::Prefix) {
        toLog(LogLevel::Error,
              QString("Puloon: Invalid first byte (prefix) = %1, need %2")
    ...
    if (aAnswer[3] != aRequest[3]) {
        toLog(LogLevel::Error,
              QString("Puloon: Invalid answer command = %1, need = %2.")
...
//--------------------------------------------------------------------------------
TResult Puloon::processCommand(const QByteArray &aCommandData,
                               TAnswerList &aAnswerList,
                               int aTimeout) const {
...
//--------------------------------------------------------------------------------
bool Puloon::execCommand(const QByteArray &aCommandPacket,
                         TAnswerList &aAnswerList,
                         int aTimeout) const {
    int countRequest = 0;
    ...
    do {
        ...
        if (!aAnswerList.isEmpty() && !aAnswerList[0].isEmpty() &&
            (aAnswerList[0][0] == ASCII::NAK)) {
            toLog(LogLevel::Warning,
                  "Puloon: Answer contains NAK, iteration " + QString::number(countRequest + 1));
            countRequest++;
        } else {
            return true;
        }
