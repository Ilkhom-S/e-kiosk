...
CCTalkCAProtocol::CCTalkCAProtocol()
    : m_Address(CCCTalk::Address::Unknown), m_Type(CCCTalk::UnknownType) {}

//--------------------------------------------------------------------------------
void CCTalkCAProtocol::setAddress(uchar aAddress) {
    if (m_Address != aAddress) {
...
bool CCTalkCAProtocol::check(QByteArray &aAnswer) {
    if (aAnswer.size() < CCCTalk::MinAnswerSize) {
        toLog(LogLevel::Error,
              QString("ccTalk: Too few bytes in answer = %1, need min = %2")
                  .arg(aAnswer.size())
                  .arg(CCCTalk::MinAnswerSize));
        return false;
    }
    ...
    if (destinationAddress != CCCTalk::Address::Host) {
        toLog(LogLevel::Error,
              QString("ccTalk: Wrong destination address = %1, need = %2")
                  .arg(uchar(destinationAddress))
                  .arg(uchar(CCCTalk::Address::Host)));
        return false;
    }
    ...
    if (length != dataSize) {
        toLog(LogLevel::Error,
              QString("ccTalk: Wrong length = %1, need = %2").arg(length).arg(dataSize));
        return false;
    }

    if (m_Type == CHardware::CashDevice::CCTalkTypes::CRC8) {
        char deviceAddress = aAnswer[2];

        if (deviceAddress != m_Address) {
            toLog(LogLevel::Error,
                  QString("ccTalk: Wrong address = %1, need = %2")
                      .arg(uchar(deviceAddress))
                      .arg(m_Address));
            return false;
        }
    }
    ...
    if (m_Type == CHardware::CashDevice::CCTalkTypes::CRC8) {
        ...
        if (dataCRC != answerCRC) {
            toLog(LogLevel::Error,
                  QString("ccTalk: Wrong CRC = %1, need = %2")
                      .arg(answerCRC)
                      .arg(uchar(uchar(0) - dataCRC)));
            return false;
        }
    } else if (m_Type == CHardware::CashDevice::CCTalkTypes::CRC16) {
...
TResult CCTalkCAProtocol::processCommand(const QByteArray &aCommandData, QByteArray &aAnswerData) {
    ...
    if (m_Address == CCCTalk::Address::Unknown) {
    ...
    if (m_Type == CHardware::CashDevice::CCTalkTypes::CRC8) {
        ...
        toLog(LogLevel::Error,
              "ccTalk: Failed to process command due to unknown ccTalk type " + m_Type);
        return CommandResult::Driver;
    }
    ...
    do {
        ...
        if (!m_Port->write(request) || !getAnswer(aAnswerData, request)) {
        ...
        if (nak || busy) {
            toLog(LogLevel::Normal,
                  QString("ccTalk: %1 in answer, %2")
                      .arg(nak ? "NAK" : "BYSY")
                      .arg((busyNAKRepeat <= CCCTalk::MaxBusyNAKRepeats)
...
bool CCTalkCAProtocol::getAnswer(QByteArray &aAnswer, const QByteArray &aCommandData) {
    ...
    } while ((timer.elapsed() < CCCTalk::Timeouts::Reading) &&
