...
bool CCTalkCAProtocol::check(QByteArray &aAnswer) {
    if (aAnswer.size() < CCCTalk::MinAnswerSize) {
        toLog(LogLevel::Error, QString("ccTalk: Too few bytes in answer = %1, need min = %2")
                                   .arg(aAnswer.size())
    ...
    if (destinationAddress != CCCTalk::Address::Host) {
        toLog(LogLevel::Error, QString("ccTalk: Wrong destination address = %1, need = %2")
    ...
    if (mType == CHardware::CashDevice::CCTalkTypes::CRC8) {
        ...
        if (deviceAddress != mAddress) {
            toLog(LogLevel::Error,
                  QString("ccTalk: Wrong address = %1, need = %2").arg(uchar(deviceAddress)).arg(uchar(mAddress)));
            return false;
        }
    }
    ...
    if (mType == CHardware::CashDevice::CCTalkTypes::CRC8) {
        ...
        if (dataCRC != answerCRC) {
            toLog(LogLevel::Error,
                  QString("ccTalk: Wrong CRC = %1, need = %2").arg(answerCRC).arg(uchar(uchar(0) - dataCRC)));
            return false;
        }
        ...
        if (dataCRC != answerCRC) {
            toLog(LogLevel::Error, QString("ccTalk: Wrong CRC = %1, need = %2")
