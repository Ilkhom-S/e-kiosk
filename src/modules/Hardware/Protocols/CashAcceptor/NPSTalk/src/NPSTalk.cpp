...
bool NPSTalkProtocol::check(const QByteArray &aCommandData, const QByteArray &aAnswerData) {
    ...
    if (aAnswerData.size() < NPSTalk::MinAnswerSize) {
        toLog(LogLevel::Error,
              QString("Too few bytes in answer = %1, need min = %2")
                  .arg(aAnswerData.size())
                  .arg(NPSTalk::MinAnswerSize));
        return false;
    }

    if (aAnswerData[0] != NPSTalk::Prefix) {
        toLog(LogLevel::Error,
              QString("Wrong prefix = %1, need = %2")
                  .arg(toHexLog(aAnswerData[0]))
                  .arg(toHexLog(NPSTalk::Prefix)));
        return false;
    }
    ...
    if (postfix != NPSTalk::Postfix) {
        toLog(LogLevel::Error,
              QString("Wrong postfix = %1, need = %2")
                  .arg(toHexLog(postfix))
                  .arg(toHexLog(NPSTalk::Postfix)));
        return false;
    }

    if (aCommandData[0] != aAnswerData[0]) {
        toLog(LogLevel::Error,
              QString("Wrong command in answer = %1, need = %2")
                  .arg(toHexLog(aCommandData[0]))
                  .arg(toHexLog(aAnswerData[0])));
        return false;
    }

    char dataCRC = 0;
    std::for_each(aAnswerData.begin() + 1, aAnswerData.end() - 1, [&dataCRC](char element) {
        dataCRC ^= element;
    });
    ...
    if (dataCRC != answerCRC) {
        toLog(LogLevel::Error,
              QString("Wrong CRC = %1, need = %2").arg(toHexLog(answerCRC)).arg(toHexLog(dataCRC)));
        return false;
    }
...
TResult NPSTalkProtocol::processCommand(const QByteArray &aCommanddata, QByteArray &aAnswerData) {
    ...
    char result = 0;
    std::for_each(
        request.begin() + 1, request.end(), [&result](char element) { result ^= element; });
...
bool NPSTalkProtocol::getAnswer(QByteArray &aAnswer) {
    ...
    } while ((timer.elapsed() < NPSTalk::Timeout) &&
             ((aAnswer.size() < 4) || (aAnswer[aAnswer.size() - 2] != NPSTalk::Postfix)));
