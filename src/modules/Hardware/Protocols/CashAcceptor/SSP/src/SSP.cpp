...
TResult SSPProtocol::check(const QByteArray &aAnswer) {
    ...
    if (aAnswer.size() < CSSP::MinAnswerSize) {
        toLog(LogLevel::Error,
              QString("SSP: Too few bytes in answer = %1, need min = %2").arg(aAnswer.size()).arg(CSSP::MinAnswerSize));
    ...
    if (prefix != CSSP::Prefix[0]) {
        toLog(LogLevel::Error,
              QString("SSP: Invalid prefix = %1, need = %2").arg(toHexLog(prefix)).arg(toHexLog(CSSP::Prefix[0])));
        return CommandResult::Protocol;
    }
    ...
    if (address != mAddress) {
        toLog(LogLevel::Error,
              QString("SSP: Invalid address = %1, need = %2").arg(toHexLog(address)).arg(toHexLog(mAddress)));
    ...
    if (sequenceFlag != mSequenceFlag) {
        toLog(LogLevel::Error,
              QString("SSP: Invalid sequence flag = %1, need = %2").arg(int(sequenceFlag)).arg(int(mSequenceFlag)));
        return CommandResult::Id;
    }
...
//--------------------------------------------------------------------------------
TResult SSPProtocol::processCommand(const QByteArray &aCommandData, QByteArray &aAnswerData,
