...
bool KasbiFRProtocol::check(const QByteArray &aAnswer) {
    ...
    if (prefix != answerPrefix) {
        toLog(LogLevel::Error,
              QString("Kasbi: Invalid prefix = 0x%1, need = 0x%2").arg(answerPrefix).arg(prefix));
        return false;
    }
    ...
    if (size != answerSize) {
        toLog(LogLevel::Error,
              QString("Kasbi: Invalid size = %1, need %2").arg(answerSize).arg(size));
        return false;
    }
    ...
    if (CRC != answerCRC) {
        toLog(LogLevel::Error,
              QString("Kasbi: Invalid CRC = %1, need %2")
...
//--------------------------------------------------------------------------------
TResult KasbiFRProtocol::processCommand(const QByteArray &aCommandData,
...
bool KasbiFRProtocol::read(QByteArray &aAnswer, int aTimeout) {
    ...
    auto getChar = [&aAnswer](int aIndex, int aShift) -> int {
        return int(uchar(aAnswer[aIndex])) << (8 * aShift);
    };
