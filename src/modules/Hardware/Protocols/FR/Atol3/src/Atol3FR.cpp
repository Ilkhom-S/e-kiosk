...
bool Atol3FRProtocol::check(const QByteArray &aAnswer)
{
    ...
    {
        toLog(LogLevel::Error, QString("Kasbi: Invalid answer length = %1, need %2 minimum")
                                   .arg(aAnswer.size())
    ...
    if (CAtol3FR::Prefix != aAnswer[0])
    {
        toLog(
            LogLevel::Error,
            QString("ATOL3: Invalid prefix = %1, need = %2").arg(toHexLog(aAnswer[0])).arg(toHexLog(CAtol3FR::Prefix)));
        return false;
    }
    ...
    if ((aAnswer[3] != char(mId)) && (aAnswer[3] != CAtol3FR::AsyncId))
    {
        toLog(LogLevel::Error,
              QString("ATOL3: Invalid Id = %1, need = %2").arg(toHexLog(aAnswer[3])).arg(toHexLog(mId)));
        return false;
    }
    ...
    if (CRC != answerCRC)
    {
        toLog(LogLevel::Error, QString("ATOL3: Invalid CRC = %1, need %2").arg(toHexLog(answerCRC)).arg(toHexLog(CRC)));
        return false;
    }
...
TResult Atol3FRProtocol::processCommand(uchar aTId, const QByteArray &aCommandData, QByteArray &aUnpackedAnswer)
{
    QByteArray commandData =
        CAtol3FR::TaskFlags::NeedResult + QByteArray(1, char(aTId)) + CAtol3FR::Password + aCommandData;
...
{
    return execCommand(CAtol3FR::Commands::Req, QByteArray(1, char(aTId)), aUnpackedAnswer,
