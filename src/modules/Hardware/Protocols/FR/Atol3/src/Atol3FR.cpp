...
bool Atol3FRProtocol::check(const QByteArray &aAnswer) {
    ...
    if (aAnswer.size() < CAtol3FR::MinAnswerSize) {
        toLog(LogLevel::Error,
              QString("Kasbi: Invalid answer length = %1, need %2 minimum")
                  .arg(aAnswer.size())
                  .arg(CAtol3FR::MinAnswerSize));
        return false;
    }
    ...
    if (CAtol3FR::Prefix != aAnswer[0]) {
        toLog(LogLevel::Error,
              QString("ATOL3: Invalid prefix = %1, need = %2")
                  .arg(toHexLog(aAnswer[0]))
                  .arg(toHexLog(CAtol3FR::Prefix)));
        return false;
    }
    ...
    if (size != answerSize) {
        toLog(LogLevel::Error,
              QString("ATOL3: Invalid size = %1, need %2").arg(answerSize).arg(size));
        return false;
    }
    ...
    if (CRC != answerCRC) {
        toLog(LogLevel::Error,
              QString("ATOL3: Invalid CRC = %1, need %2")
...
//--------------------------------------------------------------------------------
TResult Atol3FRProtocol::execCommand(char aCommand,
...
//--------------------------------------------------------------------------------
TResult Atol3FRProtocol::processCommand(uchar aTId,
                                        const QByteArray &aCommandData,
                                        QByteArray &aUnpackedAnswer) {
    QByteArray commandData = CAtol3FR::TaskFlags::NeedResult + QByteArray(1, char(aTId)) +
                             CAtol3FR::Password + aCommandData;

    return execCommand(
        CAtol3FR::Commands::Add, commandData, aUnpackedAnswer, CAtol3FR::Timeouts::AddRequest);
}

//--------------------------------------------------------------------------------
TResult Atol3FRProtocol::getResult(uchar aTId, QByteArray &aUnpackedAnswer) {
    return execCommand(CAtol3FR::Commands::Req,
                       QByteArray(1, char(aTId)),
                       aUnpackedAnswer,
                       CAtol3FR::Timeouts::GetResult);
}
...
TResult Atol3FRProtocol::sendACK(uchar aTId) {
    ...
    return execCommand(
        CAtol3FR::Commands::ACK, QByteArray(1, char(aTId)), commandData, CAtol3FR::Timeouts::ACK);
}
...
int Atol3FRProtocol::getSize(const QByteArray &aAnswer, int aStart) {
    ushort answerSize = ushort(aAnswer[aStart + 1] & CAtol3FR::SizeMask) |
                        (ushort(uchar(aAnswer[aStart + 2])) << 7);
