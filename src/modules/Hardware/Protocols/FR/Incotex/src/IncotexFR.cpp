...
//--------------------------------------------------------------------------------
bool IncotexFR::packData(FRProtocolCommands::Enum aCommand,
...
bool IncotexFR::unpackData(const QByteArray &aPacket, QByteArray &aData) {
    ...
    if (aPacket.size() < CIncotexFR::Constants::MinAnswerSize) {
        toLog(LogLevel::Error,
              QString("Incotex: Length of the packet is too small, %1 < %2")
                  .arg(aPacket.size())
    ...
    QByteArray trimmedAnswer =
        aPacket.mid(CIncotexFR::Positions::Command,
                    aPacket.size() - (CIncotexFR::Positions::CRC + CIncotexFR::Positions::Command));

    // проверяем контрольную сумму
    uchar localCRC = calcCRC(trimmedAnswer);
    QByteArray CRCBuffer =
        aPacket.right(CIncotexFR::Positions::CRC).left(CIncotexFR::Positions::CRCsize);
    uchar CRC;
...
bool IncotexFR::processCommand(SDK::Driver::IIOPort *aPort,
    ...
    return localProcessCommand(
        aPort, aCommand, aCommandData, static_cast<CIncotexFR::SUnpackedData *>(aUnpackedData));
}
...
bool IncotexFR::processAnswer(SDK::Driver::IIOPort *aPort,
    ...
    if (aAnswerData.size() > 1) {
        ...
        if (parseAnswer(aAnswerData, unpackedData)) {
            ...
            if (!processSuccess) {
                toLog(LogLevel::Error,
                      QString("Incotex: Error: %1")
                          .arg(CIncotexFR::Errors::Descriptions[unpackedData.commandResult]));
            }
            ...
            case CIncotexFR::Errors::Codes::CashierNotRegistered: {
                if (localProcessCommand(
                        aPort, FRProtocolCommands::CashierRegistration, commandData)) {
...
bool IncotexFR::localProcessCommand(SDK::Driver::IIOPort *aPort,
    ...
    return isESCCommand(aCommand)
               ? true
               : processAnswer(aPort, aCommand, aCommandData, unpacketAnswer, aUnpackedData);
}

//--------------------------------------------------------------------------------
bool IncotexFR::execCommand(SDK::Driver::IIOPort *aPort,
...
bool IncotexFR::getCommandPacket(FRProtocolCommands::Enum aCommand,
    ...
    case FRProtocolCommands::GetFRParameters: {
        ...
        // Тип параметра
        UShort2ASCIIBuffer(aLocalCommandData,
                           ushort(aCommandData[CHardware::StatusType].toUInt()),
                           10,
                           CIncotexFR::Constants::Length::FRParameter);
    ...
    case FRProtocolCommands::XReport: {
        ...
        // Тип операции
        UChar2ASCIIBuffer(aLocalCommandData,
                          CIncotexFR::Constants::XReport,
                          10,
    ...
    case FRProtocolCommands::Sale: {
        ...
        // строка шапки 1
        addFiscalElement(
            aLocalCommandData, CIncotexFR::FiscalData::Elements::DocumentCap1, x, ++y, emptyData);

        // строка шапки 2
        addFiscalElement(
            aLocalCommandData, CIncotexFR::FiscalData::Elements::DocumentCap2, x, ++y, emptyData);

        // строка шапки 3
        addFiscalElement(
            aLocalCommandData, CIncotexFR::FiscalData::Elements::DocumentCap3, x, ++y, emptyData);

        // строка шапки 4
        addFiscalElement(
            aLocalCommandData, CIncotexFR::FiscalData::Elements::DocumentCap4, x, ++y, emptyData);

        // номер ККМ
        addFiscalElement(
            aLocalCommandData, CIncotexFR::FiscalData::Elements::KKMNumber, x, ++y, emptyData);

        // ИНН владельца терминала
        addFiscalElement(
            aLocalCommandData, CIncotexFR::FiscalData::Elements::INN, x + 15, y, emptyData);

        // номер документа
        addFiscalElement(
            aLocalCommandData, CIncotexFR::FiscalData::Elements::DocumentNumber, x, ++y, emptyData);

        // номер чека
        addFiscalElement(aLocalCommandData,
                         CIncotexFR::FiscalData::Elements::ReceiptNumber,
                         x + 15,
                         y,
                         emptyData);
        ...
        // номер кассира
        addFiscalElement(aLocalCommandData,
                         CIncotexFR::FiscalData::Elements::CashierNumber,
                         x + 30,
                         y,
                         emptyData);
        ...
        // дата и время
        addFiscalElement(
            aLocalCommandData, CIncotexFR::FiscalData::Elements::DateTime, x, ++y, emptyData);

        // цена услуги
        QByteArray sum = QByteArray::number(
            int(aCommandData[CHardware::FiscalPrinter::Amount].toDouble() * 100));
        sum.insert(sum.size() - 2, ASCII::Dot);

        QByteArray priceData = emptyData;
        priceData.replace(CIncotexFR::Positions::PaidSum, sum.size(), sum);
        addFiscalElement(aLocalCommandData,
                         CIncotexFR::FiscalData::Elements::Price,
                         x,
                         ++y,
                         priceData,
        ...
        // итоговая сумма
        addFiscalElement(
            aLocalCommandData, CIncotexFR::FiscalData::Elements::TotalSum, x, ++y, emptyData);

        // уплаченная сумма
        addFiscalElement(
            aLocalCommandData,
            CIncotexFR::FiscalData::Elements::PaidSum,
            x,
            ++y,
            sum.leftJustified(CIncotexFR::Constants::Length::ElementData::Default, ASCII::NUL));

        // сумма сдачи
        int elementAmount = addFiscalElement(
            aLocalCommandData, CIncotexFR::FiscalData::Elements::OddSum, x, ++y, emptyData);

        // количество передаваемых реквизитов
        QByteArray amountData;
        UChar2ASCIIBuffer(
            amountData, uchar(elementAmount), 10, CIncotexFR::Constants::Length::ElementsQuantity);
    ...
    case FRProtocolCommands::Encashment: {
        ...
        // Тип операции
        UChar2ASCIIBuffer(aLocalCommandData,
                          CIncotexFR::FiscalData::Encashment,
                          10,
                          CIncotexFR::Constants::Length::Operation);
        aLocalCommandData.append(CIncotexFR::Constants::Separator);
        ...
        // получаем строки терминального чека из реестра, который мы запомнили при вызове функции
        // печати чека в классе принтера
        typedef QList<QByteArray> TReceiptBuffer;
        TReceiptBuffer *receipt = dynamic_cast<TReceiptBuffer *>(
            aCommandData[CHardware::Printer::Receipt].value<TReceiptBuffer *>());
        ...
        for (int i = 0; i < receipt->size(); ++i) {
            ...
            do {
                addFiscalElement(
                    aLocalCommandData,
                    CIncotexFR::FiscalData::Elements::Free,
                    x,
                    ++y,
                    receipt->value(i)
                        .mid(start, CIncotexFR::Constants::Length::ElementData::Encashment)
                        .leftJustified(CIncotexFR::Constants::Length::ElementData::Default,
                                       ASCII::NUL));
        ...
        // цена услуги
        QByteArray sum =
            QByteArray::number(aCommandData[CHardware::FiscalPrinter::Amount].toDouble(), 'f', 2)
                .right(11);
        QByteArray priceData = emptyData;
        priceData.replace(CIncotexFR::Positions::PaidSum, sum.size(), sum);
        addFiscalElement(aLocalCommandData,
    ...
    case FRProtocolCommands::GetFRRegisters: {
        ...
        // Номер кассира
        UShort2ASCIIBuffer(
            aLocalCommandData, ushort(aCommandData[CHardware::Register].toUInt()), 10, 4);
        aLocalCommandData.append(CIncotexFR::Constants::Separator);
    ...
    default: {
        toLog(LogLevel::Error,
              QString("Incotex: The commmand %1 is not implemented")
                  .arg(FRProtocolCommands::Description[aCommand]));
        return false;
    }
    }
...
bool IncotexFR::getAnswer(SDK::Driver::IIOPort *aPort, QByteArray &aAnswer) {
    ...
    auto adjust = [](QByteArray &aAnswer) {
        ...
        if (position != -1)
            aAnswer = aAnswer.mid(
                position, aAnswer.indexOf(CIncotexFR::Constants::Postfix, position) - position + 1);
    };
...
bool IncotexFR::parseAnswer(const QByteArray &aUnpackedData, CIncotexFR::SUnpackedData &aAnswer) {
    ...
    aAnswer.FiscalMemoryNearEnd =
        ProtocolUtils::getBit(FRStatus, CIncotexFR::Answer::Errors::FiscalMemoryNearEnd, true);
    aAnswer.FiscalMemoryEnd =
        ProtocolUtils::getBit(FRStatus, CIncotexFR::Answer::Errors::FiscalMemoryEnd, true);
    ...
    aAnswer.TechnoMode =
        !ProtocolUtils::getBit(PrinterStatus, CIncotexFR::Answer::Errors::TechnoMode);
    aAnswer.PrinterError =
        !ProtocolUtils::getBit(PrinterStatus, CIncotexFR::Answer::Errors::PrinterError);
    ...
    aAnswer.OfflineError =
        ProtocolUtils::getBit(PrinterStatus, CIncotexFR::Answer::Errors::OfflineError);
    ...
    QString errorLog =
        QString("Incotex: Wrong sections in answer : %1, need min ").arg(answerData.size());
    ...
    case CIncotexFR::Commands::GetModelInfo: {
        if (answerData.size() < (CIncotexFR::Answer::Sections::SoftVersion + 1)) {
            toLog(LogLevel::Error,
                  errorLog + QString::number(CIncotexFR::Answer::Sections::SoftVersion + 1));
            return false;
        }
    ...
    case CIncotexFR::Commands::GetFRParameters: {
        if (answerData.size() < (CIncotexFR::Answer::Sections::StatusEKLZ + 1)) {
            toLog(LogLevel::Error,
                  errorLog + QString::number(CIncotexFR::Answer::Sections::StatusEKLZ));
            return false;
        }
        ...
        if (ParameterFR == CIncotexFR::Constants::EKLZStatusRequest) {
            aAnswer.EKLZError =
                ProtocolUtils::getBit(StatusEKLZ, CIncotexFR::Answer::Errors::EKLZError, true);
            aAnswer.EKLZNearEnd =
                ProtocolUtils::getBit(StatusEKLZ, CIncotexFR::Answer::Errors::EKLZNearEnd, true);
        }
    ...
    case CIncotexFR::Commands::GetRegister: {
        if (answerData.size() < (CIncotexFR::Answer::Sections::Register + 1)) {
            toLog(LogLevel::Error,
                  errorLog + QString::number(CIncotexFR::Answer::Sections::Register));
            return false;
        }

        bool convertCorrect;
        aAnswer.Register =
            answerData[CIncotexFR::Answer::Sections::Register].toDouble(&convertCorrect);

        if (!convertCorrect) {
            toLog(LogLevel::Error,
                  "Incotex: wrong money register format: " +
                      answerData[CIncotexFR::Answer::Sections::Register]);
            return false;
        }
...
//--------------------------------------------------------------------------------
const bool IncotexFR::programmFRParameter(SDK::Driver::IIOPort * /*aPort*/,
                                          uchar /*aNumber*/,
                                          uchar /*aValue*/) {
    // TODO: реализовать
    return true;
...
const bool IncotexFR::isESCCommand(const QByteArray &aPacket) {
    return aPacket.isEmpty() ? false
                             : ((aPacket[0] != CIncotexFR::Constants::Prefix) &&
                                (aPacket != CIncotexFR::Commands::SetFiscalMode));
}
...
void IncotexFR::UShort2ASCIIBuffer(QByteArray &aData, ushort aNumber, int aBase, int aSize) {
    QString filledNumber =
        QString::number(aNumber, aBase).rightJustified(2 * sizeof(aNumber), ASCII::Zero, true);
...
//--------------------------------------------------------------------------------
const int IncotexFR::addFiscalElement(QByteArray &aLocalData,
                                      char elementNumber,
                                      uchar aX,
                                      uchar aY,
...
bool IncotexFR::getStatus(SDK::Driver::IIOPort *aPort, CIncotexFR::SUnpackedData &aUnpacketAnswer) {
    ...
    if (localProcessCommand(aPort, FRProtocolCommands::GetStatus, commandData, &aUnpacketAnswer)) {
        ...
        return localProcessCommand(
            aPort, FRProtocolCommands::GetFRParameters, commandData, &aUnpacketAnswer);
    }
