...
bool IncotexFR::unpackData(const QByteArray& aPacket, QByteArray& aData)
{
	...
	QByteArray trimmedAnswer = aPacket.mid(
		CIncotexFR::Positions::Command, aPacket.size() - (CIncotexFR::Positions::CRC + CIncotexFR::Positions::Command));
								 ...
								 const QVariantMap& aCommandData)
{
	...
	case FRProtocolCommands::GetFRParameters:
	{
		...
		// Тип параметра
		UShort2ASCIIBuffer(aLocalCommandData, ushort(aCommandData[CHardware::StatusType].toUInt()), 10,
	...
	case FRProtocolCommands::XReport:
	{
		...
		// Тип операции
		UChar2ASCIIBuffer(aLocalCommandData, CIncotexFR::Constants::XReport, 10, CIncotexFR::Constants::Length::Report);
		aLocalCommandData.append(CIncotexFR::Constants::Separator);
	...
	case FRProtocolCommands::ZReport:
	{
		...
		// Тип операции
		UChar2ASCIIBuffer(aLocalCommandData, CIncotexFR::Constants::ZReport, 10, CIncotexFR::Constants::Length::Report);
		aLocalCommandData.append(CIncotexFR::Constants::Separator);
	...
	case FRProtocolCommands::Sale:
	{
		...
		// цена услуги
		QByteArray sum = QByteArray::number(int(aCommandData[CHardware::FiscalPrinter::Amount].toDouble() * 100));
		...
		addFiscalElement(aLocalCommandData, CIncotexFR::FiscalData::Elements::Price, x, ++y, priceData,
						 CIncotexFR::FiscalData::Flags::DefaultFont | CIncotexFR::FiscalData::Flags::SaleForMoney);
	...
	case FRProtocolCommands::Encashment:
	{
		...
		// получаем строки терминального чека из реестра, который мы запомнили при вызове функции печати чека в классе
		// принтера
		typedef QList<QByteArray> TReceiptBuffer;
		TReceiptBuffer* receipt =
			dynamic_cast<TReceiptBuffer*>(aCommandData[CHardware::Printer::Receipt].value<TReceiptBuffer*>());
			...
			do
			{
				addFiscalElement(aLocalCommandData, CIncotexFR::FiscalData::Elements::Free, x, ++y,
								 receipt->value(i)
									 ...
									 .leftJustified(CIncotexFR::Constants::Length::ElementData::Default, ASCII::NUL));
		...
		// цена услуги
		QByteArray sum =
			QByteArray::number(aCommandData[CHardware::FiscalPrinter::Amount].toDouble(), 'f', 2).right(11);
		QByteArray priceData = emptyData;
		priceData.replace(CIncotexFR::Positions::PaidSum, sum.size(), sum);
		addFiscalElement(aLocalCommandData, CIncotexFR::FiscalData::Elements::Price, x, ++y, priceData,
						 CIncotexFR::FiscalData::Flags::DefaultFont | CIncotexFR::FiscalData::Flags::SaleForMoney);
