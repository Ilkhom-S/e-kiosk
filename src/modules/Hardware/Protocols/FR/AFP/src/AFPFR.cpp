...
TResult AFPFRProtocol::check(const QByteArray &aRequest, const QByteArray &aAnswer) {
    ...
    if (size < CAFPFR::MinAnswerSize) {
        toLog(LogLevel::Error,
              QString("AFP: Too few bytes in answer = %1, need min = %2")
                  .arg(size)
                  .arg(CAFPFR::MinAnswerSize));
        return CommandResult::Protocol;
    }

    if (aAnswer[0] != CAFPFR::Prefix) {
        toLog(LogLevel::Error,
              QString("AFP: Invalid prefix = %1, need = %2")
    ...
    if (requestCommand != answerCommand) {
        toLog(LogLevel::Error,
              QString("AFP: Invalid command in answer = 0x%1, need = 0x%2")
    ...
    if (postfix != CAFPFR::Postfix) {
        toLog(LogLevel::Error,
              QString("AFP: Invalid postfix = %1, need = %2")
    ...
    if (!OK) {
        toLog(LogLevel::Error,
              QString("AFP: Failed to parse answer CRC = 0x%1").arg(answerCRCData.toHex().data()));
        return CommandResult::Protocol;
    }

    if (dataCRC != answerCRC) {
        toLog(LogLevel::Error,
              QString("AFP: Wrong CRC = %1, need = %2")
...
//---------------------------------------------------------------------------------
TResult
AFPFRProtocol::processCommand(const QByteArray &aCommandData, QByteArray &aAnswer, int aTimeout) {
    QByteArray request;
    ...
    QByteArray CRCData =
        QByteArray::number(CRC, 16).rightJustified(2, ASCII::Zero).right(2).toUpper();
