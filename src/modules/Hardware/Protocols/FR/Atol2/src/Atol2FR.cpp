...
bool Atol2FRProtocol::unpack(QByteArray &aAnswer) {
    ...
    if (aAnswer.size() < CAtol2FR::MinAnswerSize) {
        toLog(LogLevel::Error,
              QString("ATOL: The length of the packet is less than %1 byte")
                  .arg(CAtol2FR::MinAnswerSize));
        return false;
    }
    ...
    for (int i = 0; i < aAnswer.size() - 1; ++i) {
        ...
        if ((aAnswer[i + 1] == ASCII::DLE || aAnswer[i + 1] == ASCII::ETX) &&
            aAnswer[i] == ASCII::DLE) {
            // маскируемый байт
            i++;
            data.append(aAnswer[i]);
    ...
    if (!findPrefix) {
        toLog(LogLevel::Error,
              QString("ATOL: Failed to find prefix = %1").arg(toHexLog(CAtol2FR::Prefix)));
        return false;
    }
    ...
    if (data.size() < (CAtol2FR::MinAnswerSize - 1)) {
        toLog(LogLevel::Error,
              QString("ATOL: The length of the final packet = %1, need %2 minimum")
                  .arg(data.size())
    ...
    if (postfix != CAtol2FR::Postfix) {
        toLog(LogLevel::Error,
              QString("ATOL: Invalid postfix = %1, need = %2")
                  .arg(toHexLog(postfix))
                  .arg(toHexLog(CAtol2FR::Postfix)));
        return false;
    }
    ...
    if (CRC != answerCRC) {
        toLog(LogLevel::Error,
              QString("ATOL: Invalid CRC of the answer = %1, need %2")
                  .arg(toHexLog(answerCRC))
                  .arg(toHexLog(CRC)));
        return false;
    }

    aAnswer = data.left(dataSize - 1)
                  .replace(CAtol2FR::DLEMask, QByteArray(1, ASCII::DLE))
                  .replace(CAtol2FR::ETXMask, QByteArray(1, ASCII::ETX));

    return true;
}

//--------------------------------------------------------------------------------
TResult Atol2FRProtocol::processCommand(const QByteArray &aCommandData,
...
bool Atol2FRProtocol::read(QByteArray &aData) {
    ...
    do {
        ...
        if (!answerData.isEmpty()) {
            ...
            // если получили постфикс и перед ним не стоит маска, значит это постфикс и надо
            // выходить
            if ((size > 2) && (aData[size - 2] == CAtol2FR::Postfix) &&
