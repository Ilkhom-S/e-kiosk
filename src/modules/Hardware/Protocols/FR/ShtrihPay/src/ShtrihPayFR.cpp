...
bool ShtrihPayFRProtocol::unpackData(const QByteArray &aPacket, QByteArray &aData) {
    if (aPacket.size() < CShtrihPayFR::MinPacketAnswerSize) {
        toLog(LogLevel::Error,
              QString("ShtrihPay: The length of the packet is less than %1 byte")
                  .arg(CShtrihPayFR::MinPacketAnswerSize));
        return false;
    }
    ...
    do {
        if (count++) {
            toLog(LogLevel::Normal,
                  "ShtrihPay: session opening, attempt #" + QString::number(count));
        }
        ...
        if (answerData.startsWith(ASCII::ACK)) {
            toLog(LogLevel::Normal,
                  "ShtrihPay: Device returns ACK, getting the last answer and sending ACK");
            ...
            toLog(LogLevel::Error,
                  QString("ShtrihPay: Device returns unknown answer {%1}")
...
//--------------------------------------------------------------------------------
TResult ShtrihPayFRProtocol::processCommand(const QByteArray &aCommandData,
    ...
    do {
        toLog(LogLevel::Normal,
              QString("ShtrihPay: >> {%1} - iteration %2")
                  .arg(commandData.toHex().data())
                  .arg(repeatCount + 1));
        QByteArray answer;
...
//--------------------------------------------------------------------------------
bool ShtrihPayFRProtocol::execCommand(const QByteArray &aCommandPacket,
                                      QByteArray &aAnswerData,
                                      int aTimeout) {
    int countRequest = 0;
    ...
    do {
        ...
        if (!aAnswerData.isEmpty() && (aAnswerData[0] == ASCII::NAK)) {
            toLog(LogLevel::Warning,
                  "ShtrihPay: Answer contains NAK, iteration " + QString::number(countRequest + 1));
            countRequest++;
        } else {
            return true;
        }
