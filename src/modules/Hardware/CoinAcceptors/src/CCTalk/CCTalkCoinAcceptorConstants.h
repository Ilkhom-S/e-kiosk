...
public:
    ErrorData() {
        ...
        add(4,
            BillAcceptorStatusCode::MechanicFailure::JammedCoin,
            false,
            "Wake-up timeout"); // м.б.
        add(5,
            BillAcceptorStatusCode::MechanicFailure::JammedCoin,
            false,
            "Validation timeout"); // м.б.
        add(6,
            BillAcceptorStatusCode::MechanicFailure::JammedCoin,
            false,
            "Credit sensor timeout"); // м.б.
        add(7,
            BillAcceptorStatusCode::MechanicFailure::JammedCoin,
            false,
            "Sorter opto timeout"); // м.б.
        ...
        add(27,
            ...
            false,
            "Games overload"); // будут поддержаны при реализации этих устройств
        add(28,
            DeviceStatusCode::Error::Unknown,
            false,
            "Max. coin meter pulses exceeded"); // будут поддержаны при реализации этих устройств
        ...
        for (uchar i = 128; i < 160; ++i) {
            add(i,
                BillAcceptorStatusCode::Reject::InhibitNote,
                true,
                QString("Inhibited coin (Type %1)").arg(i).toLocal8Bit().data());
        }

        add(253,
            BillAcceptorStatusCode::OperationError::Unknown,
            false,
            "Data block request (note ?)"); // "not yet used", запрос каких-то данных от хоста
        add(254,
