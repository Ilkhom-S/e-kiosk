...
template <class T> bool TCPDeviceBase<T>::checkConnectionAbility()
{
    ...
    const QString address = this->mIOPort->getDeviceConfiguration().value(CHardwareSDK::Port::TCP::IP).toString();
    ...
    return this->checkError(
           ...
           this->checkError(
               IOPortStatusCode::Error::NotConfigured, [&address]() { return !address.isEmpty(); },
...
#define MAKE_TCP_PORT_PARAMETER(aName, aType)                                                                          \
    ...
    if (aName.isEmpty())                                                                                               \
    {                                                                                                                  \
        this->toLog(LogLevel::Error, this->mDeviceName + QStringLiteral(": %1 are empty").arg(#aName));                \
        return false;                                                                                                  \
    }
