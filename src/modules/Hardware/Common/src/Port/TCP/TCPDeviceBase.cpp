...
bool TCPDeviceBase<T>::checkConnectionAbility()
{
	...
	return checkError(
			   IOPortStatusCode::Error::NotConnected, [&]() -> bool
			   { return mIOPort->isExist() || mIOPort->deviceConnected(); }, "IO port is not connected") &&
		   checkError(
			   IOPortStatusCode::Error::NotConfigured, [&]() -> bool { return !address.isEmpty(); },
...
#define MAKE_TCP_PORT_PARAMETER(aName, aType)                                        \
	TTCPDevicePortParameter aName = mPortParameters[CHardwareSDK::Port::TCP::aType]; \
	if (aName.isEmpty())                                                             \
	{                                                                                \
		toLog(LogLevel::Error, mDeviceName + QString(": %1 are empty").arg(#aName)); \
		return false;                                                                \
	}
