...
template <class T> bool SerialDeviceBase<T>::checkConnectionAbility()
{
    ...
    return this->checkError(
               IOPortStatusCode::Error::NotConnected, [this]()
               { return this->mIOPort->isExist() || this->mIOPort->deviceConnected(); }, "IO port is not connected") &&
           this->checkError(
...
// Explicit template instantiation for SerialDeviceBase constructor (ProtoModem)
template SerialDeviceBase<PortDeviceBase<DeviceBase<ProtoModem>>>::SerialDeviceBase();
// Explicit template instantiation for SerialDeviceBase constructor (ProtoPrinter)
template SerialDeviceBase<PortPollingDeviceBase<ProtoPrinter>>::SerialDeviceBase();
