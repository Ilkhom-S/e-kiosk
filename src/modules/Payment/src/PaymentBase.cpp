...
qint64 PaymentBase::getProvider(bool aMNP) const {
    ...
    qint64 gatewayIn = getParameter(PPSDK::CPayment::Parameters::MNPGatewayIn).value.toLongLong();
    ...
    PPSDK::SProvider provider = dealerSettings->getMNPProvider(myProvider, gatewayIn, gatewayOut);
...
void PaymentBase::updateParameter(const SParameter &aParameter) {
    ...
    if (aParameter.name == PPSDK::CPayment::Parameters::Provider) {
        ...
        if (!mProviderSettings.isNull()) {
            ...
            // В базе всегда храним список полей провайдера на случай, если провайдера удалят из
            ...
            setParameter(SParameter(
                PPSDK::CPayment::Parameters::ProviderFields,
                providerFields.join(PPSDK::CPayment::Parameters::ProviderFieldsDelimiter),
                true));
        }
    }
}
...
bool PaymentBase::getLimits(double &aMinAmount, double &aMaxAmount) {
    ...
    aMaxAmount =
        myEngine.evaluate(maxLimit.isEmpty() ? limits.system : maxLimit).toString().toDouble();
...
PaymentBase::calculateCommission(const QList<PPSDK::IPayment::SParameter> &aParameters) {
    ...
    if (!qFuzzyIsNull(amountAll)) {
        ...
        if (limits.min == limits.max) {
            if (lessOrEqual(amountAll, maxAmountAll)) {
                ...
                com = dealerSettings->getCommission(
                    getProvider(true), credentials, amountAll > maxAmount ? maxAmount : amountAll);
            } else {
        ...
        commission = com.getValueFor(
            targetSum, getMNPProviderSettings().processor.feeType == PPSDK::SProvider::FeeByAmount);
    }

    if (greatOrEqual(amountAll, maxAmountAll)) {
        amount =
            qFuzzyCompare(maxAmount, limits.system.toDouble()) ? maxAmount - commission : maxAmount;
    } else {
    ...
    double processingCommission =
        dealerSettings->getProcessingCommission(getProvider(true)).getValue(amount, amountAll);
    ...
    result << SParameter(
           ...
           << SParameter(
                  PPSDK::CPayment::Parameters::Fee, QString::number(commission, 'f', 2), true)
           << SParameter(PPSDK::CPayment::Parameters::Change, QString::number(change, 'f', 2), true)
           << SParameter(PPSDK::CPayment::Parameters::ProcessingFee,
                         QString::number(calcFees(processingCommission, commission).first, 'f', 2),
                         true)
           << SParameter(PPSDK::CPayment::Parameters::DealerFee,
                         QString::number(calcFees(processingCommission, commission).second, 'f', 2),
                         true)
...
void PaymentBase::calculateSums() {
    foreach (auto p,
             calculateCommission(QList<SParameter>()
                                 << getParameter(PPSDK::CPayment::Parameters::AmountAll))) {
        updateParameter(p);
    }
}
...
bool PaymentBase::restore(const QList<SParameter> &aParameters) {
    ...
    try {
        ...
        requiredParameters << PPSDK::CPayment::Parameters::ID << PPSDK::CPayment::Parameters::Type
        ...
        if (getProviderSettings().isNull()) {
            ...
            if (id == -1) {
                toLog(LogLevel::Debug,
                      QString("Payment %1. Restore provider settings failed.").arg(id));
            } else {
                // Проведение платежа невозможно, но данные из него извлечь можно, а в случае
                // необходимости и поменять провайдера.
                toLog(LogLevel::Warning,
                      QString("Payment %1. Failed to load provider settings.").arg(id));
            }
        }
...
QList<SDK::PaymentProcessor::IPayment::SParameter> PaymentBase::serialize() const {
    ...
    while (i != j) {
        ...
        if (parameter.crypted) {
            ...
            if (cryptEngine->encryptLong(
                    -1, parameter.value.toString().toUtf8(), encryptedValue, error)) {
                parameter.value = encryptedValue;
            } else {
                toLog(
                    LogLevel::Error,
                    QString(
                        "Payment %1. Failed to encrypt parameter %2 while serializing. Error: %3.")
                        .arg(getID())
                        .arg(parameter.name)
                        .arg(error));
