      ...
      mParametersLock(QReadWriteLock::Recursive)
{
    ...
    if (!states.isEmpty())
    {
        ...
        QString crc = QString::fromLatin1(
            CCryptographicHash::hash(states.join(";").toLatin1(), CCryptographicHash::Sha256).toHex());
#else
        QString crc = QString::fromLatin1(
            QCryptographicHash::hash(states.join(";").toLatin1(), QCryptographicHash::Sha256).toHex());
#endif
...
void PaymentBase::updateParameter(const SParameter &aParameter)
{
    ...
    if (aParameter.name == PPSDK::CPayment::Parameters::Provider)
    {
        ...
        if (!mProviderSettings.isNull())
        {
            ...
            // В базе всегда храним список полей провайдера на случай, если провайдера удалят из конфига, а платежи по
            // нему ещё останутся.
            ...
            setParameter(SParameter(PPSDK::CPayment::Parameters::ProviderFields,
                                    providerFields.join(PPSDK::CPayment::Parameters::ProviderFieldsDelimiter), true));
        }
    }
}
...
PaymentBase::calculateCommission(const QList<PPSDK::IPayment::SParameter> &aParameters)
{
    ...
    if (!qFuzzyIsNull(amountAll))
    {
        ...
        if (limits.min == limits.max)
        {
            if (lessOrEqual(amountAll, maxAmountAll))
            {
                ...
                com = dealerSettings->getCommission(getProvider(true), credentials,
                                                    amountAll > maxAmount ? maxAmount : amountAll);
            }
            else
            {
        ...
        commission =
            com.getValueFor(targetSum, getMNPProviderSettings().processor.feeType == PPSDK::SProvider::FeeByAmount);
    }
    ...
    result << SParameter(PPSDK::CPayment::Parameters::AmountAll, QString::number(amountAll, 'f', 2), true)
           ...
           << SParameter(PPSDK::CPayment::Parameters::ProcessingFee,
                         QString::number(calcFees(processingCommission, commission).first, 'f', 2), true)
           << SParameter(PPSDK::CPayment::Parameters::DealerFee,
                         QString::number(calcFees(processingCommission, commission).second, 'f', 2), true)
...
bool PaymentBase::restore(const QList<SParameter> &aParameters)
{
    ...
    try
    {
            ...
            if (parameter.crypted)
            {
                ...
                else
                {
                    toLog(LogLevel::Error,
                          QString("Payment %1. Failed to decrypt parameter %2 while restoring. Error: %3.")
                              .arg(getID())
                              .arg(parameter.name)
                              .arg(error));
                }
        ...
        requiredParameters << PPSDK::CPayment::Parameters::ID << PPSDK::CPayment::Parameters::Type
                           << PPSDK::CPayment::Parameters::CreationDate << PPSDK::CPayment::Parameters::Provider
                           << PPSDK::CPayment::Parameters::Status << PPSDK::CPayment::Parameters::Priority;
...
QList<SDK::PaymentProcessor::IPayment::SParameter> PaymentBase::serialize() const
{
    ...
    while (i != j)
    {
        ...
        if (parameter.crypted)
        {
            ...
            {
                toLog(LogLevel::Error,
                      QString("Payment %1. Failed to encrypt parameter %2 while serializing. Error: %3.")
                          .arg(getID())
                          .arg(parameter.name)
                          .arg(error));
