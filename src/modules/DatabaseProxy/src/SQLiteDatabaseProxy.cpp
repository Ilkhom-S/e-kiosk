...
//---------------------------------------------------------------------------
bool SQLiteDatabaseProxy::open(const QString &aDbName, const QString &aUser, const QString &aPassword,
                               const QString &aHost, const int /*aPort*/)
{
    ...
    {
        mCurrentBase =
            QDir::cleanPath(BasicApplication::getInstance()->getWorkingDirectory() + QDir::separator() + mCurrentBase);
    }
    ...
    {
        toLog(LogLevel::Error, QString("Cannot open database %1. Following error occured: %2.")
                                   .arg(mCurrentBase)
    ...
    while (tryCount < CSQLiteDatabaseProxy::MaxTryCount)
    {
        if (!aQuery->exec(aQueryMessage))
        {
            ...
            toLog(LogLevel::Error,
                  QString("Can't execute query: %1. Error: %2.").arg(aQueryMessage).arg(aQuery->lastError().text()));
